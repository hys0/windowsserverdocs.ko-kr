...
|[1.7 Active Directory Domain Services 계획](#17-plan-active-directory-domain-services)|도메인 컨트롤러, Active Directory 요구 사항, 클라이언트 인증 및 여러 도메인을 계획합니다.|  
...
-   [1.1.2 IPv6 인트라넷 연결 계획](#112-plan-ipv6-intranet-connectivity)  
  
-   [1.1.3 강제 터널링 계획](#113-plan-for-force-tunneling)  
...
-   [1.3.1 IPsec 인증용 컴퓨터 인증서 계획](#131-plan-computer-certificates-for-ipsec-authentication)  
...
-   [1.3.2 IP-HTTPS 용 인증서 계획](#132-plan-certificates-for-ip-https)  
...
-   [1.4.1 DNS 서버 요구 사항에 대 한 계획](#141-plan-for-dns-server-requirements)  
  
-   [1.4.2 로컬 이름 확인 계획](#142-plan-for-local-name-resolution)  
...
-   [1.7.1 클라이언트 인증 계획](#171-plan-client-authentication)  
...
WINS(Windows 인터넷 이름 서비스)가 다중 도메인 환경에 배포된 경우 DNS에 WINS 정방향 조회 영역을 배포해야 합니다. 자세한 내용은 참조 하세요. **단일 레이블 이름을** 에 [1.4.2 로컬 이름 확인 계획](#142-plan-for-local-name-resolution) 이 문서의 이전 섹션을 참조 합니다.  
...
-   [1.8.1 자동으로 만들어진된 Gpo 구성](#181-configure-automatically-created-gpos)  
  
-   [1.8.2 수동으로 만든된 Gpo 구성](#182-configure-manually-created-gpos)  
  
-   [1.8.3 다중 도메인 컨트롤러 환경에서 Gpo 관리](#183-manage-gpos-in-a-multi-domain-controller-environment)  
  
-   [1.8.4 제한 된 권한으로 원격 액세스 Gpo 관리](#184-manage-remote-access-gpos-with-limited-permissions)  
  
-   [1.8.5 삭제 된 gpo 복구](#185-recover-from-a-deleted-gpo)  
