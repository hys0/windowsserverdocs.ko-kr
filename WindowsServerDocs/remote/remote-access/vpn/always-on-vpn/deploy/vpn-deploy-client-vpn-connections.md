...
이 단계에서는 있습니다 ProfileXML 옵션 및 스키마에 대 한 알아봅니다 VPN 연결을 사용 하 여 해당 인프라와 통신 하는 Windows 10 클라이언트 컴퓨터 구성.
...
## <a name="manually-create-a-template-connection-profile"></a>템플릿 연결 프로필을 수동으로 만들기

이 단계를 사용 하 여 인증 프로토콜 PEAP (Protected Extensible) 클라이언트와 서버 간의 통신을 보호 합니다. 간단한 사용자 이름 및 암호와 달리이 연결에 작동 하도록 VPN 프로필의 고유한 EAPConfiguration 섹션에 필요 합니다.
...
3. 마우스 오른쪽 단추로 클릭 **가상 개인 네트워크 (VPN) 연결**, 클릭 **속성**합니다.

4. 클릭 합니다 **제약 조건** 탭을 클릭 **인증 방법을**합니다.

5. EAP 종류에서 **Microsoft: 보호 된 EAP (PEAP)** , 클릭 **편집**합니다.

6. 에 대 한 값을 기록 **에 발급 된 인증서** 하 고 **발급자**합니다.

    예정 된 VPN 템플릿 구성에서 이러한 값을 사용합니다. 예를 들어 서버의 FQDN이 nps01.corp.contoso.com 경우 호스트 이름은 NPS01 인증서 이름을 기반으로 서버의 FQDN 또는 DNS 이름 — 예를 들어 nps01.corp.contoso.com 합니다.
...
### <a name="configure-the-template-vpn-profile-on-a-domain-joined-client-computer"></a>도메인에 가입 된 클라이언트 컴퓨터에서 VPN 프로필 템플릿 구성

되었으니 이제 도메인에 가입 된 클라이언트 컴퓨터에서 VPN 프로필 템플릿을 구성 하는 데 필요한 정보입니다. 사용자 유형의 계정을 사용 하 여 (즉, 표준 사용자 또는 관리자) 프로세스의이 부분에 대 한 중요 하지 않습니다.
...
6.  서버 이름 또는 주소를 입력 합니다 **외부** VPN 서버의 FQDN (예를 들어 **vpn.contoso.com**).
...
13. 클릭 **속성** 보호 된 EAP 속성 대화 상자를 열고 다음 단계를 완료 합니다.
    ...
    c.  연결 전에 알림, 클릭 **새 서버 또는 신뢰할 수 있는 Ca를 허가 하도록 사용자를 묻지**합니다.
...
14. 클릭 **확인** 템플릿 속성 대화 상자를 닫습니다.

15. 네트워크 연결 창을 닫습니다.

16. 클릭 하 여 VPN 설정에서 테스트 **템플릿을**를 클릭 하 고 **Connect**합니다.
...
이 섹션을 완료 하기 전에 템플릿 VPN 연결을 테스트를 만들고 있는지 확인 하는 섹션 [템플릿 연결 프로필을 수동으로 만들](#manually-create-a-template-connection-profile) 에 대해 설명 합니다. 프로필에서 VPN에 연결 하는 데 필요한 모든 정보가 포함 되어 있는지 확인 해야 하는 VPN 연결을 테스트 합니다.
...
1. 동일한 사용자를 사용 하 여 VPN 프로필 템플릿이 포함 된 도메인에 가입 된 클라이언트 컴퓨터에 로그인 계정을 섹션 [템플릿 연결 프로필을 수동으로 만들](#manually-create-a-template-connection-profile) 설명 합니다.

2. 목록 1 Windows PowerShell ISE (통합된 스크립팅 환경)을 붙여넣고은 주석에 설명 된 매개 변수 사용자 지정 합니다. 다음은 $Template, $ProfileName, $Servers, $DnsSuffix, $DomainName, $TrustedNetwork, $DNSServers입니다. 각 설정에 대 한 전체 설명은 주석입니다.
...
>[!NOTE]
>전체 예제 스크립트를 보려면 섹션을 참조 하세요 [MakeProfile.ps1 전체 스크립트](#makeprofileps1-full-script)합니다.
...
- FQDN-정규화 된 도메인 이름
- 접미사-도메인 접미사를 DNS 확인을 위해 짧은 이름이 쿼리에 추가 됩니다. 접미사를 지정 하려면 DNS 접미사에 마침표 (.)를 추가 합니다.
...
다음은 예제 아래 명령에서 사용 된 매개 변수 값입니다. 사용자 환경에 대 한 이러한 값을 변경 하는 것을 확인 합니다.
...
System Center Configuration Manager에서 Windows PowerShell에서 했던 것과 마찬가지로 ProfileXML CSP 노드를 사용 하 여 VPN 프로필을 배포할 수 있습니다. 섹션에서 만든 VPN_Profile.ps1 Windows PowerShell 스크립트를 사용 하는 이때 [ProfileXML 구성 파일을 만들](#create-the-profilexml-configuration-files)합니다.
...
4.  멤버 관리 규칙 페이지에서 다음 단계를 수행 합니다.
    ...
    c.  리소스 페이지에 대 한 검색에서 **값**를 추가 하려는 사용자의 이름을 입력 합니다. 리소스 이름에는 사용자의 도메인이 포함 됩니다. 부분 일치를 기준으로 결과 포함 하려면 삽입 합니다 **%** 검색 기준의 한쪽 끝에서 문자입니다. 예를 들어, 입력 "lori" 문자열이 포함 된 모든 사용자를 찾으려면 **%lori%** 합니다. **다음**을 클릭합니다.
...
패키지 및 프로그램 배포 섹션에서 만든 Windows PowerShell 구성 스크립트를 만들면 VPN 프로필을 받을 사용자 그룹을 만든 후 [ProfileXML 구성 파일을 만들](#create-the-profilexml-configuration-files)합니다.
...
4.  패키지 페이지에서 다음 단계를 수행 합니다.
    ...
    c. 원본 폴더 설정 대화 상자에서 클릭 **찾아보기**VPN_Profile.ps1, 포함 된 파일 공유를 선택 하 고 클릭 **확인**합니다.
        로컬 경로가 아닌 네트워크 경로 선택 했는지 확인 합니다. 즉, 경로와 같아야  *\\fileserver\\vpnscript*아니라 *c:\\vpnscript*합니다.
...
Windows 10 원격 액세스 Always On VPN 프로필을 배포 하려면 Intune을 사용 하려면 ProfileXML CSP 노드 섹션에서 만든 VPN 프로필을 사용 하 여 구성할 수 있습니다 [ProfileXML 구성 파일을 만들](#create-the-profilexml-configuration-files), 기본 EAP를 사용할 수 있습니다 아래에 제공 된 XML 샘플입니다.
...
1.   아래 **플랫폼**를 선택 **Windows 10 이상**를 선택 하 고 **VPN** 프로필 유형 드롭다운 목록에서.
...
2. 아래는 **기본 VPN** 탭, 확인 또는 다음 설정을 지정 합니다.
...
4.  대체는  **\<TrustedRootCA > 5a 89 fe cb 5b 49 a7 0b 1a 52 63 b7 35 ee d 7 1c c2 68 4b 수 <\/TrustedRootCA >** 온-프레미스 루트 인증 기관 인증서 지문 사용 하 여 샘플 양쪽 모두에서.
...
5.  대체는  **\<servernames에 대 > NPS.contoso.com\</ServerNames >** 인증이 발생 하는 도메인에 가입 된 NPS의 FQDN 사용 하 여 XML 샘플에서입니다. 

6.  수정 된 XML 문자열을 복사 및 붙여 합니다 **EAP Xml** 기본 VPN 탭 아래에서 상자 하 고 클릭 **확인**합니다.
    EAP를 사용 하 여 Always On VPN 장치 구성 정책에는 Intune에서 생성 됩니다.
...
|하려는 경우...  |다음을 참조 하는 중...  |
|---------|---------|
|VPN에 대 한 조건부 액세스를 구성 합니다.    |[7단계. (선택 사항) Azure AD를 사용 하 여 VPN 연결에 대 한 조건부 액세스를 구성](../../ad-ca-vpn-connectivity-windows10.md): 이 단계에서는 미세 조정할 수 있습니다 어떻게 권한이 부여 된 VPN 사용자 액세스를 사용 하 여 리소스 [Azure Active Directory (Azure AD) 조건부 액세스](https://docs.microsoft.com/azure/active-directory/active-directory-conditional-access-azure-portal)합니다. 가상 사설망 (VPN) 연결에 대 한 Azure AD 조건부 액세스를 사용 하 여 VPN 연결을 보호할 수 있습니다. 조건부 액세스는 Azure AD(Azure Active Directory)에 연결된 응용 프로그램에 대한 액세스 규칙을 만드는 데 사용되는 정책 기반 평가 엔진입니다.         |
