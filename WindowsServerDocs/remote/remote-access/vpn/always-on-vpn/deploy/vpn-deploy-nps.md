...
&#171;  [**다음:** 3단계. VPN에 대 한 원격 액세스 서버를 항상 구성](vpn-deploy-ras.md)<br>
&#187;  [**Next:** 5단계. DNS 및 방화벽 설정 구성](vpn-deploy-dns-firewall.md)


이 단계에서는 VPN 서버에서 전송 되는 연결 요청을 처리에 대 한 서버 NPS (네트워크 정책)를 설치 합니다.
...
1.  서버 관리자에서 클릭 **관리**, 클릭 **역할 및 기능 추가**합니다. <p>역할 및 기능 추가 마법사가 열립니다.
...
1.  설치 유형 선택에서 했는지 **역할 기반 또는 기능 기반 설치** 을 선택 하 고 클릭 **다음**합니다.

2.  선택 대상 서버에 있는지를 확인 **서버 풀에서 서버 선택** 을 선택 합니다.

3.  서버 풀에서 로컬 컴퓨터 선택 되어 있는지를 확인 하 고 클릭 **다음**합니다.

4.  서버 역할 선택 **역할**를 선택 **네트워크 정책 및 액세스 서비스**합니다. 네트워크 정책 및 액세스 서비스에 필요한 기능을 추가 해야 하는 경우 묻는 대화 상자가 열립니다.

5.  클릭 **기능 추가**를 클릭 하 고 **다음**

6.  기능 선택, 클릭 **다음**, 네트워크 정책 및 액세스 서비스에서 제공 된 정보를 검토 하 고 클릭 **다음**합니다.

7.  역할 선택 서비스를 클릭 **네트워크 정책 서버**합니다.

8.  네트워크 정책 서버에 필요한 기능에 대 한 클릭 **기능 추가** 클릭 **다음**합니다.

9.  설치 선택 확인, 클릭 **필요한 경우 자동으로 대상 서버를 다시 시작**합니다.

10. 클릭 **Yes** 선택한, 확인 하 고 클릭 한 다음 **설치**합니다. <p>설치 진행률 페이지는 설치 프로세스 중 상태를 표시합니다. 프로세스가 완료 되 면 메시지 "에서 설치가 완료 되었습니다 *ComputerName*" 표시 됩니다. 여기서 *ComputerName* 네트워크 정책 서버를 설치 하는 컴퓨터의 이름입니다.
...
2.  NPS 콘솔에서 마우스 오른쪽 단추로 클릭 **NPS (로컬)**, 클릭 **Active Directory에 서버 등록** 하 여 선택 합니다.<p>네트워크 정책 서버 대화 상자가 열립니다.

3.  네트워크 정책 서버 대화 상자에서 클릭 **확인** 두 번입니다.
...
2.  마우스 오른쪽 단추로 클릭 **RADIUS 클라이언트** 누릅니다 **새로 만들기**합니다. 새 RADIUS 클라이언트 대화 상자가 열립니다.

3.  있는지 확인 합니다 **이 RADIUS 클라이언트를 사용 하도록 설정** 확인란을 선택 합니다.

4.  **이름을**를 VPN 서버에 대 한 표시 이름을 입력 합니다.

5.  **주소 (IP 또는 DNS)**, NAS IP 주소 또는 FQDN을 입력 합니다.<p>FQDN을 입력 하는 경우 클릭 **확인** 이름을 올바르고 유효한 IP 주소로 매핑합니다 있는지를 확인 하려는 경우.
...
6.  **공유 비밀**를 수행 합니다.
    ...
    3.  공유 비밀 확인에서에서 공유 암호를 다시 입력 하십시오.

7.  **확인**을 클릭합니다. VPN 서버는 NPS 서버에 구성 RADIUS 클라이언트의 목록에 나타납니다.
...
2.  클릭 **VPN 또는 전화 접속 구성**합니다.<p>전화 접속 또는 VPN 구성 마법사가 열립니다.

3.  클릭 **가상 개인 네트워크 (VPN) 연결**, 클릭 **다음**합니다.

4.  지정 전화 접속 또는 VPN 서버에서 RADIUS 클라이언트에서 이전 단계에서 추가한 VPN 서버의 이름을 선택 합니다.<p>예를 들어 VPN 서버 NetBIOS 이름은 RAS1, 클릭 **RAS1**합니다.
...
6.  인증 방법 구성에서 다음 단계를 수행 합니다.
    ...
    2.  클릭 합니다 **Extensible Authentication Protocol** 확인란을 선택 합니다.

    3.  유형 (액세스 및 네트워크 구성 방법에 따라)에서 클릭 **Microsoft: 보호 된 EAP (PEAP)**, 클릭 **구성**합니다.<p>보호 된 EAP 속성 편집 대화 상자가 열립니다.

    4.  클릭 **제거** 보안 암호 (EAP-MSCHAP v2) EAP 종류를 제거 합니다.

    5.  **추가**를 클릭합니다. EAP 추가 대화 상자가 열립니다.

    6.  클릭 **스마트 카드 또는 기타 인증서**, 클릭 **확인**합니다.

    7.  클릭 **확인** 를 보호 된 EAP 속성 편집을 닫습니다.
...
8.  사용자 지정 그룹에서 다음 단계를 수행 합니다.
    ...
    2.  형식 **VPN 사용자** 누릅니다 **확인**합니다.

    3.  **다음**을 클릭합니다.

9.  IP 필터 지정에서 클릭 **다음**합니다.

10. 암호화 설정 지정, 클릭 **다음**합니다. 변경 하지 마십시오.<p>이러한 설정은이 시나리오를 지원 하지 않는 MPPE Microsoft 지점 간 암호화 () 연결에만 적용 됩니다.

11. 영역 이름 지정, 클릭 **다음**합니다.

12. **마침** 을 클릭하여 마법사를 닫습니다.
...
[5 단계입니다. Always On VPN에 대 한 DNS 및 방화벽 설정 구성](vpn-deploy-dns-firewall.md): 이 단계에서는 Windows PowerShell 또는 서버 관리자 추가 역할 및 기능 마법사 중 하나를 사용 하 여 네트워크 정책 (NPS 서버)를 설치 합니다. 모든 인증, 권한 부여 및 연결에 대 한 회계 업무를 처리 하는 NPS를 구성할 수도 있습니다는 VPN 서버에서 수신한 요청 합니다.
