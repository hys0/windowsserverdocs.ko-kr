...
스플릿 브레인 DNS 환경에서 리소스를 사용할 수의 두 버전 모두를 하려는 경우 인터넷에서 사용 되는 이름과 중복 되지 않는 이름의 인트라넷 리소스에 구성 합니다. 그런 다음 대체 이름을 사용 하 여 인트라넷의 리소스에 액세스할 때 사용자가 지시 합니다. 예를 들어 내부 이름 www\.contoso.com에 대해 www\.internal.contoso.com이라는 대체 이름을 구성합니다.  
