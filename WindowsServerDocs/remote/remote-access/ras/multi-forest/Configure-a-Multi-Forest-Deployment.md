...
2.  Forest2의 DNS 접미사가 Forest1의 DNS 접미사의 일부가 아니면 Forest2의 클라이언트 도메인 접미사에 NRPT 규칙을 추가 하 여 인증을 위해 도메인 컨트롤러에 액세스할 수 있도록 하 고, 선택적으로 Forest2의 도메인 접미사를 DNS suf에 추가 합니다. 검색 목록을 수정 합니다. 
...
## <a name="OTPMultiForest"></a>다중 포리스트 배포에서 OTP 구성  
...
이 절차에는 PowerShell 스크립트인 PKISync.ps1이 필요합니다. @No__t-0AD CS: 크로스 포리스트 인증서 등록 @ no__t-0에 대 한 Pkisync.ps1 스크립트  
...
### <a name="BKMK_CertPub"></a>Ca를 인증서 게시자로 구성  
...
4.  관리자 권한 명령 프롬프트에서 다음 명령을 실행하여 루트 CA 인증서를 추출합니다.  
    ...
    2.  리소스 포리스트 인증서 템플릿에 대 한 읽기/쓰기 권한을 \<Account 포리스트 @ no__t-1 @ no__t-2 < Administrator account @ no__t-3에 부여 합니다.  
...
### <a name="BKMK_DelImp"></a>OTP 인증서 템플릿 삭제 및 가져오기  
...
### <a name="BKMK_Extract"></a>CA 추출 및 동기화  
...
### <a name="NRPT_DNSSearchSuffix"></a>NRPT 규칙 및 DNS 접미사 추가  
