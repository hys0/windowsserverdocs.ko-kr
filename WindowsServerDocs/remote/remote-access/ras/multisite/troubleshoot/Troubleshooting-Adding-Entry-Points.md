...
**오류가 수신**되었습니다. <로 구성 되지 않은 >에서 원격 액세스가 구성 되지 않았습니다. 멀티 사이트 배포에 속하는 서버 이름을 지정하십시오.  
  
**가능한 원인**  
  
*ComputerName* 매개 변수로 지정된 컴퓨터 또는 명령을 수행하는 컴퓨터에 원격 액세스가 구성되지 않았습니다.  
  
새 진입점을 멀티 사이트 배포에 추가 하는 경우 두 개의 매개 변수를 지정 해야 합니다. *ComputerName* 및 *RemoteAccessServer*. *ComputerName*은 이미 멀티 사이트 배포의 일부인 서버의 이름이며 *RemoteAccessServer*는 새로운 진입점으로서 추가하고자 하는 서버의 이름입니다. 멀티 사이트 배포의 일부인 컴퓨터에서 실행 중이라면 ComputerName 매개 변수는 필요하지 않습니다.  
...
**오류가 수신**되었습니다. DirectAccess 클라이언트가 RemoteAccess 서버에서 연결 하는 주소 (< 주소 >)는 네트워크 위치 서버 주소와 동일 합니다. 다른 값을 지정하십시오.  
...
**오류가 수신**되었습니다. 서버 > < 서버에서 VPN 설치가 검색 되었습니다. 원격 액세스가 설치되지 않은 다른 서버를 지정하거나 서버에서 VPN 구성을 제거하십시오.  
  
또는  
  
서버 < 서버 \ >에 원격 액세스가 이미 설치 되어 있습니다. DirectAccess를 실행하지 않는 다른 서버를 지정하거나 서버에서 기존 DirectAccess 구성을 제거하십시오.  
...
**오류가 수신**되었습니다. 구성 된 IPsec 루트 인증서를 서버 < 서버 없는 >에서 찾을 수 없습니다.  
...
> [!NOTE]  
> 인증서는 동일한 지문을 사용 하는 동일한 인증서 여야 합니다.  
...
-   **문제 1**  
    ...
    **해결 방법**  
    ...
    전체 내부 네트워크가 IPv6 및 IPv4 주소를 사용하여 구성되었다면 IPv6 기술의 이점을 누릴 수 있게 IPv6+IPv4 배포로 전환하는 것을 고려해볼 수 있습니다. @No__t-0Step 3의 "순수한 IPv4에서 IPv6 + IPv4 회사 네트워크로 전환"을 참조 하세요. 멀티 사이트 배포를 계획 @ no__t-0.  
...
-   **문제 1**  
    ...
    **오류가 수신**되었습니다. ServerGpoName 매개 변수 < server_GPO >에 지정 된 도메인이 없습니다. 대신 도메인 < domain_name >를 지정 합니다.  
...
-   **문제 2**  
    ...
    **오류가 수신**되었습니다. 서버 GPO는 원격 액세스 서버 도메인에 위치 해야 합니다. ServerGpoName 매개 변수에서 도메인 < domain_name >를 지정 합니다.  
...
**경고가 수신**되었습니다. DNS_suffix > < DNS 접미사에 대 한 NRPT 항목에는 클라이언트 컴퓨터에서 원격 액세스 서버에 연결 하는 데 사용 하는 공개 이름이 포함 됩니다. 주소 > < 이름을 NRPT의 예외로 추가 합니다.  
...
**경고가 수신**되었습니다. GPO 업데이트는 <에서 사용할 수 없는 >에 적용할 수 없습니다. 변경 사항이 다음 번에 정책을 다시 새로 고칠 때까지 적용되지 않습니다.  
