...
서버 > < 서버에서 원격 액세스가 구성 되지 않았습니다. DirectAccess를 사용하는 컴퓨터를 지정하십시오.  
...
**오류가 수신**되었습니다. 도메인 컨트롤러 < domain_controller >은 (는) 읽기 전용입니다. 읽기 전용이 아닌 도메인 컨트롤러를 지정하십시오.  
...
-   **문제 1**  
    ...
    **오류가 수신**되었습니다. 도메인 컨트롤러 < previous_domain_controller >의 GPO < GPO_name >는 동일한 도메인에 있지 않기 때문에 도메인 컨트롤러 < replacement_domain_controller >에서 검색할 수 없습니다.  
...
-   **문제 2**  
    ...
    **오류가 수신**되었습니다. 도메인 컨트롤러 < previous_domain_controller >의 GPO < GPO_name >를 도메인 컨트롤러 < replacement_domain_controller >에서 검색할 수 없습니다. 도메인 복제가 완료될 때까지 기다린 다음 다시 시도하십시오.  
...
**오류가 수신**되었습니다. 진입점 < entry_point_name >는 멀티 사이트 배포의 일부가 아닙니다. 다른 값을 지정하십시오.  
...
-   **문제 1**  
    ...
    **오류가 수신**되었습니다. 진입점 < entry_point_name >에서 서버 < 서버를 >에 액세스할 수 없습니다.  
...
-   **문제 2**  
    ...
    **오류가 수신**되었습니다. 진입점 < entry_point_name >에서 서버 < 서버 >의 레지스트리에 대 한 설정을 저장할 수 없습니다.  
...
-   **문제 3**  
    ...
    **오류가 수신**되었습니다. GPO 업데이트는 <에서 사용할 수 없는 >에 적용할 수 없습니다. 변경 사항이 다음 번에 정책을 다시 새로 고칠 때까지 적용되지 않습니다.  
