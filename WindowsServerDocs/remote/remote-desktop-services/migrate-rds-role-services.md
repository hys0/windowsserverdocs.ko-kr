...
> [!IMPORTANT]
> 이전 단계를 완료 한 후에 세션 컬렉션을 마이그레이션하세요 [마이그레이션하려면 RD 연결 브로커 서버](#migrate-rdconnection-broker-servers)합니다.
...
> [!IMPORTANT]
> 이전 단계를 완료 한 후에 가상 데스크톱 컬렉션을 마이그레이션하세요 [마이그레이션하려면 RD 연결 브로커 서버](#migrate-rdconnection-broker-servers)합니다.
...
- 원격 데스크톱 서비스 배포에서 원본 서버를 제거 합니다.
