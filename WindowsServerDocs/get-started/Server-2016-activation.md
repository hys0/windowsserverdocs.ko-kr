...
다음 정보는 Windows Server 2016과 관련된 KMS(키 관리 서비스) 정품 인증을 확인해야 할 때 고려해야 하는 초기 계획 사항에 대한 요약입니다. 여기에 나열 된 이전 운영 체제를 포함 하는 KMS 정품 인증에 대 한 정보를 참조 하세요. [1 단계: 검토 하 고 정품 인증 방법 선택](https://technet.microsoft.com/library/jj134256(WS.11).aspx)합니다.
...
컴퓨터를 KMS 호스트, MAK 또는 Windows 정품 버전에서 KMS 클라이언트로 변환하는 경우 해당하는 KMS 클라이언트 설정 키를 설치합니다. 자세한 내용은 [KMS 클라이언트 설정 키](KMSclientkeys.md)합니다. 
