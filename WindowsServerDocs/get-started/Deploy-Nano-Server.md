...
또한 찾기 하 고 PackageManagement (OneGet) PowerShell 모듈의 NanoServerPackage 공급자를 사용 하 여 이러한 패키지를 설치할 수 있습니다. 이 토픽의 "온라인으로 역할 및 기능 설치" 섹션을 참조하세요.  
...
|System Center Operations Manager 에이전트| 별도 설치되었습니다. System Center Operations Manager 설명서에 대 한 자세한 내용은 참조 https://technet.microsoft.com/system-center-docs/om/manage/install-agent-on-nano-server.|
...
### -Clustering 매개 변수로 설치된 장애 조치(Failover) 클러스터링 항목
...
### -Storage 매개 변수로 설치된 파일 및 저장소 항목
...
3.  \\\Path\To\Media\server_en-us에서 .\Base로 설치 미디어 복사  
...
`psEdit C:\Windows\Logs\DISM\dism.log` 를 실행하거나 `psEdit .\myScript.ps1`  
...
`Find-Package -ProviderName NanoServerPackage -Culture en-us` `Find-Package -Culture en-us -DisplayCulture`를 사용할 수 있습니다.
...
`Find-NanoServerPackage *dcb* | Install-NanoServerPackage` 이름에 "dcb"가 포함된 패키지를 찾아서 설치합니다.

`Find-Package *nanoserver-compute-* | Install-Package` 이름에 "nanoserver-compute-"가 포함된 패키지를 찾아서 설치합니다.
  
`Find-NanoServerPackage -Name *nanoserver-compute* | Install-NanoServerPackage -ToVhd C:\MyNanoVhd.vhd` 이름에 "compute"가 포함된 패키지를 찾아서 오프라인 이미지에 설치합니다.

`Find-Package -ProviderName NanoserverPackage *nanoserver-compute-* | Install-Package -ToVhd C:\MyNanoVhd.vhd` 이름에 "nanoserver-compute-"가 포함된 패키지와 똑같은 방식으로 작동합니다.
...
`Save-Package -ProviderName NanoServerPackage -Name Microsoft-NanoServer-IIS-Package -Path C:\ -Culture it-IT -MinimumVersion 10.0.14393.0` 특정 버전을 이탈리아 언어 및 로캘로 저장합니다.
...
5.  다음 명령을 실행하여 패키지와 관련 언어 팩이 올바르게 설치되었는지 확인합니다.  
    ...
    **dism /online /get-packages**  
    ...
    "패키지 ID: Microsoft-NanoServer-IIS-Package~31bf3856ad364e35~amd64~en-US~10.0.10586.0"이 두 번 표시되어야 합니다. 한 번은 릴리스 종류: 언어 팩에 대해, 한 번은 릴리스 종류: 기능 팩에 대해 표시됩니다.  
  
## 기존 Nano 서버 VHD 사용자 지정  
...
## New-NanoServerImage 및 Edit-NanoServerImage로 할 수 있는 추가 작업  
...
### 이미지 생성의 일환으로 사용자 지정 PowerShell 스크립트 실행
...
1. WSA 패키지를 만드는 데 필요한 MakeAppx, MakeCert, Pvk2Pfx, SignTool 도구가 포함되어 있는 [Windows 10 SDK](https://developer.microsoft.com/windows/downloads/windows-10-sdk)를 설치합니다.
...
7. 보안을 위해 이 명령을 사용하여 신뢰할 수 있는 호스트 목록에서 Nano 서버를 제거합니다.
...
"odjblob" 파일을 열고(메모장에서), 그 내용을 복사한 다음 아래 Unattend.xml 파일의 \<AccountData> 섹션에 붙여 넣습니다.  
...
## 부록: Nano 서버를 도메인에 가입하는 샘플 Unattend.xml 파일  
