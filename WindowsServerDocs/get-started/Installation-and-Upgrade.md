...
ms.locfileid: "6121482"
---
# Windows Server 설치 및 업그레이드

>적용 대상: Windows Server 2016, Windows Server 2012 R2, Windows Server 2012, Windows Server 2008 R2, Windows Server 2008

> [!IMPORTANT]
> Windows Server 2008 R2 및 Windows Server 2008에 대한 연장 지원이 2020년 1월에 종료됩니다. [업그레이드 옵션에 자세히 알아보세요](#upgrading-from-windows-server-2008-r2-or-windows-server-2008).
...
## 마이그레이션(Windows Server 2016에 권장)
...
Windows Server 2016 Standard 평가판을 Windows Server 2016 Standard 일반 정품 버전 또는 Datacenter 일반 정품 버전으로 변환할 수 있습니다.

Windows Server 2016 Datacenter 평가판을 Windows Server 2016 Datacenter 일반 정품 버전으로 변환할 수 있습니다.
 
## Windows Server 2012 R2로 업그레이드
...
|Windows Server2008R2 Enterprise sp1|Windows Server 2012 R2 Standard 또는 Windows Server 2012 R2 Datacenter|
|Windows Server2008R2 Standard sp1|Windows Server 2012 R2 Standard 또는 Windows Server 2012 R2 Datacenter|
|Windows 웹 Server2008R2 sp1|Windows Server 2012 R2 Standard|
...
|Windows Server2008R2 Standard SP1 또는 Windows Server2008R2 Enterprise sp1|Windows Server 2012 Standard, Windows Server 2012 Datacenter|
|Windows Server2008R2 Datacenter sp1|Windows Server 2012 Datacenter|
|Windows 웹 Server2008R2|Windows Server 2012 Standard|
...
## Windows Server 2008 R2 또는 Windows Server 2008에서 업그레이드

[업그레이드 Windows Server 2008 및 Windows Server 2008 r 2에서](modernize-windows-server-2008.md)설명한 대로 Windows Server 2008 R2/Windows Server 2008에 대 한 연장된 지원이 2020 년 1 월 끝납니다. 간격 없는 지원에 위해, Windows Server의 지원 되는 버전으로 업그레이드 하거나 [특수 Windows Server 2008 R2 Vm](uploading-specialized-WS08-image-to-azure.md)으로 이동 하 여 Azure에서 다시 호스팅하 해야 합니다. 마이그레이션/업그레이드를 계획에 대 한 고려 사항 및 정보에 대 한 [Windows Server에 대 한 마이그레이션 가이드를](https://go.microsoft.com/fwlink/?linkid=872689) 확인 합니다.

온-프레미스 서버에 대 한 Windows Server 2008 R2 또는 Windows Server 2016 이상에서 직접 업그레이드 경로가 없는 경우. 대신, Windows Server 2012 R2 및 다음 [Windows Server 2016으로 업그레이드](#Upgrading-to-Windows-Server-2016)하려면 먼저 업그레이드 합니다.

업그레이드를 계획 하는 대로 Windows Server 2012 r 2로 업그레이드 하는 중간 단계에 대 한 다음 지침에 주의 합니다.

  - 32 비트에서 64 비트 아키텍처로의 현재 위치 업그레이드를 수행 하거나 하나에서 다른 (예: 검사를 fre) 형식 빌드 수 없습니다.

  - 전체 업그레이드는 동일한 언어로 지원 됩니다. 다른 언어에서 업그레이드할 수 없습니다.

  - (Windows Server의 "전체 데스크톱으로 서버" 이라고 함) 서버 GUI를 사용 하 여 Windows Server 2012 r 2를 Windows Server 2008 server core 설치에서 마이그레이션할 수 없습니다. Windows Server 2012 r 2 에서만 전체 바탕 화면을 사용 하 여 서버에 업그레이드 된 server core 설치를 전환할 수 있습니다. Windows Server 2016 및 이후 *하지 않는* server core에서 전체 바탕 화면으로 전환 하는 지원 Windows Server 2016으로 업그레이드 하기 전에 해당 스위치를 확인 하므로 합니다.
  
자세한 내용은 체크아웃 [평가 버전 및 Windows에 대 한 옵션 업그레이드 Server 2012](https://docs.microsoft.com/previous-versions/windows/it-pro/windows-server-2012-R2-and-2012/jj574204\(v=ws.11\)), 역할별 업그레이드 세부 정보를 포함 합니다.
