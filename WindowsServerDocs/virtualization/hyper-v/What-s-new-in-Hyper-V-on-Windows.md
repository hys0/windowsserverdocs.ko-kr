...
이 문서에서는 Windows Server 2019, Windows Server 2016 및 Microsoft Hyper-v Server 2016에서 Hyper-v의 새로운 기능과 변경 된 기능을 설명 합니다. Windows Server 2012 R2를 사용 하 여 만든 및 이동 하거나 가져온 Windows Server 2019에서 Hyper-v를 실행 하는 서버 또는 Windows Server 2016 가상 머신에서 새 기능을 사용 하려면 가상 머신 구성 버전을 수동으로 업그레이드 해야 합니다. 자세한 내용은 [가상 컴퓨터를 업그레이드 버전](deploy/Upgrade-virtual-machine-version-in-Hyper-V-on-Windows-or-Windows-Server.md)합니다.  
  
다음은이 문서에 포함 된 항목 및 기능은 새로운 또는 업데이트 된 여부입니다.  

## <a name="windows-server-version-1903"></a>Windows Server 버전 1903

### <a name="add-hyper-v-manager-to-server-core-installations-updated"></a>(업데이트)는 Server Core 설치에 Hyper-v 관리자를 추가 합니다.
...
이 버전의 응용 프로그램 호환성 기능에 따라서 도구를 하나 더 추가한 고객 피드백에 따라: Hyper-v 관리자 (virtmgmt.msc)입니다.

자세한 내용은 참조 하세요. [Server Core 응용 프로그램 호환성 기능](../../get-started-19/install-fod-19.md)합니다.

## <a name="windows-server-2019"></a>Windows Server 2019

### <a name="security-shielded-virtual-machines-improvements-new"></a>보안: (신규) 보호 된 가상 컴퓨터 기능 향상
...
### <a name="compatible-with-connected-standby-new"></a>연결 된 대기 상태와 호환 \(새\)
...
### <a name="host-resource-protection-new"></a>호스트 리소스 보호 \(새\)
...
### <a name="hot-add-and-remove-for-network-adapters-and-memory-new"></a>핫 추가 및 네트워크 어댑터 및 메모리에 대 한 제거 \(새\)
...
### <a name="hyper-v-manager-improvements-updated"></a>Hyper-v 관리자 향상 \(업데이트\) 
...
-   **이전 버전 관리** -Hyper-v 관리자와 Windows Server 2019, Windows Server 2016 및 Windows 10, Windows Server 2012, Windows 8, Windows Server 2012 R2 및 Windows 8.1 Hyper-v를 실행 하는 컴퓨터를 관리할 수 있습니다.  
...
### <a name="integration-services-delivered-through-windows-update-updated"></a>Integration services Windows Update를 통해 전달 \(업데이트\) 
...
-   이상으로 실행 하려면 Windows Server 2019, Windows Server 2016 또는 Windows 10 실제 Hyper-v 호스트와 가상화 된 호스트에서.  
...
### <a name="rolling-hyper-v-cluster-upgrade-new"></a>Hyper-v 클러스터 업그레이드 롤링 \(새\)

이제 Windows Server 2012 R2를 실행 하는 노드를 사용 하 여 Hyper-v 클러스터에 Windows Server 2019 또는 Windows Server 2016을 실행 하는 노드를 추가할 수 있습니다. 이 옵션을 사용 하면 가동 중지 시간 없이 클러스터를 업그레이드할 수 있습니다. 클러스터의 모든 노드를 업그레이드 하 고 Windows PowerShell cmdlet으로 클러스터 기능 수준을 업데이트 될 때까지 Windows Server 2012 R2 기능 수준에서 실행 되는 클러스터 [업데이트 ClusterFunctionalLevel](https://docs.microsoft.com/powershell/module/failoverclusters/Update-ClusterFunctionalLevel)합니다.  
  
> [!IMPORTANT]  
> 클러스터 기능 수준을 업데이트 한 후 Windows Server 2012 r 2로 반환할 수 없습니다.  
  
Windows Server 2012 R2를 실행 하는 노드를 사용 하 여 Windows Server 2012 R2의 기능 수준 사용 하 여 Hyper-v 클러스터의 경우 Windows Server 2019 및 Windows Server 2016 다음 사항에 유의 합니다.  
  
-   Windows Server 2016 또는 Windows 10을 실행 하는 노드에서 클러스터, Hyper-v 및 가상 컴퓨터를 관리 합니다.  
  
-   모든 Hyper-v 클러스터의 노드 간에 가상 컴퓨터를 이동할 수 있습니다.  
  
-   모든 노드에서 새 Hyper-v 기능을 사용 하려면 Windows Server 2016을 실행 해야 하거나 클러스터 기능 수준을 업데이트 해야 합니다.  
...
### <a name="shared-virtual-hard-disks-updated"></a>공유 가상 하드 디스크가 \(업데이트\)
...
### <a name="shielded-virtual-machines-new"></a>보호 된 가상 머신을 \(새\)
...
### <a name="start-order-priority-for-clustered-virtual-machines-new"></a>클러스터 된 가상 컴퓨터에 대 한 우선 순위 순서를 시작 \(새\)
...
.  
### <a name="storage-quality-of-service-qos-updated"></a>저장소 서비스 품질 (QoS) \(업데이트\)
...
### <a name="virtual-machine-configuration-file-format-updated"></a>가상 머신 구성 파일 형식을 \(업데이트\)
