...
description: 클러스터 되지 않은 환경의 실시간 마이그레이션 설정에 대 한 지침을 제공 합니다.
...
- 원본 및 대상 서버에 설치 된 Windows Server 2012 R2 또는 Windows Server 2016의 Hyper-v 역할입니다. 가상 컴퓨터 하나 이상 있으면 Windows Server 2016 및 Windows Server 2012 R2를 실행 하는 호스트 간에 실시간 마이그레이션을 할 수 있는 버전 5입니다. <br>버전 업그레이드 지침은 [Windows 10 또는 Windows Server 2016에서 Hyper-v에서 가상 머신을 업그레이드 버전](Upgrade-virtual-machine-version-in-Hyper-V-on-Windows-or-Windows-Server.md)합니다. 설치 지침은 [Windows 서버에 Hyper-v 역할 설치](../get-started/Install-the-Hyper-V-role-on-Windows-Server.md)합니다.  
  
- 동일한 Active Directory 도메인에 속하거나 서로 신뢰 하는 도메인에 속해 있는 원본 및 대상 컴퓨터.    
- 도구는 원본 또는 대상 서버에 설치 되어 있지 않으면 Windows Server 2016 또는 Windows 10을 실행 하는 컴퓨터에 설치 된 Hyper-v 관리 도구 서버에서 도구를 실행 합니다.  
  
## <a name="consider-options-for-authentication-and-networking"></a>인증 및 네트워킹에 대 한 옵션을 고려  
  
다음을 설정 하려면 어떻게 하는 것이 좋습니다.  
  
-  **인증**: 원본 및 대상 서버 간의 실시간 마이그레이션 트래픽을 인증 하는 프로토콜이 사용 됩니다. 선택 하는 실시간 마이그레이션을 시작 하기 전에 원본 서버에 로그온 해야 하는지 여부를 결정 합니다.   
...
-   **성능**: 것이 합리적 성능 옵션을 구성 하 시겠습니까? 이러한 옵션 수 네트워크 및 CPU 사용량을 줄일 뿐 실시간 마이그레이션 속도 더 빠르게 확인 하십시오. 요구 사항 및 인프라를와 고려 결정 하려면 서로 다른 구성을 테스트 합니다. 옵션 2 단계 후에 설명 되어 있습니다.  
  
-  **기본 설정 네트워크**: 사용 가능한 네트워크를 통해 실시간 마이그레이션 트래픽을 허용할 것입니까, 아니면 특정 네트워크로 트래픽을 격리할 것입니까? 보안을 유지하는 가장 좋은 방법으로 실시간 마이그레이션 트래픽은 네트워크를 통해 전송될 때 암호화되지 않으므로 트래픽을 신뢰할 수 있는 프라이빗 네트워크로 격리하는 것이 좋습니다. 네트워크 격리는 물리적으로 떨어진 네트워크나 VLAN 등의 또 다른 신뢰할 수 있는 네트워킹 기술을 통해 구현할 수 있습니다.  
  
## <a name="BKMK_Step1"></a>1 단계: (선택 사항) 제한 된 위임 구성  
...
5.  위임 탭에서 선택 **만 지정한 서비스에 대 한 위임용으로이 컴퓨터 트러스트** 선택한 후 **모든 인증 프로토콜을 사용 하 여**입니다.  
...
둘 중 발생 한 후 구성 변경 내용을 적용 합니다.  
  ...
  -  도메인 컨트롤러에는 새 Kerberos 티켓을 발급합니다.  
  
## <a name="BKMK_Step2"></a>2 단계: 실시간 마이그레이션 위한 원본 및 대상 컴퓨터 설정  
...
클러스터 되지 않은 호스트에서 실시간 마이그레이션을 구성 하는 데 사용할 수 있는 세 가지 cmdlet: [Enable-vmmigration](https://technet.microsoft.com/library/hh848544.aspx)하십시오 [Set-vmmigrationnetwork](https://technet.microsoft.com/library/hh848467.aspx), 및 [Set-vmhost](https://technet.microsoft.com/library/hh848524.aspx)합니다. 이 예제에서는 세 가지 모두를 사용 하 여 하 고 다음 작업을 수행 합니다.   
...
|----------|---------------|  
    ...
    |압축|TCP/IP 연결을 통해 대상 서버로 복사 하기 전에 가상 컴퓨터의 메모리 내용이 압축 합니다. **참고:** 이 **기본** 설정 합니다.|  
