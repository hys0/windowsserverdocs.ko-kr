...
또는 실행할 수 있습니다 분석기 BPA (모범 사례) 검색 하거나 서버 관리자에서 BPA GUI를 사용 하 여 Windows PowerShell에서 cmdlet을 사용 하 여 합니다. Windows Server 2012 부터는 검사할 수 있습니다 하나 역할 여러 역할 한 번에 여러 서버에서 모범 사례 분석기 타일을 사용 하 여 서버 관리자 콘솔에서 또는 Windows PowerShell cmdlet을 실행 하려면 검색 합니다. 확인하지 않을 검사 결과는 제외하거나 무시하도록 BPA에 지정할 수도 있습니다.
...
-   [역할에서 검사 Best Practices Analyzer를 수행 합니다.](#BKMK_BPAscan)
...
## <a name="BKMK_how"></a>BPA의 작동 원리
BPA는 효율성, 신뢰성 및 안정성의 모범 사례 규칙 8 가지 다른 범주에 있는 역할의 호환성을 측정 하 여 작동 합니다. 측정 결과는 다음 표에 설명된 세 개의 심각도 수준 중 하나일 수 있습니다.

|심각도|설명|
|---------|--------|
|Error|오류 결과는 역할이 모범 사례 규칙 조건을 충족하지 않을 때 반환되며 기능상의 문제가 예상될 수 있습니다.|
|정보|역할이 모범 사례 규칙 조건을 충족할 경우 정보 결과가 반환됩니다.|
|경고|경고 결과는 변경 작업을 수행하지 않을 경우 규칙 위반으로 인해 문제가 발생될 수 있을 때 반환됩니다. 응용 프로그램이 현재 작동하면서 정책을 준수하고 있지만 해당 구성이나 정책 설정을 변경하지 않으면 규칙 조건을 충족하지 않을 수도 있습니다. 예를 들어 라이선스 서버를 역할에 사용할 수 없는 경우 원격 데스크톱 서비스 검사에서 경고 결과를 표시할 수 있는데, 이는 검사 시에 활성 상태인 원격 연결이 없더라도 라이선스가 서버가 없으면 유효한 클라이언트 라이선스를 새 원격 연결에서 획득할 수 없기 때문입니다.|
...
|정책|정책 규칙은 최적으로, 안전 하 게 작동 하는 역할에 대 한 수정이 필요할 수 있는 그룹 정책 또는 Windows 레지스트리 설정을 식별에 적용 됩니다.|
...
## <a name="BKMK_BPAscan"></a>역할에서 검사 Best Practices Analyzer를 수행 합니다.
수행할 수 있습니다 BPA 검사 역할 중 하나를 사용 하 여 BPA GUI를 서버 관리자에서 또는 Windows PowerShell cmdlet을 사용 하 여.
...
> [!NOTE]
> 이 섹션의 절차에는 모든 BPA cmdlet 및 매개 변수가 나와 있지 않습니다. Windows PowerShell 세션에서 Windows PowerShell에서 BPA 작업에 대 한 자세한 정보 입력 **get-help***BPACmdlet***-전체**여기서 *BPACmdlet* 하나일 수 있습니다 다음 값입니다. BPA cmdlet 도움말 항목을 찾을 수도 있습니다는 [Windows Server TechCenter](https://go.microsoft.com/fwlink/p/?LinkId=240177)합니다.
...
1.  관리자 권한으로 Windows PowerShell을 실행 하려면 다음 중 하나를 수행 합니다.
    ...
    -   관리자 권한으로 Windows PowerShell을 실행 하는 **시작** 화면에서 마우스 오른쪽 단추로 클릭 합니다 **Windows PowerShell** 타일를 **앱** 결과 및 다음 앱 바에서를 클릭 **관리자 권한으로 실행**합니다.
...
2.  Windows PowerShell 3.0 부터는 cmdlet 모듈을 자동으로 가져온 모듈에서 cmdlet을 사용 하는 처음으로 Windows PowerShell 세션입니다. 따라서 BPA cmdlet 모듈을 가져오거나 로드할 필요가 없습니다.

3.  bpa 검사를 수행할 수를 입력 하 여 모든 역할의 모델 id를 검색 합니다 **Get-bpamodel** , 다음 예제에서와 같이 cmdlet.
...
5.  다음 명령 중 하나를 입력하여 특정 역할에 대한 BPA 검사를 시작합니다. 역할이 여러 개인 경우에는 쉼표로 모델 ID를 구분합니다.
    ...
    **예:**`Invoke-BPAmodel -modelId Microsoft/Windows/DNSServer,Microsoft/Windows/FileServices`
...
#### <a name="BKMK_allroles"></a>Windows PowerShell cmdlet을 사용 하 여 모든 역할을 검사 하려면
...
GUI에서 BPA 검사를 완료하고 나면 BPA 타일에서 검사 결과를 볼 수 있습니다. 타일에 표시된 결과를 선택하면 역할이 관련 모범 사례를 준수하는지를 나타내는 등 결과 속성이 타일 내 미리 보기 창에 표시됩니다. 결과 규격이 아닙니다. 결과 속성에 설명 된 문제를 해결 하는 방법을 알고 싶은 경우 오류 및 경고 결과 속성의 하이퍼링크는 Windows Server TechCenter에서 자세한 해결 방법 도움말 항목을 엽니다.
...
BPA 검사에서 자주 발생 하지만 해결 하는 결과 같은 일부 BPA 결과 확인 해야 하는 경우에 Windows PowerShell에서 BPA GUI 또는 BPA cmdlet을 사용 하 여 결과 제외할 수 있습니다. 결과는 언제든 다시 포함할 수 있습니다.
...
#### <a name="BKMK_exclude"></a>검사 결과 제외 합니다.
...
2.  다음 명령을 실행하여 모델 검사에서 특정 결과를 제외합니다.
    ...
    앞의 명령을 나타내는 모델 ID에 대 한 BPA 검사 결과 항목을 검색 *모델 ID*합니다.
    ...
    **예:**`Get-BPAResult -Microsoft/Windows/FileServices | Where { $_.Severity -eq "Information"} | Set-BPAResult -Exclude $true`
...
##### <a name="BKMK_gui"></a>GUI를 사용 하 여 검색 결과 포함 하려면
...
##### <a name="BKMK_cmdlets"></a>Windows PowerShell cmdlet을 사용 하 여 검색 결과 포함 하려면
...
2.  다음 명령을 입력한 다음 **Enter** 키를 눌러 모델 검사에서 특정 결과를 포함합니다.
    ...
    가 나타내는 모델에 대 한 BPA 검사 결과 항목을 검색 하는 앞의 명령을 *모델 Id*합니다.
    ...
    **예:**`Get-BPAResult -Microsoft/Windows/FileServices | Where { $_.Severity -eq "Information"} | Set-BPAResult -Exclude $false`
...
#### <a name="BKMK_recentPS"></a>Windows PowerShell을 사용 하 여 역할의 최신 검사 결과 보려면

1.  관리자 권한으로 Windows PowerShell 세션을 엽니다.

2.  지정된 모델 ID에 대한 최신 검사 결과를 가져옵니다. 모델은 표현에 다음을 입력 *모델 ID*, 누릅니다 **Enter**합니다. 쉼표로 모델 ID를 구분하여 여러 모델 ID에 대한 결과를 얻을 수 있습니다.

    `Get-BPAResult <model ID>`

    **예:** `Get-BPAResult Microsoft/Windows/DNSServer,Microsoft/Windows/FileServices`

    모델의 서브 모델을 검색 하는 경우 역할 서비스와 같은 결과를 가져옵니다 해당 서브 모델에 대 한 cmdlet에 서브 모델 ID를 포함 하 여.

    **예:** `Get-BPAResult Microsoft/Windows/FileServices -SubmodelID FSRM`

#### <a name="BKMK_formats"></a>보거나 다른 형식으로 Windows PowerShell 세션에서 BPA 결과 저장 하려면
...
-   Windows PowerShell에서 각 BPA 결과 다음과 같습니다.
    ...
    -   BPA 결과를 표 형식으로 나타내려면 다음의 cmdlet과 같이 이전 예에서 보려는 결과 속성을 추가하여 실행합니다.
        ...
        **예:**`Get-BPAResult Microsoft/Windows/FileServices | format-Table -Property modelId,SubmodelId,computerName,Source,Severity,Category,Title,Problem,Impact,Resolution,compliance,help`
    ...
    -   BPA 결과를 보관 하거나 전자 메일 받는 사람, 다음 cmdlet을 실행을 보낼 수 있는 HTML 파일로 내보내려면 위치 *경로* 는 HTML 결과가 저장 하려면 경로 파일 이름을 나타냅니다.

        `Get-BPAResult <model ID> | convertTo-Html | Set-Content <path>`

        **예:**`Get-BPAResult Microsoft/Windows/FileServices | convertTo-Html | Set-Content C:\BPAResults\FileServices.htm`

    -   다음 cmdlet을 실행 하는 쉼표로 구분 된 값 (CSV) 텍스트 파일을 BPA 결과 내보낼 위치 *경로* 는 CSV 결과 저장 하려면 경로 파일 이름을 나타냅니다. CSV 결과 Microsoft Excel 또는 데이터가 스프레드시트나 그리드 표시 하는 다른 프로그램으로 가져올 수 있습니다.

        `Get-BPAResult <model ID> | Export-CSV <path>`

        **예:**`Get-BPAResult Microsoft/Windows/FileServices | Export-CSV C:\BPAResults\FileServices.txt`

## <a name="see-also"></a>관련 항목
[모범 사례 분석기 해결 방법 콘텐츠는 Windows Server TechCenter](https://go.microsoft.com/fwlink/p/?LinkId=241597)
[필터, 정렬 및 서버 관리자 타일에서 데이터를 쿼리 하](filter-sort-and-query-data-in-server-manager-tiles.md)
[Manage Multiple, remote Servers 서버 관리자를 사용 하 여](manage-multiple-remote-servers-with-server-manager.md)
