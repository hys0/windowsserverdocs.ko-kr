...
서버 관리자 콘솔이 원격 서버 관리 도구의 일부로 클라이언트 컴퓨터에서 실행 되 고 있는지 없는 **로컬 서버** 미리 보기.  

미리 보기에는 역할, 서버 및 서버 그룹의 상태와 관리 효율성이 간략하게 표시됩니다. 미리 보기 제목 행의 색상이 변경 (및 강조 표시 된 숫자가 왼쪽된 여백에 표시 됩니다) 때 이벤트, 성능 카운터, 모범 사례 분석기 결과, 서비스 또는 일반적인 관리 효율성 문제가 를구성하는조건을충족**자세히 보기** 미리 보기 행을 클릭 하 여 대화 상자가 열립니다. 다음 표에서는 미리 보기에 표시되는 데이터에 대해 설명합니다.  
...
|이벤트|지정한 심각도 수준, 원본, 기간, 서버 또는 이벤트 ID와 일치하는 이벤트가 기록될 때 경고를 표시하도록 미리 보기의 **이벤트** 행을 구성할 수 있습니다. 이벤트에 대 한 세부 정보를 확인 하 고 클릭 하 여 확인 하려는 경고를 변경 합니다 **이벤트** 행 및 열을 **이벤트 자세히 보기** 역할이 나 서버 그룹에 대 한 대화 상자.|  
|서비스|구성할 수 있습니다 합니다 **Services** 서비스 시작 유형, 서비스 상태, 서비스 이름 및 서버에서 지정 하는 일치 하는 역할 또는 서버 그룹에서 찾으면 경고를 표시 하는 행을 **서비스 자세히 보기**  대화 상자.<br /><br />서버 관리자 서버 풀에 서버를 추가한 후에 관리 되는 서버에 로그온 사용자가 없을 경우 셸 하드웨어 검색 서비스에 대 한 서비스 알림은 표시할 수 있습니다. 이는 사용자가 관리 서버에 로그온되어 있거나 관리 서버의 원격 데스크톱 세션에 연결되어 있는 경우에만 셸 하드웨어 검색 서비스가 실행되기 때문입니다. 이 경우 셸 하드웨어 검색 서비스 경고가 표시되지 않도록 하려면 **모든 서버** 그룹이 포함된 서버 그룹의 미리 보기에서 **서비스** 를 클릭합니다. 에 **서비스 자세히 보기** 대화 상자의 **Services** 드롭 다운 목록에서 확인란의 선택을 취소에 대 한 **셸 하드웨어 검색**, 클릭 하 고 **확인**.|  
|성능|구성할 수 있습니다 합니다 **성능** 리소스 유형, 서버 또는에 지정 된 기간 동안 일치 하는 성능 경고가 발생 하면 역할 또는 서버 그룹에 대 한 경고를 표시 하려면 해당 행을 **성능 자세히 보기** 대화 상자.<br /><br />기본적으로 성능 카운터는 꺼져 있습니다. 관리 되는 서버 운영 체제를 Windows Server 2003 이후 버전 실행 되는 및에 대 한 성능 카운터 시작 되지 않은, 일반적으로 관리 효율성 상태 오류 표시의 **온라인-성능 카운터 시작 되지 않은** 에 **서버** 역할이 나 그룹 페이지의 타일입니다. 성능 카운터에 대해 설정 하려면 관리 되는 서버에는 **모든 서버** 페이지에서 항목을 마우스 오른쪽 단추로 클릭 합니다 **성능** 타일를 **카운터 상태** 값 **Off**를 클릭 하 고 **성능 카운터 시작**합니다. 서버에 대 한 항목을 마우스 오른쪽 단추로 클릭 하 여 성능 카운터를 시작할 수도 있습니다는 **서버** 역할 및 그룹 페이지를 클릭 한 다음 타일 **성능 카운터 시작**합니다.|  
|BPA 결과|구성할 수 있습니다 합니다 **BPA 결과** 표시할 행에 대 한 역할 또는 서버 그룹 BPA 검사 결과가 일치 하는 심각도 수준, 서버 또는 BPA 범주에서 지정 하는 경고는 **BPA 결과 자세히 보기** 대화 상자.|  
...
3.  선택한 그룹의 서버에서 수집 하려는 이벤트 심각도 선택 합니다. 기본적으로 **위험**, **오류** 및 **경고** 심각도 수준이 선택됩니다.  
...
1.  서버 관리자가 이미 열려 있으면 다음 단계로 이동 합니다. 서버 관리자가 아직 열려 있지 않으면 다음 중 하나를 수행하여 엽니다.  
...
3.  에 **이벤트 자세히 보기** 대화 상자에서 표시 하려는 이벤트 심각도 수준을 추가 합니다. 기본적으로 미리 보기에는 위험 이벤트를 강조하는 경고만 표시됩니다. 이벤트의 수에 표시 되는 **자세히 보기** 대화 상자에 대 한 경고를 표시 하려면 심각도 수준을 추가할 때 증가 합니다.  

4.  **이벤트 원본** 필드에서는 경고를 발생시킬 이벤트 원본을 선택합니다. 기본값은 **All**입니다.  

5.  이 미리 보기에 대 한 경우 여러 서버 또는 여러 서버 그룹에 설치 된 역할에서 이벤트 경고 하려는 서버를 선택할 수 있습니다 합니다 **서버** 드롭 다운 목록.  
