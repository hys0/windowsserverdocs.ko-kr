...
"일반 최상의 집합을 처리 하기 위해 워크 로드 선택? Windows Server 워크 로드입니다. 물론이 집합이 아닙니다 실제 서버 환경의 전체 범위를 대표 하 합니다.
...
모든 테스트 거친된 벤치 마크 성능 메트릭으로 처리량을 사용합니다. 응답 시간 (기본 메트릭을 경우 SAP) 제외 하 고 이러한 워크 로드에는 SLA 요구 사항으로 간주 됩니다. 예를 들어, 실행 하는 벤치 마크를 유효한 것으로 간주 "? 평균 또는 최대 응답 시간을 특정 값 보다 작으면 합니다.
...
-   **프로세서 성능 증가 정책의** – "단일? 정책 느려지는 증가 지속적인 활동에 대 한 성능 응답 "Rocket? 정책 증가 작업에 신속 하 게 반응

-   **프로세서 성능 감소 정책의** – "단일? 정책에서 더 이상 유휴 기간;이 성능을 더 서서히 감소 "Rocket? 정책 삭제 power 매우 신속 하 게 유휴 기간을 입력 하는 경우
...
워크 로드에는 "실시간 하는 경우? (예를 들어, 고장 또는 기타에 취약 표시 최종 사용자에 영향을 줍니다)에 이르러 매우 빡빡한 응답성 요구 사항이 (예를 들어 증권 거래소는) 및 에너지 소비 환경에 대 한 기본 기준 없으면 있을 것 전환 해야 합니다 **고성능** 전원 계획입니다. 이 고, 그렇지 워크 로드, 응답 시간 요구 사항을 이해 하 고도 이러한 요구 사항을 충족 하는 최상의 가능한 전원 효율성 PPM 매개 변수를 조정 해야 합니다.

### <a name="understand-underlying-workload-characteristics"></a>기본 워크 로드 특성을 이해

워크 로드 인지 알고 있어야 하 고 튜닝 하는 것에 대 한 실험 매개 변수 집합을 디자인 해야 합니다. 예를 들어 CPU 코어의 빈도 매우 과정인 수 해야 하는 경우 (유휴 상태 유효 기간을 사용 하 여 버스 티 작업 경우가 있습니다 하지만 새 트랜잭션을 함께 하는 경우 매우 빠른 응답성이 필요할) 빠르고-다음 프로세서 성능 향상 정책 "rocket로 설정 해야 할 수 있습니다.? (, 이름에서 알 수 있듯이 촬영 CPU 코어 빈도 시간 동안의 단계별로 실행 하는 대신 해당 최 댓 값).
