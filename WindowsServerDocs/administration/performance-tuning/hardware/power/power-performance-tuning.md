...
서버에서 계량 및 예산에 대 한 지원을 제공 하는지 여부를 확인 하는 한 가지 방법은 [Windows Server 카탈로그](http://www.windowsservercatalog.com)를 검토 하는 것입니다. 서버 모델이 Windows 하드웨어 인증 프로그램의 향상 된 새 전원 관리 자격을 충족 하는 경우 계량 및 예산 기능을 지원 하도록 보장 됩니다.
...
현재 선택 된 계획 이외의 전원 계획에 대해이 값을 설정 하려면 구성표\_\_\_\_대신최대구성표(전원보호기),구성표MIN(고성능)및구성표균형조정(균형조정)등의별칭을사용할수있습니다.현재. 해당 전원 계획을 사용 하도록 설정 하려면 이전에 표시 된 powercfg setactive 명령의 "scheme current"를 원하는 별칭으로 바꿉니다.
...
서버에 특정 코어 파킹 요구 사항이 있는 경우 **프로세서 성능 코어 파킹 최대 코어** 매개 변수 또는 **프로세서 성능 코어 파킹 최소 코어를 사용 하 여 파킹에 사용할 수 있는 코어 수를 제어할 수 있습니다.** Windows Server 2016의 매개 변수입니다.
...
이러한 매개 변수에 대 한 값은 0-100 범위의 백분율입니다. **프로세서 성능 코어 파킹 최대 코어** 매개 변수는 언제 든 지 파킹 가능 (스레드를 실행할 수 있음) 할 수 있는 코어의 최대 비율을 제어 하는 반면, **프로세서 성능 코어 파킹 최소 코어** 매개 변수는 파킹 해제 될 수 있는 코어의 최소 비율을 제어 합니다. 코어 파킹 기능을 해제 하려면 다음 명령을 사용 하 여 **프로세서 성능 코어 파킹 최소 코어** 매개 변수를 100%로 설정 합니다.
