...
|일정 유형|보조키|날짜 값 (/d)|설명|
|-------------|--------|---------------|-----------|
|매주|1 - 52|월요일-일요일 [, 월요일-일요일...] | *|(선택 사항) (월)에는 기본값입니다. 와일드 카드 값 (*)은 매일을 의미 합니다.|
...
"The week day" 일정은 주별 일정의 변형입니다. 주간 일정에서는 **/sc 매주** 매개 변수는 필수입니다. **/mo** (한정자) 매개 변수는 선택 사항이 며 작업의 각 실행 사이의 주 수를 지정 합니다. 기본값 **/mo** 1 (매주)입니다. 합니다 **/d** 선택 사항인 매개 변수를 지정 된 요일 또는 매일 (*)에서 실행 되도록 작업을 예약 합니다. 기본값은 월요일 (월요일). 매일 옵션 (**/d \***)은 매일 작업을 예약 하는 것과 같습니다.
...
이 일정 유형에 **/sc monthly** 매개 변수는 **/mo** (한정자) 매개 변수 및 **/d** (일) 매개 변수는 필요 합니다. **/mo** (한정자) 매개 변수는 작업이 실행 되는 주를 지정 합니다. **/d** 매개 변수는 요일을 지정 합니다. (이 일정 유형에 대 한 주 1 일에만 지정할 수 있습니다.) 이 일정에는 선택적 **/m** (월) 매개 변수를 사용 하면 특정 월 또는 매월 실행 하는 것에 대 한 작업을 예약 (*). 기본값은 **/m** 매개 변수는 매월 (*).
...
합니다 **/m** (월) 매개 변수는이 일정 유형에 대 한 선택 사항이 며 기본값은 매월 (*). **Schtasks** 로 지정 된 달에 발생 하지 않는 날짜에 대 한 작업을 예약할 수는 **/m** 매개 변수입니다. 그러나 경우 생략 된 **/m** 매개 변수 및 일정 달에는 31 번째 날, 다음 작업 등 특정 달에 표시 되지 않는 날짜에 대 한 작업 실행 되지 않습니다. 월의 마지막 날에 대 한 작업을 예약 하려면 마지막 날 일정 유형을 사용 합니다.
...
|/ru {[\<Domain>\]<User> | System}|작업이 실행 되는 사용자 계정을 변경 하려면이 옵션을 지정 합니다. 유효한 항목은 로컬 시스템 계정 지정, "", "NT AUTHORITY\SYSTEM" 또는 "SYSTEM"입니다.</br>사용자 계정을 변경 하면 사용자 암호도 변경 해야 합니다. 명령에는 **/ru** 매개 변수가 아닌는 **/rp** 매개 변수를 **schtasks** 새 암호를 묻는 메시지를 표시 합니다.</br>로컬 시스템 계정의 사용 권한으로 실행 되는 작업 또는 필요 없는 암호를 묻는 메시지입니다.|
...
|용어|정의|
|----|----------|
|/tn {\<작업 이름 >| *}|필수 사항입니다. 삭제할 작업을 식별 합니다.</br>**\<작업 이름 >** -명명 된 작업을 삭제 합니다.</br>**<\*>** -컴퓨터에 있는 모든 예약 된 작업을 삭제합니다.|
...
-   **삭제 \*** 명령은 예약 된 모든 작업에 컴퓨터 뿐 아니라 현재 사용자가 예약 된 작업을 삭제 합니다.
...
```
응답으로 **SchTasks.exe** 다음과 같은 확인 메시지가 표시 됩니다. 작업을 삭제 하려면 Y를 눌러**합니다.** 명령을 취소 하려면 다음을 입력 **n**:
```
...
다음 명령을 다른 사용자가 예약 된 작업을 비롯 하 여 로컬 컴퓨터의 일정에서 모든 작업을 삭제 합니다. 사용 하 여는 **/tn \*** 를 컴퓨터에 있는 모든 작업을 나타내는 매개 변수 및 **/f** 를 확인 메시지를 표시 하지 않는 매개 변수입니다.
```
...
|/fo {테이블| 목록| CSV}|출력 형식을 지정합니다. **테이블** 값이 기본값입니다.|
