...
|         /b          | NTFS 전용: 볼륨의 잘못 된 클러스터 목록을 지우고 모든 할당 된 클러스터 및 사용 가능한 클러스터에서 오류를 다시 검사. **/b** 는 **/r**의 기능을 포함 합니다. 새 하드 디스크 드라이브에 볼륨 이미징 후이 매개 변수를 사용 합니다.            |
| /scan               | NTFS 전용: 볼륨에서 온라인 검색을 실행 합니다. |
| /forceofflinefix    | NTFS 전용: 는 "/scan"과 함께 사용 해야 합니다. 모든 온라인 복구 무시 발견 된 모든 결함은 오프 라인 복구를 위해 큐에 대기 됩니다 (예: "chkdsk/spotfix"). |
| /perf               | NTFS 전용: 는 "/scan"과 함께 사용 해야 합니다. 더 많은 시스템 리소스를 사용 하 여 fast aspossible 검색을 완료 합니다. 이는 시스템에서 실행 되는 다른 작업에 부정적인 영향을 미칠 수 있습니다.|
| /spotfix            | NTFS 전용: 볼륨에서 스폿 수정을 실행 합니다. |
| /sdcleanup          | NTFS 전용: 불필요 한 보안 설명자 데이터 (/F 함축)를 수집 합니다. |
| /offlinescanandfix  | 볼륨에 대 한 오프 라인 검사 및 픽스를 실행 합니다. |
| /freeorphanedchains | FAT/FAT32/exFAT만: 는 콘텐츠를 복구 하는 대신 분리 된 클러스터 체인을 해제 합니다. |
| /markclean          | FAT/FAT32/exFAT만: /F가 지정 되지 않은 경우에도 손상이 감지 되지 않은 경우 볼륨 정리를 표시 합니다. |
|         /?          | 명령 프롬프트에 도움말을 표시합니다.                       |
...
- 논리 디스크 오류 수정
  ...
  **Y**키를 누르면 Windows는 루트 디렉터리에 있는 손실 된 각 체인을 파일 @ no__t-1nnnn >의 형식으로 이름으로 파일에 저장 합니다. 때 **chkdsk** 완료 되 면 모든 필요한 데이터를 포함 하는 경우이 파일을 확인할 수 있습니다. 키를 누르면 **N**, Windows는 디스크 오류를 수정 하지만 손실 된 할당 단위의 내용을 저장 하지 않습니다.
