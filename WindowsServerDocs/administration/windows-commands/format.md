...
|    /fs: {FAT    |                                                                                                                                                                                                                       FAT32                                                                                                                                                                                                                        |
|  /v:\<레이블 >   |                           볼륨 레이블을 지정합니다. 생략 하면는 **/v** 명령줄 옵션을 선택 하거나 볼륨 레이블을 지정 하지 않고 사용 **형식** 서식을 완료 한 후 볼륨 레이블을 묻습니다. 볼륨 레이블을 묻는 메시지를 표시하지 않으려면 **/v:** 구문을 사용합니다. 단일 **format** 명령을 사용하여 둘 이상의 디스크를 포맷하는 경우 모든 디스크에 동일한 볼륨 레이블이 지정됩니다.                            |
...
| /n:\<섹터 >  |                                                         트랙 당 섹터 수를 지정 합니다. 가능 하면 **/n**대신 **/f** 명령줄 옵션을 사용 합니다. **/n**을 사용하는 경우 **/t**도 사용해야 합니다. 이 두 옵션은 함께 사용될 경우 포맷할 디스크의 크기를 지정하는 대체 방법을 제공합니다. 이 옵션은 **/f** 옵션에 유효하지 않습니다.                                                         |
