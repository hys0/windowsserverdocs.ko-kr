...
|to={on | off}|무한 시간 제한 처리 또는 해제 여부를 지정 합니다. 기본값은 off입니다.|
|xon={on | off}|데이터 흐름 제어를 xon 또는 xoff 프로토콜 또는 해제 여부를 지정 합니다.|
|odsr={on | off}|준비 DSR (Data Set) 회로 사용 하 여 출력 핸드셰이킹 또는 해제 여부를 지정 합니다.|
