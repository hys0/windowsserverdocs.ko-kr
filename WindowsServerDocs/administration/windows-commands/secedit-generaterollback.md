...
|cfg|필수입니다.</br>분석을 위해 데이터베이스로 가져올 보안 서식 파일에 대 한 경로 파일 이름을 지정 합니다.</br>이 /cfg 옵션은 함께 사용할만 `/db \<database file name>` 매개 변수입니다. 지정 하지 않으면 이미 데이터베이스에 저장 된 모든 구성에 대해 분석이 수행 됩니다.|
...
이 명령이 성공적으로 실행 되 면 "태스크가 성공적으로 완료 되었습니다." 상태가 됩니다. 및 로그 명시 된 보안 템플릿과 보안 정책 구성 간의 불일치만 합니다. 이러한 불일치는 scesrv.log에 나열 됩니다.
