---
title: diskraid
description: 독립 된 (또는 저렴 한) 디스크 (RAID) 저장소 하위 시스템의 중복 배열을 구성 하 고 관리 하는 데 사용할 수 있는 명령줄 도구인 diskraid에 대 한 참조 항목입니다.
...
VDS 1.1 공급자를 사용 하는 LUN에 포트를 연결 하 고 추가 하는 방법을 보여 줍니다.
```
DISKRAID> SEL LUN 5
...
### <a name="exit"></a><a name=BKMK_10></a>끝낸
...
LUN을 \<만들 때 사용할 드라이브의 drive_number> 지정 합니다. **Size =** 매개 변수가 지정 되지 않은 경우 생성 된 LUN은 지정 된 모든 드라이브에서 허용 되는 최대 크기입니다. 공급자는 가능 하면 지정 된 순서로 드라이브를 사용 합니다.
...
### <a name="help"></a><a name=BKMK_13></a>도움말
...
부하 분산 정책을 지정 합니다. 형식을 지정 하지 않은 경우에는 **path** 매개 변수를 지정 해야 합니다. Type은 다음 중 하나일 수 있습니다.
...
### <a name="offline"></a><a name=BKMK_24></a>라인인
...
## <a name="example-interactively-view-status-of-subsystem"></a>예: 하위 시스템의 대화형 뷰 상태

컴퓨터에서 하위 시스템 0의 상태를 확인 하려면 명령줄에 다음을 입력 합니다.
```
diskraid
```
Enter 키를 누릅니다. 다음이 표시됩니다.
```
...
select subsystem 0
```
Enter 키를 누릅니다. 다음과 비슷한 출력이 표시됩니다.
```
