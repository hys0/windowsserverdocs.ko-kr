...
다음 명령은 카운터 만듭니다 perf_log processor (_total) 카운터 범주에서 % Processor time 카운터를 사용 하 여 호출 합니다.  
```  
logman create counter perf_log -c "\Processor(_Total)\% Processor time"  
```  
다음 명령은 카운터 만듭니다 perf_log processor (_total) 카운터 범주에서 % Processor time 카운터를 사용 하 여, 10MB의 최대 크기를 사용 하 여 로그 파일 만들기 및 1 분 0 초가 대 한 데이터 수집을 호출 합니다.  
```  
