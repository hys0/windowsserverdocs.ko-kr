...
|[/insert | /overstrike]|삽입 또는 텍스트를 입력할 때 덮어쓸 것인지 지정 합니다. 사용 하는 경우 **/삽입**, 새 텍스트 줄에 입력 하는 기존 텍스트의 가운데에 삽입 됩니다. 사용 하는 경우 **겹쳐서 표시/**, 기존 텍스트 새 텍스트 바꿉니다. 기본 설정은 **겹쳐서 표시/** 합니다.|
...
-   매크로 만들기
    ...
    |$B 또는 $b|명령 매크로 출력을 보냅니다. 이러한 특수 문자는 파이프를 사용 하 여 (* *|*)를 명령줄에서.|
    ...
    |$*|매크로 이름을 입력할 때를 지정 하려는 모든 명령줄 정보를 나타냅니다. 특수 문자 **$ \*** 는 일괄 처리 매개 변수 유사한 대체 가능 매개 변수 **$1** 를 통해 **$9**, 중요 한 하나를 사용 하 여 차이: 매크로 이름을 대체 후 명령줄에 입력 모든는 **$ \*** 매크로에서.|
-   실행 되는 **doskey** 매크로
