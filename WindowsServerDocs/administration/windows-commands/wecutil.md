...
|옵션|설명|
|------|-----------|
|/f:\<형식 >|표시 되는 정보의 형식을 지정 합니다. \<형식 >은 XML 또는 간결한 형식일 수 있습니다. 경우 <Format> 는 XML 출력은 XML 형식으로 표시 됩니다. \<형식 > 간결 하 게 표시 되는 경우 출력은 이름-값 쌍에 표시 됩니다. 기본값은 Terse.|
...
|/tp:\<포트 >|원격 이벤트 소스에 연결할 때 전송에 의해 사용 되는 포트 번호를 설정 합니다.|
...
|/wct:\<형식 >|이 원격 액세스에 대 한 자격 증명 형식을 설정합니다. \<형식 >는 기본값, negotiate, digest, basic 또는 localmachine 값 중 하나 여야 합니다. 기본값은 default입니다.|
...
|/q: [\<Quiet >]|구성 프로시저에서 확인 메시지를 표시할지 여부를 지정 합니다. \<Quiet > true 또는 false 일 수 있습니다. <Quiet> true 이면 구성 절차에서 확인 메시지를 표시 하지 않습니다. 이 옵션의 기본값은 false입니다.|
