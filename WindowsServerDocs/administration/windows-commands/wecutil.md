...
원격 컴퓨터에서 전달 되는 이벤트에 대 한 구독을 만들고 관리할 수 있습니다. 원격 컴퓨터는 WS 관리 프로토콜을 지원 해야 합니다. 이 명령을 사용하는 방법의 예는 [예](#BKMK_examples)를 참조하세요.
...
|{gs \| get-subscription} \< Subid > [/f: \<Format >] [/a: \< 유니코드 >]|원격 구독 구성 정보를 표시합니다. \<Subid >는 구독을 고유 하 게 식별 하는 문자열입니다. \<Subid >은 구독을 만드는 데 사용 된 XML 구성 파일의 \<SubscriptionId > 태그에 지정 된 문자열과 같습니다.|
|{gr \| get-subscriptionruntimestatus} \<Subid > [\<Eventsource > ...]|구독의 런타임 상태를 표시 합니다. \<Subid >는 구독을 고유 하 게 식별 하는 문자열입니다. \<Subid >은 구독을 만드는 데 사용 된 XML 구성 파일의 \<SubscriptionId > 태그에 지정 된 문자열과 같습니다. \<Eventsource >는 이벤트의 원본으로 사용 되는 컴퓨터를 식별 하는 문자열입니다. \<Eventsource >는 정규화 된 도메인 이름, NetBIOS 이름 또는 IP 주소 여야 합니다.|
|{ss \| 설정-구독} \<Subid > [/e: [\<Subenabled >]] [/esa: \<Address >] [/ese: [\<Srcenabled >]] [/ese] [/res] [/un: \< 사용자 이름 >] [/up: \<Password >] [/d: \<Desc >] [/uri: @no __t-8Uri >] [/cm: \<Configmode >] [/예: 0Expires >] [/q: 1Query >] [/dia: 2Dialect >] [/tn: 3Transportname >] [/tp: 4Transportport >] [/sdm: 5Deliverymode >] [/a s: @no__ t-16Deliverymax >] [/dmlt: 7Deliverytime >] [/안녕하세요: 8Heartbeat 비트 >] [/tf: 9Content >] [/l: 0Locale >] [/ree: [1Readexist >]] [/lf: 2Logfile >] [/pn: 3Publishername >] [/ essp: 4Enableport >] [/h n: 5Hostname >] [/sct: 6Type >]</br>로 구분하거나 여러</br>{ss \| set-subscription/c: \<D> [/cun: \<Comusername >/cup: \<Compassword >]|등록 구성을 변경합니다. 구독 ID 및 구독 매개 변수를 변경 하려면 적절 한 옵션을 지정할 수 있습니다 또는 구독 매개 변수를 변경 하는 XML 구성 파일을 지정할 수 있습니다.|
|{cs \| create-subscription} \<9-> [/cun: \<Username >/cup: \<Password >]|원격 구독을 만듭니다. \<D>는 구독 구성이 포함 된 XML 파일의 경로를 지정 합니다. 절대 또는 현재 디렉터리에 상대적 경로일 수 있습니다.|
...
|{rs \| 다시 시도-구독} \<Subid > [\<Eventsource > ...]|비활성 구독에 원격 구독 요청을 보내고 연결을 설정 하는 다시 시도 합니다. 모든 이벤트 소스를 다시 활성화 하려고 하거나 이벤트 소스를 지정 합니다. 사용할 수 없는 소스를 다시 시도 하지 않습니다. \<Subid >는 구독을 고유 하 게 식별 하는 문자열입니다. \<Subid >은 구독을 만드는 데 사용 된 XML 구성 파일의 \<SubscriptionId > 태그에 지정 된 문자열과 같습니다. \<Eventsource >는 이벤트의 원본으로 사용 되는 컴퓨터를 식별 하는 문자열입니다. \<Eventsource >는 정규화 된 도메인 이름, NetBIOS 이름 또는 IP 주소 여야 합니다.|
...
|옵션|설명|
|------|-----------|
|/f: \<Format >|표시 되는 정보의 형식을 지정 합니다. \<Format >은 XML 또는 간결한 형식일 수 있습니다. 경우 <Format> 는 XML 출력은 XML 형식으로 표시 됩니다. @No__t-0Format > 간결한 경우 출력은 이름-값 쌍에 표시 됩니다. 기본값은 Terse.|
|/c: \<2->|구독 구성이 포함 된 XML 파일의 경로를 지정 합니다. 절대 또는 현재 디렉터리에 상대적 경로일 수 있습니다. 이 옵션에만 사용할 수는 **/cun** 및 **컵/** 옵션 및 다른 모든 옵션과 함께 사용할 수 없습니다.|
|/e: [\<Subenabled >]|구독을 사용 하지 않도록 설정 하거나 사용 합니다. \<Subenabled > true 또는 false 일 수 있습니다. 이 옵션의 기본값은 true입니다.|
|/esa: \<Address >|이벤트 소스의 주소를 지정 합니다. \<Address >는 정규화 된 도메인 이름, NetBIOS 이름 또는 이벤트의 원본으로 사용 되는 컴퓨터를 식별 하는 IP 주소를 포함 하는 문자열입니다. 이 옵션은 함께 사용할 수는 **/ese**, **/aes**, **/res**, 또는 **/un** 및 **/up** 옵션입니다.|
...
|/q: \<Query >|구독에 대 한 쿼리 문자열을 지정합니다. @No__t-0Query >의 형식은 URI 값 마다 다를 수 있으며 구독의 모든 원본에 적용 됩니다.|
...
|/dm: @no__t 0Deliverymode >|배달 모드를 지정합니다. \<Deliverymode >는 pull 또는 push 일 수 있습니다. 이 옵션은만 유효 하는 경우는 **/cm** 옵션 사용자 지정을 설정 합니다.|
|/dmi: \<Deliverymax >|일괄 처리 된 배달에 대 한 항목의 최대 수를 설정 합니다. 이 옵션은 유효만 경우 **/cm** Custom으로 설정 합니다.|
|/dmlt: @no__t 0Deliverytime >|이벤트의 일괄 처리를 제공 합니다. 최대 대기 시간을 설정 합니다. \<Deliverytime >는 밀리초 수입니다. 이 옵션은 유효만 경우 **/cm** Custom으로 설정 합니다.|
|/hi: \<Heartbeat 비트 >|하트 비트 간격을 정의합니다. \<Heartbeat 비트 >는 밀리초 수입니다. 이 옵션은 유효만 경우 **/cm** Custom으로 설정 합니다.|
|/cf: \<Content >|반환 되는 이벤트의 형식을 지정 합니다. \<Content >는 이벤트 또는 RenderedText 수 있습니다. RenderedText 값을 사용 하는 경우 이벤트 이벤트에 연결 된 지역화 된 문자열 (예: 이벤트 설명)으로 반환 됩니다. 기본값은 RenderedText 합니다.|
...
|/pn: \<Saername >|게시자 이름을 지정합니다. 이 소유 하거나에서 지정 된 로그를 가져오는 게시자 여야는 **/lf** 옵션입니다.|
...
|/h n: \<Hostname >|로컬 컴퓨터의 DNS 이름을 지정합니다. 이 이름에 이벤트를 다시 밀어 원격 이벤트 소스에 사용 되 고 밀어넣기 구독에만 사용 해야 합니다.|
|/sct: \<Type >|이 원격 액세스에 대 한 자격 증명 형식을 설정합니다. \<Type >는 기본값, negotiate, digest, basic 또는 localmachine 값 중 하나 여야 합니다. 기본값은 default입니다.|
...
|/컵: \<Compassword >|공유 사용자 자격 증명에 대 한 사용자 암호를 설정합니다. @No__t-0Compassword > * (별표)로 설정 된 경우 콘솔에서 암호를 읽습니다. 이 옵션은만 사용할 때의 **/cun** 옵션을 지정 합니다.|
|/q: [\< 자동 >]|구성 프로시저에서 확인 메시지를 표시할지 여부를 지정 합니다. \< 자동 > true 또는 false 일 수 있습니다. @No__t-0이 true 이면 구성 프로시저에서 확인 메시지를 표시 하지 않습니다. 이 옵션의 기본값은 false입니다.|
