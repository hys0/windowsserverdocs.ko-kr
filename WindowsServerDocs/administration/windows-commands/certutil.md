...
InfoName-CA 속성을 표시 (아래 참조)을 나타냅니다. 사용 하 여 "\*" 모든 속성에 대 한 합니다.
...
RegistryValueName: 레지스트리 값 이름 (사용 하 여 "이름\*"를 접두사 일치)
...
명령줄에서 지정 된 암호가 쉼표로 구분 된 암호 목록입니다.  둘 이상의 암호를 지정 하면 출력 파일에 대 한 마지막 암호가 사용 됩니다.  하나의 암호를 제공 하는 경우에 또는 마지막 암호는 경우 "\*"를 출력 파일 암호를 묻는 메시지가 사용자입니다.
