...
-   기본 명령줄 구문에 대해를 실행 합니다.`certreq -?`
-   특정 동사와 함께 certutil을 사용 하는 구문에 대해 **certreq**  *\<verb >* **-?** 를 실행 합니다.
...
|KeyUsage|어떤 인증서 키에 사용할지를 정의 합니다.|CERT_DIGITAL_SIGNATURE_KEY_USAGE-80 (128)</br>팁:  표시 되는 값은 각 비트 정의에 대 한 16 진수 (10 진수) 값입니다. 오래 된 구문을 사용할 수도 있습니다: 여러 개의 단일 16 진수 값을 비트 기호 표현 하는 대신 집합입니다. 예를 들어 KeyUsage 0xa0 =.</br>CERT_NON_REPUDIATION_KEY_USAGE-40 (64)</br>CERT_KEY_ENCIPHERMENT_KEY_USAGE-20 (32)</br>CERT_DATA_ENCIPHERMENT_KEY_USAGE-10 (16)</br>CERT_KEY_AGREEMENT_KEY_USAGE-8</br>CERT_KEY_CERT_SIGN_KEY_USAGE-4</br>CERT_OFFLINE_CRL_SIGN_KEY_USAGE-2</br>CERT_CRL_SIGN_KEY_USAGE-2</br>CERT_ENCIPHER_ONLY_KEY_USAGE-1</br>CERT_DECIPHER_ONLY_KEY_USAGE-8000 (32768)|KeyUsage = "CERT_DIGITAL_SIGNATURE_KEY_USAGE &#124; CERT_KEY_ENCIPHERMENT_KEY_USAGE"</br>팁:  여러 값에는 파이프 (&#124;) 기호 구분 기호가 사용 됩니다. INF 구문 분석 문제를 방지 하려면 여러 값을 사용 하는 경우 큰따옴표를 사용 하는 것을 확인 합니다.|
...
|EncryptionAlgorithm|사용할 암호화 알고리즘입니다.|가능한 옵션은 운영 체제 버전 및 설치 된 암호화 공급자 집합에 따라 달라 집니다. 사용 가능한 알고리즘 목록을 보려면 지정 된 CSP가 지정 된 <code>certutil -oid 2 &#124; findstr pwszCNGAlgid</code> 대칭 암호화 알고리즘과 길이를 지원 해야 합니다. 명령을 실행 합니다.|EncryptionAlgorithm 3des =|
...
|ValidityPeriodUnits</br>참고: 요청 유형이 cert 인 경우에만 사용 됩니다.|ValidityPeriod 함께 사용 되는 단위 수를 지정 합니다.|숫자|유닛의 = 3|
|ValidityPeriod</br>참고: 요청 유형이 cert 인 경우에만 사용 됩니다.|VValidityPeriod 기간은 영어 (미국) 복수 이어야 합니다.|년, 월, 주, 일, 시간, 분, 초|ValidityPeriod = 년|
...
> [!NOTE]
> SubjectAltName Nameflags를 사용 하면 INF 파일에서 현재 사용자 또는 현재 컴퓨터 속성을 기반으로 certreq에서 자동으로 채울 Subject 및 확장 필드를 지정할 수 있습니다. DNS 이름, UPN 등이 있습니다. "Template" 리터럴을 사용 하면 템플릿 이름 플래그가 대신 사용 됩니다. 따라서 단일 INF 파일을 상황에 맞는 주체 정보를 사용 하 여 요청을 생성 하는 여러 컨텍스트에서 사용할 수 있습니다.
>
...
|-Username  *\<사용자 이름 >*|인증서 등록 웹 서비스를 사용합니다. *\<사용자 이름 >* 를 SAM 이름 또는 domain\user로 대체할 수 있습니다. 이 옵션은-p 옵션으로 사용 합니다.|
|-p *\<Password>*|인증서 등록 웹 서비스를 사용합니다. *\<암호 >* 실제 사용자의 암호로 바꿉니다. 이 옵션은-UserName 옵션으로 사용 합니다.|
...
|형식|설명|
|-------|-----------|
|RequestFileIn|Base64 인코딩 또는 이진 입력 파일 이름: PKCS #10 certificate request, CMS certificate request, PKCS #7 인증서 갱신 요청, x.509 certificate to cross authentication 또는 Ssh-keygen tag format certificate request.|
...
-   [수동으로 웹 서버 SSL 인증서를 만드는 방법](http://blogs.technet.com/b/pki/archive/2009/08/05/how-to-create-a-web-server-ssl-certificate-manually.aspx)
...
-   [AD CS 단계별 가이드: 2 단계 PKI 계층 구조 배포](https://social.technet.microsoft.com/wiki/contents/articles/15037.ad-cs-step-by-step-guide-two-tier-pki-hierarchy-deployment.aspx)
