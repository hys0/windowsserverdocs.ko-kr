...
## <a name="syntax"></a>구문  
**nfsadmin server** `[` *computerName*`] [`\-u *UserName*`[`\-p *암호* `]]` \-l  
  
**nfsadmin server** `[` *computerName*`] [`\-u *UserName* `[` \-p *암호* `]]` \-r `{` *client* `|` 모든`}`  
  
**nfsadmin server** `[` *computerName*`] [`\-u *UserName* `[` \-p *암호* `]] {`시작 `|` 중지`}`  
  
**nfsadmin server** `[` *computerName*`] [`\-u *UserName* `[` \-p *암호* `]]` config *옵션*`[...]`  
  
**nfsadmin server** `[` *computerName*`] [`\-u *UserName* `[` \-p *암호* `]]` creategroup *이름*  
  
**nfsadmin server** `[` *computerName*`] [`\-u *UserName* `[` \-p *암호* `]]` listgroups  
  
**nfsadmin server** `[` *computerName*`] [`\-u *UserName* `[` \-p *암호* `]]` 주로 *이름*  
  
**nfsadmin server** `[` *computerName*`] [`\-u *UserName* `[` \-p *암호* `]]` renamegroup *OldName NewName*  
  
**nfsadmin server** `[` *computerName*`] [`\-u *UserName* `[` \-p *암호* `]]` addmembers *호스트 이름*`[...]`  
  
**nfsadmin server** `[` *computerName*`] [`\-u *UserName* `[` \-p *암호* `]]` listmembers  
  
**nfsadmin server** `[` *computerName*`] [`\-u *UserName* `[` \-p *암호* `]]` deletemembers *그룹 호스트*`[...]`  
  
**nfsadmin 클라이언트** `[` *computerName*`] [`\-u *UserName* `[` \-p *암호* `]] {`시작 `|` 중지`}`  
  
**nfsadmin 클라이언트** `[` *computerName*`] [`\-u *UserName* `[` \-p *암호* `]]` config *옵션*`[...]`  
...
**\-u** *UserName*  
자격 증명에 사용 하려는 사용자의 사용자 이름을 지정 합니다. 폼의 사용자 이름에 도메인 이름을 추가 하려면 할 수 있습니다 *도메인***\\***사용자 이름*  
...
**dotfileshidden**\={**예** | **없는**}  
파일의 지정 여부는 마침표로 시작 하는 이름을 사용 하 여 만든 \(.\) Windows 파일 시스템에서 숨겨진 상태로 표시 되며 따라서 NFS 클라이언트에서 숨겨집니다. 기본 설정은 **없는**합니다.  
...
**creategroup** *name*  
지정 된 새 클라이언트 그룹을 만듭니다 *이름을*입니다.  
  
**listgroups**  
모든 클라이언트 그룹의 이름을 표시합니다.  
  
**deletegroup** *name*  
지정한 클라이언트 그룹 제거 *이름을*입니다.  
  
**renamegroup** *OldName NewName*  
지정한 클라이언트 그룹의 이름을 바꾸는 *OldName* 에 *NewName*  
  
**addmembers** *호스트 이름*\[...\]  
추가 *호스트* 에서 지정한 클라이언트 그룹 *이름*합니다.  
  
**listmembers** *Name*  
호스트 컴퓨터에서 지정한 클라이언트 그룹에 나열 *이름을*입니다.  
  
**deletemembers** *그룹 호스트*\[...\]  
지정 된 클라이언트를 제거 *호스트* 에서 지정한 클라이언트 그룹에서 *그룹*합니다.  
...
**fileaccess\=***mode*  
-   네트워크 파일 시스템에서 만든 파일에 대 한 기본 사용 권한 모드를 지정 \(NFS\) 서버입니다. *모드* 인수는 세 자리 숫자로 0에서 7로 구성 됩니다. \(포괄\) 사용자, 그룹 및 부여 된 기본 사용 권한을 나타내는 \(각각\)합니다. 숫자 UNIX 변환할\-권한을 다음과 같은 스타일: 0\=없음, 1\=x, 2\=w, 3\=wx, 4\=r, 5\=rx, 6\=rw와 7\=rwx 합니다. 예를 들어 **fileaccess\=750** 제공 소유자에 게 rwx 권한, 그룹에 rx 권한 및 다른 사용자에 게 액세스 권한이 없습니다.  
...
명령 옵션 또는 인수를 지정 하지 않으면 **nfsadmin 클라이언트** NFS 구성 설정에 대 한 현재 클라이언트를 표시 합니다.  
