---
title: robocopy
description: Windows 및 Windows Server의 robocopy 명령을 사용 하 여 파일을 복사 하는 방법 알아보기
ms.custom: na
...
|\<File>|파일 또는 파일을 복사할 수를 지정 합니다. 와일드 카드 문자를 사용할 수 있습니다 (**&#42;** 하거나 **?**), 원하는 경우. 경우는 **파일** 매개 변수를 지정 하지 않으면 **\*.\*** 기본값으로 사용 됩니다.|
|\<옵션 >|와 함께 사용할 옵션을 지정 합니다 **robocopy** 명령입니다.|
...
|/e|복사본 하위 디렉터리입니다. 참고가이 옵션은 빈 디렉터리를 포함 합니다. 자세한 내용은 참조 [주의](#BKMK_remarks)합니다.|
|/lev:\<N >|상위 항목만 복사 *N* 소스 디렉터리 트리 수준입니다.|
|/z|다시 시작 가능 모드에서 파일을 복사 합니다.|
|/b|백업 모드에서 파일을 복사 합니다.|
|/zb|다시 시작 가능 모드를 사용합니다. 액세스를 거부 한 경우이 옵션 백업 모드를 사용 합니다.|
...
|/dcopy:\<copyflags\>|디렉터리에 복사 하려면 항목을 정의 합니다. 기본값은 da. 옵션은 D = 데이터, A = 특성 및 T = 타임 스탬프입니다.|
|수/초|보안 파일을 복사 (동일 **/copy:DATS**).|
...
|/purge|더 이상 원본에 존재 하는 대상 파일 및 디렉터리를 삭제 합니다. 자세한 내용은 참조 [주의](#BKMK_remarks)합니다.|
|/mir|디렉터리 트리를 미러링합니다 (동일 **/e** plus **제거/**). 자세한 내용은 참조 [주의](#BKMK_remarks)합니다.|
...
|/ 월:\<N >|소스를 모니터링 하 고 때 이상 다시 실행 *N* 변경 내용이 검색 됩니다.|
...
|/sl|기호화 된 링크 및 대상에 복사 합니다.|

> [!IMPORTANT]
> 사용 하는 경우는 **/SECFIX** 옵션 복사도 이러한 추가 복사 옵션 중 하나를 사용 하 여 복사할 보안 정보의 형식을 지정 합니다.</br>> -   **/COPYALL**</br>> -   **/COPY:O**</br>> -   **/COPY:S**</br>> -   **/COPY:U**</br>> -   **/SEC**
...
|/xf \<FileName>[ ...]|지정 된 이름이 나 경로 일치 하는 파일을 제외 합니다. 사실은 *FileName* 와일드 카드 문자를 포함할 수 있습니다 (**&#42;** 하 고 **?**).|
...
|/it|"조정" 파일을 포함 합니다.|
|max /:\<N >|최대 파일 크기를 지정 합니다 (보다 큰 파일을 제외 하려면 *N* 바이트)입니다.|
...
### <a name="exit-return-codes"></a>(반환) 종료 코드
값 | Description
-- | --
0 | 파일이 복사 되지 않았습니다. 오류가 발생 했습니다.  파일이 없는 된 일치 하지 않습니다. 대상 디렉터리에 파일이 이미 있습니다 따라서 복사 작업을 건너뛰었습니다.
1 | 모든 파일을 복사 했습니다.
2 | 원본 디렉터리에 존재 하지 않는 몇 가지 추가 파일이 대상 디렉터리에 있습니다. 파일이 복사 되지 않았습니다.
3 | 일부 파일이 복사 되었습니다. 추가 파일이 없습니다. 오류가 발생 했습니다.
5 | 일부 파일이 복사 되었습니다. 일부 파일 된 일치 하지 않습니다. 오류가 발생 했습니다.
6 | 추가 파일 및 일치 하지 않는 파일이 존재합니다. 파일이 복사 되지 않았습니다 하 고 없음 오류 발생 했습니다. 이 파일이 이미 대상 디렉터리에 있는 것을 의미 합니다.
7 | 파일을 복사 하 고 파일 불일치가 추가 파일이 있던 합니다.
8 | 여러 파일을 복사 하지 않았습니다.

> [!NOTE]
> 8 보다 큰 값 복사 작업 중 하나 이상의 오류가 발생 했음을 나타냅니다.
