---
title: dfsrmig
description: Dfsrmig에 대 한 참조 항목으로, FRS (파일 복제 서비스)에서 분산 파일 시스템 (DFS) 복제로 SYSvol 복제를 마이그레이션하고, 마이그레이션 진행 상황에 대 한 정보를 제공 하 고, 마이그레이션을 지원 하도록 AD DS (active directory 도메인 서비스) 개체를 수정 합니다.
