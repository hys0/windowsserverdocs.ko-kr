...
## <a name="step-1-disable-the-automatic-system-page-file-management"></a>1 단계: 자동 시스템 페이지 파일 관리 사용 안 함
...
## <a name="step-2-configure-the-destination-path-for-a-memory-dump"></a>2 단계: 메모리 덤프의 대상 경로 구성
...
2. 다음 레지스트리 하위 키를 찾아 클릭 합니다. HKEY_LOCAL_MACHINE \SYSTEM\CurrentControlSet\Control\CrashControl
...
6. **값 데이터** 형식 **\<Drive\>:\\\<\>Dedicateddumpfile** 를 입력 한 다음 **확인**을 클릭 합니다.

   >[!NOTE] 
   > \<드라이브\>를 페이징 파일을 저장할 충분 한 디스크 공간이 있는 드라이브로 바꾸고 Dedicateddumpfile\> \<을 전용 파일의 전체 경로로 바꿉니다.
...
## <a name="step-3-set-the-type-of-memory-dump"></a>3 단계: 메모리 덤프의 유형 설정
...
아래에 정의 된 것 처럼 \<값\>는 0, 1, 2 또는 3 일 수 있습니다.
...
## <a name="step-4-configure-the-server-to-restart-automatically-after-generating-a-memory-dump"></a>4 단계: 메모리 덤프를 생성 한 후 자동으로 다시 시작 하도록 서버 구성
...
## <a name="step-5-configure-the-server-to-overwrite-the-existing-memory-dump-file"></a>5 단계: 기존 메모리 덤프 파일을 덮어쓰도록 서버 구성
...
## <a name="step-6-set-an-administrative-alert"></a>6 단계: 관리 경고 설정
...
## <a name="step-7-set-the-memory-dumps-page-file-size"></a>7 단계: 메모리 덤프의 페이지 파일 크기 설정
...
## <a name="step-8-configure-the-server-to-generate-a-manual-memory-dump"></a>8 단계: 수동 메모리 덤프를 생성 하도록 서버 구성
...
## <a name="step-9-verify-that-memory-dump-files-are-being-created-correctly"></a>9 단계: 메모리 덤프 파일이 올바르게 생성 되는지 확인
