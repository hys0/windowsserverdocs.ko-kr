---
title: 2 단계-WSUS 구성
description: Windows Server Update Service (WSUS) 항목에서는-WSUS 구성 된 WSUS 배포에 대 한 4 단계 프로세스에서 2 단계
...
|태스크|설명|
|----|--------|
|[2.1. 네트워크 연결 구성](2-configure-wsus.md#BKM_ConfigurenetworkConnections)|네트워크 구성 마법사를 사용하여 클러스터 네트워크를 구성합니다.|
|[2.2. WSUS 구성 마법사를 사용 하 여 WSUS 구성](2-configure-wsus.md#BKM_ConfigureWSUSusingConfigurationWizard)|WSUS 구성 마법사를 사용하여 기본 WSUS 구성 작업을 수행합니다.|
|[2.3. WSUS 컴퓨터 그룹 구성](2-configure-wsus.md#BKMK_ConfigcomputerGroups)|WSUS 관리 콘솔에서 컴퓨터 그룹을 만들어 조직에서 업데이트를 관리합니다.|
|[2.4. 클라이언트 업데이트 구성](2-configure-wsus.md#BKM_ConfigureClientUpdates)|클라이언트 컴퓨터에 자동 업데이트를 적용하는 방법과 시기를 지정합니다.|
|[2.5. Secure Sockets Layer 프로토콜을 사용한 WSUS 보안](2-configure-wsus.md#bkmk_2.5.ConfigSSL)|WSUS(Windows Server Update Services)를 보호하기 위한 SSL(Secure Sockets Layer) 프로토콜을 구성합니다.|

## <a name="BKM_ConfigurenetworkConnections"></a>2.1. 네트워크 연결 구성
...
기본적으로 WSUS는 업데이트를 가져올 위치로 Microsoft 업데이트를 사용하도록 구성되어 있습니다. 프록시 서버를 네트워크에 있는 경우에 프록시 서버를 사용 하도록 WSUS를 구성할 수 있습니다. WSUS와 인터넷 간에 기업 방화벽이 있으면 WSUS가 업데이트를 가져올 수 있도록 방화벽을 구성 해야 합니다.
...
-   **프록시** WSUS 서버에서 프록시 설정을 구성 해야 하는 경우 WSUS가 프록시 서버를 사용 하 여 인터넷에 액세스 해야를 식별 합니다.

-   **방화벽** WSUS가 회사 방화벽 뒤에 있는 식별 하는 경우 WSUS 트래픽이 올바르게 허용 하는에 지 장치에서 수행 해야 하는 몇 가지 추가 단계가 있습니다.

### <a name="211-connection-from-the-wsus-server-to-the-internet"></a>2.1.1. WSUS 서버에서 인터넷 연결
WSUS와 인터넷 간에 기업 방화벽이 있으면 WSUS 업데이트를 가져올 수 있도록 방화벽을 구성 해야 합니다. Microsoft 업데이트에서 업데이트를 가져오기 위해 WSUS 서버는 HTTPS 프로토콜에 포트 443을 사용합니다. 이 유형의 트래픽 허용 하는 대다수 회사 방화벽, 있지만 회사 보안 정책 때문에 서버에서 인터넷 액세스를 제한 하는 일부 회사에서는 있습니다. 회사에 액세스를 제한 하는 경우 WSUS에서 다음 Url 목록에 인터넷 액세스를 허용 하는 권한을 얻어야 해야 합니다.
...
3.  관리자 권한으로 명령 프롬프트(Cmd.exe)를 엽니다. 관리자 권한으로 명령 프롬프트를 열려면 **시작**으로 이동합니다. **검색 시작**, 형식 **명령 프롬프트**합니다. 시작 메뉴의 맨 위에 있는 마우스 오른쪽 단추로 클릭 **명령 프롬프트**를 클릭 하 고 **관리자 권한으로 실행**합니다. 경우는 **사용자 계정 컨트롤** (요청) 하는 경우 적절 한 자격 증명을 입력, 새로운을 차례로 클릭 작업이 표시 되는지 확인 대화 상자가 나타나면 **계속**합니다.
...
4.  기존 프록시 서버 구성을 수정하려면 다음 옵션을 사용합니다.
    ...
    4.  선택, ff 프록시 서버는 특정 사용자 계정을 사용 해야 합니다 **사용자 자격 증명을 사용 하 여 프록시 서버에 연결할** 확인란 합니다. 해당 텍스트 상자에 필요한 사용자 이름, 도메인 및 암호를 입력 합니다.
...
## <a name="BKM_ConfigureWSUSusingConfigurationWizard"></a>2.2. 네트워크 구성 마법사를 사용하여 WSUS 구성
...
1.  서버 관리자의 탐색 창에서 **대시보드** 및 **도구**를 차례로 클릭한 다음 **Windows Server Update Services**를 클릭합니다.
    ...
    > [!NOTE]
    > 경우는 **WSUS 설치 완료** 대화 상자가 나타나면 **실행**합니다. 에 **WSUS 설치 완료** 대화 상자에서 클릭 **닫기** 설치가 성공적으로 완료 되 면 합니다.

2.  Windows Server Update Services 마법사가 열립니다. **시작하기 전** 페이지에서 정보를 검토하고 **다음**을 클릭합니다.

3.  **Microsoft 업데이트 개선 프로그램에 참여** 페이지의 지침을 읽고 참여할지를 평가합니다. 프로그램에 참여하려는 경우 기본 선택 항목인 유지 하거나 확인란의 선택을 취소 하 고, 클릭 **다음**합니다.
...
4.  **업스트림 서버 선택** 페이지에는 두 가지 옵션이 있습니다.
    ...
    2.  다른 Windows Server Update Services 서버에서 동기화
        ...
        -   복제본 서버를 선택 합니다 **이것이 업스트림 서버의 복제본** 확인란 합니다.
...
7.  특정 사용자 자격을 사용 하 여 프록시 서버에 연결 하려는 경우는 **사용자 자격 증명을 사용 하 여 프록시 서버에 연결할** 확인란을 선택한 다음 해당 사용자 이름, 도메인 및 사용자의 암호를 입력 상자입니다. 프록시 서버에 연결 하려는 사용자에 대 한 기본 인증을 사용 하도록 설정 하려는 경우는 **기본 인증 허용 (암호를 일반 텍스트로 보냄)** 확인란 합니다.

8.  **다음**을 클릭합니다. 에 **업스트림 서버에 연결** 페이지에서 클릭 **연결을 시작**합니다.

9. 서버에 연결되면 **다음** 을 클릭해 계속 진행합니다.

10. 에 **언어 선택** WSUS는 업데이트-모든 언어 또는 언어 하위 집합을 수신 하는 데에서 언어를 선택 하는 옵션이 있는 페이지. 언어의 하위 집합을 선택 하면 디스크 공간을 절약 됩니다 이지만 모든이 WSUS 서버의 모든 클라이언트에 필요한 언어를 선택 하는 중요 합니다. 특정 언어에 대해서만 업데이트를 선택 하면 선택할 **이러한 언어의 업데이트만 다운로드**, 한 다음 업데이트에 사용할;이 고, 그렇지 기본 선택을 그대로 유지 하는 언어를 선택 합니다.

    > [!WARNING]
    > 옵션을 선택 하는 경우 **이러한 언어의 업데이트만 다운로드**,이 서버에 연결 된 다운스트림 WSUS 서버를이 옵션에는 다운스트림 서버 에서도 선택한 언어만 사용 하도록 강제 적용 됩니다.

11. 배포에 사용할 언어 옵션을 적절히 선택한 후 **다음** 을 클릭해 계속 진행합니다.

12. **제품 선택** 페이지에서는 원하는 업데이트 제품을 지정할 수 있습니다. Windows, 또는 Windows Server 2008 등 특정 제품을 등의 제품 범주를 선택 합니다. 제품 범주를 선택 하면 해당 범주의 모든 제품을 선택 합니다.

13. 배포에 대 한 적절 한 제품 옵션을 선택 하 고 클릭 **다음**합니다.
...
15. **동기화 일정 설정** 페이지에서 동기화를 수동으로 수행할지, 자동으로 수행할지 선택할 수 있습니다.
    ...
    -   선택 하면 **자동으로 동기화**, 정해진 주기로 WSUS 서버가 동기화 됩니다.
...
17. **마침** 페이지에서 **초기 동기화 시작** 확인란을 선택하면 지금 동기화를 시작할 수 있습니다. 이 옵션을 선택 하지 않는 경우에 WSUS 관리 콘솔을 사용 하 여 초기 동기화를 수행 해야 합니다. 추가 설정에 대해 자세히 알아보려면 **다음**을 클릭하고, 이 마법사를 끝내고 초기 WSUS 설정을 완료하려면 **마침**을 클릭합니다.

18. **마침**을 클릭하면 WSUS 관리 콘솔이 나타납니다.

이제 기본적인 WSUS 구성을 작업을 수행했습니다. WSUS 관리 콘솔을 사용해 설정을 변경하는 방법에 대한 자세한 내용은 다음 섹션을 읽어보세요.

## <a name="BKMK_ConfigcomputerGroups"></a>2.3. WSUS 컴퓨터 그룹 구성
컴퓨터 그룹은 Windows Server Update Services (WSUS) 배포의 중요 한 부분입니다. 컴퓨터 그룹을 구성해 특정 컴퓨터의 업데이트를 테스트하고 작업 대상으로 삼을 수 있습니다. 기본 컴퓨터 그룹에는 모든 컴퓨터와 할당 되지 않은 컴퓨터입니다. 기본적으로 각 클라이언트 컴퓨터가 WSUS 서버에 처음 접속하면 서버는 해당 클라이언트 컴퓨터를 이러한 두 그룹에 모두 추가합니다.
...
1.  WSUS 관리 콘솔에서 아래 **Update Services**WSUS 서버를 확장 하 고 **컴퓨터**를 마우스 오른쪽 단추로 클릭 **모든 컴퓨터**를 클릭및**컴퓨터 그룹 추가**합니다.

2.  에 **컴퓨터 그룹 추가** 대화 상자의 **이름**새 그룹의 이름을 지정 하 고 클릭 한 다음 **추가**합니다.

3.  클릭 **컴퓨터**, 한 다음이 새 그룹에 할당 하려면 컴퓨터를 선택 합니다.

4.  이전 단계에서 선택한 컴퓨터 이름을 마우스 오른쪽 단추로 누른 **멤버 자격 변경**합니다.

5.  에 **컴퓨터 그룹 멤버 자격 설정** 대화 상자, 선택 테스트 그룹을 생성 한 후 클릭 **확인**합니다.

## <a name="BKM_ConfigureClientUpdates"></a>2.4. 클라이언트 업데이트 구성
WSUS 설치 프로그램은 WSUS 서버에 접속하는 각 클라이언트 컴퓨터에 최신 버전의 자동 업데이트를 배포하도록 IIS를 자동으로 구성합니다. 자동 업데이트를 구성하는 최상의 방법은 네트워크 환경에 따라 결정됩니다.

-   Active directory 디렉터리 서비스를 사용 하는 환경에서 기존 도메인 기반 그룹 정책 개체 (GPO)를 사용할 수도 있고 새 GPO를 만들고 키를 누릅니다.

-   Active directory가 없는 환경에서 로컬 그룹 정책 편집기를 사용 하 여 자동 업데이트를 구성 하 고 WSUS 서버에 클라이언트 컴퓨터를 가리킵니다.

> [!IMPORTANT]
> 다음 절차에서는 active directory 네트워크에 실행 되는 가정 합니다. 또한 그룹 정책에 대해 잘 알고 있으며, 이를 사용해 네트워크를 관리하는 것으로 가정합니다.
...
-   [2.3. 컴퓨터 그룹 구성](2-configure-wsus.md#BKMK_ConfigcomputerGroups) 이 항목의

### <a name="BKMK_ConfigureAU"></a>그룹 정책에서 자동 업데이트 구성
active directory에서 네트워크에 설정한 경우에 그룹 정책 개체 (GPO)를 포함 하 고 다음 WSUS 설정을 사용 하 여 해당 GPO를 구성 하 여 하나 또는 여러 컴퓨터를 동시에 구성할 수 있습니다. WSUS 설정만 포함하는 새 GPO를 만드는 것이 좋습니다.

사용자 환경에 적절 한 active directory 컨테이너에이 WSUS GPO를 연결 합니다. 단일 환경에서는 단일 WSUS GPO를 도메인에 연결할 수 있습니다. 보다 복잡한 환경에서는 여러 WSUS GPO를 몇몇 OU(조직 구성 단위)에 연결하여 다양한 WSUS 정책 설정을 다른 유형의 컴퓨터에 적용할 수 있습니다.

##### <a name="to-enable-wsus-through-a-domain-gpo"></a>도메인 GPO를 통해 WSUS를 활성화하려면

1.  에 GPMC 그룹 정책 관리 콘솔 (), WSUS를 구성 하려는 GPO를 찾은 다음 클릭 **편집**합니다.

2.  Gpmc에서 확장 **컴퓨터 구성**, 확장 **정책을**를 확장 **관리 템플릿**를 확장 하 고 **Windows 구성 요소**를 클릭 하 고 **Windows 업데이트**합니다.
...
5.  선택 **클라이언트 측 대상 사용**를 선택 **Enabled**, 다음에이 컴퓨터를 추가할 WSUS 컴퓨터 그룹의 이름을 입력 하 고는 **이 컴퓨터에 대 한 대상 그룹 이름**  상자입니다.

    > [!NOTE]
    > **클라이언트 대상 그룹 사용**을 선택하면 자동 업데이트가 WSUS 서버로 리디렉션될 때 클라이언트 컴퓨터가 자신을 WSUS 서버의 대상 컴퓨터 그룹에 추가할 수 있습니다. 상태는 Enabled로 설정 하는 경우이 컴퓨터는 자신을 특정 컴퓨터 그룹의 구성원으로이 컴퓨터에 배포 된 업데이트 확인을 사용 하 여 WSUS 서버로 정보를 보낼 때. 이 설정은 클라이언트 컴퓨터 그룹이 사용할 WSUS 서버를 나타냅니다. WSUS 서버에서 그룹을 만들고 해당 그룹에 도메인 구성원 컴퓨터를 추가해야 합니다.
...
클라이언트 컴퓨터를 설정한 후에 컴퓨터가 표시 되려면 몇 분 정도 걸립니다 합니다 **컴퓨터** WSUS 관리 콘솔의 페이지입니다. 도메인 기반 그룹 정책 개체로 구성되는 클라이언트 컴퓨터에서는 그룹 정책의 경우 클라이언트 컴퓨터에 새 정책 설정을 적용하는 데 20분 정도 걸릴 수 있습니다. 기본적으로 0-30 분의 임의 오프셋 90 분 마다 백그라운드에서 그룹 정책 업데이트 합니다. 그룹 정책을 더 자주 업데이트 하려는 경우에 클라이언트 컴퓨터와 형식 gpupdate /force에서 명령 프롬프트 창을 열면 됩니다.

로컬 그룹 정책 편집기를 사용 하 여 구성 된 클라이언트 컴퓨터에 대 한 GPO가 즉시 적용 및 업데이트 20 분 정도가 걸립니다. 검색을 수동으로 시작 하는 경우 WSUS에 연결할 클라이언트 컴퓨터에 대 일 분이 소요 필요가 없습니다.
...
## <a name="bkmk_2.5.ConfigSSL"></a>2.5. SSL(Secure Sockets Layer) 프로토콜을 사용한 WSUS 보안
...
WSUS는 업데이트 파일이 아닌 메타데이터에만 SSL을 사용합니다. Microsoft 업데이트도 이러한 방식으로 업데이트를 배포합니다. Microsoft는 각 업데이트에 서명하여 암호화되지 않은 채널을 통해 업데이트 파일을 보내는 위험을 줄입니다. 또한 해시가 계산되어 각 업데이트의 메타데이터와 함께 전송됩니다. 업데이트를 다운로드할 때 WSUS는 디지털 서명 및 해시를 확인합니다. 업데이트를 변경한 경우 설치 되지 않았습니다.

### <a name="ssllmits"></a>WSUS SSL 배포의 제한 사항
SSL을 사용 하 여 WSUS 배포를 보호 하는 경우 다음 제한 사항을 고려해 야 합니다.

1.  SSL을 사용하면 서버 워크로드가 증가합니다. 네트워크를 통해 전송되는 모든 메타데이터를 암호화해야 하므로 성능이 10% 정도 떨어집니다.

2.  원격 SQL Server 데이터베이스를 사용 하 여 WSUS를 사용 하는 경우 WSUS 서버와 데이터베이스 서버 간 연결이 SSL로 보호 되지 않습니다. 데이터베이스 연결을 보호 해야 하는 경우 다음 권장 사항을 따릅니다.

-   WSUS 서버에 WSUS 데이터베이스를 이동 합니다.
...
### <a name="consswsus"></a>WSUS 서버에서 SSL을 구성 합니다.
...
-   전체 WSUS 웹 사이트에서 SSL이 필요하도록 구성하면 WSUS 사이트로의 모든 트래픽이 암호화되므로 이렇게 구성할 수는 없습니다. WSUS는 업데이트 메타데이터만 암호화합니다. 컴퓨터를 HTTPS 포트에 업데이트 파일을 검색 하려고 하는 경우 전송 실패 합니다.
...
-   로컬 컴퓨터 신뢰할 수 있는 루트 CA 저장소 또는 다운스트림 WSUS 서버의 Windows Server Update Service 신뢰할 수 있는 루트 CA 저장소로 CA(인증 기관)의 인증서를 가져와야 합니다. 로컬 사용자 신뢰할 수 있는 루트 CA 저장소를 인증서만 가져온 경우 다운스트림 WSUS 서버가 업스트림 서버에서 인증 되지 않습니다.

    IIS에서 SSL 인증서를 사용 하는 방법에 대 한 자세한 내용은 참조 하세요. [Secure Sockets Layer 필요 (IIS 7)](https://go.microsoft.com/fwlink/?LinkID=203846)합니다.
...
-   모든 포트를 SSL에 대해 사용할 수 있습니다. 그러나 SSL을 사용하도록 설정한 포트는 WSUS에서 암호화되지 않은 HTTP 트래픽을 보낼 때 사용할 포트를 결정합니다. 다음 예를 참조하세요.
    ...
    -   HTTPS 트래픽에 대 한 업계 표준 포트인 443을 사용 하는 경우 WSUS는 암호화 되지 않은 HTTP 트래픽을 대 한 업계 표준 포트인 80을 사용 합니다.

    -   HTTPS 트래픽에 443 이외의 포트를 사용 하는 경우 WSUS는 암호화 되지 않은 HTTP 트래픽을 HTTPS에 포트 보다 앞에 오는 하는 포트를 통해 보냅니다. 예를 들어 HTTPS에 포트 8531을 사용하는 경우 HTTP에는 포트 8530이 사용됩니다.
...
### <a name="consslclient"></a>클라이언트 컴퓨터에서 SSL을 구성 합니다.
클라이언트 컴퓨터에서 SSL을 구성하는 경우 다음 문제를 고려해야 합니다.

-   WSUS 서버의 보안 포트에 대한 URL을 포함해야 합니다. 서버에서 SSL을 요구할 수 없으므로 클라이언트 컴퓨터에서 보안 채널을 사용하려면 HTTPS를 지정하는 URL을 사용해야 합니다. SSL에 443 이외의 포트를 사용 하는 경우 URL에 해당 포트도 포함 해야 합니다.

-   클라이언트 컴퓨터에서 인증서는 로컬 컴퓨터 신뢰할 수 있는 루트 CA 저장소 또는 자동 업데이트 서비스 신뢰할 수 있는 루트 CA 저장소로 가져올 해제 해야 합니다. 인증서를 로컬 사용자의 신뢰할 수 있는 루트 CA 저장소에만 가져올 수 없는 경우 자동 업데이트 서버 인증에 실패 합니다.

-   클라이언트 컴퓨터가 WSUS 서버에 바인딩하는 인증서를 신뢰해야 합니다. 사용되는 인증서의 형식에 따라 클라이언트 컴퓨터에서 WSUS 서버에 바인딩된 인증서를 신뢰하도록 서비스를 설정해야 할 수 있습니다.

### <a name="consslds"></a>다운스트림 WSUS 서버에 대해 SSL 구성
...
2.  클릭 **시작**, 클릭 **프로그램도**, 클릭 **관리 도구**를 클릭 하 고 **Windows Server Update Service**.
...
6.  에 업스트림 서버 이름을 입력 합니다 **서버 이름** 입력란입니다. SSL 연결을 위해 서버를 사용 하는 포트 번호를 입력 합니다 **포트 번호** 입력란입니다.

7.  선택 된 **업데이트 정보를 동기화 할 때 사용 하 여 SSL** 확인란을 선택한 다음 클릭 **확인**합니다.

### <a name="addlsl"></a>추가 SSL 리소스
...
기본적으로 기본 및 모든 새 IIS 웹 사이트에 대해 익명 읽기 권한이 사용됩니다. 일부 응용 프로그램, 특히 Windows SharePoint Services에 대한 익명 액세스를 제거할 수 있습니다. 이 문제가 발생 하는 경우 성공적으로 설치 하 고 WSUS 작동 전에 익명 읽기 액세스를 다시 설정 해야 합니다.
...
3.  **인증서 업데이트/해지**. 업데이트 하거나 (예: 검색 한 후 만료 된 것) 인증서를 해지 하려는 경우 WSUS는이 사용 하도록 설정 하려면 기능을 제공 합니다. 이 작업을 수행하면 수동 작업으로 전환되어 수동으로 또는 자동으로 수행하기가 매우 어려웠습니다.
