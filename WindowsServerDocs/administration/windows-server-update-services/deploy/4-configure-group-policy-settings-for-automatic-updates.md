...
---
# <a name="step-4-configure-group-policy-settings-for-automatic-updates"></a>4 단계: 자동 업데이트에 대 한 그룹 정책 설정 구성
...
-   그룹 정책 관리 편집기 사용에 대 한 일반적인 지침을 제공 하 고 그룹 정책의 업데이트 서비스 정책 확장 및 유지 관리 스케줄러 설정에 액세스 하는 방법에 대 한 정보를 제공 하는 [그룹 정책의 Windows 업데이트 설정에 액세스 합니다](#accessing-the-windows-update-settings-in-group-policy).
...
GPME에서 컴퓨터 기반 구성에 대 한 Windows 업데이트 정책은 경로: *PolicyName* > **컴퓨터 구성** > **정책** > **관리 템플릿** > **Windows 구성 요소** ** > Windows 업데이트에 있습니다.**
...
|**Enabled**|Windows 자동 업데이트 및 Microsoft Update 포함 되도록 관리자 이외의 사용자가 로그인 한 사용자는 업데이트 알림을 받고 결정할 때 지정 합니다. 관리자가 아닌 사용자는 알림을 받은 모든 선택적, 권장 및 중요 업데이트 콘텐츠를 설치할 수 있습니다. 사용자는 사용자 계정 컨트롤 창이 표시 되지 않습니다 및 사용자 인터페이스, 최종 사용자 사용권 계약 또는 Windows Update 설정 변경 내용이 포함 된 업데이트의 경우를 제외 하 고 이러한 업데이트를 설치 하려면 관리자 권한이 필요는 없습니다.<br /><br />다음 두 가지 경우가이 설정의 효과 운영 체제 컴퓨터에 따라 달라 집니다.<br /><br />1. 업데이트 **숨기기** 또는 **복원**<br />2. 업데이트 설치 **취소**<br /><br />Windows Vista 또는 Windows XP에서이 정책 설정을 사용 하는 경우 사용자는 사용자 계정 컨트롤 창이 표시 되지 않습니다 및 숨기기, 복원 또는 업데이트를 취소 하려면 높은 권한이 필요는 없습니다.<br /><br />Windows Vista에서이 정책 설정을 사용 하는 경우 사용자는 사용자 계정 컨트롤 창이 표시 되지 않습니다 및 숨기기, 복원 또는 업데이트를 취소 하려면 높은 권한이 필요는 없습니다. 이 정책 설정을 사용 하지 않는 경우 사용자는 항상 한 계정 컨트롤 창이 표시 및 숨기기, 복원 또는 업데이트를 취소 하려면 높은 권한이 필요 합니다.<br /><br />Windows 7에서는이 정책 설정은 효과가 없습니다. 사용자는 항상 한 계정 컨트롤 창이 표시 하며 이러한 작업을 수행 하려면 관리자 권한이 필요 합니다.<br /><br />Windows 8 및 Windows RT에서이 정책 설정은 효과가 없습니다.|
|**사용 안 함**|관리자만 로그온 업데이트 알림을 받도록 지정 합니다. **참고:** Windows 8 및 Windows RT에서는이 정책 설정은 기본적으로 사용 하도록 설정 합니다. Windows의 모든 이전 버전에서는 기본적으로 비활성화 됩니다.|
...
> [!NOTE]
> *PolicyName* > **컴퓨터 구성** > **정책** > **관리 템플릿** > **windows 구성 요소** ** > Windows 업데이트 > ** **"업데이트 설치 및 시스템 종료" 옵션을 표시 하지** 않는 경우이 정책 설정은 영향을 주지 않습니다.
...
|**Enabled**|이 정책 설정을 사용 하도록 설정 하는 경우 사용자의 마지막 종료 선택 (예: 최대 절전 모드 또는 다시 시작)은 **컴퓨터에서 수행할 작업을 선택** 하십시오. 메뉴에서 **업데이트 설치 및 시스템 종료** 옵션을 사용할 수 있는지 여부에 관계 없이 **Windows 종료** 대화 상자에서 기본 옵션입니다.|
...
**옵션:** 는 예약 된 다시 시작 하는 방법에 대 한 사용자가 다시 입력 하기 전에 경과 되는 시간 (분)에 지정 하려면이 설정을 옵션을 사용할 수를 사용 합니다.
...
**옵션:** 이 정책 설정을 사용 하는 경우는 컴퓨터를 다시 시작 후 시간 (분) 수를 지정 하려면 사용할 수 있습니다, 수행 되지 않은 예약된 된 설치를 이전 배치 발생 합니다.
...
**옵션:** 이 정책 설정을 사용 하는 경우 업데이트를 검색 하는 경우 WSUS 클라이언트가 사용할 하 고 인터넷 통계 서버에 업데이트 WSUS 클라이언트가 인트라넷 업데이트 서비스는 통계를 업로드 하는 지정 해야 합니다. 예제 값:
...
옵션을 선택 하면 자동 업데이트 구성 설정에서 **4-자동으로 다운로드 하 고 설치 예약**, 를 지정할 수 있습니다 일정 Windows 8 및 Windows RT를 실행 하는 컴퓨터에 대 한 GPMC에서 유지 관리 스케줄러 설정 "자동 업데이트 구성" 설정에 4 옵션을 선택 하지 않은 경우 자동 업데이트 하기 위해 이러한 설정을 구성할 필요가 없습니다. 유지 관리 스케줄러 설정은 경로: *PolicyName* > **컴퓨터 구성** > **정책** > **관리 템플릿** > **Windows 구성 요소** > **유지 관리 스케줄러**에 있습니다. 그룹 정책의 유지 관리 스케줄러 확장에는 다음 설정을 포함 합니다.
...
|**사용 안 함**|이 정책 설정을 **사용 안 함**으로 설정 하면 **작업 센터** > **자동 유지 관리**에 지정 된 매일 예약 된 시간이 제어판에 적용 됩니다.|
...
GPMC에서 컴퓨터 자동 업데이트에 대 한 사용자 설정은 경로: *PolicyName* > **사용자 구성** > **정책** > **관리 템플릿** > **Windows 구성 요소** ** > Windows 업데이트에 있습니다.** 설정은 그룹 정책의 컴퓨터 구성 및 사용자 구성 확장에 표시 되는 순서 대로 나열 됩니다 Windows 업데이트 정책의 **설정** 탭을 선택 하 여 설정을 사전순으로 정렬 합니다.
...
> [!NOTE]
> 이 정책 설정은 windows **종료 대화 상자에서 "업데이트 설치 및 시스템 종료" 옵션을 표시 하지** 않는 *PolicyName* > **사용자 구성** > **정책** > **관리 템플릿** ** >  > ** 에 영향을 주지 않습니다. > 
...
1.  그룹 정책 관리 편집기에서 다음 중 하나를 수행 합니다.
    ...
    -   **그룹 정책의 Windows 업데이트 확장 > 컴퓨터 구성을 엽니다**. *PolicyName* > **컴퓨터 구성** > **정책** / **관리 템플릿** > **Windows 구성 요소** ** > Windows 업데이트로 이동 합니다.**

    -   **사용자 구성 > 그룹 정책의 Windows Update 확장**합니다. *PolicyName* > **사용자 구성** > **정책** > **관리 템플릿** > **Windows 구성 요소** ** > Windows 업데이트으로 이동 합니다.**
...
|용어|정의|
|----|-------|
|자동 업데이트|**Windows 컴퓨터에서 실행 되는 서비스** (자동 업데이트): Microsoft Update 또는 Windows Update에서 업데이트를 받도록 SP3 운영 체제와 Microsoft Windows Vista, Windows Server 2003, Windows XP 및 Windows 2000에 내장 된 클라이언트 컴퓨터 구성 요소를 나타냅니다.<br /><br />**일반 참조** (자동 업데이트): 시기를 설명 하기 위해 용어는 Windows Update 에이전트가 자동으로 예약 하 고 업데이트를 다운로드 합니다.|
...
|Microsoft 업데이트|**인터넷 기반 Microsoft 다운로드 사이트:** 는 Microsoft 인터넷 사이트를 저장 하 고 Windows 컴퓨터 (장치 드라이버), Windows 운영 체제 및 기타 Microsoft 소프트웨어 제품에 대 한 업데이트를 배포 합니다.|
...
|Windows 업데이트|**인터넷 기반 Microsoft 다운로드 사이트:** 는 Microsoft 인터넷 사이트를 저장 하 고 Windows 컴퓨터 (장치 드라이버) 및 Windows 운영 체제에 대 한 업데이트를 배포 합니다.<br /><br />**컴퓨터 서비스:** 컴퓨터에서 실행 되는 Windows 업데이트 서비스의 이름입니다. Windows Update를 검색, 다운로드, 및 Windows 컴퓨터에 업데이트를 설치 합니다.<br /><br />컴퓨터 및 정책 구성에 따라 Windows Update 에이전트는에서 업데이트를 다운로드할 수 있습니다.<br /><br />Microsoft 업데이트<br />Windows 업데이트<br />-Microsoft Store<br />-인터넷 (네트워크) 업데이트 서비스 (WSUS)<br /><br />WSUS 기반 환경에서 관리 되지 않는 컴퓨터는 일반적으로 인터넷을 통해 직접 연결 하는 Windows 업데이트 (Windows 업데이트, Microsoft 업데이트 또는 Microsoft Store를 사용 하 여 업데이트를 가져옵니다.|
