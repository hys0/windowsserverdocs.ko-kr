...
description: 설치 하 고 Software Inventory Logging Aggregator를 관리 하는 방법을 설명 합니다.
ms.custom: na
...
ms.locfileid: "59835464"
---
# <a name="software-inventory-logging-aggregator"></a>소프트웨어 인벤토리 로깅 집계

>적용 대상: Windows Server 2012 R2

## <a name="what-is-software-inventory-logging-aggregator"></a>Software Inventory Logging Aggregator 란?
...
SILA는 Windows Server에 설치하지만 Windows Server 설치에 포함되지 않는 소프트웨어입니다. 소프트웨어를 설치 하려면 먼저 다운로드 무료로 Windows 다운로드 센터에서: [Windows Server 용 software Inventory Logging Aggregator 1.0](https://www.microsoft.com/en-us/download/details.aspx?id=49046)

소프트웨어 인벤토리 로깅 프레임워크의 목적은 IT 환경 내의 많은 서버를 통해 배포되는 Microsoft 소프트웨어의 인벤토리 작성과 관련한 운영 비용을 절감하는 데 있습니다. 이 프레임 워크 두 구성 요소,이 SIL Aggregator 및 소프트웨어 인벤토리 로깅 (SIL), Windows Server 2012 R2에 도입 된 Windows Server 기능을 구성 합니다. 이 Software Inventory Logging Aggregator 1.0은 단일 서버에 설치되며, SIL을 통해 데이터를 전달하도록 구성된 모든 Windows Server에서 인벤토리 데이터를 받습니다. 데이터 센터 관리자가 마스터 Windows Server 이미지의 SIL을 사용하여 환경에서 광범위하게 배포할 수 있도록 설계되었습니다.  이 소프트웨어 패키지는 대상 지점이며, 고객이 시간에 따른 쉬운 인벤토리 데이터 로깅을 위해 사내에 설치하도록 고안되었습니다. 또한 이 소프트웨어를 사용하면 Microsoft Excel에서 기본 인벤토리 보고서를 정기적으로 만들 수 있습니다. Software Inventory Logging Aggregator 1.0 보고서에는 Windows Server, System Center 및 SQL Server의 설치 수가 포함되어 있습니다.
...
-   최고 수 위 표시 수 및 동시에 관리 되는 실행 중인 호스트 이름 \(System Center 에이전트가 있는\) 시간이 지남에 따라 각 호스트에서 Windows Server Vm

-   관리 되는 높은 Vm에 설치 된 System Center 에이전트의 이름을 계산\-최고 수 위 표시

-   수 및 시간에 따른 SQL Server 설치 위치 \(Sku 및 라이선스를 필요로 하는 버전\)

-   추가에 설치 된 소프트웨어 목록을\/프로그램 제거

### <a name="who-will-use-sil"></a>SIL의 사용자는 누구인가요?

-   시간에 따라 중요 소프트웨어 인벤토리 데이터를 자동으로 수집하기 위한 경제적인 방법을 찾고 있는 **IT 전문가 또는 데이터 센터 관리자**

-   조직의 IT 배포에 포함된 Microsoft 엔터프라이즈 소프트웨어의 사용량을 보고해야 하는 **CIO 및 재무 관리자**
...
>[!Note]
>Windows 관리 프레임 워크를 사용 하는 경우만 SIL Aggregator에서 5.1, WMF 릴리스에서 알려진된 호환성 문제가 있습니다.  WMF 4.0 SIL Aggregator를 사용 하 여 서버에서 설치를 초과 하는 데 필요한 것입니다.

소프트웨어 인벤토리 로깅 (SIL) 설치 된 다음 업데이트를 사용 하 여 Windows Server 버전에 있습니다.
...
SIL 및 SIL Aggregator에서 인증된 통신을 위해 SSL 인증서를 사용합니다. 이 일반 구성에서는 인벤토리 데이터를 받는 웹 서비스를 호스트하기 위해 하나의 인증서(서버 이름 및 인증서 이름 일치)로 SIL Aggregator를 설치합니다. 그런 다음 SIL 기능을 사용하여 인벤토리를 만들 Windows Server에서 다른 클라이언트 인증서를 사용하여 데이터를 SIL Aggregator에 푸시합니다. PowerShell cmdlet(Set-SilAggregator, 아래의 자세한 내용 참조)을 사용하여 집계에서 연결된 데이터를 수락할 SIL Aggregator의 승인된 인증서 목록에 인증서 지문을 추가해야 합니다. SIL Aggregator에서는 인증서로 각 데이터 페이로드를 인증한 후 데이터베이스 처리 및 삽입을 계속합니다. 이 과정에 대한 자세한 내용은 **SIL Aggregator Cmdlet 세부 정보** 섹션을 참조하세요.

### <a name="polling-account-setup"></a>폴링 계정 설정
폴링 작업을 사용하기 위해 SIL Aggregator에 자격 증명을 추가할 경우 최소 권한 계정 접근 방식을 사용해야 합니다. 또한 보안을 극대화하려면 데이터 센터 또는 다른 IT 배포의 각 호스트에 대해 서로 다른 자격 증명을 사용하는 것이 좋습니다.
...
12. 권한 섹션 맨 아래에 있는지를 확인 합니다 **원격 으로부터 사용 가능** 확인란이 선택 되어 있습니다.
...
-   이 소프트웨어의 웹 서비스를 호스트하는 데 사용할 **유효한 SSL 인증서가 있어야 합니다**.
...
6.  **계정 유형 선택**에서 기본 설정에 따라 **로컬 사용자** 또는 **gMSA**를 선택합니다.
    ...
    -   Active Directory의 gMSA 사용 보안 그룹에 컴퓨터 계정을 추가한 후 서버를 다시 부팅해야 합니다.
...
|`Add-SilVmHost`|특정 호스트 이름 또는 호스트 이름, 일정 한 간격으로 폴링됩니다 배열을 추가 하는 데 \(기본값은 1 시간 간격\)합니다.|
...
-   SIL Aggregator에서:
    ...
    -   `Start-SilAggregator`를 실행합니다.
        ...
        인벤토리를 만들도록 설정했거나 설정할 서버에서 HTTPS를 통해 전달되는 데이터를 집계 중에 적극적으로 받으려면 이 명령을 실행해야 합니다. 서버에서는 최대 30일 동안 데이터 페이로드를 로컬로 캐싱하므로, 이 집계에 먼저 전달하도록 서버를 설정한 경우에도 문제가 되지 않습니다. 한 번만 집계에서 "targeturi? 중인지 및 모든 캐시 된 데이터가 집계에 한 번에 전달 됩니다 하 고 모든 데이터는 처리를 실행 합니다.
    ...
    -   실행 `Set-SilAggregator –AddCertificateThumbprint` "your client certificate's thumbprint?

        SIL 로깅을 사용하도록 설정한 Windows Server에서 HTTPS를 통해 데이터를 받으려면 이 명령을 실행해야 합니다. 지문은 SIL Aggregator에서 데이터를 수락할 지문 목록에 추가됩니다. SIL Aggregator는 유효한 엔터프라이즈 클라이언트 인증 인증서를 수락하도록 설계되었습니다. 사용 된 인증서를 설치 해야 합니다는  **\\localmachine\MY (로컬 컴퓨터-> 개인**) 데이터를 전달 하는 서버에 저장 합니다.
...
-   인벤토리를 만들 Windows Server에서 관리자 권한으로 PowerShell을 열고 다음 명령을 실행합니다.
    ...
    -   `Set-SilLogging –TargetUri “https://contososilaggregator�? –CertificateThumbprint “your client certificate’s thumbprint�?`를 실행합니다.
        ...
        -   이 명령은 Windows Server의 SIL에 인벤토리 데이터를 보낼 위치와 인증에 사용할 인증서를 알려줍니다.
            ...
            > [!IMPORTANT]
            > “https://’ 가 TargetUri 값에 있는지 확인합니다.
    ...
    -   `Start-SilLogging`를 실행합니다.
        ...
        이 명령은 SIL 로깅을 시작합니다. 매시간 임의의 간격으로 SIL은 `–targeturi` 매개 변수에 지정된 집계에 인벤토리 데이터를 전달합니다. 처음에는 전체 데이터 집합을 전달합니다. 각 후속 전달 "하트 비트 자세히 됩니다? 바로 데이터를 식별 하는 아무 것도 변경 되었습니다. 데이터 집합이 변경된 경우 다른 전체 데이터 집합이 전달됩니다.
...
SIL 푸시와 끌어오기 모드에서 작동 및 병렬 작동 하는 두 가지 구성 요소가 구성 됩니다. Windows Server 및 소프트웨어 인벤토리 로깅 집계 (SILA) 다운로드 가능한 MSI의 소프트웨어 인벤토리 로깅 (SIL) 기능입니다. 인벤토리를 만들 서버에서는 SIL을 사용하여 HTTPS를 통해 소프트웨어 인벤토리 데이터를 SIL Aggregator에 푸시합니다(1시간마다 각 시간 내의 임의의 시점에). 집계에서는 실제 하이퍼바이저 호스트를 차례로 폴링하거나 쿼리하여 1시간마다 하드웨어 인벤토리 데이터를 끌어옵니다. SIL의 전체 기능을 사용하려면 푸시와 끌어오기 모두를 적절히 구성해야 합니다. 푸시와 끌어오기를 순서에 관계없이 구성할 수 있습니다. 하지만 집계에서 큐브 처리는 하루에 한 번만 발생하므로, 집계에서 푸시 또는 끌어보기를 통해 캡처되는 데이터는 다음 날까지 보고서에 표시되지 않습니다.
...
Software Inventory Logging Aggregator 보고서의 기본 측면은 어떤 일반적으로 라고 하는 "상위 워터 마크의 캡처? Windows 서버를 동시에 실행 합니다. 이는 이러한 보고서의 Windows Server 및 System Center 수에 적용됩니다. Windows Server의 경우 각 실제 호스트에는 OS 유형에 상관없이 1개월의 과정 동안 대부분의 Windows Server VM이 동시에 실행 중인 시점이 있습니다. 이를 해당 월에 대한 최고 수위 표시라고 합니다. System Center의 경우 매월 각 실제 호스트에서 대부분의 관리되는 Windows Server가 동시에 실행 중인 시점이 있습니다. 관리되는 서버는 하나 이상의 System Center 에이전트가 있을 때 식별됩니다. 보고서에는 실제 호스트에 대한 최신 최고 수위 표시만 표시됩니다. 최고 수위 표시 이후의 데이터는 표시되지 않습니다. 해당 시점 이후에는 Windows Server VM(WS 탭) 또는 관리되는 Windows Server VM(SC 탭)의 수가 최고 수위 표시 아래로 떨어진 것으로 간주될 수 있습니다. 이 사용량 추적 및 표시 방법은 이러한 제품에 대한 라이선스 모델을 지원하고 용량 계획을 돕기 위한 것입니다.

SQL 관련 보고서에서 SQL Server 설치는 누적; 계산 탭 여 hig 워터 마크입니다. 합계는 실행 중인 SQL Server 설치 수입니다.

> [!NOTE]
> 소프트웨어 인벤토리 로깅을 사용하는 것이 해당 사용 조건에 따라 Microsoft 소프트웨어 사용을 정확히 보고해야 하는 의무를 대체하지는 않습니다.

### <a name="poll-date-time"></a>폴링 날짜/시간
Software Inventory Logging Aggregator를 사용할 경우 최고 수위 표시 수는 폴링을 기반으로 집계됨을 알고 있어야 합니다. 즉, 최고 수위 표시는 기본 실제 호스트의 폴링을 통해서만 캡처할 수 있습니다. 따라서 최고 수 위 표시 수는 직접 연결 된 해당 "폴링 날짜 시간입니다? 폴링 간격을 조정할 수 있지만, 높은 간격 값을 사용할 경우 캡처되는 최고 수위 표시의 충실도에 영향을 줍니다. 간격이 높을수록 데이터가 실제 사용을 대표하는 정도가 낮아집니다.
...
-   "알 수 없는 OS를 사용 하 여 VM이 나열 되는 경우? 집계 되지 받았는지 전체 데이터 페이로드를 SIL 통해 해당 VM에서 HTTPS를 통해 의미 합니다.

-   "알 수 없는 호스트 아래에 나열 된 Vm? Windows Server Vm을 성공적으로 집계를 집계를 HTTPS를 통해 인벤토리 데이터를 전달 되었거나 적극적으로 성공적으로 폴링하고 해당 VM에 대 한 기본 호스트입니다. 기본 호스트를 알 수 없으므로 이러한 항목의 수는 항상 0입니다. `Add-SilVMHost` cmdlet을 사용하고 올바른 자격 증명을 제공하여 폴링을 위해 호스트 또는 모든 호스트를 SIL Aggregator에 추가합니다. 폴링된 이후에는 보고서에서 VM 데이터와 호스트 데이터가 연결됩니다.
...
-   "SIL Aggregator? Software Inventory Logging Aggregator 설치 되어 있는 모든 서버 컴퓨터에 표시 됩니다.

-   실제 호스트에서 프로세서 수 또는 실제 메모리 양이 변경되면 보고서에 이전 행과 함께 새 행이 나타납니다. 새로 추가한 호스트인 것처럼 폴링 업데이트가 이전 행에서는 중단되고 새 행에서는 계속됩니다.

-   **요약** 및 **세부 정보** 탭에서 동시에 실행 중인 Windows Server 또는 관리되는 Windows Server에 대한 열이 나열되는 합계는 아래의 모든 호스트에 대한 모든 최고 수위 표시의 합계를 나타냅니다. 이러한 하이퍼바이저 호스트 되지 없습니다 vm 및 실행 하는 Vm 수 있는 서버를 실행 하는 Windows 서버를 포함 하지만 "알 수 있습니까? 데이터가 없는에서 HTTPS 통해 SIL 사용 하는 VM 내에서 수신 되는 합니다. 이러한 항목은 편의상 합계로 계산됩니다.
...
다음은 SIL Aggregator에서 만드는 Excel 기반 보고서의 **Windows 서버 세부 정보** 탭에 있는 각 열에 대한 설명입니다. 다른 데이터 탭은 동일하거나 이러한 열의 하위 집합입니다. 한 가지 예외는 것 "설치 수 있습니까? SQL Server 탭에 (참조 **최고 수 위 표시** 섹션).
...
다음은 SIL Aggregator cmdlet에 대한 세부 정보입니다. 전체 cmdlet 설명서를 참조 하세요. [SIL Aggregator PowerShell cmdlet](https://technet.microsoft.com/library/mt548455.aspx)

### <a name="publish-silreport"></a>Publish-SilReport

-   이 cmdlet은 있는 그대로 사용되며, 소프트웨어 인벤토리 로깅 보고서를 만들어 로그인한 사용자의 Documents 디렉터리에 저장합니다. cmdlet이 실행되는 컴퓨터에 Excel 2013이 있어야 합니다.
...
다음은 `Add-SilVMHost` cmdlet을 사용할 때 지원되는 호스트 유형 및 하이퍼바이저 버전입니다. 이러한 항목을 지정할 필요는 없습니다. `Add-SilVMHost` cmdlet에서는 지원되는 조합을 자동으로 검색합니다. 검색할 수 없거나 제공된 자격 증명이 잘못된 경우 프롬프트가 표시됩니다. "Y를 사용 하 여 사용자가 수락 하는 경우? 항목, 호스트 추가 되지만 폴링 되지 것입니다. 으로 추가 됩니다 "알 수 없는?.
...
-   `SilLogging` 또는 `Publish-Sildata` cmdlet 실패 또는 오류 시 확인해야 할 사항:
    ...
    -   Windows Server에 대한 모든 필요한 업데이트가 설치되어 있는지 확인합니다(SIL에 대한 필수 구성 요소 참조).  확인 하는 빠른 방법은 다음과 같습니다. 다음 cmdlet을 사용 하려면   `Get-SilWindowsUpdate *3060*, *3000*`
    ...
    -   \Windows\System32 전달/푸시 하려는 서버에서 캐시 된 SIL 파일에 대 한 다음 위치를 확인할 수 있습니다\\Logfiles\\SIL 합니다. `SilLogging`이 시작되고 1시간 이상 실행 중이거나 `Publish-SilData`를 최근에 실행했지만 이 디렉터리에 파일이 없는 경우 집계에 대한 로깅이 성공한 것입니다.

-   로그인한 사용자에게 SQL 데이터베이스 및 Analysis Services 액세스 권한이 있는지를 확인합니다.

    -   이 작업은 SIL Aggregator를 설치할 때 수행해야 합니다.

    -   이 원격으로 PowerShell을 사용 하 여 SIL Aggregator를 관리 하는 경우에 필요 합니다.
...
-   gMSA 옵션을 사용할 경우:
    ...
    -   Active Directory의 gMSA 사용 컴퓨터 그룹에 조인한 후 서버를 다시 부팅해야 합니다.

    -   설치 프로세스에서 도메인\사용자를 입력할 때 정규화된 도메인을 사용하지 마세요. 사용 예를 들어 **mydomain\gmsaaccount**합니다. 입력 하지 마세요 **mydomain.<i> </i>com\gmsaaccount**합니다.

-   Windows 관리 프레임 워크를 사용 하 여 환경의 하는 경우:

    -   SILA 설치를 사용 하 여 서버에 설치 하는 WMF 5.1 없는 해야 합니다.  DLL에 대 한 이벤트 로그에 오류가 발생 하는 것이 불가능 **'mpunits.dll'** 합니다.  이렇게 하면 제대로 작동을 하지 것입니다.  SILA는 WMF 4.0이 필요합니다.
...
**데이터 손실을 방지 하기 위해 중요 한 단계:** 서버 데이터를 SIL Aggregator에 전달 하는 하지만 대상 집계는 그대로 유지 되는 인증서를 변경 하는 데 필요한 경우 수집기로 전송에서 하는 잠재적인 데이터 손실을 방지 하려면 다음이 단계를 사용 합니다.
...
-   **중요: 데이터를 전달 하는 모든 서버를 업데이트 한 후에 이전 지문을 제거** 사용 하 여 SIL Aggregator에서 `Set-SilAggregator –RemoveCertificateThumbprint` cmdlet. 서버에서 SIL Aggregator에서 제거된 이전 인증서를 사용하여 데이터를 전달하는 경우 **데이터가 손실되고** 집계 중에 데이터베이스에 삽입되지 않습니다. 이는 서버에서 이전에 SIL Aggregator로 데이터를 전달한 다음 데이터를 수락할 SIL Aggregator의 지문 목록에서 인증서를 제거한 경우에만 해당합니다.
...
-   SIL Aggregator가 SQL Server Standard Edition 설치의 존재 여부를 처리하지 않고 이에 대해 보고하지 않는 알려진 문제가 있습니다.  이 문제를 해결하는 단계는 다음과 같습니다.
    ...
    4.  **dbo.SqlServerEdition**을 마우스 오른쪽 단추로 클릭한 다음 '**상위 200개 행 편집**'을 선택합니다.

    5.  "Standard Edition 옆에 있는 PropertyNumValue를 변경 합니다? 하 **2760240536** 1534726760에서.
...
-   행의 합계 (온 **대시보드** 탭) 및 열 (에 **요약 및 세부 정보** 탭) 레이블이 지정 된 "**동시에 실행 중인**... ?에 Windows Server 및 System Center 모두 두 위치 간에 정확히 일치 하지 않습니다. 에 **대시보드** 추가 하는 데 필요한 것이 탭에 "**Windows Server 장치 (알려진 vm이 없는**)? 값은 "**동시에 실행 중인**... ? 값에 추가하여 **요약 및 세부 정보** 탭에서 이 수를 일치시켜야 합니다.
...
[Windows Server 용 software Inventory Logging Aggregator 1.0](https://www.microsoft.com/en-us/download/details.aspx?id=49046)<br>
[SIL Aggregator PowerShell cmdlet](https://technet.microsoft.com/library/mt548455.aspx)<br>
