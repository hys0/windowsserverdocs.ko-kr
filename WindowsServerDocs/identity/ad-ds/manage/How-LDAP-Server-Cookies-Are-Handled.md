...
이러한 용량 및 로지스틱 문제로 인해 Microsoft LDAP 개발자가 "페이징된 쿼리" 라는 LDAP 확장을 만들 수 있습니다. 이 확장은 LDAP 컨트롤을 구현하여 매우 큰 쿼리를 더 작은 결과 집합 더 작은 결과 집합의 청크로 구분합니다. 그리고 [RFC 2696](http://www.ietf.org/rfc/rfc2696)과 같은 RFC 표준이 되었습니다.  
  
## <a name="cookie-handling-on-client"></a>클라이언트의 쿠키 처리  
페이징된 쿼리 메서드는 클라이언트에서 설정 하거나 [LDAP 정책](https://support.microsoft.com/kb/315071/en-us) ("maxpagesize")을 통해 설정한 페이지 크기를 사용 합니다. 클라이언트는 항상 LDAP 컨트롤을 보내 페이징을 사용하도록 설정해야 합니다.  

  
결과가 많은 쿼리를 사용하는 경우 특정 지점에서 허용되는 최대 개체 수에 도달합니다. LDAP 서버는 응답 메시지를 패키지하고 나중에 검색을 계속하는 데 필요한 정보를 포함하는 쿠키를 추가합니다.  
  
클라이언트 애플리케이션은 쿠키를 불투명 blob으로 처리해야 합니다. 응용 프로그램은 응답에서 개체 수를 검색하고 쿠키의 유무에 따라 검색을 계속할 수 있습니다. 클라이언트는 기준 개체 및 필터와 같은 동일한 매개 변수를 사용하여 다시 LDAP 서버로 쿼리를 전송하여 검색을 계속하고 이전 응답에서 반환된 쿠키 값을 포함합니다.  
  
개체 수가 페이지를 채우지 않으면 LDAP 쿼리가 완료 되 고 응답에 페이지 쿠키가 포함 되지 않습니다. 서버에서 쿠키가 반환되지 않으면 클라이언트는 페이지된 검색이 완료된 것으로 간주해야 합니다.  
...
LDAP 서버에는 [NTDS 진단 키](https://support.microsoft.com/kb/314980/en-us)의 "16 ldap 인터페이스" 범주를 통해 이벤트를 기록 하는 기능이 있습니다. 이 범주를 "2"로 설정 하면 다음과 같은 이벤트를 얻을 수 있습니다.  
...
DC/LDAP 서버에 2898 이벤트가 표시되는 경우 MaxResultSetsPerConn을 25로 설정하는 것이 좋습니다. 단일 LDAP 연결에서 25개 이상의 병렬 페이지된 검색은 일반적이지 않습니다. 2898 이벤트가 계속 표시되는 경우 오류가 발생하는 LDAP 클라이언트 애플리케이션을 조사하는 것이 좋습니다. 어떻게 하든 추가 페이지된 결과를 검색하려고 한다고 의심되면 쿠키를 보류 중 상태로 유지하고 새 쿼리를 다시 시작합니다. 따라서 특정 지점에서 해당 용도에 충분한 쿠키가 애플리케이션에 있는지 확인하려면 MaxResultSetsPerConn 값을 25 이상으로 늘릴 수도 있습니다. 도메인 컨트롤러에 이벤트 2899가 기록된 것을 확인하면 계획은 달라집니다. 메모리가 충분한(몇 GB의 사용 가능한 메모리) 컴퓨터에서 DC/LDAP 서버가 실행되는 경우에는 LDAP 서버에서 MaxResultsetSize를 >=250MB로 설정하는 것이 좋습니다. 이 제한은 매우 큰 디렉터리에 많은 양의 LDAP 페이지 검색을 수용할 만큼 충분히 큽니다.  
  
250MB 이상의 풀에서도 이벤트 2899가 계속 표시되면 많은 클라이언트에서 매우 자주 쿼리되어 매우 많은 개체가 반환되는 것일 수 있습니다. [Active Directory 데이터 수집기 집합](https://blogs.technet.com/b/askds/archive/2010/06/08/son-of-spa-ad-data-collector-sets-in-win2008-and-beyond.aspx) 을 사용하여 수집할 수 있는 데이터를 사용하면 LDAP 서버를 사용 중 상태로 유지하는 반복적인 페이징된 쿼리를 찾을 수 있습니다. 이러한 쿼리는 사용 된 페이지 크기와 일치 하는 많은 "반환 된 항목"과 함께 표시 됩니다.  
  
가능 하면 응용 프로그램 디자인을 검토 하 고이 데이터를 쿼리 하는 더 낮은 빈도, 데이터 볼륨 및/또는 더 적은 클라이언트 인스턴스를 사용 하 여 다른 방법을 구현 해야 합니다. 소스 코드 액세스 권한이 있는 응용 프로그램의 경우 [효율적인 Ad 사용 응용 프로그램을 만들기](https://msdn.microsoft.com/library/ms808539.aspx) 위한이 가이드를 통해 응용 프로그램에서 ad에 액세스할 수 있는 최적의 방법을 파악할 수 있습니다.  
  
쿼리 동작을 변경할 수 없는 경우에는 필요한 명명 컨텍스트의 복제 된 인스턴스를 더 추가 하 고 클라이언트를 재배포 하 여 결국 개별 LDAP 서버에 대 한 부하를 줄이는 방법도 있습니다.  
