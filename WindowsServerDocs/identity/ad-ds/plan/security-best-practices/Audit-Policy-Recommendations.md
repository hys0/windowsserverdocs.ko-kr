...
손상의 징후가 있는지 모니터링할 때 포함 해야 하는 포괄적인 이벤트 목록은 [Appendix L: @ No__t를 모니터링할 이벤트입니다.  
...
@No__t-0Appendix L: 모니터링할 이벤트 (예를 들어, 모니터링할 권장 이벤트 목록, 중요도 등급 및 이벤트 메시지 요약)를 no__t 합니다.  

-   가장 긴밀 하 게 모니터링 되 고 가장 보안이 엄격 구성 해야 하는 서버를 신속 하 게 식별할 수 있도록 작업의 분류를 기준으로 서버를 그룹화 합니다.  

-   다음 AD DS 그룹의 속성 및 멤버 자격에 대 한 변경 내용: EA (Enterprise Admins), DA (Domain Admins), 관리자 (BA) 및 스키마 관리자 (SA)  
...
**최고** 높은 중요도 등급을 사용 하는 이벤트 Id는 항상 경고 및 조사를 통해 즉시 발생 해야 합니다.  

**중간** 보통 중요도 등급의 이벤트 ID는 악의적인 작업을 나타낼 수 있지만 다른 비정상 상태 (예: 특정 기간 내에 발생 하는 비정상적인 숫자, 예기치 않은 항목 또는 일반적으로는 이벤트를 기록 하지 않습니다.) 중간 중요도 이벤트를 메트릭으로 수집 하 고 시간에 따라 비교할 수도 있습니다.  

**거의** 중요도가 낮은 이벤트를 포함 하는 및 이벤트 ID는 중간 또는 높은 중요도 이벤트와 상관 관계가 없는 한 garner 주의 하거나 경고를 발생 시 키 지 않아야 합니다.  
