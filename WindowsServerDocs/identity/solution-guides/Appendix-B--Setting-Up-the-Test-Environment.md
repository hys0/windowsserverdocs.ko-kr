...
1.  가상 컴퓨터를 ID_AD_Network에 연결합니다. DC1에 관리자로 로그인 암호로 **pass@word1**합니다.  
...
12. 에 **도메인 컨트롤러 옵션** 페이지, 도메인 및 포리스트 기능 수준을 Windows Server 2012로 선택 하 고, DSRM 암호를 지정 **pass@word1**를 클릭 하 고 **다음**.  
...
3.  가상 컴퓨터를 contoso.com 도메인에 가입 하 고 다음 암호를 사용 하 여 contoso\administrator로 FILE1에 로그인 **pass@word1**합니다.  
...
1.  암호를 사용 하 여 contoso\administrator로 FILE1에 로그인: **pass@word1**합니다.  
...
19. 에 **서비스 계정** 화면의 **도메인 사용자 계정을**, 클릭 **지정** 사용자 이름을 지정 합니다 (**contoso\rms**), 및 암호 (**pass@word1**)를 클릭 하 고 **확인**를 클릭 하 고 **다음**합니다.  
  
20. **암호화 모드** 화면에서 **암호화 모드 2**를 클릭합니다.  
  
21. **클러스터 키 저장소** 화면에서 **다음**을 클릭합니다.  
  
22. 에 **클러스터 키 암호** 화면의 합니다 **암호** 및 **암호 확인** 상자를 입력 **pass@word1**를 클릭 하 고 **다음**합니다.  
...
28. **결과** 화면에서 **닫기**를 클릭한 다음 **설치 진행률** 화면에서 **닫기**를 클릭합니다. 로그 오프 했다가 제공 된 암호를 사용 하 여 contoso\rms로 로그온 완료 되 면 (**pass@word1**).  
...
31. 사용자 및 권한 섹션에서 클릭 **사용자 및 사용 권한**, 클릭 **추가**, 형식 **financeadmin@contoso.com**를 클릭 하 고 **확인**합니다.  
...
1.  암호를 사용 하 여 CONTOSO\Administrator로 로그온 합니다. **pass@word1**합니다.  
...
1.  가상 컴퓨터를 ID_AD_Network에 연결합니다. DC2에 관리자로 로그인 암호로 **Pass@word1**합니다.  
...
암호를 사용 하 여 사용자 Jeff Low를 만들고 **pass@word1**를 회사 특성 값으로 할당 하 고 **Adatum**합니다.  
...
7.  **확인** 을 세 번 클릭하여 작업을 마치고 Active Directory 관리 센터로 돌아갑니다.  
    ...
    ![솔루션 가이드](media/Appendix-B--Setting-Up-the-Test-Environment/PowerShellLogoSmall.gif)Windows PowerShell 해당 명령을 * * *  
...
1.  Hyper-V 관리자에서 FILE1 서버에 연결합니다. 암호를 사용 하 여 contoso\administrator로 로그인을 사용 하 여 서버에 로그인 **pass@word1**합니다.  
