...
ms.locfileid: "66188313"
---
# <a name="when-to-use-an-authorization-claim-rule"></a>권한 부여 클레임 규칙을 사용하는 경우
Active Directory Federation Services에서이 규칙을 사용할 수 있습니다 \(AD FS\) 들어오는 클레임 유형을 가져오고 사용자 허용 되거나 액세스 거부 될 지 여부를 결정 하는 작업을 적용 해야 할 때 값을 기반으로 이 규칙에 지정 합니다. 이 규칙을 사용하는 경우 규칙에서 구성하는 옵션 중 하나에 따라 다음 규칙 논리와 일치하는 클레임을 통과 또는 변환합니다.  
...
들어오는 클레임에 따라 사용자 허용 또는 거부 규칙 템플릿을 사용하여 규칙을 만들고 허용 조건을 설정하는 경우 들어오는 클레임의 유형과 값에 따라 신뢰 당사자에 대한 특정 사용자의 액세스를 허용할 수 있습니다. 예를 들어 이 규칙 템플릿을 사용하여 값이 Domain Admins인 그룹 클레임이 있는 사용자만 허용하는 규칙을 만들 수 있습니다. 한 규칙은 사용자가 신뢰 당사자에 액세스할 수 있도록 허용하고 다른 규칙은 신뢰 당사자에 대한 사용자 액세스를 거부하는 경우 거부 결과가 허용 결과를 재정의하며 사용자 액세스가 거부됩니다.  
...
들어오는 클레임에 따라 사용자 허용 또는 거부 규칙 템플릿을 사용하여 규칙을 만들고 거부 조건을 설정하는 경우 들어오는 클레임의 유형과 값에 따라 신뢰 당사자에 대한 사용자의 액세스를 거부할 수 있습니다. 예를 들어 이 규칙 템플릿을 사용하여 값이 Domain Users인 그룹 클레임이 있는 사용자를 모두 거부하는 규칙을 만들 수 있습니다.  
  
거부 조건을 사용하면서도 특정 사용자가 신뢰 당사자에 액세스할 수 있게 하려는 경우 신뢰 당사자에 대한 해당 사용자의 액세스를 허용하는 허용 조건이 포함된 권한 부여 규칙을 나중에 명시적으로 추가해야 합니다.  
  
사용자 클레임 발급 엔진이 규칙 집합을 처리할 때 액세스 거부 되 면 추가 규칙 처리가 종료 되 고 AD FS 사용자의 요청에 "액세스 거부" 오류를 반환 합니다.  
  
## <a name="authorizing-users"></a>사용자 권한 부여  
AD FS에서 권한 부여 규칙 허가 발급 하거나 사용자 또는 사용자 그룹에 있는지 여부를 결정 하는 클레임을 거부 하 되 \(사용 하는 클레임 형식에 따라\) 웹에 액세스 하도록 허용할지\-기반 리소스에서 지정 된 신뢰 여부 당사자입니다. 권한 부여 규칙은 신뢰 당사자 트러스트에만 설정할 수 있습니다.  
...
AD FS에는 사용자가 허용 또는 거부 여부를 결정 하는 데 사용 되는 두 클레임 형식을 정의 합니다. 이러한 클레임 유형 Uniform Resource Identifier \(Uri\) 는 다음과 같습니다.  
  
1.  **허용**: http:\/\/schemas.microsoft.com\/권한 부여\/클레임\/허용  
  
2.  **거부**: http:\/\/schemas.microsoft.com\/권한 부여\/클레임\/거부  
  
## <a name="how-to-create-this-rule"></a>이 규칙을 만드는 방법  
클레임 규칙 언어를 사용 하거나 사용 하 여 권한 부여 규칙을 만들 수 있습니다 합니다 **모든 사용자 허용** 규칙 서식 파일 또는 **허용 또는 거부 들어오는 클레임에 따라 사용자가** AD FS에서 규칙 템플릿을 관리 스냅인\-에서 합니다. 모든 사용자 허용 규칙 템플릿은 구성 옵션을 제공하지 않습니다. 그러나 들어오는 클레임에 따라 사용자 허용 또는 거부 규칙 템플릿은 다음과 같은 구성 옵션을 제공합니다.  
...
이 템플릿을 만드는 방법에 대 한 자세한 내용은 참조 하세요. [모든 사용자 허용 규칙을 만들어](https://technet.microsoft.com/library/ee913577.aspx) 하거나 [들어오는 클레임에 허용 또는 거부 사용자 기반 규칙 만들기](https://technet.microsoft.com/library/ee913594.aspx) AD FS 배포 가이드에서.  
...
클레임 규칙 언어 구문을 사용하여 클레임에 권한을 부여하는 경우 사용자의 원래 클레임에 있는 여러 클레임의 현재 상태에 따라 클레임을 발급할 수도 있습니다. 다음 규칙은 사용자가 Editors 그룹의 구성원이고 Windows 인증을 사용하여 인증하는 경우에만 권한 부여 클레임을 발급합니다.  
...
지정된 페더레이션 서비스에 대한 프록시 트러스트를 만들 수 있는 사용자를 지정하려는 경우 다음 위임 방법 중 하나를 사용할 수 있습니다. 이 방법 목록은 가장 안전하고 최소 문제가 있는 AD FS 제품 팀의 권장 위임 방법을 기반으로 하며 우선 순위 순서로 표시됩니다. 조직의 요구에 따라 이러한 방법 중 하나만 사용해야 합니다.  
  
1.  Active Directory에 도메인 보안 그룹을 만들고 \(예: FSProxyTrustCreators\)각 팜에 페더레이션 서버에서 로컬 Administrators 그룹에이 그룹을 추가 하 고 추가 하려는 사용자 계정만 새 그룹에이 권한을 위임 합니다. 이것은 기본적으로 사용되는 방법입니다.  
  
2.  팜에 있는 각 페더레이션 서버의 관리자 그룹에 사용자의 도메인 계정을 추가합니다.  
...
3.  어떤 이유로든 이러한 방법 중 하나를 사용할 수 없는 경우 이 용도로 권한 부여 규칙을 만들 수도 있습니다. 이 규칙을 제대로 작성하지 않을 경우 발생할 수 있는 가능한 복잡성으로 인해 권장되지는 않지만 사용자 지정 권한 부여 규칙을 사용하여 지정된 페더레이션 서비스와 연결된 모든 페더레이션 서버 프록시 간에 트러스트를 만들거나 제거할 수 있는 Active Directory 도메인 사용자 계정을 위임할 수 있습니다.  
    ...
    방법 3을 선택 하는 경우 다음 규칙 구문은 지정된 된 사용자 수 있도록 권한 부여 클레임을 발급 하 따르면 \(이 예에서 contoso\\frankm\) 트러스트를 만들 하나 이상의 페더레이션 서버 프록시에는 페더레이션 서비스입니다. Windows PowerShell 명령을 사용 하 여이 규칙을 적용 해야 합니다 **설정할\-에서 AddProxyAuthorizationRules**합니다.  
    ...
    나중에 사용자가 더 이상 프록시 트러스트를 만들 수 없도록 사용자를 제거하려는 경우 기본 프록시 트러스트 권한 부여 규칙으로 되돌려 페더레이션 서비스에 대한 프록시 트러스트를 만들 수 있는 사용자 권한을 제거할 수 있습니다. Windows PowerShell 명령을 사용 하 여이 규칙을 적용 해야 **설정할\-에서 AddProxyAuthorizationRules**합니다.  
