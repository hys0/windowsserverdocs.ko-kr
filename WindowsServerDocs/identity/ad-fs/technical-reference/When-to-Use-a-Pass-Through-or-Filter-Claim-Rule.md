...
ms.locfileid: "66188347"
---
# <a name="when-to-use-a-pass-through-or-filter-claim-rule"></a>통과 또는 필터 클레임 규칙을 사용하는 경우
Active Directory Federation Services에서이 규칙을 사용할 수 있습니다 \(AD FS\) 들어오는 클레임의 값에 따라 특정 들어오는 클레임 유형을 가져오고 출력을 사용할지 결정 하는 작업을 적용 해야 합니다. 이 규칙을 사용하는 경우 규칙에서 구성하는 옵션 중 하나에 따라 다음 표의 규칙 논리와 일치하는 클레임을 통과 또는 필터링합니다.  
...
예를 들어, 다음 그림과 같이 UPN으로 키 지정 된 들어오는 클레임을 필터링 하는 규칙 조건을 사용 하 여 설정 된 경우 클레임 유형 및 끝나야 @fabrikam.com, 다른 모든 들어오는 클레임은이 조건을 충족 하지 않는 경우 무시 됩니다. E 클레임 유형의 들어오는 클레임도 여기에\-메일 주소 클레임 값에 종료 하는 경우에 @fabrikam.com합니다. 이 예에서 값이 포함 된 클레임만 Nick@fabrikam.com 당사자에 게 전송 됩니다.  
  
![패스를 사용 하는 경우를 통해](media/adfs2_filter.gif)  
  
## <a name="configuring-this-rule-on-a-claims-provider-trust"></a>클레임 공급자 트러스트에서 이 규칙 구성  
클레임 공급자 트러스트를 사용하는 경우 클레임 공급자에서 들어오는 클레임 중 특정 제약 조건과 일치하는 클레임만 통과시키도록 이 규칙을 구성할 수 있습니다. 예를 들어 e에만 적용 하도록 경우가\-메일은 클레임 공급자에서 클레임;이 규칙 템플릿을 사용 하 여 e에 적용할 것 이므로\-클레임 공급자의 도메인 이름 시스템에서 종료 되는 클레임 형식 메일 \(DNS\) 이름입니다.  
...
필터를 논리적으로 수 AND\-적용 합니다. 예를 들어, 다음과 같은 규칙이 모든 e 수락할\-클레임 값을 사용 하 여 메일 johndoe@fabrikam.com:  
...
|클레임 속성|설명|  
|------------------|---------------|  
|형식|클레임 유형이 \(일반적으로 Uri로 표현\) 암시적 클레임에 어떤 종류의 정보를 전달 하는 방법에 대 한 페더레이션 파트너 간의 규약을 반영 합니다. 예를 들어 http 형식 클레임:\/\/schemas.xmlsoap.org\/ws\/2005\/05\/identity\/클레임\/emailaddresse이포함됩니다\-메일은 사용자의 주소입니다.|  
|값|클레임의 값입니다. 예를 들어 http 형식 클레임:\/\/schemas.xmlsoap.org\/ws\/2005\/05\/identity\/클레임\/emailaddress의 값을 가질 수 johndoe@fabrikam.com|  
|ValueType|ValueType은 클레임의 값에 포함된 정보를 해석하는 방법을 나타냅니다. 일반적으로 ValueType를 http로 설정 됩니다.\/\/www.w3.org\/2001\/XMLSchema\#문자열 이지만 클레임 값 Base64Binary 인코딩된 데이터를 포함할 수 \(예를 들어 이미지 \) 날짜, 부울 및 등입니다.|  
...
|OriginalIssuer|이 클레임 속성은 원래 클레임을 발급한 페더레이션 서버를 전달하는 데 사용됩니다. 클레임의 발급자 속성이 토큰에 서명한 페더레이션 서버를 마지막으로 설정 되어 있으므로 원래 발급자는 둘 이상의 페더레이션 서버를 통해 클레임에 이동 하는 시나리오에서 유용 \(토큰을 수신 하는 신뢰 당사자 예를 들어, 페더레이션 공급자 페더레이션 서버 관심을 가질 수는 사용자를 인증 하는 특정 클레임 공급자 페더레이션 서버\)|  
