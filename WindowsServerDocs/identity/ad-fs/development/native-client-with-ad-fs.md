---
title: 공용 클라이언트 OAuth를 사용 하 여 AD FS 2016을 사용 하 여 네이티브 클라이언트 응용 프로그램 빌드
description: 공용 클라이언트 OAuth 및 AD FS 2016을 사용 하 여 네이티브 클라이언트 응용 프로그램을 작성 하는 지침을 제공 하는 연습
author: billmath
ms.author: billmath
ms.reviewer: anandy
manager: mtillman
...
ms.locfileid: "59846484"
---
# <a name="build-a-native-client-application-using-oauth-public-clients-with-ad-fs-2016"></a>공용 클라이언트 OAuth를 사용 하 여 AD FS 2016을 사용 하 여 네이티브 클라이언트 응용 프로그램 빌드

## <a name="overview"></a>개요

이 문서에는 AD FS 2016으로 보호 되는 웹 API를 사용 하 여 상호 작용 하는 네이티브 응용 프로그램을 빌드하는 방법을 보여 줍니다.
...
2. 액세스 토큰은 TodoListService web API의 /todolist 였습니다. 그리고 끝점을 호출할 때 사용자를 인증 하는 전달자 토큰으로 사용 됩니다.
 에서는 여기에서 Azure AD에 대 한 응용 프로그램 예제를 사용 하 여 되며 다음 AD FS 2016에 대 한 수정 합니다.
...
* 키를 추가 **ida: 기관** AD를 나타내는 값을 사용 하 여 FS 서비스, 예 https://fs.contoso.com/adfs/
* 수정할 **ida: ClientId** 의 값을 사용 하 여 키를 **네이티브 응용 프로그램** AD FS에서 응용 프로그램 그룹을 만드는 동안 페이지입니다. 에 대 한 3f07368b-6efd-4f50-a330-d93853f4c855 예
* 수정 된 **todo:TodoListBaseAddress** Web API의 기준 주소에 예: https://localhost:44321/
* 값을 설정할 **ida: RedirectUri** AD FS에서 응용 프로그램 그룹을 추가 하는 동안 네이티브 응용 프로그램 페이지에 넣으면 값입니다.
* 읽기 쉽도록 제거 / 키에 대 한 주석 수 있습니다 **ida: 테 넌 트** 하 고 **ida: AADInstance**합니다.

**MainWindow.xaml.cs**

* AadInstance에 대 한 줄을 제거
...
* 함수의 **MainWindow**로 authContext 인스턴스화를 변경 합니다.
