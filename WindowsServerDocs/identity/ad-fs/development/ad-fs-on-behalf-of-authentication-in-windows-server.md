...
title: On-Behalf-Of (OBO) OAuth를 사용 하 여 ad FS 2016을 사용 하 여 다중 계층 응용 프로그램 빌드
description: ''
author: billmath
ms.author: billmath
manager: mtillman
...
ms.locfileid: "59858944"
---
# <a name="build-a-multi-tiered-application-using-on-behalf-of-obo-using-oauth-with-ad-fs-2016"></a>On-Behalf-Of (OBO) OAuth를 사용 하 여 ad FS 2016을 사용 하 여 다중 계층 응용 프로그램 빌드

>적용 대상: Windows Server 2016

이 연습에서는 AD FS를 사용 하 여 Windows Server 2016 TP5에 대 한 대리 (OBO) 인증을 구현 하기 위한 지침을 제공 합니다. 읽으십시오 OBO 인증에 자세히 알아보려면 [개발자를 위한 AD FS 시나리오](../../ad-fs/overview/AD-FS-Scenarios-for-Developers.md)
...
1. 클라이언트가 인증 AD FS 권한 부여 끝점 하 고 권한 부여 코드 요청
2. 클라이언트에 인증 코드를 반환 하는 권한 부여 끝점
3. 클라이언트 인증 코드를 사용 하 고 WebAPI로 중간 계층 웹 서비스에 대 한 액세스 토큰 요청에 AD FS 토큰 끝점에 표시
4. AD FS는 중간 계층 웹 서비스에 액세스 토큰을 반환합니다. 추가 기능에 대 한 중간 계층 서비스는 백 엔드 WebAPI에 액세스 해야
...
WebAPI-OnBehalfOf-DotNet.sln 솔루션을 열면 즉시 알게 될 것-솔루션에 두 개의 프로젝트가 있는 경우

* **ToDoListClient**: 이 역할을 사용자 상호 작용 하는 OpenID 클라이언트
* **ToDoListService**: 이 중간 계층 웹 앱 / 서비스에 인증된 된 사용자 다른 백 엔드 WebAPI OBO 상호 작용
...
AD FS 관리 MMC를 열고 새 응용 프로그램 그룹을 추가 합니다. WebAPI-네이티브-응용 프로그램 템플릿을 선택 합니다.

![AD FS OBO](media/AD-FS-On-behalf-of-Authentication-in-Windows-Server-2016/ADFS_OBO2.PNG)

다음에 클릭 하 고 클라이언트 응용 프로그램에 대 한 정보를 제공 하기 위한 페이지와 함께 나타납니다. AD FS에서 응용 프로그램 클라이언트는 적절 한 이름을 제공 합니다. 클라이언트 식별자를 복사 하 고이 visual studio에서 응용 프로그램 구성에 필요한 수 만큼 나중에 액세스할 수 어딘가에 저장 합니다.

>참고: 리디렉션 URI 가능 임의의 URI 실제로 사용 되지 않는 네이티브 클라이언트의 경우
...
**응용 프로그램 그룹에서 클라이언트로 ToDoListService를 추가합니다.**

이 단계에서 리소스 뿐만 아니라 클라이언트와 작동 하도록 웹 서버 응용 프로그램에 대 한 AD FS에서 추가 항목을 확인 해야 합니다. 방금 만든 응용 프로그램 그룹을 열고 응용 프로그램 추가 클릭 합니다.

![AD FS OBO](media/AD-FS-On-behalf-of-Authentication-in-Windows-Server-2016/ADFS_OBO15.PNG)

"MySampleGroup 새 응용 프로그램 추가" 페이지와 함께 나타납니다. 이 페이지에서 "서버 응용 프로그램 또는 웹 사이트" 독립 실행형 응용 프로그램으로 선택

![AD FS OBO](media/AD-FS-On-behalf-of-Authentication-in-Windows-Server-2016/ADFS_OBO19.PNG)

다음을 클릭 하 고 응용 프로그램 정보를 제공 하는 페이지와 함께 나타납니다. 구성 항목의 이름 섹션에 대 한 적절 한 이름을 제공 합니다. 클라이언트 식별자는 ToDoListServiceWebAPI에 대 한 식별자와 같음 인지 확인

![AD FS OBO](media/AD-FS-On-behalf-of-Authentication-in-Windows-Server-2016/ADFS_OBO20.PNG)

다음에 클릭 하 고 응용 프로그램 자격 증명을 구성 하는 페이지와 함께 나타납니다. "공유 암호 생성" 클릭 합니다. 자동으로 생성 되는 암호와 나타납니다. 로 서이 필요한 ToDoListService visual studio에서 구성 하는 동안 동일한 위치에서 암호를 복사 합니다.
...
응용 프로그램 구성에서 테 넌 트 정보를 읽는 줄을 주석
...
MySampleGroup 응용 프로그램 그룹을 엽니다. 추가 응용 프로그램 및 Web API 템플릿 선택을 클릭 하 고 클릭 합니다.
...
마법사의 나머지 부분에서 동일한으로 계속 ToDoListService WebAPI을 구성 했습니다. 끝에 응용 프로그램 그룹 아래 같이 표시 됩니다.
...
|ida: ADFSMetadata| 이 AD FS 메타 데이터를 URL에 대 한 예: https://fs.anandmsft.com/federationmetadata/2007-06/federationmetadata.xml|
...
AD FS에서 Nmae 클레임을 실행 하는 것 하지만 하지 NameIdentifier 클레임을 실행 하는 것입니다. 샘플 할 일 항목의 고유 키를 NameIdentifier를 사용합니다. 간단히 하기 위해 코드에서 이름 클레임을 가진 NameIdentifier를 안전 하 게 제거할 수 있습니다. 찾기 및 NameIdentifier의 모든 항목 이름으로 바꿉니다.

**Post 루틴 및 callgraphapionbehalfofuser ()를 수정 합니다.**
...
첫 번째 상호 작용을 토큰 끝점에 액세스 코드를 제공 하 고 액세스에 대 한 토큰 가져오기 https://localhost:44321/ ![AD FS OBO](media/AD-FS-On-behalf-of-Authentication-in-Windows-Server-2016/ADFS_OBO22.PNG)

두 번째 상호 토큰 끝점을 확인할 수 있습니다 했으므로 **requested_token_use** 으로 설정 **on_behalf_of** 즉,중간계층웹서비스에대해얻은액세스토큰을사용하는것을 https://localhost:44321/ 어설션으로-대신-의 토큰을 가져옵니다.
