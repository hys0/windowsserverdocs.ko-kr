...
ExpressRoute를 사용하는 것이 좋습니다. Express 경로를 사용 하면 온-프레미스 또는 공동 배치 환경의 인프라와 Azure 데이터 센터 간에 개인 연결을 만들 수 있습니다. ExpressRoute 연결은 공용 인터넷을 통해 이동하지 않습니다. Express 경로는 일반적인 인터넷을 통한 연결보다 안정적이고 속도가 빠르며 대기 시간이 짧고 보안성이 높습니다.
...
내부 DNS 서버를 사용 하 여 ILB에 대 한 A 레코드를 만듭니다. A 레코드는 ILB의 IP 주소를 가리키는 IP 주소를 사용 하 여 페더레이션 서비스에 대 한 것 이어야 합니다. 예를 들어 ILB IP 주소가 10.3.0.8이 고 페더레이션 서비스가 설치 된 경우 fs.contoso.com를 가리키는 fs.contoso.com에 대 한 A 레코드를 만듭니다.
이렇게 하면 fs.contoso.com trasmitted 모든 데이터를 ILB에서 종료 하 고 적절 하 게 라우팅됩니다. 

> [!NOTE]
>또한 배포에서 i p v 6을 사용 하는 경우 해당 AAAA 레코드를 만들어야 합니다.
>
>
