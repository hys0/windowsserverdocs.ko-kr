...
클레임 @ no__t-0based id 모델에서 Active Directory Federation Services \(AD FS @ no__t-2를 페더레이션 서비스로 사용 하는 기능은 클레임 집합이 포함 된 토큰을 발급 하는 것입니다. 클레임 규칙은 문제를 AD FS 하는 클레임에 대 한 결정을 제어 합니다. 클레임 규칙 및 모든 서버 구성 데이터는 AD FS 구성 데이터베이스에 저장 됩니다.  
