---
title: Windows Server 2012 R2에서 AD FS를 AD FS 2.0 페더레이션 서버 마이그레이션 준비
description: Windows Server 2012 R2로 AD FS 서버 마이그레이션 준비에 정보를 제공 합니다.
author: billmath
ms.author: billmath
manager: femila
ms.date: 07/10/2017
...
-   [Windows Server 2012 R2의 AD FS 요구 사항](prepare-migrate-ad-fs-server-r2.md#ad-fs-requirements-in-windows-server-2012-r2)  
  
-   [Windows PowerShell도 증가](prepare-migrate-ad-fs-server-r2.md#increasing-your-windows-powershell-limits)  
...
다음 설정은 Windows Server 2012 R2 설치 CD의 \support\adfs 폴더에 있는 스크립트를 통해 마이그레이션됩니다.  
...
**AD FS 로그인 페이지의 설정 및 사용자 지정 변경 내용**  
-   Windows Server 2012 R2에서 AD FS에서 관리자와 사용자 모두에 대 한 로그인 환경을 개선 하기 위한 몇 가지 변경 사항이 있습니다. 이전 버전의 AD FS에서 제공된 IIS 호스팅 웹 페이지가 이제 제거되었습니다. AD FS 로그인 웹 페이지의 모양과 느낌이 AD FS에서 자체 호스팅되며, 이제 사용자 환경에 맞게 사용자 지정할 수 있습니다. 변경 사항에는 다음이 포함됩니다.  
...
Windows Server 2012 R2로 마이그레이션하려는 기존 AD FS 팜에 웹 페이지 사용자 지정에서 설정한 경우에 Windows Server 2012 R2에서 새 사용자 지정 기능을 사용 하 여 마이그레이션 프로세스의 일부로 다시 만들 수 있습니다.  
  
-   **기타 변경 내용**  
  
    -   Windows Server 2012 R2에서 AD FS에서 Windows Identity Foundation (WIF) 3.5를 WIF 4.5가 아니라 기반 합니다. 따라서 일부 특정 기능 (예: Kerberos 클레임 및 동적 액세스 제어)에 WIF 4.5의 Windows Server 2012 R2의 AD FS에서 지원 되지 않습니다.  
  
    -   포트 443에서 작동 하는 Windows Server 2012 R2에서 장치 등록 서비스 (DRS) 사용자 인증서 인증을 위한 ClientTLS는 포트 49443에서 작동합니다.  
...
##  <a name="ad-fs-requirements-in-windows-server-2012-r2"></a>Windows Server 2012 R2의 AD FS 요구 사항  
 AD FS 팜을 Windows Server 2012 r2를 성공적으로 마이그레이션하려면 하기 위해 다음 요구 사항을 충족 해야 합니다.  
  
 AD FS가 작동 하려면에 대 한 페더레이션 하려는 각 컴퓨터를 도메인에 가입 해야 합니다.  
...
-   동일한 페더레이션 서버 팜 이름 및 Windows를 마이그레이션할 때 레거시 AD FS 팜에서 사용한 서비스 계정 id 이름이 유지 하는 Windows Server 2012 R2 설치 CD의 \support\adfs 폴더에 있는 마이그레이션 스크립트 필요 Server 2012 R2입니다.  
