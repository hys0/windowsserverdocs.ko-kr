...
|-----|-----|-----|-----|
|단일 페이지 앱</br> | [ADAL을 사용 하](../development/Single-Page-Application-with-AD-FS.md) 는 &bull; 샘플|[명시적](#implicit-grant-flow)|Public| 
|사용자가 로그인 하는 웹 앱</br> | [OWIN를 사용 하](../development/enabling-openid-connect-with-ad-fs.md) 는 &bull; 샘플|[인증 코드](#authorization-code-grant-flow)|공개, 기밀|  
|네이티브 앱이 Web API를 호출 합니다.</br>|[MSAL을 사용 하](../development/msal/adfs-msal-native-app-web-api.md) 는 &bull; 샘플</br>[ADAL을 사용 하](../development/native-client-with-ad-fs.md) 는 &bull; 샘플|[인증 코드](#authorization-code-grant-flow)|Public|   
|웹 앱이 Web API를 호출 합니다.</br>|[MSAL을 사용 하](../development/msal/adfs-msal-web-app-web-api.md) 는 &bull; 샘플</br>[ADAL을 사용 하](../development/enabling-oauth-confidential-clients-with-ad-fs.md) 는 &bull; 샘플|[인증 코드](#authorization-code-grant-flow)|Confidential| 
|Web API는 (OBO) 사용자를 대신 하 여 다른 웹 API를 호출 합니다.</br>|[MSAL을 사용 하](../development/msal/adfs-msal-web-api-web-api.md) 는 &bull; 샘플</br>[ADAL을 사용 하](../development/ad-fs-on-behalf-of-authentication-in-windows-server.md) 는 &bull; 샘플|[대리](#on-behalf-of-flow)|웹 앱은 기밀 역할을 합니다.| 
...
처음에 사용자를 앱에 로그인 하려면 Openid connect Connect 인증 요청을 보내고 AD FS 끝점에서 id_token 및 액세스 토큰을 가져올 수 있습니다.  
...
|response_type|필수|Openid connect Connect 로그인에 대 한 `id_token` 를 포함 해야 합니다. Response_type `token`포함 될 수도 있습니다. 여기에서 토큰을 사용 하면 앱이 토큰 끝점에 대 한 두 번째 요청을 수행 하지 않고도 권한 부여 끝점에서 즉시 액세스 토큰을 받을 수 있습니다.| 
|redirect_uri|필수|앱에서 인증 응답을 보내고 받을 수 있는 앱의 redirect_uri입니다. AD FS에서 구성한 redirect_uris 중 하 나와 정확 하 게 일치 해야 합니다.| 
|임시|필수|앱에서 생성 한 요청에 포함 된 값으로, 결과 id_token에 클레임으로 포함 됩니다. 그러면 앱에서 토큰 재생 공격을 완화 하기 위해이 값을 확인할 수 있습니다. 값은 일반적으로 요청의 출처를 식별 하는 데 사용할 수 있는 임의의 고유 문자열입니다. Id_token 요청 된 경우에만 필요 합니다.|
|범위|옵션|공백으로 구분 된 범위 목록입니다. Openid connect Connect의 경우 `openid`범위를 포함 해야 합니다.|
|resource|옵션|웹 API의 url입니다.</br>참고 – MSAL 클라이언트 라이브러리를 사용 하는 경우 리소스 매개 변수가 전송 되지 않습니다. 대신 리소스 url은 범위 매개 변수의 일부로 전송 됩니다. `scope = [resource url]//[scope values e.g., openid]`</br>리소스가 여기에 전달 되지 않거나 범위에서 ADFS는 기본 리소스 urn: microsoft: userinfo를 사용 합니다. MFA, 발급 또는 권한 부여 정책과 같은 사용자 정보 리소스 정책은 사용자 지정할 수 없습니다.| 
|response_mode|옵션| 결과 토큰을 앱에 다시 보내는 데 사용 해야 하는 메서드를 지정 합니다. 기본값은 `fragment`입니다.| 
|state|옵션|토큰 응답에도 반환 되는 요청에 포함 된 값입니다. 원하는 모든 콘텐츠의 문자열일 수 있습니다. 임의로 생성 된 고유 값은 일반적으로 교차 사이트 요청 위조 공격을 방지 하는 데 사용 됩니다. 상태는 인증 요청이 발생 하기 전에 앱에서 사용자 상태에 대 한 정보 (예: 설정 된 페이지 또는 보기)를 인코딩하는 데도 사용 됩니다.| 
|prompt|옵션|필요한 사용자 상호 작용의 유형을 나타냅니다. 현재 유일 하 게 유효한 값은 로그인 이며 없음입니다.</br> `prompt=login` 를 -사용자가 해당 요청에 대 한 자격 증명을 입력 하 여 single sign-on을 부정 합니다. </br>- `prompt=none` 는 반대입니다. 그러면 사용자에 게 대화형 프롬프트가 표시 되지 않습니다. Single sign-on을 통해 요청을 자동으로 완료할 수 없는 경우 AD FS은 interaction_required 오류를 반환 합니다.| 
|login_hint|옵션|사용자의 사용자 이름을 미리 알고 있는 경우 사용자에 대 한 로그인 페이지의 사용자 이름/이메일 주소 필드를 미리 채우는 데 사용할 수 있습니다. 앱에서 다시 인증 하는 동안이 매개 변수를 사용 하 여 `id_token`에서 `upn` 클레임을 사용 하 여 이전 로그인에서 사용자 이름을 이미 추출 했습니다.| 
|domain_hint|옵션|포함 되는 경우 사용자가 로그인 페이지에서 이동 하는 도메인 기반 검색 프로세스를 건너뛰고 약간 더 간소화 된 사용자 환경을 제공 합니다.| 

이 시점에서 사용자에 게 자격 증명을 입력 하 고 인증을 완료 하 라는 메시지가 표시 됩니다. 사용자가 인증 하면 AD FS 권한 부여 끝점은 response_mode 매개 변수에 지정 된 메서드를 사용 하 여 표시 된 redirect_uri에서 앱에 대 한 응답을 반환 합니다.  
 
### <a name="successful-response"></a>성공적인 응답 
 
 `response_mode=fragment and response_type=id_token+token` 를 사용 하는 성공적인 응답은 다음과 같습니다.  
...
|매개 변수|설명| 
|-----|-----|
|access_token|Response_type에 `token`포함 되어 있는 경우 포함 됩니다.|
|token_type|Response_type에 `token`포함 되어 있는 경우 포함 됩니다. 는 항상 전달자입니다.| 
|expires_in| Response_type에 `token`포함 되어 있는 경우 포함 됩니다. 캐싱을 위해 토큰이 유효한 시간 (초)을 나타냅니다.| 
|범위| Access_token를 사용할 범위를 나타냅니다.|  
|id_token|Response_type에 `id_token`포함 되어 있는 경우 포함 됩니다. JWT (서명 된 JSON Web Token)입니다. 앱은이 토큰의 세그먼트를 디코드 하 여 로그인 한 사용자에 대 한 정보를 요청할 수 있습니다. 앱은 값을 캐시 하 고 표시할 수 있지만 권한 부여 또는 보안 경계에는이 값을 사용 하지 않아야 합니다.| 
|state|요청에 state 매개 변수가 포함 되어 있으면 동일한 값이 응답에 표시 되어야 합니다. 앱은 요청 및 응답의 상태 값이 동일한 지 확인 해야 합니다.|

### <a name="refresh-tokens"></a>토큰 새로 고침 
암시적 권한 부여는 새로 고침 토큰을 제공 하지 않습니다.  `id_tokens`와 `access_tokens`는 모두 짧은 시간 후에 만료 되므로 앱이 주기적으로 이러한 토큰을 새로 고칠 수 있도록 준비 해야 합니다. 두 가지 유형의 토큰을 새로 고치려면 `prompt=none` 매개 변수를 사용 하 여 위와 동일한 숨겨진 iframe 요청을 수행 하 여 id 플랫폼의 동작을 제어할 수 있습니다. `new id_token`받으려면 `response_type=id_token`를 사용 해야 합니다. 

## <a name="authorization-code-grant-flow"></a>인증 코드 부여 흐름 
 
OAuth 2.0 인증 코드 부여는 웹 Api와 같은 보호 된 리소스에 대 한 액세스 권한을 얻기 위해 웹 앱에서 사용할 수 있습니다. OAuth 2.0 인증 코드 흐름은 [oauth 2.0 사양의 섹션 4.1](https://tools.ietf.org/html/rfc6749)에 설명 되어 있습니다. 웹 앱 및 기본적으로 설치 된 앱을 비롯 하 여 대부분의 앱 형식에서 인증 및 권한 부여를 수행 하는 데 사용 됩니다. 흐름을 사용 하면 앱이 AD FS를 신뢰 하는 리소스에 액세스 하는 데 사용할 수 있는 access_tokens을 안전 하 게 가져올 수 있습니다.  
...
|redirect_uri|필수|앱에서 인증 응답을 보내고 받을 수 있는 앱의 `redirect_uri`입니다. 클라이언트의 AD FS에 등록 한 redirect_uris 중 하 나와 정확 하 게 일치 해야 합니다.|  
|resource|옵션|웹 API의 url입니다.</br>참고 – MSAL 클라이언트 라이브러리를 사용 하는 경우 리소스 매개 변수가 전송 되지 않습니다. 대신 리소스 url은 범위 매개 변수의 일부로 전송 됩니다. `scope = [resource url]//[scope values e.g., openid]`</br>리소스가 여기에 전달 되지 않거나 범위에서 ADFS는 기본 리소스 urn: microsoft: userinfo를 사용 합니다. MFA, 발급 또는 권한 부여 정책과 같은 사용자 정보 리소스 정책은 사용자 지정할 수 없습니다.| 
|범위|옵션|공백으로 구분 된 범위 목록입니다.|
|response_mode|옵션|결과 토큰을 앱에 다시 보내는 데 사용 해야 하는 메서드를 지정 합니다. 다음 중 하나일 수 있습니다. </br>-쿼리 </br>-조각 </br>-form_post</br>`query` 는 리디렉션 URI에 대 한 쿼리 문자열 매개 변수로 코드를 제공 합니다. 코드를 요청 하는 경우 쿼리, 조각 또는 form_post을 사용할 수 있습니다. `form_post` 는 리디렉션 URI에 대 한 코드를 포함 하는 POST를 실행 합니다.|
|state|옵션|토큰 응답에도 반환 되는 요청에 포함 된 값입니다. 원하는 모든 콘텐츠의 문자열일 수 있습니다. 임의로 생성 된 고유 값은 일반적으로 교차 사이트 요청 위조 공격을 방지 하는 데 사용 됩니다. 또한이 값은 인증 요청이 발생 하기 전에 앱에서 사용자 상태에 대 한 정보 (예: 설정 된 페이지 또는 보기)를 인코딩할 수 있습니다.|
|prompt|옵션|필요한 사용자 상호 작용의 유형을 나타냅니다. 현재 유일 하 게 유효한 값은 로그인 이며 없음입니다.</br> `prompt=login` 를 -사용자가 해당 요청에 대 한 자격 증명을 입력 하 여 single sign-on을 부정 합니다. </br>- `prompt=none` 는 반대입니다. 그러면 사용자에 게 대화형 프롬프트가 표시 되지 않습니다. Single sign-on을 통해 요청을 자동으로 완료할 수 없는 경우 AD FS은 interaction_required 오류를 반환 합니다.|
|login_hint|옵션|사용자의 사용자 이름을 미리 알고 있는 경우 사용자에 대 한 로그인 페이지의 사용자 이름/이메일 주소 필드를 미리 채우는 데 사용할 수 있습니다. 앱에서 다시 인증 하는 동안이 매개 변수를 사용 하 여 `id_token`의 `upn`클레임을 사용 하 여 이전 로그인에서 사용자 이름을 이미 추출 했습니다.|
|domain_hint|옵션|포함 되는 경우 사용자가 로그인 페이지에서 이동 하는 도메인 기반 검색 프로세스를 건너뛰고 약간 더 간소화 된 사용자 환경을 제공 합니다.|
|code_challenge_method|옵션|Code_challenge 매개 변수의 code_verifier 인코딩하는 데 사용 되는 메서드입니다. 다음 값 중 하나입니다. </br>-일반 </br>- S256 </br>제외 된 경우 `code_challenge` 포함 된 경우 code_challenge는 일반 텍스트로 간주 됩니다. AD FS는 일반 및 S256을 모두 지원 합니다. 자세한 내용은 [Pkce RFC](https://tools.ietf.org/html/rfc7636)를 참조 하세요.|
|code_challenge|옵션| 인증 코드의 보안을 유지 하는 데 사용 됩니다.  `code_challenge_method` 포함 된 경우 필수입니다. 자세한 내용은 [Pkce RFC](https://tools.ietf.org/html/rfc7636) 를 참조 하세요.|

이 시점에서 사용자에 게 자격 증명을 입력 하 고 인증을 완료 하 라는 메시지가 표시 됩니다. 사용자가 인증 되 면 AD FS는 `response_mode` 매개 변수에 지정 된 메서드를 사용 하 여 표시 된 `redirect_uri`에서 앱에 대 한 응답을 반환 합니다.  
 
### <a name="successful-response"></a>성공적인 응답 
 
Response_mode = 쿼리를 사용 하는 성공적인 응답은 다음과 같습니다. 
...
|매개 변수|설명|
|-----|-----|
|code|앱이 요청한 `authorization_code`입니다. 앱은 권한 부여 코드를 사용 하 여 대상 리소스에 대 한 액세스 토큰을 요청할 수 있습니다. Authorization_codes 수명이 짧고, 일반적으로 약 10 분 후에 만료 됩니다.|
...
`authorization_code`를 획득 하 고 사용자가 사용 권한을 부여 했으므로 이제 `access_token` 에 대 한 코드를 원하는 리소스에 대 한 코드를 사용할 수 있습니다. 이 작업을 수행 하려면/stoken 끝점에 POST 요청을 보냅니다.  
...
|-----|-----|-----| 
|client_id|필수|앱에 할당 AD FS는 응용 프로그램 (클라이언트) ID입니다.| 
|grant_type|필수|인증 코드 흐름에 대 한 `authorization_code` 여야 합니다.| 
|code|필수|흐름의 첫 번째 레그에서 얻은 `authorization_code`입니다.| 
|redirect_uri|필수|`authorization_code`를 가져오는 데 사용 된 것과 동일한 `redirect_uri` 값입니다.| 
|client_secret|웹 앱에 필요|AD FS에서 앱을 등록 하는 동안 만든 응용 프로그램 암호입니다. Client_secrets 장치에 안정적으로 저장할 수 없기 때문에 네이티브 앱에서 응용 프로그램 암호를 사용 하면 안 됩니다. 서버 쪽에 안전 하 게 client_secret를 저장할 수 있는 웹 앱과 web Api에 필요 합니다. 클라이언트 암호를 보내기 전에 URL로 인코딩해야 합니다. 이러한 앱은 JWT에 서명 하 고이를 client_assertion 매개 변수로 추가 하 여 키 기반 인증을 사용할 수도 있습니다.| 
|code_verifier|옵션|Authorization_code를 가져오는 데 사용 된 것과 동일한 `code_verifier`입니다. 권한 부여 코드 부여 요청에서 PKCE를 사용한 경우에 필요 합니다. 자세한 내용은 [Pkce RFC](https://tools.ietf.org/html/rfc7636)를 참조 하세요.</br>참고 – AD FS 2019 이상에 적용 됩니다.| 
...
Access_tokens 수명이 짧고 리소스에 계속 액세스할 수 있도록 만료 된 후 새로 고쳐야 합니다. 이렇게 하려면 다른 POST 요청을 `/token` 끝점에 제출 합니다. 이번에는 코드 대신 refresh_token을 제공 합니다. 새로 고침 토큰은 클라이언트에서 이미 액세스 토큰을 받은 모든 사용 권한에 대해 유효 합니다. 
...
새로 고침 토큰은 새 액세스 토큰을 획득 하는 데 사용 될 때 해지 되지 않지만 이전 새로 고침 토큰을 삭제 해야 합니다. OAuth 2.0 spec: "권한 부여 서버에서 새 새로 고침 토큰을 발급할 수 있습니다 .이 경우 클라이언트는 이전 새로 고침 토큰을 삭제 하 고 새 새로 고침 토큰으로 바꾸어야 합니다. 권한 부여 서버에서 새 새로 고침 토큰을 클라이언트에 발급 한 후 이전 새로 고침 토큰을 해지할 수 있습니다. " 
...
|-----|-----|-----|
|client_id|필수|앱에 할당 AD FS는 응용 프로그램 (클라이언트) ID입니다.| 
|grant_type|필수|이 인증 코드 흐름 레그의 `refresh_token` 이어야 합니다.| 
|resource|옵션|웹 API의 url입니다.</br>참고 – MSAL 클라이언트 라이브러리를 사용 하는 경우 리소스 매개 변수가 전송 되지 않습니다. 대신 리소스 url은 범위 매개 변수의 일부로 전송 됩니다. `scope = [resource url]//[scope values e.g., openid]`</br>리소스가 여기에 전달 되지 않거나 범위에서 ADFS는 기본 리소스 urn: microsoft: userinfo를 사용 합니다. MFA, 발급 또는 권한 부여 정책과 같은 사용자 정보 리소스 정책은 사용자 지정할 수 없습니다.|
|범위|옵션|공백으로 구분 된 범위 목록입니다.| 
|refresh_token|필수|흐름의 두 번째 레그에서 얻은 refresh_token입니다.| 
|client_secret|웹 앱에 필요| 앱에 대 한 앱 등록 포털에서 만든 응용 프로그램 암호입니다. Client_secrets 장치에 안정적으로 저장할 수 없기 때문에 네이티브 앱에서 사용 하면 안 됩니다. 서버 쪽에 안전 하 게 client_secret를 저장할 수 있는 웹 앱과 web Api에 필요 합니다. 이러한 앱은 JWT에 서명 하 고이를 client_assertion 매개 변수로 추가 하 여 키 기반 인증을 사용할 수도 있습니다.|
...
![흐름에 대 한](media/adfs-scenarios-for-developers/obo.png)
  ...
  참고: AD FS OBO flow를 구성 하는 동안 범위 `user_impersonation` 선택 되어 있는지 확인 하 고 클라이언트에서 요청 `user_impersonation` 범위를 요청 합니다. 
  2. API A는 AD FS 토큰 발급 끝점에 대해 인증 하 고 API B에 액세스 하는 토큰을 요청 합니다. 참고:이 AD FS 흐름을 구성 하는 동안 api A가 API A의 리소스 ID와 동일한 값을 갖는 서버 응용 프로그램으로도 등록 되어 있는지 확인 합니다. 자세한 내용은 여기에 있는 샘플 추가 링크를 참조 하세요.  
...
|requested_token_use|필수|요청을 처리 하는 방법을 지정 합니다. OBO 흐름에서 값을로 설정 해야 on_behalf_of| 
...
|범위|옵션|토큰 요청에 대 한 공백으로 구분 된 범위 목록입니다.| 

#### <a name="example"></a>예 
 
다음 `HTTP POST`는 액세스 토큰과 새로 고침 토큰을 요청 합니다. 
...
매개 변수는 client_secret 매개 변수가 client_assertion_type 및 client_assertion의 두 매개 변수로 대체 된다는 점을 제외 하 고 공유 암호 요청의 경우와 거의 동일 합니다. 
...
클라이언트 자격 증명 부여를 사용 하 여 토큰을 가져오려면/토큰 AD FS 끝점에 `POST` 요청을 보냅니다.  
...
|-----|-----|-----| 
|client_id|필수|앱에 할당 AD FS는 응용 프로그램 (클라이언트) ID입니다.| 
|범위|옵션|사용자가 동의할 수 있도록 할 공백으로 구분 된 범위 목록입니다.| 
|client_secret|필수|앱 등록 포털에서 앱에 대해 생성 한 클라이언트 암호입니다. 클라이언트 암호를 보내기 전에 URL로 인코딩해야 합니다.| 
|grant_type|필수| `client_credentials`으로 설정 해야 합니다.|
...
|client_assertion|필수|응용 프로그램에 대 한 자격 증명으로 등록 한 인증서를 사용 하 여 만들고 서명 해야 하는 어설션 (JSON 웹 토큰)입니다.|  
|grant_type|필수| `client_credentials`으로 설정 해야 합니다.|
|client_id|옵션|앱에 할당 AD FS는 응용 프로그램 (클라이언트) ID입니다. 이는 client_assertion의 일부 이므로 여기에 전달할 필요가 없습니다.| 
|범위|옵션|사용자가 동의할 수 있도록 할 공백으로 구분 된 범위 목록입니다.| 
...
|refresh_token|원래 범위 매개 변수가 offline_access 포함 된 경우 발급 됩니다.|
...
클라이언트는 인증을 시작 하는 데 사용 되는 장치 및 사용자 코드에 대 한 인증 서버를 먼저 확인 해야 합니다. 클라이언트는/devicecode 끝점에서이 요청을 수집 합니다. 이 요청에서 클라이언트는 사용자 로부터 획득 해야 하는 사용 권한을 포함 해야 합니다. 이 요청이 전송 된 순간부터 사용자는 15 분 동안만 로그인 (expires_in의 일반적인 값) 되므로 사용자가 로그인 할 준비가 되었음을 나타내는 경우에만이 요청을 수행 합니다. 
...
|verification_uri_complete|사용자가 로그인 하기 위해 user_code로 이동 해야 하는 URI입니다. 사용자가 입력 하지 않아도 되도록 user_code으로 미리 채워집니다 user_code| 
|expires_in|Device_code 및 user_code 만료 되기까지의 시간 (초)입니다.| 
...
User_code 및 verification_uri를 받은 후 클라이언트는 사용자에 게 이러한 항목을 표시 하 여 휴대폰 또는 PC 브라우저를 사용 하 여 로그인 하도록 지시 합니다. 또한 클라이언트는 QR 코드 또는 유사한 메커니즘을 사용 하 여 verfication_uri_complete을 표시할 수 있습니다. 그러면 사용자에 대 한 user_code를 입력 하는 단계가 수행 됩니다. 사용자가 verification_uri에서 인증 하는 동안 클라이언트는 device_code를 사용 하 여 요청 된 토큰에 대 한/stoken 끝점을 폴링합니다. 
...
|client_id|필수|초기 요청에 사용 되는 client_id 일치 해야 합니다.| 
