---
title: 권한 있는 액세스 워크스테이션이 조직의 보안을 유지 하는 데 도움이 되는 이유
description: 조직의 보안 상태를 PAW 수 있는 방법
...
PAW(Privileged Access Workstation)는 인터넷 공격 및 위협 벡터로부터 보호되는 중요한 작업에 대한 전용 운영 체제를 제공합니다. 이러한 중요 한 작업 및 계정을 매일 사용 하는 워크스테이션 및 장치에서 분리 하면 피싱 공격, 응용 프로그램 및 OS 취약점, 다양 한 가장 공격 및 자격 증명 도난 공격 (예: 키 입력) 으로부터 매우 강력한 보호 기능을 제공 합니다. 로깅, [해시 전달](https://aka.ms/pth)및 티켓 전달입니다.

## <a name="what-is-a-privileged-access-workstation"></a>권한 있는 액세스 워크스테이션 이란?
...
가장 높은 수준의 보안을 제공 하기 위해 Paw는 항상 가장 최신의 안전한 운영 체제를 실행 해야 합니다. Microsoft는 다른 버전 (특히 [Credential guard](https://technet.microsoft.com/library/mt483740%28v=vs.85%29.aspx) 및 [Device Guard](https://technet.microsoft.com/library/dn986865(v=vs.85).aspx))에서 사용할 수 없는 몇 가지 추가 보안 기능을 포함 하는 Windows 10 Enterprise를 적극 권장 합니다.
...
PAW 보안 컨트롤은 높은 영향 및 위험의 높은 확률 위험을 완화 하는 데 중점을 두었습니다. 여기에는 환경에 대 한 공격 완화 및 시간에 따른 PAW 컨트롤의 효율성을 저하 시킬 수 있는 위험이 포함 됩니다.
...
* **유용성 위험** - PAW가 일상적인 작업에 사용하기에 너무 어려운 경우 관리자는 작업을 보다 쉽게 수행할 수 있는 해결책을 만들어야 합니다. 종종 이러한 해결책은 관리 워크스테이션 및 계정을 상당한 보안 위험에 노출시키므로 이러한 유용한 문제를 안전하게 완화하기 위해 PAW 사용자를 참여시키고 권한을 부여하는 것이 중요합니다. 이러한 작업을 수행 하려면 자신의 의견을 듣고, 작업을 수행 하는 데 필요한 도구와 스크립트를 설치 하 고, 모든 관리 담당자가 PAW를 사용 해야 하는 이유와 PAW 무엇 인지, 그리고 올바르게 사용 하는 방법을 알고 있어야 합니다.
* **환경 위험** - 환경 내의 여러 컴퓨터 및 계정이 직접 또는 간접적으로 인터넷 위험에 노출되기 때문에 프로덕션 환경 내 손상된 자산의 공격으로부터 PAW를 보호해야 합니다. 이렇게 하려면 이러한 특수 워크스테이션의 보안 및 모니터링을 위해 Paw에 액세스할 수 있는 관리 도구 및 계정의 사용을 최소화 해야 합니다.
* 공급망 **변조** -하드웨어 및 소프트웨어에 대 한 공급망의 손상 가능성을 모두 제거 하는 것은 불가능 하지만 몇 가지 주요 작업을 수행 하면 공격자가 쉽게 사용할 수 있는 중요 한 공격 벡터를 완화할 수 있습니다. 여기에는 모든 설치 미디어의 무결성 확인([클린 소스 원칙](https://aka.ms/cleansource)) 및 신뢰할 수 있고 평판 좋은 하드웨어 및 소프트웨어 공급업체 이용 등이 포함됩니다.
...
관리자 직원은 표준 사용자가 PAW 뿐 아니라 전자 메일을 확인 하 고, 웹을 찾아보고, 회사의 비즈니스 응용 프로그램에 액세스 하는 표준 사용자 워크스테이션이 필요 합니다.  관리자의 생산성과 보안을 둘 다 유지하는 것은 모든 PAW 배포의 성공에 필수적입니다.  사용자는 생산성을 현격히 제한하는 보안 솔루션 대신 보안이 보장되지 않더라도 생산성을 향상시키는 솔루션을 선택할 것입니다.

보안의 필요성과 생산성의 필요성에 대한 균형을 유지하려면 다음 PAW 하드웨어 프로필 중 하나를 사용하는 것이 좋습니다.

* **전용 하드웨어** -사용자 작업 및 관리 작업에 대해 별도의 전용 장치입니다.
...
> [!NOTE]
> 이러한 모든 시나리오에서 관리 담당자는 지정 된 관리 계정과 별도의 표준 사용자 계정을 발급 하는 것이 중요 합니다. 관리 계정은 PAW 관리 운영 체제에만 사용해야 합니다.
...
Windows 10 Hyper-v를 사용 하는 게스트 가상 컴퓨터 (Windows 10도 실행)에는 비즈니스용 Skype와 같은 사운드, 비디오 및 인터넷 통신 응용 프로그램을 비롯 한 풍부한 사용자 환경이 있을 수 있습니다.
...
## <a name="how-microsoft-is-using-administrative-workstations"></a>Microsoft에서 관리 워크스테이션을 사용 하는 방법

Microsoft에서는 내부 시스템뿐만 아니라 고객에게도 PAW 아키텍처 접근 방식을 사용합니다. Microsoft는 microsoft IT 인프라, Microsoft 클라우드 패브릭 인프라 개발 및 운영 및 기타 높은 가치 자산 관리를 비롯 한 여러 용량에서 내부적으로 관리 워크스테이션을 사용 합니다.
...
ESAE 관리 포리스트에 대 한 자세한 내용은 [권한 있는 액세스 보안 참조 자료](../securing-privileged-access/securing-privileged-access-reference-material.md#esae-administrative-forest-design-approach)에서 *Esae 관리 포리스트 디자인 방법* 섹션을 참조 하세요.
...
* **관리 권한** -paw은 높은 영향을 주는 IT 관리 역할 및 작업에 대 한 보안 강화를 제공 합니다. 이 아키텍처는 Active Directory 도메인 및 포리스트 Microsoft Azure Active Directory 테 넌 트, Office 365 테 넌 트, PCN (프로세스 제어 네트워크), 감독 기관과 이러한 제어 및 데이터 취득을 비롯 한 다양 한 유형의 시스템 관리에 적용 될 수 있습니다. SCADA) 시스템, Atm (자동화 인출기 Machines) 및 PoS (Point of Sale) 장치를 지정 합니다.
* **높은 민감도 정보 근로자** -PAW에서 사용 되는 방법은 미리 알림 합병 및 구매 활동, 시험판 재무 등의 매우 중요 한 정보 근로자 작업 및 직원을 위한 보호를 제공할 수도 있습니다. 보고서, 조직 사회 미디어의 현재 상태, 임원 커뮤니케이션, 특허 되지 않은 거래 비밀, 중요 한 연구 또는 기타 독점 데이터 나 중요 한 데이터입니다. 이 지침에서는 이러한 정보 근로자의 구성에 대해 자세히 다루지 않으며 이 시나리오가 기술 지침에 포함되어 있지도 않습니다.
...
* [**1 단계-Active Directory 관리자를 위한 즉시 배포**](#phase-1-immediate-deployment-for-active-directory-administrators) 이는 온-프레미스 도메인 및 포리스트 관리 역할을 보호할 수 있는 PAW를 신속 하 게 제공 합니다.
* [**2 단계-모든 관리자로 PAW 확장**](#phase-2-extend-paw-to-all-administrators) 이 기능을 사용 하면 Office 365 및 Azure, 엔터프라이즈 서버, 엔터프라이즈 응용 프로그램 및 워크스테이션과 같은 클라우드 서비스 관리자가 보호할 수 있습니다.
* [**3 단계-고급 PAW 보안**](#phase-3-extend-and-enhance-protection) 이는 추가 보호 및 PAW 보안 고려 사항에 대해 설명 합니다.

### <a name="why-dedicated-workstations"></a>전용 워크스테이션은 무엇 인가요?

조직의 현재 위협 환경은 정교한 피싱 및 기타 인터넷에 노출된 계정과 워크스테이션에 대한 지속적인 보안 침해 위험을 초래하는 인터넷 공격으로 가득 차 있습니다.

이 위협 환경에서는 조직에서 관리 계정 및 중요 한 비즈니스 자산과 같은 높은 가치 자산에 대 한 보호를 설계할 때 "위반 가정" 보안 상태를 채택 해야 합니다. 이러한 중요 자산은 직접적인 인터넷 위협뿐만 아니라 환경 내 다른 워크스테이션, 서버 및 장치에서 탑재되는 공격으로부터 모두 보호되어야 합니다.
...
이 섹션에서는 대체 접근 방식의 보안을 PAW와 비교하는 방법 및 이러한 접근 방식을 PAW 아키텍처 내에 올바르게 통합하는 방법에 대한 정보를 제공합니다. 이러한 모든 접근 방식은 격리에서 구현 될 때 심각한 위험을 발생 하지만 일부 시나리오에서는 PAW 구현에 값을 추가할 수 있습니다.

#### <a name="credential-guard-and-windows-hello-for-business"></a>Credential Guard 및 비즈니스용 Windows Hello

Windows 10에 도입된 [Credential Guard](https://technet.microsoft.com/library/mt483740%28v=vs.85%29.aspx)는 하드웨어 및 가상화 기반 보안을 사용하여 파생된 자격 증명을 보호하는 방식으로 Pass-the-Hash와 같은 일반적인 자격 증명 도난 공격을 완화합니다. [비즈니스용 Windows Hello](https://aka.ms/passport) 에서 사용 되는 자격 증명의 개인 키를 TPM (신뢰할 수 있는 플랫폼 모듈) 하드웨어로 보호할 수도 있습니다.

이것은 강력한 완화 방법 이지만 자격 증명을 Credential Guard 또는 비즈니스용 Windows Hello로 보호 하는 경우에도 워크스테이션은 특정 공격에 취약할 수 있습니다. 공격은 손상 된 장치에서 직접 자격 증명을 사용 하 고 자격 증명을 사용 하 여 자격 증명 보호 및 워크스테이션에서 취약 한 응용 프로그램 구성을 사용 하기 전에 이전에 도난당 한 자격 증명을 다시 사용 합니다.

이 섹션의 PAW 지침에는 민감도가 높은 계정 및 작업에 이러한 여러 기술을 사용하는 방법이 포함되어 있습니다.

#### <a name="administrative-vm"></a>관리 VM

관리 VM (관리 가상 컴퓨터)은 표준 사용자 데스크톱에서 호스트 되는 관리 작업을 위한 전용 운영 체제입니다. 이 접근 방식은 관리 작업 전용 OS를 제공한다는 점에서 PAW와 유사하지만 관리 VM은 보안상 표준 사용자 데스크톱을 기반으로 한다는 점에서 심각한 결함이 있습니다.

아래 다이어그램에서는 공격자가 사용자 워크스테이션의 관리 VM을 통해 제어 체인을 따라 관심 있는 대상 개체에 접근할 수 있다는 점과 역방향 구성에서는 경로를 만들기가 어렵다는 점을 보여 줍니다.

PAW 아키텍처는 사용자 워크스테이션에서 관리 VM을 호스트 하는 것을 허용 하지 않지만, 표준 회사 이미지를 사용 하는 사용자 VM은 관리자 PAW에 호스트 하 여 직원에 게 모든 책임에 대 한 단일 PC를 제공할 수 있습니다.

![PAW 아키텍처 다이어그램](../media/privileged-access-workstations/PAWFig9.JPG)

#### <a name="shielded-vm-based-paws"></a>차폐 VM 기반 Paw

관리 VM 모델의 보안 변형은 [보호 된 가상 컴퓨터](../../security/guarded-fabric-shielded-vm/guarded-fabric-and-shielded-vms.md) 를 사용 하 여 사용자 VM과 함께 하나 이상의 관리 vm을 호스트 하는 것입니다.
보호 된 Vm은 잠재적으로 신뢰할 수 없는 사용자 또는 코드가 물리적 컴퓨터의 표준 사용자 데스크톱에서 실행 될 수 있는 환경에서 보안 작업을 실행 하도록 설계 되었습니다.
보호 된 VM에는 미사용 데이터를 암호화 하는 데 사용할 수 있는 가상 TPM이 있으며, 기본 콘솔 액세스, PowerShell Direct 및 VM 디버그 기능과 같은 여러 관리 컨트롤을 사용 하지 않도록 설정 하 여 표준 사용자 데스크톱에서 VM을 추가로 격리할 수 있습니다. 기타 Vm.
보호 된 VM에 대 한 키는 신뢰할 수 있는 키 관리 서버에 저장 됩니다 .이 서버에는 VM을 시작 하기 위해 키를 릴리스하기 전에 물리적 장치가 해당 id 및 상태를 증명 해야 합니다.
이렇게 하면 보호 된 Vm이 의도 된 장치 에서만 시작 하 고 해당 장치에서 알려진 신뢰할 수 있는 소프트웨어 구성을 실행 하 게 됩니다.

보호 된 Vm은 서로 격리 되 고 표준 사용자 데스크톱과 격리 되기 때문에 해당 관리자 Vm이 다른 계층을 관리 하는 경우에도 단일 호스트에서 여러 차폐 PAW Vm을 실행 하는 것이 허용 됩니다.

자세한 내용은 아래의 [보호 된 패브릭을 사용 하 여 paw 배포](#deploy-paws-using-a-guarded-fabric) 섹션을 참조 하세요.
...
> 계층 모델에 대 한 자세한 내용은 [https://aka.ms/tiermodel](https://aka.ms/tiermodel) 을 참조 하세요. 계층 0 그룹에 대 한 자세한 내용은 [권한 있는 액세스 보안 참조 자료](../securing-privileged-access/securing-privileged-access-reference-material.md)의 계층 0과 같은 항목을 참조 하세요.

MIM (Microsoft Identity Manager) PAM (권한 있는 액세스 관리)을 배포 하는 방법에 대 한 자세한 내용은 [https://aka.ms/mimpamdeploy](https://aka.ms/mimpamdeploy) 을 참조 하세요.
...
> [!NOTE]
> 이 지침에서는 인터넷의 특정 서비스 (예: Azure 및 Office 365 관리 포털)에 대 한 액세스 요구와 모든 호스트 및 서비스의 "인터넷 열기"를 명시적으로 구분 합니다.

계층 지정에 대한 자세한 내용은 [계층 모델 페이지](https://aka.ms/tiermodel)를 참조하세요.

|**시나리오**|**PAW를 사용 하 시겠습니까?**|**범위 및 보안 고려 사항**|
|---------|--------|---------------------|
|Active Directory 관리자 - 계층 0|예|1단계 지침에 따라 구축한 PAW면 이 역할에 충분합니다.<br /><br />-   이 시나리오에 가장 강력한 보호를 제공하기 위해 관리 포리스트를 추가할 수 있습니다. ESAE 관리 포리스트에 대한 자세한 내용은 [ESAE 관리 포리스트 디자인 방법](../securing-privileged-access/securing-privileged-access-reference-material.md#esae-administrative-forest-design-approach)을 참조하세요.<br />-   PAW를 사용하여 관리되는 여러 도메인 또는 여러 포리스트를 관리할 수 있습니다.<br />-도메인 컨트롤러가 IaaS (Infrastructure as a Service) 또는 온-프레미스 가상화 솔루션에서 호스트 되는 경우 해당 솔루션의 관리자에 대해 Paw 구현 우선 순위를 지정 해야 합니다.|
|Azure IaaS 및 PaaS 서비스 관리자 - 계층 0 또는 계층 1(범위 및 디자인 고려 사항 참조)|예|2단계에서 제공된 지침에 따라 구축된 PAW면 이 역할에 충분합니다.<br /><br />-   최소한 전역 관리자 및 구독 비용 청구 관리자에게 PAW를 사용해야 합니다. 중요하거나 민감한 서버의 위임된 관리자에게도 PAW를 사용해야 합니다.<br />-Paw 및 Active Directory Federation Services (ADFS) [Azure AD Connect](https://azure.microsoft.com/documentation/articles/active-directory-aadconnect/) 와 같은 클라우드 서비스에 대 한 디렉터리 동기화 및 id 페더레이션을 제공 하는 운영 체제 및 응용 프로그램을 관리 하는 데 사용 해야 합니다.<br />-   아웃바운드 네트워크 제한에서 2단계의 지침을 사용하는 권한 있는 클라우드 서비스에 대한 연결만 허용해야 합니다. 공개 인터넷 액세스는 PAW에서 허용되어서는 안 됩니다.<br />-Windows Defender Exploit Guard는 워크스테이션에서 구성 해야 합니다 **. 참고:**     도메인 컨트롤러 또는 다른 계층 0 호스트가 구독에 있는 경우에는 포리스트에 대해 구독이 계층 0으로 간주 됩니다. Azure에서 호스트되는 계층 0 서버가 없는 경우 구독은 계층 1입니다.|
|Office 365 테넌트 관리자 <br />- 계층 1|예|2단계에서 제공된 지침에 따라 구축된 PAW면 이 역할에 충분합니다.<br /><br />-   최소한 구독 비용 청구 관리자, 전역 관리자, Exchange 관리자, SharePoint 관리자 및 사용자 관리 관리자 역할에 PAW를 사용해야 합니다. 또한 매우 중요하거나 민감한 데이터의 위임된 관리자에게도 PAW를 사용하는 것이 좋습니다.<br />-워크스테이션에서 Windows Defender Exploit Guard를 구성 해야 합니다.<br />-   아웃바운드 네트워크 제한에서 2단계의 지침을 사용하는 Microsoft 서비스에 대한 연결만 허용해야 합니다. 공개 인터넷 액세스는 PAW에서 허용되어서는 안 됩니다.|
|다른 IaaS 또는 PaaS 클라우드 서비스 관리자<br />- 계층 0 또는 계층 1(범위 및 디자인 고려 사항 참조)|예|2단계에서 제공된 지침에 따라 구축된 PAW면 이 역할에 충분합니다.<br /><br />-   에이전트 설치 권한, 하드 디스크 파일 내보내기 권한, 운영 체제, 민감한 데이터 또는 비즈니스에 중요한 데이터가 저장된 저장소 액세스 권한 등 클라우드 호스트된 VM에 대한 관리 권한을 가진 모든 역할에 PAW를 사용해야 합니다.<br />-   아웃바운드 네트워크 제한에서 2단계의 지침을 사용하는 Microsoft 서비스에 대한 연결만 허용해야 합니다. 공개 인터넷 액세스는 PAW에서 허용되어서는 안 됩니다.<br />-워크스테이션에서 Windows Defender Exploit Guard를 구성 해야 합니다. **참고:** 도메인 컨트롤러 또는 다른 계층 0 호스트가 구독에 있는 경우 포리스트의 계층 0 구독입니다. Azure에서 호스트되는 계층 0 서버가 없는 경우 구독은 계층 1입니다.|
|가상화 관리자<br />- 계층 0 또는 계층 1(범위 및 디자인 고려 사항 참조)|예|2단계에서 제공된 지침에 따라 구축된 PAW면 이 역할에 충분합니다.<br /><br />-   에이전트 설치 권한, 가상 하드 디스크 파일 내보내기 권한, 게스트 운영 체제 정보, 민감한 데이터 또는 비즈니스에 중요한 데이터가 저장된 저장소 액세스 권한 등 VM에 대한 관리 권한을 가진 모든 역할에 PAW를 사용해야 합니다. **참고:** 도메인 컨트롤러 또는 다른 계층 0 호스트가 구독에 있는 경우 가상화 시스템 (및 해당 관리자)은 포리스트에 대해 계층 0으로 간주 됩니다. 가상화 시스템에서 호스트되는 계층 0 서버가 없는 경우 구독은 계층 1입니다.|
...
|사용자 워크스테이션 관리자 <br />- 계층 2|예|2단계에서 제공된 지침에 따라 구축된 PAW면 최종 사용자 장치(예: 지원 센터 및 데스크 측 지원 역할)에 대한 관리 권한이 있는 역할에 충분합니다.<br /><br />-   티켓 관리 및 기타 지원 기능을 사용하려면 PAW에 추가 응용 프로그램을 설치해야 할 수도 있습니다.<br />-워크스테이션에서 Windows Defender Exploit Guard를 구성 해야 합니다.<br />    PAW에서 보다 대규모의 관리자를 처리하려면 전용 관리 도구를 추가해야 할 수도 있습니다.|
|SQL, SharePoint 또는 LOB(기간 업무) 관리자<br />- 계층 1|예|2단계 지침에 따라 구축한 PAW면 이 역할에 충분합니다.<br /><br />-   관리자가 서버에 연결하지 않고도 원격 데스크톱을 사용하여 응용 프로그램을 관리할 수 있도록 하려면 PAW에 추가 관리 도구를 설치해야 할 수도 있습니다.|
...
|VIP 사용자(임원, 연구원 등)|부분적으로|2 단계에서 제공 된 지침을 사용 하 여 빌드한 PAW는 이러한 역할에 대 한 보안을 제공 하기 위한 시작 점으로 사용할 수 있습니다.<br /><br />-   이 시나리오는 표준 사용자 데스크톱과 유사하지만 일반적으로 응용 프로그램 프로필이 더 작고 단순하며 잘 알려져 있습니다. 이 시나리오에서는 일반적으로 중요한 데이터, 서비스 및 응용 프로그램(데스크톱에 설치되거나 설치되지 않은)을 검색하고 보호해야 합니다.<br />-   이러한 역할에는 일반적으로 높은 보안 수준과 매우 높은 유용성 수준이 필요하므로 사용자 요구 사항을 충족하려면 디자인 변경이 필요합니다.|
|산업용 제어 시스템(예: SCADA, PCN 및 DC)|부분적으로|대부분의 ICS 콘솔(SCADA 및 PCN과 같은 일반적인 표준 포함)에서는 공개 인터넷을 검색하고 전자 메일을 확인할 필요가 없으므로 2단계에서 제공된 지침에 따라 구축한 PAW를 시작점으로 사용하여 이러한 역할에 대한 보안을 제공할 수 있습니다.<br /><br />-물리적 기계를 제어 하는 데 사용 되는 응용 프로그램을 통합 하 고 호환성을 테스트 하 고 적절 하 게 보호 해야 합니다.|
...
> 이러한 경우에 유의 해야 하는 주요 규칙은 항상 계층 모델 규칙을 따라야 한다는 점입니다. 자세한 내용은 계층 모델 페이지를 참조하세요.
...
## <a name="paw-phased-implementation"></a>PAW 단계적 구현
...
* [1 단계-Active Directory 관리자를 위한 즉시 배포](#phase-1-immediate-deployment-for-active-directory-administrators)
...
용도: 온-프레미스 도메인 및 포리스트 관리 역할을 보호할 수 있는 PAW를 신속 하 게 제공 합니다.

범위 Enterprise Admins, Domain Admins (모든 도메인의 경우) 및 기타 신뢰할 수 있는 id 시스템 관리자를 포함 하는 계층 0 관리자
...
* OU(조직 구성 단위)
   * 새 최상위 Ou 6 개:  지점 그룹과 계층 1 서버 워크스테이션 사용자 계정 및 컴퓨터 격리.  각 최상위 OU에는 여러 자식 Ou가 포함 됩니다.
* 그룹
   * 새로운 6 가지 보안 사용 글로벌 그룹:  계층 0 복제 유지 관리 계층 1 서버 유지 관리 서비스 데스크 운영자; 워크스테이션 유지 관리 PAW 사용자 PAW 유지 관리.

또한 다음과 같은 몇 가지 그룹 정책 개체를 만듭니다. PAW 구성-컴퓨터; PAW 구성-사용자 RestrictedAdmin 필수-컴퓨터; PAW 아웃 바운드 제한 워크스테이션 로그온 제한; 서버 로그온 제한
...
4. **필요한 Windows 10 Enterprise Edition 및 응용 프로그램 소프트웨어를 얻고 유효성을 검사**합니다. PAW에 필요한 소프트웨어를 구입하고 [설치 미디어에 대한 클린 소스](https://aka.ms/cleansource)의 지침에 따라 유효성을 검사합니다.
...
1. [TechNet 갤러리](https://aka.ms/PAWmedia)에서 PAW 스크립트 라이브러리를 다운로드합니다.
   ...
   > [!NOTE]
   > 모든 파일을 다운로드 하 여 동일한 디렉터리에 저장 하 고 아래 지정 된 순서 대로 실행 합니다.  Create-PAWGroups는 Create-PAWOUs를 통해 만들어진 OU에 따라 달라지고, Set-PAWOUDelegation은 Create-PAWGroups를 통해 만들어진 그룹에 따라 달라집니다.
...
#### <a name="move-tier-0-accounts-to-the-admintier-0accounts-ou"></a>계층 0 계정을 Admin\Tier 0 \ 계정 OU로 이동 합니다.
...
이 섹션에서는 이러한 Paw에 대 한 특정 보호를 제공 하는 새 "PAW Configuration-Computer" GPO를 만들고 계층 0 장치 OU (계층 0 \ 관리자의 "장치"에 연결 합니다.
...
2. **로컬 그룹 구성원 제한** - 이 설정은 워크스테이션의 로컬 관리자 그룹의 구성원이 항상 비어 있도록 합니다.
   ...
   2. 다음 추가 그룹에 대해 위 단계를 완료합니다.
      ...
      * 리플리케이터
   3. **PAW Logon 제한** -이 설정은 PAW에 로그온 할 수 있는 계정을 제한 합니다. 이 설정을 구성하려면 아래 단계를 따릅니다.
      1. 컴퓨터 구성\정책\Windows 설정\보안 설정\로컬 정책\사용자 권한 할당\로컬 로그온 허용으로 이동합니다.
      2. 이 정책 설정 정의를 선택 하 고 "PAW Users" 및 Administrators를 추가 합니다 (관리자 선택에는 찾아보기 단추를 사용 하지 않음).
   4. **인바운드 네트워크 트래픽 차단** - 이 설정은 무단 인바운드 네트워크 트래픽이 PAW에 허용되지 않도록 합니다. 이 설정을 구성하려면 아래 단계를 따릅니다.
      1. 컴퓨터 구성\정책\Windows 설정\보안 설정\고급 보안이 포함된 Windows 방화벽\고급 보안이 포함된 Windows 방화벽으로 이동하여 아래 단계를 따릅니다.
         ...
         4. **확인**을 클릭합니다.
            ...
            > WFW 파일의 설정은 모든 방화벽 프로필에 대해 "차단 - 기본값" 모드의 방화벽을 사용하도록 설정하고, 규칙 병합을 해제하며, 삭제된 패킷과 성공한 패킷을 둘 다 기록하도록 합니다. 이러한 설정은 PAW에서 시작 된 연결에 대 한 양방향 통신을 허용 하는 동시에, 로컬 관리자 액세스 권한이 있는 사용자가 GPO 설정을 재정의 하는 로컬 방화벽 규칙을 만들지 못하도록 방지 하는 동시에 원치 않는 트래픽을 차단 합니다. PAW에서 들어오고 나가는 트래픽이 기록 되는지 확인 합니다.
   ...
   5. **WSUS에 대한 Windows 업데이트 구성** - 아래 단계에 따라 설정을 변경하여 PAW에 대한 Windows 업데이트를 구성합니다.
      1. Computer Configuration\Policies\Administrative)로 이동 하 여 다음 단계를 수행 합니다.
         ...
         4. **인트라넷 Microsoft 업데이트 서비스 위치 정책 지정** 옵션을 사용 하도록 설정 하 고 두 옵션 모두에 ESAE WSUS 서버의 URL을 지정 합니다.
   6. 다음과 같이 "PAW" GPO를 연결 합니다.

         |정책|링크 위치|
         |-----|---------|
         |PAW Configuration - Computer |Admin\Tier 0\Devices|

#### <a name="create-paw-configuration---user-group-policy-object-gpo"></a>"PAW 구성-사용자" GPO (그룹 정책 개체) 만들기

이 섹션에서는 이러한 Paw에 대 한 특정 보호를 제공 하는 새 "PAW Configuration-User" GPO와 계층 0 계정 OU (계층 0 \ 관리자의 "계정")에 대 한 링크를 만듭니다.

   > [!NOTE]
   > 이러한 설정을 기본 도메인 정책에 추가하지 마세요.

1. **인터넷 검색 차단** - 의도치 않은 인터넷 검색을 방지하기 위해 루프백 주소의 프록시 주소(127.0.0.1)를 설정합니다.
   1. 사용자 구성 \ 기본 Settings\Registry.로 이동 합니다. 레지스트리를 마우스 오른쪽 단추로 클릭 하 고 **새로 만들기** > **레지스트리 항목** 을 선택 하 고 다음 설정을 구성 합니다.
      1. 작업:  바꾸기
      2. 하이브의 HKEY_CURRENT_USER
   ...
   2. 사용자 구성 \ 기본 Settings\Registry.로 이동 합니다. 레지스트리를 마우스 오른쪽 단추로 클릭 하 고 **새로 만들기** > **레지스트리 항목** 을 선택 하 고 다음 설정을 구성 합니다.

      * 작업: 바꾸기
      * 하이브의 HKEY_CURRENT_USER
...
2. 사용자 Configuration\Policies\Administrative를 사용 하 여 다음 옵션을 사용 하도록 설정 합니다. 이러한 설정은 관리자가 프록시 설정을 수동으로 재정의하는 것을 방지합니다.
   1. **자동 구성 설정 변경할 수 없음** 설정을 사용하도록 설정합니다.
   2. **프록시 설정 변경 방지** 설정을 사용하도록 설정합니다.

#### <a name="restrict-administrators-from-logging-onto-lower-tier-hosts"></a>관리자가 하위 계층 호스트에 로그온 하지 못하도록 제한
...
2. 로컬 관리자 계정의 고유하고 복잡한 암호를 설정합니다.  환경에서 다른 계정에 사용된 암호를 사용하지 마세요.
   ...
   > [!NOTE]
   > [LAPS (로컬 관리자 암호 솔루션)](https://www.microsoft.com/en-us/download/details.aspx?id=46899) 를 사용 하 여 paw를 비롯 한 모든 워크스테이션의 로컬 관리자 암호를 관리 하는 것이 좋습니다.  LAPS를 사용하는 경우 PAW Maintenance 그룹에만 PAW의 LAPS 관리되는 암호를 읽을 수 있는 권한을 부여해야 합니다.
...
9. 필드 Active Directory 관리자에 대 한 추가 필수 도구를 설치 합니다. 작업을 수행하는 데 필요한 다른 도구 또는 스크립트를 설치합니다. PAW에 추가하기 전에 모두 도구에 대해 대상 컴퓨터에서 자격 증명 노출 위험을 평가해야 합니다. 관리 도구 및 연결 방법에 대해 자격 증명 노출 위험을 평가하는 방법에 대한 자세한 내용은 [이 페이지](https://aka.ms/logontypes)를 참조하세요. [설치 미디어에 대한 클린 소스](https://aka.ms/cleansource)의 지침에 따라 모든 설치 미디어를 가져와야 합니다.

   > [!NOTE]
   > 이러한 도구의 중앙 위치에 점프 서버를 사용하면 보안 경계 역할을 하지 않더라도 복잡성을 줄일 수는 있습니다.

10. 필드 필요한 원격 액세스 소프트웨어를 다운로드 하 여 설치 합니다. 관리자가 관리를 위해 원격으로 PAW를 사용하는 경우 원격 액세스 솔루션 공급업체의 보안 지침에 따라 원격 액세스 소프트웨어를 설치합니다. 설치 미디어에 대한 클린 소스의 지침에 따라 모든 설치 미디어를 가져와야 합니다.

    > [!NOTE]
    > PAW를 통해 원격 액세스를 허용 하는 것과 관련 된 모든 위험을 신중 하 게 고려해 야 합니다.  모바일 PAW는 재택 근무를 포함하여 중요한 많은 시나리오를 지원하지만 원격 액세스 소프트웨어는 잠재적으로 공격에 취약하고 PAW를 손상시키는 데 사용될 수 있습니다.
...
11. 아래 단계에 따라 모든 설정이 적절히 완료되었는지 검토 및 확인하여 PAW 시스템의 무결성에 대한 유효성을 검사합니다.
    ...
    2. 아래 단계에 따라 PAW의 권한 있는 그룹에 구성원으로 속한 추가 사용자 계정이 없는지 확인합니다.
       ...
       2. 또한 **로컬 사용자 및 그룹 편집**을 사용하여 다음 그룹에 구성원이 없는지 확인합니다. Backup Operators 암호화 운영자 Hyper-v 관리자 네트워크 구성 운영자 Power Users 원격 데스크톱 사용자 복제기

12. 필드 조직에서 SIEM (보안 정보 및 이벤트 관리) 솔루션을 사용 하는 경우 PAW가 [WEF (Windows 이벤트 전달)를 사용 하 여 시스템에 이벤트를 전달 하도록 구성](http://blogs.technet.com/b/jepayne/archive/2015/11/24/monitoring-what-matters-windows-event-forwarding-for-everyone-even-if-you-already-have-a-siem.aspx) 되어 있는지, 아니면 siem이 되도록 솔루션에 등록 되어 있는지 확인 합니다. PAW에서 이벤트 및 정보를 적극적으로 수신 합니다.  이 작업의 세부 정보는 SIEM 솔루션에 따라 크게 다릅니다.

    > [!NOTE]
    > SIEM에 PAW의 시스템 또는 로컬 관리 계정으로 실행되는 에이전트가 필요한 경우 SIEM이 도메인 컨트롤러 및 ID 시스템과 동일한 신뢰 수준으로 관리되는지 확인합니다.

13. 필드 LAPS를 배포 하 여 PAW의 로컬 관리자 계정에 대 한 암호를 관리 하도록 선택한 경우 암호가 성공적으로 등록 되었는지 확인 합니다.
...
### <a name="phase-2-extend-paw-to-all-administrators"></a>2 단계: 모든 관리자에 게 PAW 확장

범위 중요 업무용 응용 프로그램 및 종속성에 대 한 관리 권한이 있는 모든 사용자입니다.  여기에는 최소한 응용 프로그램 서버, 작업 상태 및 보안 모니터링 솔루션, 가상화 솔루션, 저장소 시스템 및 네트워크 장치의 관리자가 포함되어야 합니다.

> [!NOTE]
> 이 단계의 지침에서는 1단계가 완전히 완료된 것으로 가정합니다.  1 단계의 모든 단계를 완료할 때까지 2 단계를 시작 하지 않습니다.

모든 단계가 완료된 것을 확인했으면 아래 단계를 수행하여 2단계를 완료합니다.

#### <a name="recommended-enable-restrictedadmin-mode"></a>바람직하지 **RestrictedAdmin** 모드 사용

기존 서버 및 워크스테이션에서이 기능을 사용 하도록 설정한 다음이 기능의 사용을 적용 합니다. 이 기능을 실행 하려면 대상 서버에서 windows Server 2008 R2 이상을 실행 하 고 대상 워크스테이션에서 Windows 7 이상을 실행 해야 합니다.
...
3. 아래 정책 옵션을 사용하여 **RestrictedAdmin** Required - Computer를 적절한 계층 1 및/또는 계층 2 장치에 연결합니다.
   * PAW Configuration - Computer
      * -> 링크 위치: Admin\Tier 0\Devices (기존)
   * PAW Configuration - User
      * -> 링크 위치: Admin\Tier 0 \ 계정
...
5. 필드 Cloud Services에 대 한 연결을 사용 하도록 설정 합니다. 이 단계에서는 적절한 보안 보증을 지원하는 Azure 및 Office 365와 같은 클라우드 서비스를 관리할 수 있도록 합니다. 이 단계는 Microsoft Intune에서 PAW를 관리하는 데에도 필요합니다.
   ...
   * PAW 서비스에 대한 연결을 설정하려면 아래 단계를 따릅니다.
   ...
   1. 권한 있는 인터넷 대상만 허용하도록 PAW를 구성합니다.  클라우드 관리를 사용하도록 PAW 배포를 확장하는 경우 공격자가 관리자를 보다 쉽게 도용할 수 있는 공개 인터넷에서의 액세스를 필터링하면서 권한 있는 서비스에 대한 액세스를 허용해야 합니다.
      ...
      1. **Cloud Services 관리자** 그룹을 만들고 인터넷에서 클라우드 서비스에 액세스 해야 하는 모든 계정을 추가 합니다.
      ...
      3. *proxy.pac* 파일을 구성했으면 PAW Configuration - User GPO를 업데이트합니다.
         1. 사용자 구성 \ 기본 Settings\Registry.로 이동 합니다. 레지스트리를 마우스 오른쪽 단추로 클릭 하 고 **새로 만들기** > **레지스트리 항목** 을 선택 하 고 다음 설정을 구성 합니다.
            ...
            5. 값 형식: REG_SZ
            6. 값 데이터: http://및 파일 이름 (@no__t 예:-1)을 포함 하 여 *프록시 .pac* 파일의 전체 URL을 입력 합니다.  URL은 단일 레이블 URL 일 수도 있습니다 (예: http://proxy/proxy.pac ).

               > [!NOTE]
               > file://server.fabrikan.com/share/proxy.pac 구문을 사용하여 파일 공유에서 PAC 파일을 호스트할 수도 있지만 그러려면 file:// 프로토콜을 허용해야 합니다. 다음을 참조 하십시오. 필요한 레지스트리 값 구성에 대 한 자세한 내용은이 [이해 웹 프록시 구성](http://blogs.msdn.com/b/ieinternals/archive/2013/10/11/web-proxy-configuration-and-ie11-changes.aspx) 블로그의 File://-based 프록시 스크립트를 사용 하지 않음 "섹션을 사용 하세요.
   ...
   2. Windows 10 보안 기준 및 클라우드 서비스 액세스를 적용합니다. 아래 단계를 사용하여 Windows 및 클라우드 서비스(필요한 경우)에 대한 보안 기준을 올바른 OU에 연결합니다.
      ...
      2. 이러한 GPO를 만들고 [정책 설정을 가져온](https://technet.microsoft.com/library/cc753786.aspx) 다음 아래 표에 따라 [연결](https://technet.microsoft.com/library/cc732979.aspx)합니다. 각 정책을 각 위치에 연결하고 순서가 표에 따르는지 확인합니다(표의 아래쪽에 있는 항목이 나중에 적용되고 우선 순위가 더 높음).
         ...
         **방침**
         ...
         |SCM Windows 10 TH2-컴퓨터|Admin\Tier 0\Devices|
...
6. 필드 계층 1 관리자에 대 한 추가 필수 도구를 설치 합니다. 작업을 수행하는 데 필요한 다른 도구 또는 스크립트를 설치합니다. PAW에 추가하기 전에 모두 도구에 대해 대상 컴퓨터에서 자격 증명 노출 위험을 평가해야 합니다. 관리 도구 및 연결 방법에 대해 자격 증명 노출 위험을 평가하는 방법에 대한 자세한 내용은 [이 페이지](https://aka.ms/logontypes)를 참조하세요. 설치 미디어에 대한 클린 소스의 지침에 따라 모든 설치 미디어를 가져와야 합니다.
7. 관리에 필요한 소프트웨어 및 응용 프로그램을 식별하고 안전하게 가져오기.  이는 1단계에서 수행한 작업과 유사하지만 보안을 유지해야 하는 응용 프로그램, 서비스 및 시스템 수가 증가했기 때문에 범위가 더 넓습니다.

   > [!NOTE]
   > Windows Defender Exploit Guard에서 제공 하는 보호로 옵트인 하 여 이러한 새 응용 프로그램 (웹 브라우저 포함)을 보호 해야 합니다.

   추가 소프트웨어 및 응용 프로그램의 예:

      * Microsoft Azure PowerShell
      * Office 365 PowerShell(Microsoft Online Services 모듈이라고도 함)
      * Microsoft Management Console을 기반으로 하는 응용 프로그램 또는 서비스 관리 소프트웨어
      * 독점(비 MMC 기반) 응용 프로그램 또는 서비스 관리 소프트웨어

         > [!NOTE]
         > 현재 다양한 클라우드 서비스를 포함하여 많은 응용 프로그램이 웹 브라우저를 통해 단독으로 관리됩니다.  따라서 PAW에 설치해야 하는 응용 프로그램 수는 감소하지만 브라우저 상호 운용성 문제의 위험이 발생합니다.  특정 서비스를 관리하기 위해 특정 PAW 인스턴스에 타사 웹 브라우저를 배포해야 할 수도 있습니다.  추가 웹 브라우저를 배포하는 경우 모든 클린 소스 원칙을 준수하고 공급업체의 보안 지침에 따라 브라우저의 보안을 유지해야 합니다.

8. 필드 필요한 관리 에이전트를 다운로드 하 여 설치 합니다.
...
9. PAW에서 제공하는 추가 보안 보호가 필요한 시스템을 식별하기 위해 인프라 평가.  보호해야 하는 시스템을 정확히 알아야 합니다.  리소스 자체에 대해 다음과 같은 중요한 질문을 합니다.
   ...
   * 몇 개의 시스템이 있습니까?
   * 이러한 시스템이 다른 시스템(가상화, 저장소 등)에 종속되어 있습니까? 그렇다면 그러한 시스템은 어떻게 관리되고 있습니까?  중요한 시스템이 이러한 종속성에 어떻게 노출되어 있으며, 이러한 종속성과 관련된 추가적인 위험은 무엇입니까?
   * 관리되는 서비스가 얼마나 중요하며, 이러한 서비스가 손상된 경우 예상되는 손실은 어느 정도입니까?

      > [!NOTE]
      > 이 평가에 클라우드 서비스를 포함합니다. 보안되지 않은 클라우드 배포를 대상으로 하는 공격자가 점점 늘어나고 있으므로 업무에 중요한 온-프레미스 응용 프로그램처럼 이러한 서비스를 안전하게 관리하는 것이 매우 중요합니다.

        이 평가를 사용하여 추가 보호가 필요한 특정 시스템을 식별한 다음 해당 시스템의 관리자로 PAW 프로그램을 확장합니다.  PAW 기반 관리의 이점을 크게 활용할 수 있는 시스템의 일반적인 예로는 SQL Server(온-프레미스와 SQL Azure 모두), HR 응용 프로그램, 재무 소프트웨어 등이 있습니다.

        > [!NOTE]
        > 리소스가 Windows 시스템에서 관리되는 경우 PAW로 관리할 수 있으며, 이는 응용 프로그램 자체가 Windows가 아닌 다른 운영 체제 또는 타사 클라우드 플랫폼에서 실행되는 경우에도 마찬가지입니다.  예를 들어 Amazon Web Services 구독 소유자는 PAW만 사용하여 해당 계정을 관리해야 합니다.

10. 조직의 규모에 맞게 PAW를 배포하기 위한 요청 배포 방법 개발.  2단계에서 배포하려고 선택한 PAW 수에 따라 프로세스를 자동화해야 할 수도 있습니다.

    * 관리자가 PAW를 얻는 데 사용할 수 있는 공식적인 요청 및 승인 프로세스를 개발하는 것이 좋습니다.  이 프로세스는 배포 프로세스를 표준화하고, PAW 장치에 대한 책임을 보장하며, PAW 배포의 차이를 식별하는 데 도움이 됩니다.
...
1. **권한 있는 계정에 대해 multi-factor authentication을 사용 하도록 설정**합니다.  다단계 인증은 사용자에게 자격 증명 외에 물리적 토큰을 제공하도록 하여 계정 보안을 강화합니다.  다단계 인증은 인증 정책을 보완할 뿐만 아니라 배포용 인증 정책에 종속되지도 않습니다(마찬가지로 인증 정책에서는 다단계 인증을 요구하지 않음).  다음 형태 중 하나의 다단계 인증을 사용하는 것이 좋습니다.

   * **스마트 카드**: 스마트 카드는 Windows 로그온 프로세스를 진행 하는 동안 두 번째 확인을 제공 하는 변조 방지 및 휴대용 물리적 장치입니다.  로그온하려면 개인이 카드를 소지해야 하므로 도난된 자격 증명이 원격으로 재사용되는 위험을 줄일 수 있습니다.  Windows에서의 스마트 카드 로그온에 대한 자세한 내용은 [스마트 카드 개요](https://technet.microsoft.com/library/hh831433.aspx) 문서를 참조하세요.
   * **가상 스마트 카드**:  가상 스마트 카드를 사용 하면 실제 스마트 카드와 동일한 보안 이점을 제공 하므로 특정 하드웨어에 연결 되는 이점이 추가 됩니다.  배포 및 하드웨어 요구 사항에 대 한 자세한 내용은 [가상 스마트 카드 개요](https://technet.microsoft.com/library/dn593708.aspx) 및 [ 가상 스마트 카드 시작을 참조 하세요. 연습 가이드 @ no__t-0.
   * **비즈니스용 Windows Hello**: 비즈니스용 Windows Hello를 통해 사용자는 FIDO (Fast ID Online) 인증을 지 원하는 Microsoft 계정, Active Directory 계정, Microsoft Azure Active Directory (Azure AD) 계정 또는 타사 서비스에 인증할 수 있습니다. 비즈니스용 windows Hello 등록 중에 초기 2 단계 인증 후에는 사용자의 장치에 비즈니스용 Windows Hello가 설정 되며 사용자는 Windows Hello 또는 PIN이 될 수 있는 제스처를 설정 합니다. 비즈니스용 Windows Hello 자격 증명은 비대칭 키 쌍으로, Tpm (신뢰할 수 있는 플랫폼 모듈)의 격리 된 환경 내에서 생성 될 수 있습니다.
      비즈니스용 Windows Hello에 대 한 자세한 내용은 비즈니스용 [Windows hello](https://docs.microsoft.com/windows/security/identity-protection/hello-for-business/hello-identity-verification) 문서를 참조 하세요.
   * **Azure multi-factor authentication**:  Azure MFA (multi-factor authentication)는 모니터링 및 기계 학습 기반 분석을 통해 향상 된 보호 뿐만 아니라 두 번째 확인 요소의 보안을 제공 합니다.  Azure MFA는 Azure 관리자는 물론, 웹 응용 프로그램, Azure Active Directory, 온-프레미스 솔루션(예: 원격 액세스 및 원격 데스크톱) 등 다른 많은 솔루션의 보안도 유지할 수 있습니다.  Azure 다단계 인증에 대한 자세한 내용은 [다단계 인증](https://azure.microsoft.com/services/multi-factor-authentication)을 참조하세요.

2. **Windows Defender 응용 프로그램 제어 및/또는 AppLocker를 사용 하 여 신뢰할 수 있는 응용 프로그램을 허용 목록**.  신뢰할 수 없거나 서명되지 않은 코드를 PAW에서 실행할 수 있는 권한을 제한하면 악의적인 활동 및 손상의 가능성을 더욱 줄일 수 있습니다.  Windows에서는 응용 프로그램 제어에 대한 두 가지 기본 옵션을 제공합니다.

   * **AppLocker**:  AppLocker를 사용 하면 관리자가 지정 된 시스템에서 실행할 수 있는 응용 프로그램을 제어할 수 있습니다.  AppLocker는 그룹 정책을 통해 중앙에서 제어하고 특정 사용자 또는 그룹(PAW 사용자를 대상으로 하는 응용 프로그램의 경우)에 적용할 수 있습니다.  AppLocker에 대한 자세한 내용은 [AppLocker 개요](https://technet.microsoft.com/library/hh831440.aspx) TechNet 문서를 참조하세요.
   * **Windows Defender 응용 프로그램 제어**: 새 Windows defender 응용 프로그램 제어 기능은 AppLocker와는 달리 영향을 받는 장치에서 재정의할 수 없는 향상 된 하드웨어 기반 응용 프로그램 제어 기능을 제공 합니다.  AppLocker와 마찬가지로, Windows Defender 응용 프로그램 제어는 그룹 정책을 통해 제어 하 고 특정 사용자를 대상으로 지정할 수 있습니다.  Windows Defender 응용 프로그램 제어를 사용 하 여 응용 프로그램 사용을 제한 하는 방법에 대 한 자세한 내용은 [Windows Defender 응용 프로그램 제어 배포 가이드](https://docs.microsoft.com/windows/security/threat-protection/windows-defender-application-control/windows-defender-application-control-deployment-guide)를 참조 하세요.
...
PAW에서 추가적인 구성 관리, 운영 모니터링 및 보안 관리를 사용할 수도 있지만 각 관리 기능에는 해당 도구를 통한 PAW 손상 위험도 따르므로 이러한 기능을 통합할 때는 신중히 고려해야 합니다. 고급 관리 기능을 도입 하는 것이 적절 한지 여부는 다음과 같은 몇 가지 요인에 따라 달라 집니다.
...
|접근 방식|고려 사항|
|------|---------|
|기본적으로 PAW<br /><br />-   Windows Server Update Services<br />-   Windows Defender|-   추가 비용 없음<br />-   기본적인 필수 보안 기능 수행<br />-   이 지침에 포함된 설명|
|[Intune](https://technet.microsoft.com/library/jj676587.aspx)으로 관리|<ul><li>클라우드 기반 가시성 및 제어 제공<br /><br /><ul><li>소프트웨어 배포</li><li>o   소프트웨어 업데이트 관리</li><li>Windows 방화벽 정책 관리</li><li>맬웨어 방지 보호</li><li>원격 지원</li><li>소프트웨어 라이선스 관리.</li></ul></li><li>필요한 서버 인프라 없음</li><li>2단계의 "클라우드 서비스 연결 사용" 단계를 따라야 함</li><li>PAW 컴퓨터가 도메인에 가입되지 않은 경우 보안 기준 다운로드에 제공된 도구를 사용하여 로컬 이미지에 SCM 기준을 적용해야 함</li></ul>|
...
|기존 관리 도구로 PAW 관리|-기존 관리 인프라를 Paw의 보안 수준으로 설정 하지 않은 경우 Paw 손상에 대 한 상당한 위험을 만듭니다 **.**     Microsoft는 일반적으로 조직에서 사용 해야 하는 특별 한 이유가 없다면이 접근 방식을 사용 하지 않습니다. Microsoft 환경에서는 일반적으로 이러한 모든 도구 (및 해당 보안 종속성)를 Paw의 보안 수준까지 가져오는 비용이 매우 높습니다.<br />-   이러한 도구는 대부분 구성, 소프트웨어 배포 및 보안 업데이트에 대한 가시성 및 제어를 제공함|
|관리 액세스가 필요한 보안 검사 또는 모니터링 도구|에이전트를 설치하는 도구를 포함하거나 로컬 관리 액세스 계정이 필요함<br /><br />-   도구 보안 보증을 PAW 수준까지 강화해야 함<br />-   도구 기능(포트 열기, Java 또는 기타 미들웨어 설치 등)을 지원하기 위해 PAW의 보안 상태를 낮추거나 보안 위험 감수에 대한 의사 결정이 필요함|
|SIEM(보안 정보 및 이벤트 관리)|<ul><li>SIEM에 에이전트가 없는 경우<br /><br /><ul><li>**Event Log Readers** 그룹의 계정을 사용하여 관리 액세스 없이 PAW에서 이벤트에 액세스할 수 있음</li><li>SIEM 서버의 인바운드 트래픽을 허용하도록 네트워크 포트를 열어야 함</li></ul></li><li>SIEM에 에이전트가 필요로 하는 경우에는 **관리 액세스가 필요한 보안 검사 또는 모니터링 도구** 행 참조</li></ul>|
...
## <a name="deploy-paws-using-a-guarded-fabric"></a>보호 된 패브릭을 사용 하 여 Paw 배포

[보호 된 패브릭을](https://aka.ms/shieldedvms) 사용 하 여 랩톱 또는 점프 서버의 차폐 가상 머신에서 PAW 작업을 실행할 수 있습니다.
이 접근 방식을 채택 하려면 추가 인프라 및 운영 단계가 필요 하지만, PAW 이미지를 정기적으로 다시 배포 하는 것이 더 쉬울 뿐만 아니라 여러 계층화 된 (또는 분류) Paw를 실행 하는 가상 컴퓨터에 통합할 수 있습니다. 단일 장치에서 나란히.
보호 된 패브릭 토폴로지와 보안 약속에 대 한 자세한 설명은 [보호 된 패브릭 설명서](https://aka.ms/shieldedvms)를 참조 하세요.

### <a name="changes-to-the-paw-gpos"></a>PAW Gpo에 대 한 변경 내용

차폐 VM 기반 Paw를 사용 하는 경우 위에 정의 된 [권장 GPO 설정을](#create-paw-configuration---computer-group-policy-object-gpo) 수정 하 여 가상 머신 사용을 지원 해야 합니다.

1. 물리적 PAW 호스트의 새 OU를 만듭니다. 실제 및 가상 Paw는 서로 다른 보안 요구 사항을 가지 며 적절 하 게 Active Directory으로 구분 해야 합니다.
2. PAW 컴퓨터 GPO는 실제 및 가상 PAW Ou 모두에 연결 되어야 합니다.
3. 물리적 PAW에 대 한 새 GPO를 만들어 PAW 사용자를 Hyper-v 관리자 그룹에 추가 합니다. 관리자가 관리자 Vm에 연결 하 고 필요에 따라 설정/해제할 수 있게 하려면이 작업이 필요 합니다. 물리적 PAW에 로그인 하는 사용자에 게는 관리자 권한이 없거나, 인터넷에 액세스할 수 없거나, 악성 가상 컴퓨터 데이터를 네트워크 공유 또는 외부 저장 장치에서 실제 PAW로 복사 하는 기능이 중요 합니다.
4. PAW 사용자를 Remote Desktop Users 그룹에 추가 하려면 관리 Vm에 대 한 새 GPO를 만듭니다. 이렇게 하면 사용자가 더 나은 사용자 환경을 제공 하 고 VM에 대 한 스마트 카드 통과를 사용할 수 있도록 하는 Hyper-v 고급 콘솔 세션을 사용할 수 있습니다.

### <a name="set-up-the-host-guardian-service"></a>호스트 보호자 서비스 설정

호스트 보호자 서비스는 실제 PAW 장치의 id 및 상태를 증명 하는 일을 담당 합니다.
HGS에 알려진 컴퓨터와 신뢰할 수 있는 [코드 무결성 정책을](https://docs.microsoft.com/windows/security/threat-protection/windows-defender-application-control/windows-defender-application-control) 실행 하는 컴퓨터만 보호 된 vm을 시작할 수 있습니다.
이렇게 하면 사용자 데스크톱 환경 위협 으로부터 신뢰할 수 있는 작업을 실행 하 여 계층화 된 리소스를 관리 하는 보호 된 Vm을 보호할 수 있습니다.

HGS는 PAW Vm을 실행할 수 있는 장치를 결정 하는 일을 담당 하므로 계층 0 리소스로 간주 됩니다.
다른 계층 0 리소스와 함께 배포 되 고 권한이 없는 물리적 및 논리적 액세스 로부터 보호 되어야 합니다.
HGS는 클러스터 된 역할 이므로 크기를 배포 하기 위해 쉽게 확장할 수 있습니다.
일반적으로 최소 3 개 노드를 사용 하 여 모든 1000 장치에 대해 1 개의 HGS 서버를 계획 하는 것이 일반적입니다.

1. 첫 번째 HGS 서버를 설치 하려면 [hgs-요새 포리스트 설치](../../security/guarded-fabric-shielded-vm/guarded-fabric-install-hgs-in-a-bastion-forest.md) 문서를 시작 하 고 계층 0 도메인에 hgs를 조인 합니다.

2. 그런 다음 엔터프라이즈 인증 기관을 사용 하 여 [HGS 용 인증서를 만듭니다](../../security/guarded-fabric-shielded-vm/guarded-fabric-obtain-certs.md) .
HGS 암호화 및 서명 인증서를 소유 하는 모든 사용자는 보호 된 VM의 암호를 해독할 수 있으므로 개인 키를 보호 하기 위해 하드웨어 보안 모듈에 액세스할 수 있는 경우 HSM을 사용 하 여 이러한 인증서를 생성 하는 것이 좋습니다.
더 강력한 보안을 위해 4096 비트 보다 크거나 같은 키 크기를 선택 합니다.

3. 마지막으로 **TPM 모드**에서 [HGS 서버를 초기화](../../security/guarded-fabric-shielded-vm/guarded-fabric-initialize-hgs-tpm-mode-bastion.md) 하는 단계를 수행 합니다.
초기화는 Paw에서 사용 하는 증명 및 키 보호 웹 서비스를 설정 합니다.
이러한 통신을 보호 하려면 [eap-tls 인증서를 사용 하 여](../../security/guarded-fabric-shielded-vm/guarded-fabric-configure-hgs-https.md) hgs를 구성 해야 하며, 신뢰할 수 없는 네트워크에서 hgs로 포트 443만 열어야 합니다.

4. 단계에 따라 두 번째, 세 번째 및 추가 HGS 노드에 대 한 [노드를 더 추가](../../security/guarded-fabric-shielded-vm/guarded-fabric-configure-additional-hgs-nodes.md) 합니다.

5. HGS 서버에서 Windows Server 2019 이상을 실행 하는 경우 선택적 기능을 사용 하도록 설정 하 여 Paw의 보호 된 Vm에 대 한 키를 캐시 하 여 오프 라인에서 사용할 수 있습니다. 키는 다른 컴퓨터에서 캐시 된 키를 사용 하거나 보안 되지 않은 상태의 동일한 컴퓨터를 사용 하지 못하도록 시스템의 현재 보안 구성에 봉인 됩니다. PAW 사용자가 인터넷에 액세스 하지 않고 이동 했지만 여전히 PAW Vm에 로그인 해야 하는 경우에는이 방법이 유용할 수 있습니다. 이 기능을 사용 하려면 모든 HGS 서버에서 다음 명령을 실행 합니다.

      ```powershell
      Set-HgsKeyProtectionConfiguration -AllowKeyMaterialCaching:$true
      ```

### <a name="set-up-the-physical-paw-device"></a>물리적 PAW 장치 설정

물리적 PAW 장치는 보호 된 패브릭 솔루션에서 기본적으로 신뢰할 수 없는 것으로 간주 됩니다.
증명 프로세스 중에 신뢰할 수 있음을 증명할 수 있으며, 그 후에는 보호 된 관리 VM을 시작 하는 데 필요한 키를 가져올 수 있습니다.
장치에서 Hyper-v를 실행할 수 있어야 하며, 보호 된 [호스트 필수 조건을](../../security/guarded-fabric-shielded-vm/guarded-fabric-guarded-host-prerequisites.md)충족 하기 위해 보안 부팅 및 TPM 2.0을 사용할 수 있어야 합니다.
모든 PAW 기능을 지 원하는 최소 운영 체제 버전은 **Windows 10 버전 1803**입니다.

물리적 PAW는 다른 모든 PAW와 마찬가지로 설정 해야 합니다. 단, 모든 사용자가 Hyper-v 관리자 여야 관리 VM을 켜고 연결할 수 있습니다.
대화방 환경에서는 관리 Vm에 대 한 보호 된 호스트로 배포 하는 각각의 고유한 하드웨어/소프트웨어 조합에 대해 골든 구성을 만들어야 합니다.
각 골든 구성에서 다음 작업을 완료 합니다.

1. 컴퓨터에 Windows, 드라이버 및 펌웨어에 대 한 최신 업데이트와 타사 관리 또는 모니터링 에이전트를 설치 합니다.
2. 고유한 TPM id (인증 키), 부팅 측정 (TCG 로그) 및 컴퓨터에 대 한 코드 무결성 정책을 포함 하 여 [필요한 기준 정보를 캡처합니다](../../security/guarded-fabric-shielded-vm/guarded-fabric-tpm-trusted-attestation-capturing-hardware.md).
3. 이러한 아티팩트를 HGS 서버에 복사 하 고 이전 아티클의 HGS 증명 명령을 실행 하 여 호스트를 등록 합니다. 모든 호스트에서 동일한 코드 무결성 정책을 사용 하거나 동일한 하드웨어 구성을 사용 하는 경우 코드 무결성 정책/a s i 로그를 한 번 등록 해야 합니다.

### <a name="create-the-signed-template-disk"></a>서명 된 템플릿 디스크 만들기

보호 된 Vm은 서명 된 템플릿 디스크를 사용 하 여 생성 됩니다.
이 서명은 배포 시에 관리자 암호와 같은 암호를 VM으로 릴리스하기 전에 디스크 무결성과 신뢰성을 확인 하기 위해 확인 됩니다.

서명 된 템플릿 디스크를 만들려면 일반적인 2 세대 가상 컴퓨터에서 1 단계 배포 단계를 수행 합니다.
이 컴퓨터는 관리 VM의 골든 이미지가 됩니다.
다른 컨텍스트에서 사용할 수 있는 특수 한 도구를 포함 하기 위해 둘 이상의 템플릿 디스크를 만들 수 있습니다.

VM이 원하는 대로 구성 되 면 `C:\Windows\System32\sysprep\sysprep.exe`을 실행 하 고 디스크를 **일반화** 하도록 선택 합니다. 일반화가 완료 되 면 OS를 **종료** 합니다.

마지막으로 VM의 VHDX 파일에서 [템플릿 디스크 마법사](../../security/guarded-fabric-shielded-vm/guarded-fabric-create-a-shielded-vm-template.md) 를 실행 하 여 BitLocker 구성 요소를 설치 하 고 디스크 서명을 생성 합니다.

#### <a name="create-the-shielding-data-file"></a>보호 데이터 파일 만들기

일반화 된 템플릿 디스크는 차폐 VM을 프로 비전 하는 데 필요한 암호를 포함 하는 보호 데이터 파일과 쌍을 이룹니다.
보호 데이터 파일에는 다음이 포함 됩니다.
   - VM을 실행할 수 있는 패브릭을 정의 하는 보호자 목록입니다. 각 HGS 클러스터는 보호 하는 PAW 장치에 대 한 보호자입니다.
   - 배포용으로 신뢰할 수 있는 디스크 서명 목록입니다. 보호 데이터 파일은 권한 있는 원본 미디어를 사용 하 여 만든 Vm에 대해서만 암호를 해제 합니다.
   - 호스트의 VM을 보호 하기 위해 추가 보호를 설정 하 고 VM을 다른 컴퓨터로 이동할 수 있는지 여부를 결정 하는 보안 정책입니다. PAW admin Vm은 항상 완전히 보호 되어야 합니다.
   - Windows에서 자동으로 설치를 완료 하 고 로컬 관리자 암호와 같은 암호를 포함할 수 있도록 하는 unattend.xml 특수화 파일입니다.
   - RDP 또는 VPN 인증서와 같은 추가 파일입니다.

보호 데이터 파일을 만드는 방법에 대 한 단계는 [보호 데이터 파일 문서](../../security/guarded-fabric-shielded-vm/guarded-fabric-tenant-creates-shielding-data.md) 를 참조 하세요.

보호 된 Vm에 대 한 소유자 키는 매우 중요 하므로 HSM에 유지 하거나 안전한 위치에 오프 라인으로 저장 해야 합니다.
이러한 정보는 응급 중단 유리 시나리오에서 사용 하 여 HGS가 없어도 보호 된 VM을 부팅할 수 있습니다.

PAW 관리 Vm에 대 한 보호 데이터에는 VM이 부팅 되는 첫 번째 물리적 호스트에 VM을 잠그기 위한 설정이 포함 되어 있는 것이 좋습니다.
이렇게 하면 다른 사람이 동일한 환경에서 다른 PAW로 관리 VM을 이동할 수 없습니다.
이 기능을 사용 하려면 PowerShell을 사용 하 여 보호 데이터 파일을 만들고 **BindToHostTpm** 매개 변수를 포함 합니다.
...
템플릿 디스크 및 보호 데이터 파일이 준비 되 면 HGS에 등록 된 모든 PAW에 관리 VM을 배포할 수 있습니다.

1. 템플릿 디스크 (.vhdx) 및 보호 데이터 파일 (pdk)을 신뢰할 수 있는 PAW 장치에 복사 합니다.
2. 지침에 따라 [PowerShell을 사용 하 여 새 보호 된 VM을 배포](../../security/guarded-fabric-shielded-vm/guarded-fabric-create-a-shielded-vm-using-powershell.md) 합니다.
3. 배포 프로세스의 1 단계에서 나머지 단계를 완료 하 여 VM 운영 체제를 보호 하 고 원하는 역할에 맞게 구성 합니다.


## <a name="related-topics"></a>관련 항목

[Microsoft 사이버 보안 Services 참여](https://www.microsoft.com/en-us/microsoftservices/campaigns/cybersecurity-protection.aspx)

[ 프리미어의 기능: 해시 전달 및 기타 형태의 자격 증명 도난 완화 방법 @ no__t-0
...
[보안 관리 워크스테이션을 사용 하 여 높은 가치의 자산 보호](https://msdn.microsoft.com/library/mt186538.aspx)

[Windows 10의 격리 된 사용자 모드 (Dave Probert) (Channel 9)](https://channel9.msdn.com/Blogs/Seth-Juarez/Isolated-User-Mode-in-Windows-10-with-Dave-Probert)

[Logan Gabriel를 사용 하 여 Windows 10의 격리 된 사용자 모드 프로세스 및 기능 (Channel 9)](http://channel9.msdn.com/Blogs/Seth-Juarez/Isolated-User-Mode-Processes-and-Features-in-Windows-10-with-Logan-Gabriel)

[Dave Probert를 사용 하는 Windows 10 격리 된 사용자 모드의 프로세스 및 기능에 대 한 추가 정보 (Channel 9)](https://channel9.msdn.com/Blogs/Seth-Juarez/More-on-Processes-and-Features-in-Windows-10-Isolated-User-Mode-with-Dave-Probert)

[Windows 10 격리 된 사용자 모드를 사용 하 여 자격 증명 도난 완화 (Channel 9)](https://channel9.msdn.com/Blogs/Seth-Juarez/Mitigating-Credential-Theft-using-the-Windows-10-Isolated-User-Mode)

[Windows Kerberos에서 엄격한 KDC 유효성 검사 사용](https://www.microsoft.com/en-us/download/details.aspx?id=6382)

[Windows Server 2012에 대 한 Kerberos 인증의 새로운 기능](https://technet.microsoft.com/library/hh831747.aspx)

[Windows Server 2008 R2 단계별 가이드의 AD DS에 대 한 인증 메커니즘 보증](https://technet.microsoft.com/library/dd378897(v=ws.10).aspx)
