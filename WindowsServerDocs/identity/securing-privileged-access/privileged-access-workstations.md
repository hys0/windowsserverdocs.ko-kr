...
PAW 보안 제어는 중대 한 영향 및 손상 가능성이 높음 위험 완화 집중 합니다. 환경 및 시간이 지남에 따라 PAW 제어의 효율성을 줄일 수 있는 위험에 대 한 공격을 완화 하는 것이 다음과 같습니다.
...
이 위협 환경에 관리 계정과 마찬가지로 귀중 한 자산 및 중요 한 비즈니스 자산에 대 한 보호를 디자인할 때 "침입 가정" 보안 태세를 채택 하는 조직에 필요 합니다. 이러한 중요 자산은 직접적인 인터넷 위협뿐만 아니라 환경 내 다른 워크스테이션, 서버 및 장치에서 탑재되는 공격으로부터 모두 보호되어야 합니다.
...
관리 VM 모델의 안전한 변형을 사용 하는 것 [보호 된 가상 머신을](../../security/guarded-fabric-shielded-vm/guarded-fabric-and-shielded-vms.md) 사용자 VM은 함께 하나 이상의 관리 Vm을 호스트 합니다.
...
> 계층 모델에 대 한 자세한 내용은 참조 하세요. [ https://aka.ms/tiermodel ](https://aka.ms/tiermodel) 계층 0 그룹에 대 한 자세한 내용은에서 계층 0 동일성을 참조 하세요. [권한 있는 액세스 보안 참조 자료](../securing-privileged-access/securing-privileged-access-reference-material.md)합니다.

Microsoft Identity Manager (MIM) 권한 있는 액세스 관리 (PAM)를 배포 하는 방법에 대 한 자세한 내용은 참조 하세요. [https://aka.ms/mimpamdeploy](https://aka.ms/mimpamdeploy)
...
> [!NOTE]
> 이 지침은 명시적으로 구별 "공개 인터넷" 인터넷 (예: Azure 및 Office 365 관리 포털)에 특정 서비스에 대 한 액세스가 필요한 모든 호스트 및 서비스의 합니다.

계층 지정에 대한 자세한 내용은 [계층 모델 페이지](https://aka.ms/tiermodel)를 참조하세요.

|**시나리오**|**PAW 사용?**|**범위 및 보안 고려 사항**|
|---------|--------|---------------------|
|Active Directory 관리자 - 계층 0|예|1단계 지침에 따라 구축한 PAW면 이 역할에 충분합니다.<br /><br />-   이 시나리오에 가장 강력한 보호를 제공하기 위해 관리 포리스트를 추가할 수 있습니다. ESAE 관리 포리스트에 대한 자세한 내용은 [ESAE 관리 포리스트 디자인 방법](../securing-privileged-access/securing-privileged-access-reference-material.md#esae-administrative-forest-design-approach)을 참조하세요.<br />-   PAW를 사용하여 관리되는 여러 도메인 또는 여러 포리스트를 관리할 수 있습니다.<br />-도메인 컨트롤러는 서비스 (IaaS) 또는 온-프레미스 가상화 솔루션 인프라에서 호스트 되므로 하는 경우 해당 솔루션의 관리자가 Paw 구현 우선 순위를 매기 있습니다.|
|Azure IaaS 및 PaaS 서비스 관리자 - 계층 0 또는 계층 1(범위 및 디자인 고려 사항 참조)|예|2단계에서 제공된 지침에 따라 구축된 PAW면 이 역할에 충분합니다.<br /><br />-   최소한 전역 관리자 및 구독 비용 청구 관리자에게 PAW를 사용해야 합니다. 중요하거나 민감한 서버의 위임된 관리자에게도 PAW를 사용해야 합니다.<br />-Paw를 사용 하 여 운영 체제를 관리 해야 하 고 디렉터리 동기화 및 Id 페더레이션에 대 한 제공 응용 프로그램을 클라우드 서비스와 같은 [Azure AD Connect](https://azure.microsoft.com/documentation/articles/active-directory-aadconnect/) 및 Active Directory Federation Services (ADFS).<br />-   아웃바운드 네트워크 제한에서 2단계의 지침을 사용하는 권한 있는 클라우드 서비스에 대한 연결만 허용해야 합니다. 공개 인터넷 액세스는 PAW에서 허용되어서는 안 됩니다.<br />워크스테이션에서 Windows Defender Exploit Guard 구성 해야 **참고 합니다.**     구독 계층 0 포리스트 도메인 컨트롤러 또는 다른 계층 0 호스트가 구독에 있는 경우 되도록 간주 됩니다. Azure에서 호스트되는 계층 0 서버가 없는 경우 구독은 계층 1입니다.|
|Office 365 테넌트 관리자 <br />- 계층 1|예|2단계에서 제공된 지침에 따라 구축된 PAW면 이 역할에 충분합니다.<br /><br />-   최소한 구독 비용 청구 관리자, 전역 관리자, Exchange 관리자, SharePoint 관리자 및 사용자 관리 관리자 역할에 PAW를 사용해야 합니다. 또한 매우 중요하거나 민감한 데이터의 위임된 관리자에게도 PAW를 사용하는 것이 좋습니다.<br />워크스테이션에서 Windows Defender Exploit Guard 구성 되어야 합니다.<br />-   아웃바운드 네트워크 제한에서 2단계의 지침을 사용하는 Microsoft 서비스에 대한 연결만 허용해야 합니다. 공개 인터넷 액세스는 PAW에서 허용되어서는 안 됩니다.|
|다른 IaaS 또는 PaaS 클라우드 서비스 관리자<br />- 계층 0 또는 계층 1(범위 및 디자인 고려 사항 참조)|예|2단계에서 제공된 지침에 따라 구축된 PAW면 이 역할에 충분합니다.<br /><br />-   에이전트 설치 권한, 하드 디스크 파일 내보내기 권한, 운영 체제, 민감한 데이터 또는 비즈니스에 중요한 데이터가 저장된 저장소 액세스 권한 등 클라우드 호스트된 VM에 대한 관리 권한을 가진 모든 역할에 PAW를 사용해야 합니다.<br />-   아웃바운드 네트워크 제한에서 2단계의 지침을 사용하는 Microsoft 서비스에 대한 연결만 허용해야 합니다. 공개 인터넷 액세스는 PAW에서 허용되어서는 안 됩니다.<br />워크스테이션에서 Windows Defender Exploit Guard 구성 되어야 합니다. **참고:** 도메인 컨트롤러 또는 다른 계층 0 호스트가 구독에 있는 경우 구독은 계층 0 포리스트를입니다. Azure에서 호스트되는 계층 0 서버가 없는 경우 구독은 계층 1입니다.|
|가상화 관리자<br />- 계층 0 또는 계층 1(범위 및 디자인 고려 사항 참조)|예|2단계에서 제공된 지침에 따라 구축된 PAW면 이 역할에 충분합니다.<br /><br />-   에이전트 설치 권한, 가상 하드 디스크 파일 내보내기 권한, 게스트 운영 체제 정보, 민감한 데이터 또는 비즈니스에 중요한 데이터가 저장된 저장소 액세스 권한 등 VM에 대한 관리 권한을 가진 모든 역할에 PAW를 사용해야 합니다. **참고:** 가상화 시스템 (및 해당 관리자)는 도메인 컨트롤러 또는 다른 계층 0 호스트가 구독에 있으면 계층 0 포리스트 간주 됩니다. 가상화 시스템에서 호스트되는 계층 0 서버가 없는 경우 구독은 계층 1입니다.|
...
|사용자 워크스테이션 관리자 <br />- 계층 2|예|2단계에서 제공된 지침에 따라 구축된 PAW면 최종 사용자 장치(예: 지원 센터 및 데스크 측 지원 역할)에 대한 관리 권한이 있는 역할에 충분합니다.<br /><br />-   티켓 관리 및 기타 지원 기능을 사용하려면 PAW에 추가 응용 프로그램을 설치해야 할 수도 있습니다.<br />워크스테이션에서 Windows Defender Exploit Guard 구성 되어야 합니다.<br />    PAW에서 보다 대규모의 관리자를 처리하려면 전용 관리 도구를 추가해야 할 수도 있습니다.|
...
또한 여러 그룹 정책 개체를 만듭니다. PAW 구성-컴퓨터 PAW 구성-사용자 RestrictedAdmin Required-Computer; PAW Outbound Restrictions; 워크스테이션 로그온 제한 서버 로그온을 제한 합니다.
...
13. (선택 사항) PAW에서 로컬 관리자 계정의 암호를 관리 하기 위해 LAPS를 배포 하기로 선택한 경우 암호를 성공적으로 등록 되어 있는지 확인 합니다.
...
### <a name="phase-2-extend-paw-to-all-administrators"></a>2 단계: 모든 관리자로 PAW 확장

범위: 업무용 응용 프로그램 및 종속성에 대 한 관리자 권한 가진 모든 사용자입니다.  여기에는 최소한 응용 프로그램 서버, 작업 상태 및 보안 모니터링 솔루션, 가상화 솔루션, 저장소 시스템 및 네트워크 장치의 관리자가 포함되어야 합니다.

> [!NOTE]
> 이 단계의 지침에서는 1단계가 완전히 완료된 것으로 가정합니다.  1 단계에서에서 모든 단계를 완료 될 때까지 2 단계를 시작 하지 마십시오.

모든 단계가 완료된 것을 확인했으면 아래 단계를 수행하여 2단계를 완료합니다.

#### <a name="recommended-enable-restrictedadmin-mode"></a>(권장) 사용 하도록 설정 **RestrictedAdmin** 모드

기존 서버 및 워크스테이션에서이 기능을 사용 하도록 설정 다음이 기능의 사용을 적용 합니다. 이 기능은 Windows Server 2008 R2를 실행 하려면 대상 서버 해야 이상 워크스테이션에서 Windows 7 이상을 실행 하 고 있습니다.
...
#### <a name="move-tier-1-objects-to-the-appropriate-ous"></a>적절 한 Ou 계층 1 개체 이동
...
3. 관련 그룹에 적절한 구성원 추가
   * **Tier 1 Admins** - 이 그룹은 계층 2 호스트에 로그온하는 것이 제한된 계층 1 관리자를 포함합니다. 모든 계층 1 관리 그룹 서버 또는 인터넷 서비스를 통해 관리 권한을 보유 하는 추가 합니다.
...
5. (선택 사항) 클라우드 서비스에 대 한 연결을 사용 하도록 설정 합니다. 이 단계에서는 적절한 보안 보증을 지원하는 Azure 및 Office 365와 같은 클라우드 서비스를 관리할 수 있도록 합니다. 이 단계는 Microsoft Intune에서 PAW를 관리하는 데에도 필요합니다.
   ...
   * PAW 서비스에 대한 연결을 설정하려면 아래 단계를 따릅니다.
   ...
   1. 권한 있는 인터넷 대상만 허용하도록 PAW를 구성합니다.  클라우드 관리를 사용하도록 PAW 배포를 확장하는 경우 공격자가 관리자를 보다 쉽게 도용할 수 있는 공개 인터넷에서의 액세스를 필터링하면서 권한 있는 서비스에 대한 액세스를 허용해야 합니다.
      ...
      1. 만들 **Cloud Services Admins** 그룹화 하 고 인터넷에서 클라우드 서비스에 액세스 해야 하는 모든 계정을 추가 합니다.
...
6. (선택 사항) Tier 1 Admins에 필요한 추가 도구를 설치 합니다. 작업을 수행하는 데 필요한 다른 도구 또는 스크립트를 설치합니다. PAW에 추가하기 전에 모두 도구에 대해 대상 컴퓨터에서 자격 증명 노출 위험을 평가해야 합니다. 관리 도구 및 연결 방법에 대해 자격 증명 노출 위험을 평가하는 방법에 대한 자세한 내용은 [이 페이지](https://aka.ms/logontypes)를 참조하세요. 설치 미디어에 대한 클린 소스의 지침에 따라 모든 설치 미디어를 가져와야 합니다.
7. 관리에 필요한 소프트웨어 및 응용 프로그램을 식별하고 안전하게 가져오기.  이는 1단계에서 수행한 작업과 유사하지만 보안을 유지해야 하는 응용 프로그램, 서비스 및 시스템 수가 증가했기 때문에 범위가 더 넓습니다.

   > [!NOTE]
   > 이러한 새 응용 프로그램 (웹 브라우저 포함)를 보호 하는 선택 하 여 확인 하 여 Windows Defender Exploit Guard 제공 하는 보호 합니다.
...
8. (선택 사항) 다운로드 하 고 필요한 관리 에이전트를 설치 합니다.
...
12. 확장된 PAW 배포를 수반하는 문화권, 운영, 통신 및 교육 요구 사항 고려.   이러한 관리 모델의 상당한 변화는 자연스럽게 어느 정도의 변경 관리를 필요로 하며, 배포 프로젝트 자체에 기본 제공되어야 합니다.  적어도 다음을 고려해야 합니다.
    ...
    * 새 프로세스의 준수를 보장할 방법  PAW 모델 권한 있는 자격 증명의 노출을 방지 하기 위해 여러 기술적 컨트롤을 포함 하는 동안 순수 하 게 제어 기술 만으로 가능한 모든 노출을 완벽히 방지 하는 것이 불가능 합니다.  예를 들어 관리자가 권한 있는 자격 증명으로 사용자 데스크톱에 로그온하지 못하도록 할 수 있지만 로그온을 시도하는 것만으로도 해당 사용자 데스크톱에 설치된 맬웨어에 자격 증명이 노출될 수 있습니다.  따라서 PAW 모델의 이점뿐 아니라 준수하지 않을 경우의 위험도 명시해야 합니다.  이를 감사 및 경고로 보완하여 자격 증명 노출을 신속하게 감지하고 해결할 수 있도록 해야 합니다.

### <a name="phase-3-extend-and-enhance-protection"></a>3 단계: 확장 및 향상 된 보호

범위: 이러한 보호 기능은 다단계 인증과 네트워크 액세스 규칙을 비롯 한 고급 기능을 사용 하 여 기본 보호를 거둔 단계 1에서 구축한 시스템을 강화 합니다.
...
1. **권한 있는 계정에 대 한 multi-factor authentication 사용**합니다.  다단계 인증은 사용자에게 자격 증명 외에 물리적 토큰을 제공하도록 하여 계정 보안을 강화합니다.  다단계 인증은 인증 정책을 보완할 뿐만 아니라 배포용 인증 정책에 종속되지도 않습니다(마찬가지로 인증 정책에서는 다단계 인증을 요구하지 않음).  다음 형태 중 하나의 다단계 인증을 사용하는 것이 좋습니다.

   * **스마트 카드**: 스마트 카드는 Windows 로그온 프로세스 중 두 번째 인증을 제공 하는 변조 방지 및 휴대용 물리적 장치입니다.  로그온하려면 개인이 카드를 소지해야 하므로 도난된 자격 증명이 원격으로 재사용되는 위험을 줄일 수 있습니다.  Windows에서의 스마트 카드 로그온에 대한 자세한 내용은 [스마트 카드 개요](https://technet.microsoft.com/library/hh831433.aspx) 문서를 참조하세요.
   * **가상 스마트 카드**:  가상 스마트 카드에는 특정 하드웨어에 연결 된다는 추가 혜택을 사용 하 여 실제 스마트 카드와 동일한 보안 이점을 제공 합니다.  배포 및 하드웨어 요구 사항에 대 한 내용은 문서를 참조 하십시오 [가상 스마트 카드 개요](https://technet.microsoft.com/library/dn593708.aspx) 고 [가상 스마트 카드를 사용 하 여 시작 합니다. 연습 가이드](https://technet.microsoft.com/library/dn579260.aspx)합니다.
   * **비즈니스용 Windows Hello**: Windows Hello for Business 수 있도록 Microsoft 계정, Active Directory 계정, Microsoft Azure Active Directory (Azure AD) 계정 또는 ID Online FIDO (Fast) 인증을 지 원하는 타사 서비스에 인증 합니다. 초기 2 단계 확인 하는 동안 Windows Hello 등록 후는 Windows Hello for Business에 설정 된 사용자의 장치 및 사용자 Windows Hello 또는 PIN 일 수 있는 제스처를 설정 합니다. Windows Hello 비즈니스 자격 증명은 신뢰할 수 있는 플랫폼 모듈 (Tpm)의 격리 된 환경 내에서 생성 될 수 있는 비대칭 키 쌍을 합니다.
      대 한 자세한 내용은 Windows Hello 비즈니스 읽기 [Windows Hello for Business](https://docs.microsoft.com/windows/security/identity-protection/hello-for-business/hello-identity-verification) 문서.
   * **Azure multi-factor authentication**:  Azure multi-factor authentication (MFA) 모니터링 및 기계 학습 기반 분석을 통해 향상 된 보호 뿐만 아니라는 두 번째 확인 요소의 보안을 제공합니다.  Azure MFA는 Azure 관리자는 물론, 웹 응용 프로그램, Azure Active Directory, 온-프레미스 솔루션(예: 원격 액세스 및 원격 데스크톱) 등 다른 많은 솔루션의 보안도 유지할 수 있습니다.  Azure 다단계 인증에 대한 자세한 내용은 [다단계 인증](https://azure.microsoft.com/services/multi-factor-authentication)을 참조하세요.
...
PAW에서 추가적인 구성 관리, 운영 모니터링 및 보안 관리를 사용할 수도 있지만 각 관리 기능에는 해당 도구를 통한 PAW 손상 위험도 따르므로 이러한 기능을 통합할 때는 신중히 고려해야 합니다. 고급 관리 기능을 소개 하는 데 적합 여부를 비롯 한 여러 요인에 따라 달라 집니다.
...
|기존 관리 도구로 PAW 관리|-기존 관리 인프라가 Paw의 보안 수준으로 가동 하지 않는 한 paw 손상 시킬 위험이 만듭니다 **참고 합니다.**     일반적으로 Microsoft는 조직에 사용 하 여 특별 한 이유가 없는 한이 접근 방식을 억제 해야 합니다. 경험에 따르면 한지 일반적으로 이러한 모든 도구 (및 해당 보안 종속성)를 전환 하는 매우 많은 비용이 Paw의 보안 수준입니다.<br />-   이러한 도구는 대부분 구성, 소프트웨어 배포 및 보안 업데이트에 대한 가시성 및 제어를 제공함|
...
A [패브릭에 보호 된](https://aka.ms/shieldedvms) 실드 된 가상 컴퓨터에서 랩톱 PAW 워크 로드를 실행 하 여 점프 서버를 사용할 수 있습니다.
추가 인프라 및 운영 단계 필요 하지만 정기적으로 PAW 이미지를 다시 배포 하려면 쉽게 수 있으며 실행 중인 가상 컴퓨터에 여러 다른 계층 구성 (또는 분류) Paw를 통합할 수 있습니다이 접근 방식을 채택 -side-by-side 단일 장치에서.
보호 된 패브릭 토폴로지 및 보안 약속의 자세한 내용은 참조는 [보호 된 패브릭 설명서](https://aka.ms/shieldedvms)합니다.

### <a name="changes-to-the-paw-gpos"></a>PAW Gpo 변경

보호 된 VM 기반 Paw를 사용 하 여 때 합니다 [권장 GPO 설정을](#create-paw-configuration---computer-group-policy-object-gpo) 정의 위에 virtual machines의 사용을 지원 하도록 수정 해야 합니다.

1. 실제 PAW 호스트에 대 한 새 OU를 만듭니다. 실제 및 가상 Paw 보안 요구 사항이 다릅니다 및 그에 따라 Active Directory에서 분리 해야 합니다.
2. PAW 컴퓨터가 GPO 모두 실제 및 가상 PAW Ou에 연결 되어야 합니다.
3. Hyper-v 관리자 그룹에 PAW 사용자를 추가 하려면 물리적 PAW에 대 한 새 GPO를 만듭니다. 이 관리자 관리자 Vm에 연결 하 고 필요에 따라 설정/해제 하 여 설정할 수 있도록 해야 합니다. 실제 PAW 사용자 로그인 인터넷 또는 네트워크 공유 또는 물리적 PAW에 외부 저장 장치에서 악성 가상 컴퓨터 데이터를 복사 하는 기능에 대 한 액세스 관리 권한이 없는 것입니다.
4. Remote Desktop Users 그룹에 PAW 사용자를 추가 하려면 Vm 관리에 대 한 새 GPO를 만듭니다. 이 세션을 사용 하도록 Hyper-v가 향상 된 콘솔, 더 나은 사용자 환경을 제공 하는 사용자 수 및 VM에 스마트 카드 통과 사용 하도록 설정 합니다.
...
다른 계층 0 리소스와 함께 배포 되 고 실제 및 논리에 대 한 무단된 액세스 로부터 보호 될 해야 합니다.
HGS는 스케일 아웃 배포의 크기를 쉽게 클러스터 된 역할을 합니다.
일반 규칙 1 HGS 서버에 있는 노드 3 개를 사용 하 여 모든 1,000 개의 장치에 대 한 계획 하는 것입니다.

1. 첫 번째 HGS 서버를 설치 하려면 사용을 시작 합니다 [HGS 설치-배스 천 포리스트](../../security/guarded-fabric-shielded-vm/guarded-fabric-install-hgs-in-a-bastion-forest.md) 문서를 HGS에 계층 0 도메인에 가입 합니다.

2. 그런 다음 [HGS에 대 한 인증서를 만드는](../../security/guarded-fabric-shielded-vm/guarded-fabric-obtain-certs.md) 엔터프라이즈 인증 기관 사용 합니다.
보호 된 VM의 암호를 해독할 수 HGS 암호화 및 서명 인증서를 소유한 모든 사용자가 않도록 HSM을 사용 하 여 이러한 인증서를 생성 하는 권장 개인 키를 보호 하는 하드웨어 보안 모듈에 대 한 액세스를 사용 하는 경우.
더 강력한 보안을 위해 키 크기가 4096 비트 보다 크거나 같은 경우를 선택 합니다.

3. 마지막으로, 하는 단계를 따릅니다 [HGS 서버를 초기화할](../../security/guarded-fabric-shielded-vm/guarded-fabric-initialize-hgs-tpm-mode-bastion.md) 에 **TPM 모드**합니다.
초기화 증명 및 Paw에서 사용 하는 키 보호 웹 서비스를 설정 합니다.
HGS 있어야 [TLS 인증서를 사용 하 여 구성](../../security/guarded-fabric-shielded-vm/guarded-fabric-configure-hgs-https.md) 이러한 통신을 보호만 포트 443을 열어야 신뢰할 수 없는 네트워크에서 HGS를 합니다.

4. 다음 단계에 따라 [노드 추가](../../security/guarded-fabric-shielded-vm/guarded-fabric-configure-additional-hgs-nodes.md) 에 두 번째에 대 한 세 번째 및 추가 HGS 노드.

5. HGS 서버에서 Windows Server 2019를 실행 하는 경우 나중에 오프 라인으로 사용할 수 있도록 Paw에 보호 된 Vm에 대 한 키를 캐시 하는 선택적 기능을 사용할 수 있습니다. 키 캐시 된 키를 사용 하 여 다른 컴퓨터에는 안전 하지 않은 상태에서 동일한 컴퓨터에서 사용자를 방지 하기 위해 시스템의 현재 보안 구성과 봉인 됩니다. 이 PAW 사용자가 인터넷에 액세스 하지 않고 이동 하지만 PAW Vm에 로그인 할 수 있어야 하는 경우 유용한 솔루션을 수 있습니다. 이 기능을 사용 하려면 모든 HGS 서버에서 다음 명령을 실행 합니다.
...
물리적 PAW 장치 보호 된 패브릭 솔루션에는 기본적으로 신뢰할 수 없는 것으로 간주 됩니다.
관리자가 보호 된 VM을 시작 하는 데 필요한 키를 얻을 수 있는 증명 프로세스 중 신뢰할 수 있는를 증명할 수 것입니다.
장치 보안 부팅을 있고 Hyper-v를 실행 하는 일을 할 수 있어야 하 고 TPM 2.0에 맞게 사용 하도록 설정 합니다 [필수 구성 요소 호스트를 보호](../../security/guarded-fabric-shielded-vm/guarded-fabric-guarded-host-prerequisites.md)합니다.
모든 PAW 기능을 지 원하는 최소 운영 체제 버전이 **Windows 10 버전 1803**합니다.

VM 관리자를 설정 하 고 연결 하려면 Hyper-v 관리자가 모든 PAW 사용자가 필요로 하는 예외를 사용 하 여 다른 같은 물리적 PAW 설정 해야 합니다.
정리 방 환경의 관리자 Vm에 대 한 보호 된 호스트로 배포 하는 각 고유한 하드웨어/소프트웨어 조합에 대해 골든 구성을 만들려면 해야 합니다.
각 골든 구성에서 다음 작업을 완료 합니다.

1. Windows, 드라이버 및 펌웨어 컴퓨터 뿐만 아니라 타사 관리 또는 모니터링 에이전트에 대 한 최신 업데이트를 설치 합니다.
2. [필수 기본 정보를 캡처할](../../security/guarded-fabric-shielded-vm/guarded-fabric-tpm-trusted-attestation-capturing-hardware.md), 고유 TPM 식별자 (인증 키)를 포함 하 여 부팅 측정값 (TCG 로그) 및 코드 무결성 정책 컴퓨터에 대 한 합니다.
3. HGS 서버에 이러한 아티팩트를 복사 하 고 호스트를 등록 하려면 이전 문서에서 HGS 증명 명령을 실행 합니다. 모든 호스트가 동일한 코드 무결성 정책을 사용 하 여 동일한 하드웨어 구성을 사용 하 여 경우, 코드 무결성 정책/TCG 로그를 한 번 등록 해야 합니다.

### <a name="create-the-signed-template-disk"></a>서명 된 템플릿 디스크 만들기

보호 된 Vm은 서명 된 템플릿 디스크를 사용 하 여 생성 됩니다.
VM에 관리자 암호와 같은 비밀을 해제 하기 전에 디스크 무결성 및 신뢰성을 확인 하려면 배포 시에 서명을 확인 합니다.

서명된 된 템플릿 디스크를 만들려면 보통, 2 세대 가상 컴퓨터에서 1 단계 배포 단계를 수행 합니다.
이 컴퓨터에 관리 VM 골든 이미지 됩니다.
다른 컨텍스트에서 사용할 수 있는 도구 특수에 둘 이상의 템플릿 디스크를 만들 수 있습니다.

VM으로 구성 된 경우 실행 원하는 `C:\Windows\System32\sysprep\sysprep.exe` 를 선택 하 고 **일반화** 디스크. **종료** 일반화 완료 되 면 OS입니다.

마지막으로 실행 합니다 [템플릿 디스크 마법사](../../security/guarded-fabric-shielded-vm/guarded-fabric-create-a-shielded-vm-template.md) BitLocker 구성 요소를 설치 하 고 디스크 서명이 생성 된 VM에서 VHDX 파일에 있습니다.

#### <a name="create-the-shielding-data-file"></a>실딩 데이터 파일 만들기

일반화 된 템플릿 디스크를 보호 된 VM을 프로 비전 하는 데 필요한 암호를 포함 하는 보호 데이터 파일을 이룹니다.
실딩 데이터 파일에는 다음이 포함 됩니다.
   - VM 실행에 허용 되는 패브릭을 정의 하는 보호자의 목록입니다. 각 HGS 클러스터가 보호 PAW 장치에 대 한 보호 합니다.
   - 목록 배포에 대 한 신뢰할 수 있는 디스크 서명입니다. 실딩 데이터 파일은 권한이 있는 원본 미디어를 사용 하 여 만든 Vm에 해당 암호 해제만 합니다.
   - 호스트에서 VM을 보호 하기 위해에서 추가 보호를 지정할 것인지 여부 및 VM을 다른 컴퓨터로 이동할 수 허용 되는지 여부를 결정 하는 보안 정책입니다. PAW 관리 Vm를 완벽 하 게 보호 항상 해야 합니다.
   - 특수화에서 unattend.xml 파일 자동으로 설치를 완료 하려면 Windows를 허용 하 고 로컬 관리자 암호와 같은 비밀을 포함 합니다.
   - RDP 또는 VPN 인증서와 같은 추가 파일입니다.

참조 된 [실딩 데이터 파일 문서](../../security/guarded-fabric-shielded-vm/guarded-fabric-tenant-creates-shielding-data.md) 실딩 데이터 파일을 만드는 방법에 대 한 단계입니다.

보호 된 Vm에 대 한 소유자 키 매우 민감하며 HSM에 보관 하거나 안전한 위치에 오프 라인으로 저장 될 해야 합니다.
이러한 할 수 응급 장애 투명 시나리오에서 HGS 없이 보호 된 VM을 부팅 합니다.

PAW 관리에 대 한 데이터 보호 Vm 포함 부팅 된 첫 번째 물리적 호스트에 VM을 잠글 수 설정을 것이 좋습니다.
이렇게 하면 누군가가 하나 PAW에서 관리 VM을 동일한 환경에서 다른 PAW 이동.
이 기능을 사용 하려면 PowerShell을 사용 하 여 보호 데이터 파일을 만들고 포함 된 **BindToHostTpm** 매개 변수:

```powershell
New-ShieldingDataFile -Policy Shielded -BindToHostTpm [...]
```

#### <a name="deploy-an-admin-vm"></a>관리 VM을 배포 합니다.

템플릿 디스크 및 보호 데이터 파일 준비를 HGS에 등록 되었던 모든 PAW의 관리 VM을 배포할 수 있습니다.

1. 신뢰할 수 있는 PAW 장치에 템플릿 디스크 (.vhdx) 및 보호 데이터 파일 (.pdk)를 복사 합니다.
2. 지침에 따라 [PowerShell을 사용 하 여 새 보호 된 VM 배포](../../security/guarded-fabric-shielded-vm/guarded-fabric-create-a-shielded-vm-using-powershell.md)
3. VM 운영 체제를 보호 하 고 해당 의도 한 역할의 경우 적절 하 게 구성 하는 배포 프로세스의 1 단계에서 나머지 단계를 완료 합니다.


## <a name="related-topics"></a>관련 항목

[Microsoft 사이버 안보 서비스 참여](https://www.microsoft.com/en-us/microsoftservices/campaigns/cybersecurity-protection.aspx)

[Taste of Premier: Pass-the-hash 및 기타 형태의 자격 증명 도난 완화 하는 방법](https://channel9.msdn.com/Blogs/Taste-of-Premier/Taste-of-Premier-How-to-Mitigate-Pass-the-Hash-and-Other-Forms-of-Credential-Theft)
...
[격리 된 사용자 모드 프로세스 및 기능-logan Windows 10에서에서 Gabriel (Channel 9)](http://channel9.msdn.com/Blogs/Seth-Juarez/Isolated-User-Mode-Processes-and-Features-in-Windows-10-with-Logan-Gabriel)

[프로세스 및 Dave probert (Channel 9) Windows 10 격리 된 사용자 모드의 기능에 대 한 자세한 on](https://channel9.msdn.com/Blogs/Seth-Juarez/More-on-Processes-and-Features-in-Windows-10-Isolated-User-Mode-with-Dave-Probert)

[Windows 10 격리 된 사용자 모드 (채널 9)를 사용 하 여 자격 증명 도난 완화](https://channel9.msdn.com/Blogs/Seth-Juarez/Mitigating-Credential-Theft-using-the-Windows-10-Isolated-User-Mode)

[Windows Kerberos에서 엄격한 KDC 유효성 검사를 사용 하도록 설정](https://www.microsoft.com/en-us/download/details.aspx?id=6382)

[Windows Server 2012 용 Kerberos 인증의 새로운 기능](https://technet.microsoft.com/library/hh831747.aspx)

[Windows Server 2008 R2 단계별 가이드에서 AD DS에 대 한 인증 메커니즘 보증](https://technet.microsoft.com/library/dd378897(v=ws.10).aspx)
