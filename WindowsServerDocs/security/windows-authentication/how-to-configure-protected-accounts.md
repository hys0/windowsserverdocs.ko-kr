...
![그룹 정책 편집기 관리 창 도메인 컨트롤러 TGT 수명 및 도메인 정책에 따라 갱신을 설정 하는 방법을 보여 주는 스크린샷](../media/how-to-configure-protected-accounts/ADDS_ProtectAcct_TGTExpiration.png)  
...
![확인 하는 위치를 보여 주는 스크린샷 * * 계정을 중요 한 위임 수 없는 * * UI 요소](../media/how-to-configure-protected-accounts/ADDS_ProtectAcct_TshootDelegation.gif)  
...
-   거부 위임 위임 무제한 또는 제한 된: 계정을 제한 하려면를 열고 Active Directory 관리 센터 (ADAC)는 **계정을 중요 한 위임 수 없는** 확인란을 선택 합니다.  
    ...
    ![제한 된 계정을 위치를 보여 주는 스크린샷](../media/how-to-configure-protected-accounts/ADDS_ProtectAcct_TshootDelegation.gif)  
...
![세 가지 유형의 Kerberos 인증 subprotocols 라고도 프로토콜 거래소를 보여 주는 스크린샷](../media/how-to-configure-protected-accounts/ADDS_ProtectAcct_KerbRefresher.gif)  
...
![사용자 기호를 제한 하는 TGT 수명 및 액세스 제어 조건 구성 초기 인증을 제한 하는 방법을 보여 주는 스크린샷](../media/how-to-configure-protected-accounts/ADDS_ProtectAcct_RestrictAS.gif)  
...
1.  기본 도메인 컨트롤러 정책 클릭 **사용** 수 있도록 **키 메일 센터 (KDC) 클라이언트 클레임, 지원 복합 인증 Kerberos armoring** 컴퓨터 구성에서 | 관리 템플릿 | 시스템 | KDC 합니다.  
    ...
    ![기본 도메인 컨트롤러 정책 클릭 * * 사용 * * 수 있도록 * * 키 메일 센터 (KDC) 클라이언트 클레임, 지원 복합 인증 Kerberos armoring * * 컴퓨터 구성에서 | 관리 템플릿 | 시스템 | KDC](../media/how-to-configure-protected-accounts/ADDS_ProtectAcct_EnableKDCClaims.gif)  
...
2.  아래에서 **옵션**, 드롭다운 목록에서 상자에서 선택 **클레임을 항상 제공**합니다.  
    ...
    ![아래에서 * * 옵션 * *, 드롭다운 목록에서 상자에서 선택 * * 클레임을 항상 제공](../media/how-to-configure-protected-accounts/ADDS_ProtectAcct_AlwaysProvideClaims.png)  
...
2.  클릭 **인증 정책**을 차례로 클릭 하 고 **새로** 정책을 새로 만들 수 있습니다.  
    ...
    ![인증 정책](../media/how-to-configure-protected-accounts/ADDS_ProtectAcct_NewAuthNPolicy.gif)  
  
    인증 정책을 표시 이름이 있어야 하며 기본적으로 적용 됩니다.  
  
3.  감사 전용 정책 만들려면 클릭 **정책 제한 감사**합니다.  
  
    ![정책 제한 감사](../media/how-to-configure-protected-accounts/ADDS_ProtectAcct_NewAuthNPolicyAuditOnly.gif)  
...
4.  구성 하려면 사용자 계정에 대해 TGT 수명 선택 하십시오는 **사용자 계정에 대 한 허용 티켓 수명 지정** 확인란을 선택 하 고 시간 분 내에 입력 합니다.  
    ...
    ![사용자 계정에 대 한 허용 티켓 수명 지정](../media/how-to-configure-protected-accounts/ADDS_ProtectAcct_TGTLifetime.gif)  
    ...
    ![기본 설정으로 도메인에 대 한 그룹 정책 편집기 관리 창](../media/how-to-configure-protected-accounts/ADDS_ProtectAcct_TGTExpiration.png)  
  
5.  장치를 선택 하 여 사용자 계정을 제한 하려면 클릭 **편집** 를 정의할 장치에 필요한 상황입니다.  
  
    ![장치를 선택 하 여 사용자 계정을 제한 하려면 클릭 * * 편집 * *](../media/how-to-configure-protected-accounts/ADDS_ProtectAcct_EditAuthNPolicy.gif)  
  
6.  에 **액세스 제어 조건 편집** 창을 클릭 **조건을 추가**합니다.  
  
    ![액세스를 제어 조건 편집](../media/how-to-configure-protected-accounts/ADDS_ProtectAcct_AddCondition.png)  
...
5.  사용자를 제한 하 고 다음을 클릭 하려면 컴퓨터의 이름을 입력 **이름 확인**합니다.  
    ...
    ![이름을 확인을 클릭 합니다.](../media/how-to-configure-protected-accounts/ADDS_ProtectAcct_ChangeObjectsCompName.gif)  
  
6.  확인을 클릭 하 고 컴퓨터 계정에 대 한 기타 조건 만듭니다.  
  
    ![확인을 클릭 하 고 컴퓨터 계정에 대 한 기타 조건 만들기](../media/how-to-configure-protected-accounts/ADDS_ProtectAcct_AddCompAddConditions.png)  
  
7.  완료 되 면 클릭 한 다음 **확인** 컴퓨터 계정에 대 한 조건에 정의 된 해당 표시 됩니다.  
  
    ![완료 되 면 클릭 * * 확인 * *](../media/how-to-configure-protected-accounts/ADDS_ProtectAcct_AddCompDone.png)  
  
##### <a name="add-computer-claim-conditions"></a>컴퓨터 클레임 조건을 추가합니다  
  
1.  구성 하려면 컴퓨터 클레임, 드롭다운 청구 선택 합니다.  
  
    ![구성 하려면 컴퓨터 클레임, 드롭다운 청구 선택](../media/how-to-configure-protected-accounts/ADDS_ProtectAcct_CompClaim.gif)  
  
    클레임은 숲에서 이미 제공 되는 경우 사용할 수만 있습니다.  
  
2.  사용자 계정에 로그인을 하면로 제한 해야, OU의 이름을 입력 합니다.  
  
    ![사용자 계정에 로그인을 하면로 제한 해야, OU의 이름을 입력 합니다.](../media/how-to-configure-protected-accounts/ADDS_ProtectAcct_CompClaimOUName.gif)  
  
3.  완료 되 면 다음 확인을 클릭 하 고 상자에 정의 된 조건과 표시 됩니다.  
  
    ![확인 클릭 끝나면 인가요](../media/how-to-configure-protected-accounts/ADDS_ProtectAcct_CompClaimComplete.gif)  
...
![컴퓨터의는 조직 단위 기반 인증을 제한 하 하는 방법을 보여 주는 스크린샷](../media/how-to-configure-protected-accounts/ADDS_ProtectAcct_RestrictComputers.gif)  
  
디바이스에 제한 사용자 로그인에 사용할 수는 클레임 선택는 **사용자** 확인란을 선택 합니다.  
  
![다음을 확인 하 여 디바이스에 로그인 사용자를 제한 하는 방법을 보여 주는 스크린샷 * * 사용자 * * 확인란을 선택 합니다.  ](../media/how-to-configure-protected-accounts/ADDS_ProtectAcct_RestrictUsersComputers.gif)  
  
#### <a name="provision-a-user-account-with-an-authentication-policy-with-adac"></a>ADAC로 인증 정책 사용 하 여 사용자 계정을 제공합니다  
  
1.  **사용자** 계정, 클릭 **정책**합니다.  
  
    ![* * 사용자 * * 계정을 클릭 * * 정책 * *](../media/how-to-configure-protected-accounts/ADDS_ProtectAcct_UserPolicy.gif)  
  
2.  선택는 **인증 정책을이 계정에 할당** 확인란을 선택 합니다.  
  
    ![선택는 * *는 인증 정책을이 계정 * * 확인란에 할당](../media/how-to-configure-protected-accounts/ADDS_ProtectAcct_UserPolicyAssign.gif)  
  
3.  사용자에 게 적용할 인증 정책을 선택 합니다.  
  
    ![사용자에 게 적용할 인증 정책을 선택합니다](../media/how-to-configure-protected-accounts/ADDS_ProtectAcct_UserPolicySelect.png)  
...
![사용 하도록 설정 하려면 그룹 정책 또는 로컬 그룹 정책 편집기를 사용 하는 방법을 보여 주는 스크린샷 * * 클레임, 복합 인증 및 Kerberos armoring Kerberos 클라이언트 지원 * *](../media/how-to-configure-protected-accounts/ADDS_ProtectAcct_KerbClientDACSupport.gif)  
...
![직접 정책이 적용 되는 계정을 보여 주는 인증 정책을의 계정 섹션 스크린샷](../media/how-to-configure-protected-accounts/ADDS_ProtectAcct_AccountsAssigned.gif)  
...
1.  열기 **Active Directory 관리 센터**, 클릭 **인증**를 마우스 오른쪽 단추로 클릭 **인증 정책 사일로**, 클릭 **새로**를 차례로 클릭 하 고 **인증 정책 기지**합니다.  
    ...
    ![열기 * * Active Directory 관리 센터 * *, 클릭 * * 인증 * * 마우스 오른쪽 단추로 클릭 * * 인증 정책 사일로 * *, 클릭 * * 다음 클릭 한 새로 * * * * 인증 정책 기지 * *](../media/how-to-configure-protected-accounts/ADDS_ProtectAcct_CreateNewAuthNPolicySilo.gif)  
...
2.  **표시 이름**, 기지 이름을 입력 합니다. **허용 계정**, 클릭 **추가**를 입력 하는 계정 이름을 클릭 한 다음 **확인**합니다. 사용자가, 컴퓨터 또는 서비스 계정 지정할 수 있습니다. 모든 사용자 또는 사용자를 유형과 정책이 나 방침의 이름에 대 한 별도 정책에 대 한 단일 정책을 사용 여부를 지정 합니다.  
    ...
    ![* * 표시 이름 * * 기지 이름을 입력 합니다. * * 허용 계정 * *, 클릭 * * 추가 * *는 계정 이름을 입력 한 다음 * * 확인 * *](../media/how-to-configure-protected-accounts/ADDS_ProtectAcct_NewAuthNPolicySiloDisplayName.gif)  
