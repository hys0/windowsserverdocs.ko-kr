...
| ------------------ | ------ |
| **서비스 이름**   | ScDeviceEnum    |
| **설명**    | 지정한 세션에 액세스할 수 있는 모든 스마트 카드 판독기에 소프트웨어 디바이스 노드를 만듭니다. 이 서비스를 사용할 수 없으면 WinRT API에서 스마트 카드 판독기를 열거할 수 없습니다.   |
| **설치**   | 항상 설치    |
| **시작 유형**   | Manual  |
| **추천 사항** | 사용하지 않도록 설정할 수 있음   |
...
| ------------------ | ------ |
| **서비스 이름**   | SCPolicySvc
| **설명**    | 스마트 카드 제거 시 사용자 데스크톱을 잠그도록 시스템을 구성할 수 있게 합니다.
| **설치**   | 항상 설치
| **시작 유형**   | Manual
| **추천 사항** | 지침 없음
| **설명**       |
...
| ------------------ | ------ |
| **서비스 이름**   | SNMPTRAP
| **설명**    | 로컬 또는 원격 SNMP(Simple Network Management Protocol) 에이전트가 만든 트랩 메시지를 받아서, 이 컴퓨터에서 실행 중인 SNMP 관리 프로그램에게 메시지를 전달합니다. 이 서비스를 중지하면 이 컴퓨터에 있는 SNMP 기반 프로그램이 SNMP 트랩 메시지를 받지 않습니다. 이 서비스를 사용하지 않으면 이 서비스에 명시적으로 종속된 모든 서비스를 시작할 수 없습니다.
| **설치**   | 항상 설치
| **시작 유형**   | Manual
| **추천 사항** | 지침 없음
| **설명**       |
...
| ------------------ | ------ |
| **서비스 이름**   | sppsvc
| **설명**    | Windows 및 Windows 애플리케이션의 디지털 라이선스를 다운로드, 설치 및 적용할 수 있도록 합니다. 이 서비스를 사용하지 않으면 운영 체제 및 정품 애플리케이션이 알림 모드에서 실행될 수도 있습니다. 소프트웨어 보호 서비스를 해제하지 않는 것이 좋습니다.
| **설치**   | 항상 설치
| **시작 유형**   | 자동
| **추천 사항** | 지침 없음
| **설명**       |
...
| ------------------ | ------ |
| **서비스 이름**   | sacsvr
| **설명**    | 관리자가 응급 관리 서비스를 사용하여 명령 프롬프트에 원격으로 액세스합니다.
| **설치**   | 항상 설치
| **시작 유형**   | Manual
| **추천 사항** | 지침 없음
| **설명**       |
...
| ------------------ | ------ |
| **서비스 이름**   | SSDPSRV
| **설명**    | UPnP 디바이스와 같이 SSDP 검색 프로토콜을 사용하는 네트워크 디바이스 및 서비스를 검색하고 로컬 컴퓨터에서 실행하는 SSDP 디바이스 및 서비스를 알립니다. 이 서비스를 중지하면 SSDP 기반 디바이스가 검색되지 않습니다. 이 서비스를 사용하지 않으면 이 서비스에 명시적으로 종속된 모든 서비스를 시작할 수 없습니다.
| **설치**   | 데스크톱 경험이 있는 경우에만
| **시작 유형**   | Manual
| **추천 사항** | 사용하지 않도록 설정할 수 있음
| **설명**       |
...
| **설명**    | 애플리케이션 모델에 대한 필수 인프라 지원을 제공합니다.
| **설치**   | 항상 설치
| **시작 유형**   | Manual
| **추천 사항** | 지침 없음
| **설명**       |
...
| ------------------ | ------ |
| **서비스 이름**   | WiaRpc
| **설명**    | 정지 이미지 인식 이벤트와 연결된 애플리케이션을 시작합니다.
| **설치**   | 데스크톱 경험이 있는 경우에만
| **시작 유형**   | Manual
| **추천 사항** | 사용하지 않도록 설정할 수 있음
| **설명**       |
...
| ------------------ | ------ |
| **서비스 이름**   | StorSvc
| **설명**    | 스토리지 공간 설정 및 외부 스토리지 공간 확장에 대한 서비스 활성화를 제공합니다.
| **설치**   | 데스크톱 경험이 있는 경우에만
| **시작 유형**   | Manual
| **추천 사항** | 지침 없음
| **설명**       |
...
| **설명**    | 시스템에 있는 모든 계층화된 스토리지 공간의 스토리지 계층에서 데이터 배치를 최적화합니다.
| **설치**   | 항상 설치
| **시작 유형**   | Manual
| **추천 사항** | 지침 없음
| **설명**       |
...
| ------------------ | ------ |
| **서비스 이름**   | OneSyncSvc
| **설명**    | 이 서비스는 메일, 연락처, 일정 및 기타 다양한 사용자 데이터를 동기화합니다. 이 기능에 종속된 메일 및 기타 애플리케이션은 이 서비스가 실행되지 않은 경우 제대로 작동하지 않습니다.
| **설치**   | 데스크톱 경험이 있는 경우에만
| **시작 유형**   | 자동
| **추천 사항** | 사용하지 않도록 설정할 수 있음
| **설명**       | 사용자 서비스 템플릿
...
| ------------------ | ------ |
| **서비스 이름**   | SENS
| **설명**    | 시스템 이벤트를 모니터링하고 구독자를 이벤트의 COM+ 이벤트 시스템에 알립니다.
| **설치**   | 항상 설치
| **시작 유형**   | 자동
| **추천 사항** | 지침 없음
| **설명**       |
...
| **설명**    | WinRT 애플리케이션의 백그라운드 작업 실행을 조정합니다. 이 서비스가 중지되었거나 사용하지 않도록 설정된 경우 백그라운드 작업이 트리거되지 않을 수 있습니다.
| **설치**   | 항상 설치
| **시작 유형**   | 자동
| **추천 사항** | 사용하지 않도록 설정할 수 없음
...
| ------------------ | ------ |
| **서비스 이름**   | 일정
| **설명**    | 사용자가 컴퓨터에서 자동화된 작업을 구성하고 예약할 수 있도록 합니다. 이 서비스는 또한 Windows 시스템에서 중요한 여러 가지 작업을 호스팅합니다. 이 서비스를 중지하거나 사용하지 않도록 설정하면 예정된 시간에 작업이 실행되지 않습니다. 이 서비스를 사용하지 않으면 이 서비스에 명시적으로 종속된 모든 서비스를 시작할 수 없습니다.
| **설치**   | 항상 설치
| **시작 유형**   | 자동
| **추천 사항** | 지침 없음
| **설명**       |
...
| ------------------ | ------ |
| **서비스 이름**   | lmhosts
| **설명**    | NetBIOS over TCP/IP(NetBT) 서비스 및 NetBIOS 이름 확인 지원을 네트워크의 클라이언트에 제공하므로 사용자가 파일을 공유하거나, 인쇄하거나, 네트워크에 로그온할 수 있습니다. 이 서비스가 중지되면 이러한 기능을 사용할 수 없습니다. 이 서비스를 사용하지 않으면 이 서비스에 명시적으로 종속된 모든 서비스를 시작할 수 없습니다.
| **설치**   | 항상 설치
| **시작 유형**   | Manual
| **추천 사항** | 지침 없음
| **설명**       |
...
| ------------------ | ------ |
| **서비스 이름**   | TapiSrv
| **설명**    | 로컬 컴퓨터에서 그리고 LAN을 통해 서비스를 실행 중인 서버에서 전화 통신 디바이스를 제어하는 프로그램에 대해 TAPI(Telephony API)를 지원합니다.
| **설치**   | 데스크톱 경험이 있는 경우에만
| **시작 유형**   | Manual
| **추천 사항** | 사용하지 않도록 설정할 수 없음
| **설명**       | 사용하지 않도록 설정하면 RRAS가 중단됩니다.
...
| **설명**    | 터치 키보드 및 필기 패널 펜 및 잉크 기능 사용
| **설치**   | 데스크톱 경험이 있는 경우에만
| **시작 유형**   | Manual
| **추천 사항** | 사용하지 않도록 설정할 수 있음
| **설명**       |
...
| ------------------ | ------ |
| **서비스 이름**   | UsoSvc
| **설명**    | Windows 업데이트를 관리합니다. 중지하면 디바이스가 최신 업데이트를 다운로드하고 설치할 수 없습니다.
| **설치**   | 항상 설치
| **시작 유형**   | Manual
| **추천 사항** | 사용하지 않도록 설정할 수 없음
...
| ------------------ | ------ |
| **서비스 이름**   | upnphost
| **설명**    | 이 컴퓨터에서 UPnP 장비 호스팅을 허용합니다. 이 서비스를 중지하면 호스팅되는 UPnP 장비의 작동이 멈추고 호스팅되는 장비를 추가할 수 없습니다. 이 서비스를 사용하지 않으면 이 서비스에 명시적으로 종속된 모든 서비스를 시작할 수 없습니다.
| **설치**   | 데스크톱 경험이 있는 경우에만
| **시작 유형**   | Manual
| **추천 사항** | 사용하지 않도록 설정할 수 있음
| **설명**       |
...
| ------------------ | ------ |
| **서비스 이름**   | UALSVC
| **설명**    | 이 서비스는 로컬 서버에 설치된 제품 및 역할의 고유 클라이언트 액세스 요청을 IP 주소 및 사용자 이름 형식으로 기록합니다. 관리자가 오프라인 CAL(클라이언트 액세스 라이선스) 관리용 서버 소프트웨어의 클라이언트 수요를 수치화해야 하는 경우 PowerShell을 통해 이 정보를 쿼리할 수 있습니다. 서비스를 사용하지 않는 경우 클라이언트 요청은 기록되지 않으며 Powershell을 통해 검색되지도 않습니다. 서비스를 중지하더라도 기록 데이터 쿼리에는 아무런 영향이 없습니다(기록 데이터 삭제 단계는 지원 설명서 참조). 로컬 시스템 관리자는 Windows Server 사용 조건을 참조하여 서버 소프트웨어를 적절히 사용하는 데 필요한 CAL 수를 결정해야 합니다. UAL 서비스 및 데이터를 사용하더라도 이 책임이 없어지는 것은 아닙니다.
| **설치**   | 항상 설치
| **시작 유형**   | 자동
| **추천 사항** | 지침 없음
| **설명**       |
...
| ------------------ | ------ |
| **서비스 이름**   | UserDataSvc
| **설명**    | 연락처 정보, 일정, 메시지, 기타 콘텐츠를 비롯하여 구조적 사용자 데이터에 대한 액세스 권한을 앱에 제공합니다. 이 서비스를 중지하거나 사용하지 않도록 설정할 경우 이 데이터를 사용하는 앱이 올바르게 작동하지 않을 수 있습니다.
| **설치**   | 데스크톱 경험이 있는 경우에만
| **시작 유형**   | Manual
| **추천 사항** | 사용하지 않도록 설정할 수 있음
| **설명**       | 사용자 서비스 템플릿
...
| ------------------ | ------ |
| **서비스 이름**   | UnistoreSvc
| **설명**    | 계약 정보, 일정, 메시지, 기타 콘텐츠를 비롯하여 구조적 사용자 데이터 스토리지를 처리합니다. 이 서비스를 중지하거나 사용하지 않도록 설정할 경우 이 데이터를 사용하는 앱이 올바르게 작동하지 않을 수 있습니다.
| **설치**   | 데스크톱 경험이 있는 경우에만
| **시작 유형**   | Manual
| **추천 사항** | 사용하지 않도록 설정할 수 있음
| **설명**       | 사용자 서비스 템플릿
...
| **설명**    | 애플리케이션 및 OS 설정 로밍에 대한 지원을 제공합니다.
| **설치**   | 데스크톱 경험이 있는 경우에만
| **시작 유형**   | 사용 안 함
| **추천 사항** | 이미 사용하지 않도록 설정됨
| **설명**       |
...
| ------------------ | ------ |
| **서비스 이름**   | UserManager
| **설명**    | 사용자 관리자는 다중 사용자 조작에 필요한 런타임 구성 요소를 제공합니다.  이 서비스를 중지하면 일부 애플리케이션이 제대로 작동하지 않을 수 있습니다.
| **설치**   | 항상 설치
| **시작 유형**   | 자동
| **추천 사항** | 지침 없음
| **설명**       |
...
| ------------------ | ------ |
| **서비스 이름**   | ProfSvc
| **설명**    | 이 서비스는 사용자 프로필을 로드하고 언로드합니다. 이 서비스가 중지되거나 사용되지 않으면 사용자가 더 이상 로그인하거나 로그아웃할 수 없고, 앱에서 사용자 데이터를 얻는 데 문제가 발생하며, 프로필 이벤트 알림을 받도록 등록된 구성 요소가 해당 알림을 받지 못합니다.
| **설치**   | 항상 설치
| **시작 유형**   | 자동
| **추천 사항** | 지침 없음
| **설명**       |
...
| ------------------ | ------ |
| **서비스 이름**   | vds
| **설명**    | 디스크, 볼륨, 파일 시스템 및 스토리지 배열에 대한 관리 서비스를 제공합니다.
| **설치**   | 항상 설치
| **시작 유형**   | Manual
| **추천 사항** | 지침 없음
| **설명**       |
...
| ------------------ | ------ |
| **서비스 이름**   | VSS
| **설명**    | 백업 및 기타 목적에 사용되는 볼륨 섀도 복사본을 관리하고 구현합니다. 이 서비스가 중단되면 섀도 복사본을 백업에 사용할 수 없게 되며 백업이 실패할 수 있습니다. 이 서비스를 사용하지 않으면 이 서비스에 명시적으로 종속된 모든 서비스를 시작할 수 없습니다.
| **설치**   | 항상 설치
| **시작 유형**   | Manual
| **추천 사항** | 지침 없음
| **설명**       |
...
| ------------------ | ------ |
| **서비스 이름**   | Audiosrv
| **설명**    | Windows 기반 프로그램의 오디오를 관리합니다.  이 서비스가 중지되면 오디오 디바이스 및 효과가 제대로 작동하지 않습니다.  이 서비스를 사용하지 않으면 이 서비스에 명시적으로 종속된 모든 서비스를 시작할 수 없습니다.
| **설치**   | 데스크톱 경험이 있는 경우에만
| **시작 유형**   | Manual
| **추천 사항** | 사용하지 않도록 설정할 수 있음
| **설명**       |
...
| **설명**    | Windows 오디오 서비스에 대한 오디오 디바이스를 관리합니다.  이 서비스가 중지되면 오디오 디바이스 및 효과가 제대로 작동하지 않습니다.  이 서비스를 사용하지 않으면 이 서비스에 명시적으로 종속된 모든 서비스를 시작할 수 없습니다.
| **설치**   | 데스크톱 경험이 있는 경우에만
| **시작 유형**   | Manual
| **추천 사항** | 사용하지 않도록 설정할 수 있음
| **설명**       |
...
| ------------------ | ------ |
| **서비스 이름**   | WbioSrvc
| **설명**    | Windows 생체 인식 서비스는 클라이언트 애플리케이션에서 생체 인식 하드웨어 또는 샘플에 직접 액세스하지 않고 생체 인식 데이터를 캡처, 비교, 조작 및 저장할 수 있도록 합니다. 이 서비스는 권한이 있는 SVCHOST 프로세스에서 호스트됩니다.
| **설치**   | 데스크톱 경험이 있는 경우에만
| **시작 유형**   | Manual
| **추천 사항** | 지침 없음
| **설명**       |
...
| ------------------ | ------ |
| **서비스 이름**   | FrameServer
| **설명**    | 여러 클라이언트가 카메라 디바이스에서 비디오 프레임에 액세스할 수 있도록 합니다.
| **설치**   | 데스크톱 경험이 있는 경우에만
| **시작 유형**   | Manual
| **추천 사항** | 사용하지 않도록 설정할 수 있음
| **설명**       |
...
| ------------------ | ------ |
| **서비스 이름**   | Wcmsvc
| **설명**    | 현재 PC에서 사용할 수 있는 네트워크 연결 옵션에 따라 자동 연결/연결 끊기를 결정하고 그룹 정책 설정에 따라 네트워크 연결 관리를 사용합니다.
| **설치**   | 데스크톱 경험이 있는 경우에만
| **시작 유형**   | 자동
| **추천 사항** | 지침 없음
| **설명**       |
...
| ------------------ | ------ |
| **서비스 이름**   | WdNisSvc
| **설명**    | 알려져 있거나 새로 발견된 네트워크 프로토콜 취약점을 공격하는 침입 시도로부터 보호합니다.
| **설치**   | 항상 설치
| **시작 유형**   | Manual
| **추천 사항** | 지침 없음
| **설명**       |
...
| ------------------ | ------ |
| **서비스 이름**   | WinDefend
| **설명**    | 맬웨어 및 기타 사용자 동의 없이 설치된 소프트웨어로부터 사용자를 보호합니다.
| **설치**   | 항상 설치
| **시작 유형**   | 자동
| **추천 사항** | 지침 없음
| **설명**       |
...
| ------------------ | ------ |
| **서비스 이름**   | wudfsvc
| **설명**    | 사용자 모드 드라이버 프로세스를 만들고 관리합니다. 이 서비스는 중지할 수 없습니다.
| **설치**   | 항상 설치
| **시작 유형**   | Manual
| **추천 사항** | 지침 없음
| **설명**       |
...
| ------------------ | ------ |
| **서비스 이름**   | WEPHOSTSVC
| **설명**    | Windows 암호화 공급자 호스트 서비스는 타사 암호화 공급자의 암호화 관련 기능을 EAS 정책을 평가하고 적용하는 데 필요한 프로세스에 연결합니다. 이 기능을 중지하면 연결된 메일 계정에서 설정한 규격 검사가 손상됩니다.
| **설치**   | 데스크톱 경험이 있는 경우에만
| **시작 유형**   | Manual
| **추천 사항** | 지침 없음
| **설명**       |
...
| ------------------ | ------ |
| **서비스 이름**   | WerSvc
| **설명**    | 프로그램에서 작업 또는 응답을 중지할 때 오류를 보고할 수 있으며 기존 해결책을 제공할 수 있습니다. 또한 진단 및 복구 서비스용으로 로그를 생성할 수 있습니다. 이 서비스가 중지되면 오류 보고가 올바르게 작동하지 않으며 진단 서비스 및 복구 결과가 표시되지 않을 수도 있습니다.
| **설치**   | 항상 설치
| **시작 유형**   | Manual
| **추천 사항** | 사용하지 않도록 설정할 수 없음
...
| ------------------ | ------ |
| **서비스 이름**   | Wecsvc
| **설명**    | 이 서비스는 WS-Management 프로토콜을 지원하는 원격 원본의 이벤트에 대한 영구 가입을 관리하며, 여기에 Windows Vista 이벤트 로그, 하드웨어 및 IPMI 사용 이벤트 원본이 포함됩니다. 서비스는 전달된 이벤트를 로컬 이벤트 로그에 저장합니다. 이 서비스를 중지하거나 사용하지 않을 경우 이벤트 가입을 만들 수 없고 전달된 이벤트를 수락할 수 없습니다.
| **설치**   | 항상 설치
| **시작 유형**   | Manual
| **추천 사항** | 사용하지 않도록 설정할 수 없음
...
| ------------------ | ------ |
| **서비스 이름**   | EventLog
| **설명**    | 이 서비스는 이벤트 및 이벤트 로그를 관리합니다. 이벤트 로깅, 이벤트 쿼리, 이벤트 가입, 이벤트 로그 보관, 이벤트 메타데이터 관리 등을 지원하며 XML 형식과 일반 텍스트 형식으로 이벤트를 표시할 수 있습니다. 이 서비스를 중지하면 시스템의 보안과 안정성이 손상될 수 있습니다.
| **설치**   | 항상 설치
| **시작 유형**   | 자동
| **추천 사항** | 지침 없음
| **설명**       |
...
| ------------------ | ------ |
| **서비스 이름**   | MpsSvc
| **설명**    | Windows 방화벽은 권한이 없는 사용자가 인터넷 또는 네트워크를 통해 컴퓨터에 액세스하는 것을 방지하여 컴퓨터를 보호해 줍니다.
| **설치**   | 항상 설치
| **시작 유형**   | 자동
| **추천 사항** | 지침 없음
| **설명**       |
...
| ------------------ | ------ |
| **서비스 이름**   | FontCache
| **설명**    | 자주 사용하는 글꼴 데이터를 캐시하여 애플리케이션의 성능을 최적화합니다. 이 서비스가 실행되고 있지 않으면 애플리케이션이 자동으로 서비스를 시작합니다. 이 서비스를 사용하지 않도록 설정할 수 있지만 그러면 애플리케이션 성능이 저하될 수 있습니다.
| **설치**   | 항상 설치
| **시작 유형**   | 자동
| **추천 사항** | 지침 없음
| **설명**       |
...
| ------------------ | ------ |
| **서비스 이름**   | stisvc
| **설명**    | 스캐너 및 카메라에 이미지 인식 서비스를 제공합니다.
| **설치**   | 데스크톱 경험이 있는 경우에만
| **시작 유형**   | Manual
| **추천 사항** | 사용하지 않도록 설정할 수 있음
| **설명**       |
...
| ------------------ | ------ |
| **서비스 이름**   | msiserver
| **설명**    | Windows Installer(*.msi, *.msp) 패키지로 제공되는 애플리케이션을 추가, 수정 및 제거합니다. 이 서비스를 사용하지 않으면 이 서비스에 명시적으로 종속된 모든 서비스를 시작할 수 없습니다.
| **설치**   | 항상 설치
| **시작 유형**   | Manual
| **추천 사항** | 지침 없음
| **설명**       |
...
| **설명**    | Microsoft 스토어에 대한 인프라 지원을 제공합니다.  이 서비스는 요청 시 시작되며, 사용하지 않도록 설정하면 Microsoft 스토어에서 구입한 콘텐츠가 제대로 작동하지 않습니다.
| **설치**   | 데스크톱 경험이 있는 경우에만
| **시작 유형**   | Manual
| **추천 사항** | 지침 없음
| **설명**       |
...
| ------------------ | ------ |
| **서비스 이름**   | Winmgmt
| **설명**    | 운영 체제, 디바이스, 애플리케이션 및 서비스 등의 관리 정보에 액세스하는 공용 인터페이스 및 개체 모델을 제공합니다. 이 서비스가 중지되면 대부분의 Windows 기반 소프트웨어가 올바르게 작동하지 않습니다. 이 서비스를 사용하지 않으면 이 서비스에 명시적으로 종속된 모든 서비스를 시작할 수 없습니다.
| **설치**   | 항상 설치
| **시작 유형**   | 자동
| **추천 사항** | 지침 없음
| **설명**       |
...
| ------------------ | ------ |
| **서비스 이름**   | icssvc
| **설명**    | 다른 디바이스와 셀룰러 데이터 연결을 공유할 수 있는 기능을 제공합니다.
| **설치**   | 데스크톱 경험이 있는 경우에만
| **시작 유형**   | Manual
| **추천 사항** | 사용하지 않도록 설정할 수 있음
| **설명**       |
...
| **설명**    | Windows 업데이트와 선택적 구성 요소의 설치, 수정 및 제거를 사용할 수 있게 합니다. 이 서비스를 사용할 수 없게 하면 이 컴퓨터에 대한 Windows 업데이트의 설치 또는 제거가 실행되지 않을 수 있습니다.
| **설치**   | 항상 설치
| **시작 유형**   | Manual
| **추천 사항** | 지침 없음
| **설명**       |
...
| ------------------ | ------ |
| **서비스 이름**   | WpnService
| **설명**    | 이 서비스는 세션 0에서 실행되며 디바이스와 WNS 서버 사이의 연결을 처리하는 알림 플랫폼 및 연결 공급자를 호스트합니다.
| **설치**   | 데스크톱 경험이 있는 경우에만
| **시작 유형**   | 자동
| **추천 사항** | 사용하지 않도록 설정할 수 있음
...
| **설명**    | 이 서비스는 로컬 및 푸시 알림을 지원하는 Windows 알림 플랫폼을 호스트합니다. 지원되는 알림은 타일, 알림 및 원시 알림입니다.
| **설치**   | 데스크톱 경험이 있는 경우에만
| **시작 유형**   | Manual
| **추천 사항** | 사용하지 않도록 설정할 수 있음
| **설명**       | 사용자 서비스 템플릿
...
| ------------------ | ------ |
| **서비스 이름**   | WSearch
| **설명**    | 파일, 이메일 및 기타 콘텐츠에 대한 콘텐츠 인덱싱, 속성 캐싱 및 검색 결과를 제공합니다.
| **설치**   | 데스크톱 경험이 있는 경우에만
| **시작 유형**   | 사용 안 함
| **추천 사항** | 이미 사용하지 않도록 설정됨
| **설명**       |
...
| ------------------ | ------ |
| **서비스 이름**   | W32Time
| **설명**    | 네트워크의 모든 클라이언트와 서버에서 날짜 및 시간 동기화를 유지 관리합니다. 이 서비스가 중지되면 날짜 및 시간 동기화를 사용할 수 없습니다. 이 서비스를 사용하지 않으면 이 서비스에 명시적으로 종속된 모든 서비스를 시작할 수 없습니다.
| **설치**   | 항상 설치
| **시작 유형**   | 자동
| **추천 사항** | 지침 없음
| **설명**       |
...
| ------------------ | ------ |
| **서비스 이름**   | wuauserv
| **설명**    | Windows와 기타 프로그램의 업데이트를 검색하고 다운로드하고 설치하도록 설정합니다. 서비스를 사용하지 않도록 설정하면 이 컴퓨터의 사용자가 Windows 업데이트나 자동 업데이트 기능을 사용할 수 없게 되며 프로그램에서 WUA(Windows 업데이트 에이전트) API를 사용할 수 없게 됩니다.
| **설치**   | 항상 설치
| **시작 유형**   | Manual
| **추천 사항** | 지침 없음
| **설명**       |
...
| **설명**    | WinHTTP는 클라이언트 HTTP 스택을 구현하며 HTTP 요청을 보내고 응답을 받을 수 있는 Win32 API 및 COM 자동화 구성 요소를 개발자에게 제공합니다. 또한 WinHTTP는 웹 프록시 자동 검색(WPAD) 프로토콜의 구현을 통한 프록시 구성의 자동 검색을 지원합니다.
| **설치**   | 항상 설치
| **시작 유형**   | Manual
| **추천 사항** | 사용하지 않도록 설정할 수 없음
...
| ------------------ | ------ |
| **서비스 이름**   | dot3svc
| **설명**    | 유선 자동 구성(DOT3SVC) 서비스는 이더넷 인터페이스에서 IEEE 802.1X 인증을 수행합니다. 현재 유선 네트워크 배포에서 802.1X 인증을 적용하는 경우 계층 2 연결을 설정하거나 네트워크 리소스에 대한 액세스를 제공하도록 DOT3SVC 서비스를 구성해야 합니다. 802.1X 인증을 적용하지 않는 유선 네트워크 연결은 DOT3SVC 서비스의 영향을 받지 않습니다.
| **설치**   | 데스크톱 경험이 있는 경우에만
| **시작 유형**   | Manual
| **추천 사항** | 지침 없음
| **설명**       |
...
| ------------------ | ------ |
| **서비스 이름**   | wmiApSrv
| **설명**    | 네트워크에 있는 클라이언트에 WMI(Windows Management Instrumentation) 공급자의 성능 라이브러리 정보를 제공합니다. 이 서비스는 성능 데이터 도우미가 활성화되어 있을 경우에만 실행됩니다.
| **설치**   | 항상 설치
| **시작 유형**   | Manual
| **추천 사항** | 지침 없음
| **설명**       |
...
| **설명**    | SMB 프로토콜을 사용하여 원격 서버에 대한 클라이언트 네트워크 연결을 만들고 유지합니다. 이 서비스가 중지되면 해당 연결을 사용할 수 없습니다. 이 서비스를 사용하지 않으면 이 서비스에 명시적으로 종속된 모든 서비스를 시작할 수 없습니다.
| **설치**   | 항상 설치
| **시작 유형**   | 자동
| **추천 사항** | 지침 없음
| **설명**       |
...
| **설명**    | Xbox Live와의 상호 작용을 위한 인증 및 승인 서비스를 제공합니다. 이 서비스를 중지하면 일부 애플리케이션이 제대로 작동하지 않을 수 있습니다.
| **설치**   | 데스크톱 경험이 있는 경우에만
| **시작 유형**   | Manual
