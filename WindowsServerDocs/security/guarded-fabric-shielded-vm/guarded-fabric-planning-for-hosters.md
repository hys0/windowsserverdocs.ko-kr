...
## <a name="decision-2-existing-hyper-v-fabric-versus-a-new-separate-hyper-v-fabric"></a>의사 결정 #2: 기존 Hyper-v 패브릭과 새 개별 Hyper-v 패브릭 비교
...
전용 물리적 서버, 보호 된 VM, 격리 된 Hyper-v 호스트의 VM (보호 중인 패브릭에서 분리 됨) 또는 다른 Azure를 사용 하 여 논리적으로 분리 된 가상 환경에서 매우 안전한 환경에 HGS (호스트 보호 서비스)를 배포 합니다. 가입은.   
...
| 영역 | 설명 |
|------|---------|
| 하드웨어 | <ul><li>호스트 키 증명: 보호 된 호스트로 기존 하드웨어를 사용할 수 있습니다. 몇 가지 예외가 있습니다. 호스트에서 Windows Server 2016부터 시작 하는 새로운 보안 메커니즘을 사용할 수 있도록 하려면 [코드 무결성을 Windows server 2016 가상화 기반 보호와 호환 되는 하드웨어](guarded-fabric-compatible-hardware-with-virtualization-based-protection-of-code-integrity.md)를 참조 하세요.</li><li>TPM에서 신뢰할 수 있는 증명: 적절 하 게 구성 된 경우 [하드웨어 보증 추가 자격이](https://msdn.microsoft.com/windows/hardware/commercialize/design/compatibility/systems#system-server-assurance) 있는 모든 하드웨어를 사용할 수 있습니다 ( [보호 된 vm과 호환 되는 서버 구성 및 코드 무결성의 가상화 기반 보호 참조). ](guarded-fabric-compatible-hardware-with-virtualization-based-protection-of-code-integrity.md)특정 구성의 경우). 여기에는 TPM 2.0 및 UEFI 버전 2.3.1 c 이상이 포함 됩니다.</li></ul> |
