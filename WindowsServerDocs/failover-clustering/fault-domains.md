...
이 가이드에서는 두 가지 예만 제공 하지만 `<Site>`, `<Rack>`, `<Chassis>` 및 @no__t 태그는 배포의 실제 토폴로지를 반영 하는 여러 가지 다른 방법으로 혼합 및 일치할 수 있습니다. 위 예에서는 이러한 태그의 유연성과 이러한 태그를 명확히 구분하는 자유형 위치 설명자의 가치를 보여드리려 했습니다.  
