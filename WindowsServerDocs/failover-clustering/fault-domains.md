...
이 가이드에서는 두 가지 예만 제공 하지만 `<Site>`, `<Rack>`, `<Chassis>`및 `<Node>` 태그를 여러 가지 방법으로 혼합 하 고 일치 시켜 배포의 실제 토폴로지를 반영할 수 있습니다. 위 예에서는 이러한 태그의 유연성과 이러한 태그를 명확히 구분하는 자유형 위치 설명자의 가치를 보여 주고자 했습니다.  
