...
- 다음 방법 중 하나를 사용 하 여 Hyper-v 노드에 SLAT (두 번째 수준 주소 지정 테이블)를 지 원하는 Cpu가 있는지 확인 합니다.  
        -@No__t를 검토 합니다. CPU가 SLATs를 지원 하는지 확인 하는 두 가지 메서드를 설명 하는 WP8 SDK Tip 01 @ no__t 문서  
...
@no__t--클러스터 OS 롤링 업그레이드의 세 단계를 보여 주는 그림: 모든 노드 Windows Server 2012 R2, 혼합 OS 모드 및 모든 노드 Windows Server 2016 @ no__t-1  
**그림 1: 클러스터 운영 체제 상태 전환 @ no__t-0  
...
초기 상태를 보여 주는 @no__t--모든 노드 Windows Server 2012 R2 @ no__t-1  
...
혼합 OS 모드에서 클러스터를 보여 주는 @no__t 그림: 예 4-노드 클러스터, 두 노드에서 Windows Server 2016를 실행 하 고 두 노드에서 Windows Server 2012 R2 @ no__t-1을 실행 합니다.  
...
@no__t-클러스터를 Windows Server 2016로 완전히 업그레이드 했으며 ClusterFunctionalLevel cmdlet이 클러스터 기능 수준을 Windows Server 2016 @ no__t-1까지 가져올 준비가 되었음을 보여 주는 그림입니다.  
**그림 4: 중간 상태: 모든 노드가 Windows Server 2016로 업그레이드 되 고 ClusterFunctionalLevel (3 단계 @no__t)를 사용할 수 있습니다.-0  

ClusterFunctionalLevelcmdlet가 실행 된 후 클러스터는 "4 단계"를 입력 합니다. 여기에서 새로운 Windows Server 2016 클러스터 기능을 사용할 수 있습니다.  

클러스터 롤링 OS 업그레이드가 성공적으로 완료 되었음을 보여 주는 ![ 그림 모든 노드가 Windows Server 2016로 업그레이드 되었으며 클러스터가 Windows Server 2016 클러스터 기능 수준 @ no__t-1에서 실행 되 고 있습니다.  
...
클러스터 업그레이드를 위한 워크플로를 보여 주는 ](media/Cluster-Operating-System-Rolling-Upgrade/Clustering_RollingUpgrade_Workflow.png)-1-1  
**Figure 6: 클러스터 OS 롤링 업그레이드 프로세스 워크플로 @ no__t-0  
...
1. 다음과 같이 운영 체제를 업그레이드 하기 위해 클러스터를 준비 합니다.  
    ...
    4. [@No__t-1](https://docs.microsoft.com/powershell/module/failoverclusters/Get-ClusterNode?view=win10-ps) cmdlet을 사용 하 여 모든 클러스터 노드가 온라인/running/up 확인 합니다 (그림 7 참조).  

        ![Screencap-Start-clusternode cmdlet @ no__t-1을 실행 한 결과를 보여 줍니다.  
        **Figure 7: Start-clusternode cmdlet @ no__t을 사용 하 여 노드 상태를 확인 하는 중  

    5. CAU (클러스터 인식 업데이트)를 실행 하는 경우 **클러스터 인식 업데이트** UI 또는 [`Get-CauRun`](https://docs.microsoft.com/powershell/module/clusterawareupdating/Get-CauRun?view=win10-ps) cmdlet (그림 8 참조)을 사용 하 여 cau가 현재 실행 되 고 있는지 확인 합니다. 클러스터 OS 롤링 업그레이드 프로세스를 진행 하는 동안 CAU에서 노드를 일시 중지 하 고 방전 하지 않도록 하려면 [`Disable-CauClusterRole`](https://docs.microsoft.com/powershell/module/clusterawareupdating/Disable-CauClusterRole?view=win10-ps) Cmdlet (그림 9 참조)을 사용 하 여 cau를 중지 합니다.  

        ![Screencap-Invoke-caurun cmdlet의 출력을 보여 주는 @ no__t-1  
        **Figure 8: [@No__t-1](https://docs.microsoft.com/powershell/module/clusterawareupdating/Get-CauRun?view=win10-ps) cmdlet을 사용 하 여 클러스터 인식 업데이트가 클러스터에서 실행 중인지 확인 합니다. @ no__t-2  

        ![Screencap Add-cauclusterrole cmdlet의 출력을 보여 주는 @ no__t-1  
        **Figure 9: [@No__t-1](https://docs.microsoft.com/powershell/module/clusterawareupdating/Disable-CauClusterRole?view=win10-ps) cmdlet @ no__t-2를 사용 하 여 클러스터 인식 업데이트 역할을 사용 하지 않도록 설정  
...
2. 클러스터의 각 노드에 대해 다음을 완료 합니다.  
    1. 클러스터 관리자 UI를 사용 하 여 노드를 선택 하 고 일시 중지를 사용 합니다.노드를 드레이닝 하기 위한 드레이닝 메뉴 옵션 (그림 10 참조) 또는 [`Suspend-ClusterNode`](https://docs.microsoft.com/powershell/module/failoverclusters/Suspend-ClusterNode?view=win10-ps) Cmdlet 사용 (그림 11 참조)  
        ...
        ![Screencap는 클러스터 관리자 UI를 사용 하 여 역할을 드레이닝 하는 방법을 보여 줍니다. @ no__t-1  
        ** 그림 10: 장애 조치(Failover) 클러스터 관리자 @ no__t를 사용 하 여 노드에서 역할 드레이닝  

        ![Screencap no__t-1 Start-clusternode cmdlet의 출력을 보여 줍니다.  
        **Figure 11: [@No__t-1](https://docs.microsoft.com/powershell/module/failoverclusters/Suspend-ClusterNode?view=win10-ps) cmdlet @ no__t-2를 사용 하 여 노드에서 역할 드레이닝  
    ...
    2.  클러스터 관리자 UI를 사용 하 여 클러스터에서 일시 중지 된 노드를 **제거** 하거나 [`Remove-ClusterNode`](https://docs.microsoft.com/powershell/module/failoverclusters/Remove-ClusterNode?view=win10-ps) cmdlet을 사용 합니다.  
        ...
        ![Screencap Start-clusternode cmdlet의 출력을 보여 주는 @ no__t-1  
        **Figure 12: [@No__t-1](https://docs.microsoft.com/powershell/module/failoverclusters/Remove-ClusterNode?view=win10-ps) cmdlet @ no__t-2를 사용 하 여 클러스터에서 노드를 제거 합니다.  

    3.  시스템 드라이브를 다시 포맷 하 고 **Custom을 사용 하 여 노드에서 Windows Server 2016의 "클린 운영 체제 설치"를 수행 합니다. Setup.exe에 Windows only (고급)**  설치 (그림 13 참조) 옵션을 설치 합니다. @No__t-0Upgrade를 선택 하지 마십시오. 클러스터 OS 롤링 업그레이드에서 전체 업그레이드를 권장 하지 않으므로 Windows를 설치 하 고 파일, 설정 및 응용 프로그램 유지 @ no__t-0 옵션을 유지 합니다.  

        ![Screencap the Windows Server 2016 설치 마법사에서 선택한 사용자 지정 설치 옵션을 보여 주는 @ no__t-1  
        **Figure 13: Windows Server 2016 @ no__t에 사용 가능한 설치 옵션-0  
    ...
    11. Hyper-v 워크 로드의 경우 Hyper-v 관리자 UI를 사용 하 여 가상 스위치 관리자 대화 상자를 시작 합니다 (그림 14 참조).  
        ...
        Hyper-v 가상 스위치 관리자 대화 상자 @ no__t-1의 위치를 보여 주는 ![Screencap  
        **Figure 14: 가상 스위치 관리자 @ no__t-0  

    12. Windows server 2016 노드 (Windows Server 2012 R2 노드를 사용 하지 않음)에서 장애 조치(Failover) 클러스터 관리자 (그림 15 참조)를 사용 하 여 클러스터에 연결 합니다.  

        클러스터 선택 대화 상자 @ no__t-1을 보여 주는 ![Screencap  
        **Figure 15: 장애 조치(Failover) 클러스터 관리자 @ no__t를 사용 하 여 노드를 클러스터에 추가 하는 중-0  
    ...
    13. 장애 조치(Failover) 클러스터 관리자 UI 또는 [`Add-ClusterNode`](https://docs.microsoft.com/powershell/module/failoverclusters/Add-ClusterNode?view=win10-ps) Cmdlet (그림 16 참조) 중 하나를 사용 하 여 노드를 클러스터에 추가 합니다.  
        ...
        ![Screencap no__t-1 Start-clusternode cmdlet의 출력을 보여 줍니다.  
        **Figure 16: [@No__t-1](https://docs.microsoft.com/powershell/module/failoverclusters/Add-ClusterNode?view=win10-ps) cmdlet을 사용 하 여 노드를 클러스터에 추가 @ no__t-2  
    ...
    14. Windows Server 2016 노드가 클러스터에 성공적으로 추가 된 후에는 다음과 같이 클러스터 전체에서 작업의 균형을 다시 조정 하기 위해 클러스터 워크 로드 중 일부를 새로 추가 된 노드로 이동할 수 있습니다 (옵션).
        ...
        ![Screencap Move-clustervirtualmachinerole cmdlet의 출력을 보여 주는 @ no__t-1  
        **Figure 17: [@No__t-1](https://docs.microsoft.com/powershell/module/failoverclusters/Move-ClusterVirtualMachineRole?view=win10-ps) cmdlet @ no__t-2를 사용 하 여 클러스터 워크 로드 (클러스터 VM 역할) 이동  
        ...
        2. 다른 클러스터 워크 **로드의 경우** 장애 조치(Failover) 클러스터 관리자 또는 [`Move-ClusterGroup`](https://docs.microsoft.com/powershell/module/failoverclusters/Move-ClusterGroup?view=win10-ps) cmdlet을 사용 합니다.  
...
3. 모든 노드가 Windows Server 2016로 업그레이드 되 고 클러스터에 다시 추가 되었거나 나머지 Windows Server 2012 R2 노드가 제거 된 경우 다음을 수행 합니다.  
    ...
    > -   [@No__t](https://docs.microsoft.com/powershell/module/failoverclusters/Update-ClusterFunctionalLevel?view=win10-ps) cmdlet이 실행 될 때까지 프로세스를 완전히 되돌릴 수 있으며 windows Server 2012 R2 노드를이 클러스터에 추가할 수 있으며 windows server 2016 노드를 제거할 수 있습니다.  
    > -   [@No__t-1](https://docs.microsoft.com/powershell/module/failoverclusters/Update-ClusterFunctionalLevel?view=win10-ps) cmdlet이 실행 된 후에는 새로운 기능을 사용할 수 있습니다.  
    ...
    1.  장애 조치(Failover) 클러스터 관리자 UI 또는 [`Get-ClusterGroup`](https://docs.microsoft.com/powershell/module/failoverclusters/Get-ClusterGroup?view=win10-ps) cmdlet을 사용 하 여 클러스터에서 모든 클러스터 역할이 예상 대로 실행 되 고 있는지 확인 합니다. 다음 예제에서는 사용 가능한 저장소를 사용 하지 않고 CSV를 사용 합니다. 따라서 사용 가능한 저장소에는 **오프 라인** 상태가 표시 됩니다 (그림 18 참조).  
        ...
        no__t cmdlet의 출력을 보여 주는 ![Screencap cmdlet @-1  
        **Figure 18: 모든 클러스터 그룹 (클러스터 역할)이 [`Get-ClusterGroup`](https://docs.microsoft.com/powershell/module/failoverclusters/Get-ClusterGroup?view=win10-ps) cmdlet @ no__t-2를 사용 하 여 실행 되 고 있는지 확인 하는 중  

    2.  모든 클러스터 노드가 온라인 상태이 고 [`Get-ClusterNode`](https://docs.microsoft.com/powershell/module/failoverclusters/Get-ClusterNode?view=win10-ps) cmdlet을 사용 하 여 실행 중인지 확인 합니다.  
    3.  [@No__t-1](https://technet.microsoft.com/library/mt589702.aspx) cmdlet 실행-오류가 반환 되어서는 안 됩니다 (그림 19 참조).  

        ![Screencap-ClusterFunctionalLevel cmdlet @ no__t-1의 출력을 보여 줍니다.  
        **Figure 19: PowerShell @ no__t를 사용 하 여 클러스터의 기능 수준 업데이트-0  

    4.  [@No__t-1](https://docs.microsoft.com/powershell/module/failoverclusters/Update-ClusterFunctionalLevel?view=win10-ps) cmdlet이 실행 된 후에는 새로운 기능을 사용할 수 있습니다.  
...
4. Windows Server 2016-정상 클러스터 업데이트 및 백업 다시 시작:  
    ...
    1. 이전에 CAU를 실행 한 경우 CAU UI를 사용 하 여 다시 시작 하거나 [`Enable-CauClusterRole`](https://docs.microsoft.com/powershell/module/clusterawareupdating/Enable-CauClusterRole?view=win10-ps) cmdlet을 사용 합니다 (그림 20 참조).  
        ...
        Add-cauclusterrole @ no__t-1의 출력을 보여 주는 ![Screencap  
        **Figure 20: [@No__t-1](https://docs.microsoft.com/powershell/module/clusterawareupdating/Enable-CauClusterRole?view=win10-ps) cmdlet @ no__t-2를 사용 하 여 클러스터 인식 업데이트 역할을 사용 하도록 설정  
...
5. Hyper-v Virtual Machines에서 Windows Server 2016 기능을 사용 하도록 설정 하 고 사용 합니다.  
    ...
    2. 클러스터의 각 Hyper-v 호스트 노드에서 [`Get-VMHostSupportedVersion`](https://docs.microsoft.com/powershell/module/hyper-v/Get-VMHostSupportedVersion?view=win10-ps) cmdlet을 사용 하 여 호스트에서 지원 되는 hyper-v VM 구성 버전을 확인 합니다.  
        ...
        ![Screencap-VMHostSupportedVersion cmdlet의 출력을 보여 주는 @ no__t-1  
        **Figure 21: 호스트에서 지원 되는 Hyper-v VM 구성 버전 보기 @ no__t-0  

   3. 클러스터의 각 Hyper-v 호스트 노드에서 Hyper-v VM 구성 버전은 사용자를 사용 하 여 간단한 유지 관리 기간을 예약 하 고, 가상 컴퓨터를 백업 하 고, 가상 컴퓨터를 해제 하 고, [`Update-VMVersion`](https://docs.microsoft.com/powershell/module/hyper-v/Update-VMVersion?view=win10-ps) cmdlet을 실행 하 여 업그레이드할 수 있습니다 (그림 22 참조). 이렇게 하면 가상 컴퓨터 버전이 업데이트 되 고 새 Hyper-v 기능이 사용 되므로 향후 Hyper-v 통합 구성 요소 (IC) 업데이트가 필요 하지 않습니다. 이 cmdlet은 VM을 호스트 하는 Hyper-v 노드에서 실행 하거나, `-ComputerName` 매개 변수를 사용 하 여 VM 버전을 원격으로 업데이트할 수 있습니다. 이 예제에서는 VM1의 구성 버전을 5.0에서 7.0으로 업그레이드 하 여 프로덕션 검사점 (응용 프로그램 일치 백업) 및 이진 VM과 같은 VM 구성 버전과 관련 된 많은 새 Hyper-v 기능을 활용 합니다. 구성 파일.  

       작업 @ no__t-1의 업데이트-VMVersion cmdlet을 보여 주는 ![Screencap  
       **Figure 22: 업데이트-VMVersion PowerShell cmdlet @ no__t를 사용 하 여 VM 버전 업그레이드  
...
**노드가 끄거나 일시 중지 된 상태 [에서 `Update-ClusterFunctionalLevel`](https://docs.microsoft.com/powershell/module/failoverclusters/Update-ClusterFunctionalLevel?view=win10-ps) cmdlet을 실행할 수 있나요?**  
    아니요. [@No__t-1](https://docs.microsoft.com/powershell/module/failoverclusters/Update-ClusterFunctionalLevel?view=win10-ps) cmdlet이 작동 하려면 모든 클러스터 노드가 켜져 있고 활성 멤버 여야 합니다.  

@no__t 클러스터 운영 체제 롤링 업그레이드는 클러스터 워크 로드에서 작동 하나요? SQL Server에 대해 작동 합니까? **  
...
**[@No__t-2](https://docs.microsoft.com/powershell/module/failoverclusters/Update-ClusterFunctionalLevel?view=win10-ps) 가 성공적으로 실행 된 후 클러스터에서 문제가 발견 되 면 어떻게 되나요?**  
    [@No__t-1](https://docs.microsoft.com/powershell/module/failoverclusters/Update-ClusterFunctionalLevel?view=win10-ps)을 실행 하기 전에 시스템 상태 백업을 사용 하 여 클러스터 데이터베이스를 백업한 경우 Windows Server 2012 R2 클러스터 노드에서 정식 복원을 수행 하 고 원래 클러스터 데이터베이스 및 구성을 복원할 수 있습니다.  
