...
ms.locfileid: "8992546"
---
# Windows Server 2019의 새로운 기능

이 항목에서는 Windows Server 2019의 새로운 기능 몇 가지를 설명합니다. Windows Server 2019는 강력한 Windows Server 2016 기반 위에 구축되어 하이브리드 클라우드, 보안, 응용 프로그램 플랫폼, HCI(하이퍼 컨버지드 인프라)라는 네 가지 테마에 수많은 혁신을 가져옵니다. Windows Server 버전 1809의 새로운 기능을 알아보려면 [Windows Server 버전 1809의 새로운 기능](../get-started/whats-new-in-windows-server-1809.md)을 참조하세요.

## 일반

### 데스크톱 환경

Windows Server 2019는 장기 서비스 채널(LTSC) 릴리스이므로 <b>데스크톱 환경</b>이 포함되어 있습니다. (반기 채널\(SAC\) 릴리스에 기본적으로 데스크톱 환경이 포함되어 있지 않기 때문에 Windows Server 버전 1709, Windows Server 버전 1803 또는 Windows Server 버전 1809에는 데스크톱 환경이 포함되어 있지 않습니다. 이것들은 엄밀히 말해 Server Core 및 Nano Server 컨테이너 이미지 릴리스입니다.) Windows Server 2016과 마찬가지로 운영 체제 설치 도중 Server Core 설치 또는 데스크톱 환경 포함 서버 설치 중 하나를 선택할 수 있습니다.
...
- **더욱 뛰어난 응용 프로그램 호환성**
    ...
    Windows 기반 응용 프로그램의 컨테이너화가 더욱 쉬워집니다. 기존 *windowsservercore* 이미지의 앱 호환성이 향상되었습니다. 추가 API 종속성이 포함된 응용 프로그램의 경우 *windows*라는 세 번째 기본 이미지가 있습니다.
