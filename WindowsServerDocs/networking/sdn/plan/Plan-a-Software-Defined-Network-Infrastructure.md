...
| **네트워크 이름** |  **서브넷**  | **마스크할** | **트럭의 VLAN ID** | **관문**  |                                                           **예약 (예제)**                                                           |
|------------------|--------------|----------|----------------------|--------------|-------------------------------------------------------------------------------------------------------------------------------------------------|
...
라우팅 정보 \((예: VIP 서브넷\) 의 다음 홉)는 SLB/MUX 및 RAS 게이트웨이에서 내부 BGP 피어 링을 사용 하 여 실제 네트워크에 보급 합니다. VIP 논리 네트워크에 할당 된 VLAN이 없으며 계층 2 스위치 (예: 랙 이전 스위치)에 미리 구성 되어 있지 않습니다.  
...
BGP 피어 링은 일반적으로 네트워크 인프라의 일부로 관리 되는 스위치 또는 라우터에서 구성 됩니다. BGP 피어는 라우팅 전용 모드에서 RAS (원격 액세스 서버) 역할이 설치 된 Windows 서버에서 구성할 수도 있습니다. 네트워크 인프라의이 BGP 라우터 피어는 자체 asn을 갖도록 구성 하 고 SDN 구성 요소 \(SLB/MUX 및 RAS 게이트웨이에\)할당 된 asn에서 피어 링을 허용 해야 합니다. 실제 라우터 또는 해당 라우터를 제어 하는 네트워크 관리자에서 다음 정보를 얻어야 합니다.
...
VXLAN 또는 NVGRE 캡슐화 헤더에 의해 발생 하는 테 넌 트 가상 네트워크 트래픽에 대 한 오버 헤드를 고려 하려면 계층 2 패브릭 네트워크 (스위치 및 호스트)의 MTU를 계층 2 이더넷을 포함 하 여 1674 \(바이트 이상으로 설정 해야 합니다. 헤더\). 

새 *Encapoverhead* 고급 어댑터 키워드를 지 원하는 nic는 네트워크 컨트롤러 호스트 에이전트를 통해 자동으로 MTU를 설정 합니다. 새 *encapoverhead* 키워드를 지원 하지 않는 nic에서는\) *JumboPacket* \(또는 동급 키워드를 사용 하 여 각 실제 호스트에서 MTU 크기를 수동으로 설정 해야 합니다. 
...
- MTU가 L2-이더넷 헤더를 \(포함 하 > = 1674 바이트로 설정 되었습니다.\)  
- L3 프로토콜 \(필요\)  
- ECMP  
...
- RFC 4893: "4 개 옥텟에 대 한 BGP 지원 (숫자 공간)"  
- RFC 4456: "BGP 경로 리플렉션: 전체 메시 내부 BGP (IBGP)의 대안  
...
- Roce를 \(사용 하는 경우에만 서비스 품질 PFC 필요\)
...
- 네트워크 컨트롤러 \(를 사용 하 여 물리적 스위치를 모니터링 하는 경우 SNMP (mib 필요\)  
- Mib-II (RFC 1213), LLDP, 인터페이스 MIB \(RFC 2863\),-mib, ip-mib, ip-전달-mib, Q-브리지-mib, 브리지-mib, lldb-mib, 엔터티-mib, IEEE8023  
...
> [!TIP]  
> 모든 가상 스위치에 동일한 이름을 사용 하는 것이 편리 하지만 반드시 필요한 것은 아닙니다. 스크립트를 사용 하 여 배포 하려는 경우 psd1 파일의 `vSwitchName` 변수와 연결 된 주석을 참조 하세요.  
...
--------|-------------------------|-------------------------  
|실제 Hyper-v 호스트|4 코어 2.66 g h z CPU<br /><br />32 GB의 RAM<br /><br />300 g b의 디스크 공간<br /><br />1gb/s (또는 더 빠른) 실제 네트워크 어댑터|부팅 Windows Server 2016<br /><br />설치 된 hyper-v 역할|  
