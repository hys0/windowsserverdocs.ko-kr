...
ms.locfileid: "59861534"
---
# <a name="performance-tuning-network-adapters"></a>네트워크 어댑터 성능 조정

>적용 대상: Windows Server (반기 채널), Windows Server 2016

이 항목에서는 Windows Server 2016을 실행 하는 컴퓨터에 설치 되는 성능 튜닝 네트워크 어댑터를 사용할 수 있습니다.
...
##  <a name="bkmk_offload"></a> 오프 로드 기능을 사용 하도록 설정

일반적으로 네트워크 어댑터 오프로드 기능을 설정하는 것이 좋습니다. 하지만 네트워크 어댑터가 높은 처리량을 가진 오프로드 기능을 처리할 수 있을 만큼 강력하지 않은 경우가 있습니다.

>[!IMPORTANT]
>오프 로드 기능을 사용 하지 **IPsec 작업 오프 로드가** 하거나 **TCP Chimney 오프 로드**합니다. 이러한 기술은 Windows Server 2016에서 사용 되지 않는 및 서버 및 네트워킹 성능이 저하 될 수 있습니다. 또한 이러한 기술은 지원 되지 않는 Microsoft에서 향후 합니다.
...
##  <a name="bkmk_rss_web"></a> 사용 하도록 설정 하면 수신 측 웹 서버에 대 한 배율 (RSS)
...
것이 중요 한 사실은 RSS 및 Hypertext Transfer Protocol 논리로 인해 \(HTTP\) 부하 분산에 대 한 성능 저하 될 수 있습니다 심각 하 게 RSS 지원-네트워크 어댑터를 하나 있는 서버에서 웹 트래픽을 허용 하는 경우 또는 더 많은 RSS 지원 네트워크 어댑터입니다. 이 경우 RSS 지원 네트워크 어댑터를 사용하거나 네트워크 어댑터 속성의 **고급 속성** 탭에서 RSS를 사용하지 않도록 설정해야 합니다. 네트워크 어댑터가 RSS를 지원하는지 여부를 확인하려면 네트워크 어댑터 속성의 **고급 속성** 탭에서 RSS 정보를 확인하면 됩니다.

### <a name="rss-profiles-and-rss-queues"></a>RSS 프로필 및 RSS 큐

기본 RSS 사전 정의 프로필은 이전 버전의 운영 체제에서 기본 동작을 변경 하는 NUMA Static입니다. RSS 프로필을 시작하려면 사용 가능한 프로필을 검토하여 RSS 프로필이 유용한 경우 및 네트워크 환경과 하드웨어에 RSS 프로필을 적용하는 방법을 이해해야 합니다.
...
##  <a name="bkmk_low"></a> 짧은 대기 시간 패킷 처리를 위한 성능 튜닝

대부분의 네트워크 어댑터는 운영 체제에서 발생하는 대기 시간을 최적화할 수 있는 옵션을 제공합니다. 대기 시간은 네트워크 드라이버에서 들어오는 패킷을 처리하여 다시 보낼 때까지 소요되는 시간입니다. 이 시간은 일반적으로 마이크로초 단위로 측정됩니다. 반면 멀리 떨어진 패킷 전송에 대 한 전송에 일반적으로 밀리초 단위로 측정 됩니다 \(배 더 큰\)입니다. 이러한 조정은 전송 중에 패킷에서 소요되는 시간을 단축하지 않습니다.

다음은 마이크로초가 중요한 네트워크에 대한 몇 가지 성능 조정 제안 사항입니다.

- 컴퓨터 BIOS를 **고성능**으로 설정하고, C-state는 사용하지 않도록 설정합니다. 그러나 이는 시스템 및 BIOS에 종속되므로 일부 시스템은 운영 체제에서 전원 관리를 제어하는 경우 더 높은 성능을 제공합니다. 확인 및에서 전원 관리 설정을 조정할 수 있습니다 **설정을** 또는 사용 하 여 합니다 **powercfg** 명령입니다. 자세한 내용은 참조 하세요. [Powercfg 명령줄 옵션](https://technet.microsoft.com/library/cc748940.aspx)
...
많은 하드웨어 시스템이 사용 하 여 시스템 관리 인터럽트 \(SMI\) 다양 한 보고 오류 수정 코드를 포함 하 여 유지 관리 기능에 대 한 \(ECC\) 메모리 오류, 레거시 USB 호환성, 팬 제어 및 BIOS 제어 전원 관리 합니다. 
...
##  <a name="bkmk_tcp"></a> TCP 성능 조정

 다음 항목을 사용하여 TCP 성능을 조정할 수 있습니다.

###  <a name="bkmk_tcp_params"></a>  TCP 수신 창 자동 조정

Windows Server 2008 이전의 네트워크 스택에 연결에 대 한 잠재적인 전체 처리량을 제한 하는 고정 크기 수신측 창을 (65,535 바이트)를 사용 합니다. TCP 스택의 가장 중요한 변경 사항 중 하나는 TCP 수신 창 자동 조정입니다. 

TCP 수신 창으로 고정된 크기를 사용 하는 경우 단일 연결의 총 처리량을 계산할 수 있습니다.

**달성 가능한 총 처리량 (바이트) = TCP 수신 창 크기 (바이트) \* (1 / 연결 대기 시간 (초))**

예를 들어, 달성 가능한 총 처리량은만 연결 대기 시간이 10ms 인 51 Mbps \(대기업 네트워크 인프라에 대 한 적절 한 값을\)입니다. 

그러나 자동 조정을 사용하면 수신측 창을 조정할 수 있으므로 보낸 사람의 요구에 맞게 확장할 수 있습니다. 1gbps 연결의 전체 회선 속도 달성 하기 위해 연결에 대 한 것 같습니다. 이전에 TCP 연결의 달성 가능한 총 처리량으로 인해 제한되었을 수 있는 네트워크 사용 시나리오에서 이제 네트워크를 완전히 사용할 수 있습니다.

#### <a name="deprecated-tcp-parameters"></a>사용 되지 않는 TCP 매개 변수

Windows Server 2003에서 다음 레지스트리 설정을 더 이상 지원 되지 않으며 이후 버전에서 무시 됩니다.

이러한 설정은 모두가 다음 레지스트리 위치:
...
Windows Vista 및 Windows Server 2008에 도입 된 Windows 필터링 플랫폼 (WFP) 패킷 처리 필터를 만들려면 Microsoft 이외의 독립 소프트웨어 공급 업체 (Isv)에 Api를 제공 합니다. 예를 들어 방화벽 및 바이러스 백신 소프트웨어가 여기에 해당합니다.

>[!NOTE]
>잘못 작성된 WFP 필터는 서버의 네트워킹 성능을 크게 저하시킬 수 있습니다. 자세한 내용은 [이식 패킷 처리 드라이버 및 WFP 앱](https://msdn.microsoft.com/windows/hardware/gg463267.aspx) Windows 개발자 센터에서.
