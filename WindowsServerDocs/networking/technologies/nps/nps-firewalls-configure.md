...
기본적으로 NPS는 사용자 데이터 그램 프로토콜 \(UDP @ no__t-1 포트 1812, 1813, 1645 및 1646을 사용 하 여 RADIUS 트래픽을 보내고 받습니다. Nps를 설치 하는 동안 NPS의 Windows Defender 방화벽이 자동으로 예외를 구성 하 여이 RADIUS 트래픽을 보내고 받을 수 있도록 합니다.
...
인터넷에 연결 된 방화벽은 해당 인터넷 인터페이스 @no__t에서 입력 및 출력 필터를 사용 하 여 구성 해야 하 고, 필요에 따라 네트워크 경계 인터페이스 @ no__t-1을 사용 하 여 NPS와 RADIUS 간에 RADIUS 메시지를 전달할 수 있도록 해야 합니다. 인터넷의 클라이언트 또는 프록시. 추가 필터를 사용 하 여 경계 네트워크의 웹 서버, VPN 서버 및 다른 서버 유형에 트래픽을 전달할 수 있습니다.
...
- \(Optional @ no__t-1 @no__t 경계 네트워크 인터페이스의 대상 IP 주소 및 UDP 대상 포트 1645 이 필터는 인터넷 기반 RADIUS 클라이언트에서 NPS로의 RADIUS 인증 트래픽을 허용 합니다. 이전 RADIUS 클라이언트에서 사용 하는 UDP 포트입니다.
- \(Optional @ no__t-1 @no__t 경계 네트워크 인터페이스의 대상 IP 주소 및 UDP 대상 포트 1646 이 필터는 인터넷 기반 RADIUS 클라이언트에서 NPS로의 RADIUS 계정 트래픽을 허용 합니다. 이전 RADIUS 클라이언트에서 사용 하는 UDP 포트입니다.
...
- \( @no__t 경계 네트워크 인터페이스의 원본 IP 주소와 1645의 UDP 원본 포트 (NPS의 20x66D @ no__t-3). 이 필터는 NPS에서 인터넷 기반 RADIUS 클라이언트에 대 한 RADIUS 인증 트래픽을 허용 합니다. 이전 RADIUS 클라이언트에서 사용 하는 UDP 포트입니다.
- \( @no__t 경계 네트워크 인터페이스의 원본 IP 주소와 1646의 UDP 원본 포트 (NPS의 20x66E @ no__t-3). 이 필터는 NPS에서 인터넷 기반 RADIUS 클라이언트에 대 한 RADIUS 계정 트래픽을 허용 합니다. 이전 RADIUS 클라이언트에서 사용 하는 UDP 포트입니다.
