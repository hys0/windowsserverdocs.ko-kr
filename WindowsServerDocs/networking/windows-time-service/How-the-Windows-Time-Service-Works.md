...
자주 적은 시간을 동기화 할 수 없거나 도메인에 가입 하지 않은 컴퓨터는 time.windows.com와 동기화 하는 기본적으로 구성 됩니다.  따라서 일시적인 있는 컴퓨터 또는 네트워크 연결에 대 한 시간 정확도 보장 하기 위해 가능한 아닙니다.  
...
Windows 시간 서비스를 실행 하는 모든 컴퓨터는 서비스를 사용 하 여 가장 정확한 시간을 유지 합니다. 도메인의 구성원 인 컴퓨터는 기본적으로 시간 클라이언트로 동작, 따라서 대부분의 경우에 필요 없는 Windows 시간 서비스를 구성 합니다. 그러나 Windows 시간 서비스는 지정 된 참조 시간 원본에서 시간을 요청 하도록 구성할 수 있습니다 및 클라이언트에 시간을 제공할 수도 있습니다.
...
-   **NtpClient 입력된 된 공급자입니다.** 이 시간 클라이언트는 하드웨어 장치 또는 NTP 서버를 다른 원본에서 시간 정보를 가져오고 로컬 시계를 동기화 하는 데 도움이 되는 시간 예제를 반환할 수 있습니다.  
...
이벤트 로그에서 오류를 생성을 방지 하려면 동기화를 비활성화할 수 있습니다. 컴퓨터를 사용할 수 있는 시간 원본과 동기화 하려고 할 때마다 이벤트 로그에 오류가 발생 합니다. 시간 서버 하는 동안 동기화를 시도 하지 않도록 하려면 클라이언트에서 동기화를 비활성화할 수 시간 원본을 예정 된 유지 관리에 대 한 네트워크를 벗어나 수행 되 고 다른 원본에서 동기화 하도록 클라이언트 다시 구성 하지 않을 경우 제공 되지 않습니다.  
...
[Windows 시간 서비스에 대 한 기술 참조](windows-time-service-tech-ref.md)
[설정과 Windows 시간 서비스 도구가](Windows-Time-Service-Tools-and-Settings.md)
