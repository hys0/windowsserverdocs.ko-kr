...
|NTP 왕복 지연| 가장 최근의 왕복 지연 시간 (마이크로초)에서 서버에서 응답을 받을 NTP 클라이언트에 의해 발생입니다. 에 요청을 전송 하는 동안 NTP 클라이언트에서 경과 된 시간입니다. |서버에서 유효한 응답을 수신 하 고 NTP 서버를 수신 합니다. 이 카운터는 NTP 클라이언트도 지연 특징을 결정 하는 데 도움이 됩니다. 더 큰 또는 다양 한 왕복 노이즈 NTP 시간 계산에 영향을 줄 수 시간 동기화를 통해 NTP의 정확성을 추가할 수 있습니다.|
...
처음 3 카운터 정확성 문제 해결에 대 한 시나리오를 대상으로 합니다. 모범 사례 아래에서 아래의 문제 해결 시간 정확도 및 NTP 섹션에 더 자세히 설명 되어 있습니다.
...
| |*폴링 빈도*|64-1024 초|해당 없음|일주일에 한 번|
| |*클록 업데이트 빈도*|1 초|해당 없음|한 시간에 한 번|
|**독립 실행형 클라이언트**||||
| |*시간 서버*|해당 없음|time.windows.com|time.windows.com|
| |*폴링 빈도*|해당 없음|하루에 한 번|일주일에 한 번|
| |*클록 업데이트 빈도*|해당 없음|하루에 한 번|일주일에 한 번|
|**도메인 컨트롤러**||||
| |*시간 서버*|PDC/GTIMESERV|해당 없음|PDC/GTIMESERV|
| |*폴링 빈도*|64-1024 초|해당 없음|1024-32768 초|
| |*클록 업데이트 빈도*|하루에 한 번|해당 없음|일주일에 한 번|
|**도메인 구성원 서버**||||
| |*시간 서버*|DC|해당 없음|DC|
| |*폴링 빈도*|64-1024 초|해당 없음|1024-32768 초|
| |*클록 업데이트 빈도*|1 초|해당 없음|5 분 마다 한 번|
|**도메인 구성원 클라이언트**||||
| |*시간 서버*|해당 없음|DC|DC|
| |*폴링 빈도*|해당 없음|1204-32768 초|1024-32768 초|
| |*클록 업데이트 빈도*|해당 없음|5 분 마다 한 번|5 분 마다 한 번|
|**Hyper-v 게스트**||||
