...
Windows 2016 Hyper-v TimeSync 서비스를 향상 시켰습니다. VM start 또는 VM 복원 및 w32time에 제공 된 샘플에 대 한 대기 시간 보정 인터럽트에 초기 시간을 더 정확 하 게 포함 하는 향상 된 기능.  이러한 향상 된이 기능을 사용 하면 로드 (75%)를 사용 하 여 컴퓨터에도 50µs의 10µs (루트 평균 제곱을 가변성을 나타내는), RMS 사용 하 여 호스트의 상태를 유지 하기. 자세한 내용은 [Hyper-v 아키텍처](https://msdn.microsoft.com/library/cc768520.aspx)합니다.

> [!NOTE]
> 부하 분산 된 프로필을 사용 하 여 prime95 벤치 마크를 사용 하 여 생성 되었습니다.

또한 게스트에 보고 하는 계층 수준을 더 투명 하 게 됩니다.  이전에 호스트의 정확성에 관계 없이 2, 고정된 계층을 발생 하 게 합니다.  Windows Server 2016에서 변경 내용을 사용 하 여 호스트 가상 게스트에 대 한 더 나은 시간 결과 호스트 계층 보다 하나 더 큰 계층을 보고 합니다.  호스트 계층은 원본 시간에 따라 일반적인 방법으로 w32time에 의해 결정 됩니다.  도메인 가입 Windows 2016 게스트가 호스트에 기본값으로 지정 하는 것이 아니라 가장 정확한 시계를 찾을 수 있습니다.  이러한 이유 때문에 수동으로 Windows 2012R2에 도메인에 가입 된 컴퓨터에 대 한 Hyper-v 시간 공급자 설정을 사용 하지 않도록 해야 한다고 것 이었습니다.
...
다음 Windows 2016 및 이전 버전 간의 각 역할에 대 한 기본 구성 변경 내용을 설명 합니다.  Windows Server 2016 및 Windows 10 1 주년 업데이트 (빌드 14393)에 대 한 설정이 이제 고유한 이기 때문에 있는 별도 열으로 표시 됩니다. 
...
| |*폴링 빈도*|64-1024 시간 (초)|NA|일주일에 한 번|
| |*시계 업데이트 빈도*|초에 한 번|NA|시간에 한 번|
|**독립 실행형 클라이언트**||||
| |*시간 서버*|NA|time.windows.com|time.windows.com|
| |*폴링 빈도*|NA|하루에 한 번|일주일에 한 번|
| |*시계 업데이트 빈도*|NA|하루에 한 번|일주일에 한 번|
|**도메인 컨트롤러**||||
| |*시간 서버*|PDC/GTIMESERV|NA|PDC/GTIMESERV|
| |*폴링 빈도*|64-1024 시간 (초)|NA|1024-32768 시간 (초)|
| |*시계 업데이트 빈도*|하루에 한 번|NA|일주일에 한 번|
|**도메인 구성원 서버**||||
| |*시간 서버*|DC|NA|DC|
| |*폴링 빈도*|64-1024 시간 (초)|NA|1024-32768 시간 (초)|
| |*시계 업데이트 빈도*|초에 한 번|NA|5 분 마다 한 번씩|
|**도메인 구성원 클라이언트**||||
| |*시간 서버*|NA|DC|DC|
| |*폴링 빈도*|NA|1204-32768 시간 (초)|1024-32768 시간 (초)|
| |*시계 업데이트 빈도*|NA|5 분 마다 한 번씩|5 분 마다 한 번씩|
|**Hyper-v 게스트**||||
| |*시간 서버*|호스트 시간과 서버의 계층에 따라 최상의 옵션 선택|호스트 시간과 서버의 계층에 따라 최상의 옵션 선택|호스트 기본값|
| |*폴링 빈도*|위의 역할에 따라|위의 역할에 따라|위의 역할에 따라|
...
부하를 상당히 CPU, 40%가 넘는 Dc를 실행 하는 경우이 거의 확실히 노이즈 NTP 응답에 추가 되며 도메인에 시간 정확도 영향을 명심 해야 이기도 합니다.  마찬가지로 실제 결과 이해 하는 사용자 환경에서 테스트 해야 합니다.
...
## <a name="BestPractices"></a>정확 하 게 계측에 대 한 모범 사례
...
#### <a name="AllowingLinux"></a>Hyper-v 호스트 시간을 사용 하는 Linux를 허용 합니다.
...
> [!NOTE]
> 참고:  Linux 게스트를 사용 하 여 정확한 시간에 대 한 지원만 업스트림 최신 Linux 커널에서 지원 되는 기능이 필요 하 고 광범위 하 게 사용할 수 있는 모든 Linux 배포판에서 아직입니다. 참조 하세요 [Windows의 Hyper-v에 대 한 지원 되는 Linux 및 FreeBSD 가상 컴퓨터](https://technet.microsoft.com/windows-server-docs/virtualization/hyper-v/supported-linux-and-freebsd-virtual-machines-for-hyper-v-on-windows) 지원 배포판에 대 한 자세한 내용은 합니다.

#### <a name="GTIMESERV"></a>GTIMESERV를 사용 하는 로컬 신뢰할 수 있는 시간 서비스를 지정 합니다.
...
#### <a name="windows-server-2016-on-3rd-party-virtual-platforms"></a>타사 파티 가상 플랫폼에서 Windows Server 2016
Windows는 가상화 된 경우 기본적으로 하이퍼바이저는 시간을 제공 합니다.  하지만 도메인 가입 멤버에서 제대로 작동 하려면 Active Directory 도메인 컨트롤러와 동기화 해야 합니다.  모든 시간 가상화 게스트와 호스트 모든 타사 가상 플랫폼을 사용 하지 않도록 설정 하는 것이 좋습니다.
...
**도메인 가상화** -Hyper-v 호스트를 사용 하 여이 레지스트리 항목을 비활성화할 수 있습니다 하는 것이 아니라 해당 도메인와 시간을 동기화 할 수 있도록 Windows 2012R2에서 가상화 된 도메인 컨트롤러를 제어 하기 위해.   Pdc를 하지 않으려는 Hyper-v 호스트 가장 안정적인 시간 원본을 제공 하는 것으로 항목을 해제 합니다.  레지스트리 항목 데이터가 변경 되는 w32time 서비스를 다시 시작 하면 필요 합니다.
...
> [!NOTE]
> NtpServer 및 EventLogFlags 설정은 System\Windows 시간 Service\Time 공급자 Windows NTP 클라이언트 구성 설정을 사용 하 여 아래에 위치 합니다.  다른 3 System\Windows 시간 서비스의 전역 구성 설정을 사용 하 여 아래에 있습니다.
...
UTC 및 하지 시간의 경과 가장 큰 정확도 필요로 하는 프로그램을 사용 해야 합니다 [GetSystemTimePreciseAsFileTime API](https://msdn.microsoft.com/library/windows/desktop/Hh706895.aspx)합니다.  이 통해 응용 프로그램을 조건으로 Windows 시간 서비스 시스템 시간을 가져옵니다.
...
Windows Server 2012 및 Windows Server 2008에 대 한 핫픽스를 먼저 설치 해야 합니다.  이 기술 자료 문서를 참조할 수 있습니다. [Windows Server 2012 및 Windows 8에서 멀티 캐스트 수신기 응용 프로그램을 실행 하면 데이터 그램 손실](https://support.microsoft.com/kb/2808584)
...
2.  "피어 응답에서" w32tm 로그에서 찾고 클록 소스입니다.   메시지 텍스트를 다음 IP 주소 또는 시간 원본과 유효성을 검사에 대 한 참조 시계는 체인에서 다음을 설명 하는 VMIC입니다.
3.  Clock w32tm 로그를 사용 하 여 유효성을 검사 하는 조건 상태 "ClockDispl 분야: \*기울이기\*시간\*"발생 합니다.  이 시간에 해당 w32tm 상태인 것을 나타냅니다.

### <a name="event-logging"></a>이벤트 로깅
전체 스토리를 가져오려면도 이벤트 로그 정보가 필요 합니다.  시스템 이벤트 로그를 수집 및 시간-서버에서 필터링 하 여 Windows 커널 부팅 Microsoft, Microsoft-Windows-커널-일반을 수 변경 된 경우, 예를 들어 타사 기타 영향 있는지 검색할 수 있도록 합니다.  이러한 로그는 외부 간섭이 해야 할 수 있습니다.  그룹 정책 이벤트 로그는 로그에 기록 됩니다 영향을 줄 수 있습니다.  사용 하 여 그룹 정책에 대 한 자세한 내용은 위의 섹션을 참조 합니다.

### <a name="W32Logging"></a>W32time 디버그 로깅을
W32tm 감사를 목적으로 사용 하려면 다음 명령을 시계의 주기적인 업데이트를 보여 주는 소스 시계를 나타냅니다 로깅을 사용 합니다.  새 로깅을 사용 하도록 서비스를 다시 시작 합니다.  
...
다음 참조 형식으로 현재 사용 되 고 있는 원본 컴퓨터를 보고 하는 분쟁에 관심된 시간 전에 마지막 보고서 로그에서 찾을 해야 합니다.  이 IP 주소, 컴퓨터 이름 또는 hyper-v 호스트와 동기화 할 나타내는 VMIC 공급자 수 있습니다.  다음 예제에서는 10.197.216.105의 IPv4 주소를 제공합니다.
...
Windows 특정 범위를 초과 하는 대신 다양 한 시계를 제외 단계 시간을 하지 않습니다.  즉, w32tm 클록 업데이트 빈도 설정, Windows Server 2016 초에 한 번에 기본값을 사용 하 여 정기적으로 시계는 빈도 조정 합니다.  빈도 계속 시계 뒤에 있는 경우와 앞 이면 저하 빈도.  그러나 간의 클럭 주파수 조정 되는 동안 컨트롤의 하드웨어 시간이 않았습니다.  펌웨어 또는 하드웨어 시계에 문제가 있으면 컴퓨터의 시간을 덜 정확 하 게 될 수 있습니다.

테스트 해야 하는 또 다른 이유 및 사용자 환경에서 초기 계획입니다.  대상으로 하는 정확도 "계산 시간 오프셋" 성능 카운터에 안정화 되지 않으면, 펌웨어를 최신 확인 하려는 수도 있습니다.  다른 테스트로 중복 된 하드웨어 같은 문제를 재현 하는 경우를 볼 수 있습니다.
...
- 시스템 이벤트 로그
- 사용 하 여 로깅을 사용 하도록 설정: w32tm 로그-w32tm /debug /enable /file:C:\Windows\Temp\w32time-test.log /size:10000000 /entries:0-300
...
클라이언트 시계는 유동| 시간 서비스 이벤트 36 시스템 이벤트 로그 및/또는 텍스트에서 설명 하는 로그 파일에: 1에서 0으로 이동 하는 "NTP 클라이언트 시간 원본 Count" 카운터|업스트림 원본 문제를 해결 하 고 성능 문제에 실행 되 고 있는지 이해 합니다.|
...
Windows Server 2012R2 성능 카운터를 없기 때문에 비교 하 여 도구로 w32tm /stripchart 하나만 있지만 초기 Windows Server 2016 및 2012 R2의 경우에 유용 합니다.  동일한 특성을 가진 두 개의 컴퓨터를 선택 하거나 컴퓨터를 업그레이드 하 고 업데이트 후 결과 비교 해야 합니다.  Windows 시간 측정 추 록 2016과 2012 간의 측정을 수행 하는 방법에 대 한 자세한 내용은 있습니다.
...
지구 회전 기간 climatic 및 geological 이벤트에 의해 발생 한 시간에 따라 달라 집니다. 일반적으로, 변형을 2 년 마다 약 1 초입니다. 원자성 시간에서 변형 너무 커지면, 때마다 1 초 (위쪽 또는 아래쪽)의 수정 사항은 삽입를 윤 초 라는 됩니다. 차이 0.9 초를 초과 하지 않습니다 하는 방식으로 수행 됩니다. 이 수정에는 6 개월 동안 실제 수정 미리 발표 됩니다. Windows Server 2016 하기 전에 Microsoft 시간 서비스 윤 초를 인식 하지 못했던 수 있지만이 확인 하는 외부 시간 서비스에서. Windows Server 2016의 증가 시간 정확도, Microsoft 윤 초 문제에 대 한 더 적합 한 솔루션에서 작동 합니다.

## <a name="secure-time-seeding"></a>시간 시드 보안
Server 2016에는 W32time 시간 시드 보안 기능을 포함 합니다. 이 기능은 나가는 SSL 연결에서 현재 대략적인 시간을 결정합니다.  시간 값이 로컬 시스템 시계를 모니터링 하 고 총 오류를 수정 됩니다. 자세한 내용은 기능에 대 한 [이 블로그 게시물](https://blogs.msdn.microsoft.com/w32time/2016/09/28/secure-time-seeding-improving-time-keeping-in-windows/)합니다.  신뢰할 수 있는 시간 원본 및 시간 오프셋에 대 한 모니터링을 포함 하는 잘 모니터링 대상된 컴퓨터의 배포에서는 시간 시드 보호 기능 사용을 대신 기존 인프라에 의존 하지 않고 선택할 수 있습니다. 

다음이 단계를 사용 하 여 기능을 비활성화할 수 있습니다.
...
2.  몇 가지 이유로 인해 즉시 컴퓨터를 다시 부팅 수 없는 경우 구성 업데이트에 대 한 W32time 서비스를 알릴 수 있습니다. 이 시간 모니터링을 중지 및 SSL 연결에서 수집 된 시간 데이터를 기반으로 적용 합니다. 

    W32tm.exe /config /update

3.  컴퓨터를 다시 부팅 설정을 효과적인 즉시 만들고 SSL 연결에서 시간 데이터 수집을 중지 해도 합니다.  두 번째 부분에는 매우 작은 오버 헤드가 및 성능 별로 중요 하지 않아야 합니다.

4.  전체 도메인에서이 설정을 적용할 그룹 정책 설정을 0으로 W32time의 UtilizeSSLTimeData 값을 설정 하 고 게시를 설정 하세요.  그룹 정책 클라이언트에서 설정을 선택, W32time 서비스 알리고 시간 모니터링 및 적용 SSL 시간 데이터를 사용 하 여 중지 됩니다. 각 컴퓨터를 다시 부팅 하는 경우 SSL 시간 데이터 수집 중지 됩니다. 이식 가능한 슬림 랩톱/태블릿 및 기타 장치 도메인이 있으면 이번 정책 변경을에서 이러한 컴퓨터를 제외 하는 것이 좋습니다. 이러한 장치 배터리가 발생할 결국 및 시간을 부트스트랩 하 기능 보안 시간 시드 작업을 해야 합니다.
