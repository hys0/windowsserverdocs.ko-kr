...
AD DS에 대 한 예제 구성 항목이 다음 표에 나와 있습니다.
...
##### <a name="BKMK_installDHCP"></a>동적 호스트 구성 프로토콜 (DHCP)를 설치 합니다.
...
### <a name="BKMK_joinlogclients"></a>클라이언트 컴퓨터를 도메인에 가입 및 로그온
...
##### <a name="to-join-computers-running-windows-10-to-the-domain"></a>도메인에 Windows 10을 실행 하는 컴퓨터를 가입 합니다.

1.  로컬 Administrator 계정을 사용하여 컴퓨터에 로그온합니다.

2.  **웹 및 Windows 검색**, 형식 **시스템**입니다. 검색 결과에서 클릭 **시스템 (제어판)** 합니다. **시스템** 대화 상자가 열립니다.

3.  **시스템**, 클릭 **고급 시스템 설정**합니다. **시스템 속성** 대화 상자가 열립니다. 클릭 합니다 **컴퓨터 이름** 탭 합니다.

4.  **컴퓨터 이름**, 클릭 **변경**합니다. **컴퓨터 이름/도메인 변경** 대화 상자가 열립니다.

5.  **컴퓨터 이름/도메인 변경** 의 **소속**, 클릭 **도메인**, 한 다음 가입할 도메인의 이름을 입력 합니다. 예를 들어 도메인 이름이 corp.contoso.com이면 **corp.contoso.com**을 입력합니다.
...
##### <a name="to-join-computers-running-windows-81-to-the-domain"></a>도메인에 Windows 8.1 실행 하는 컴퓨터를 가입 합니다.

1.  로컬 Administrator 계정을 사용하여 컴퓨터에 로그온합니다.

2.  마우스 오른쪽 단추로 클릭 **시작**를 클릭 하 고 **시스템**입니다. **시스템** 대화 상자가 열립니다.
...
##### <a name="to-log-on-to-the-domain-using-computers-running-windows-10"></a>Windows 10을 실행 하는 컴퓨터를 사용 하 여 도메인에 로그온 합니다.

1.  컴퓨터에서 로그오프하거나 컴퓨터를 다시 시작합니다.

2.  Ctrl+Alt+Del을 누릅니다. 로그온 화면이 표시됩니다.

3.  를 왼쪽 아래에서 클릭 **기타 사용자**합니다.
...
핵심 네트워크 설치 후 무선 액세스 지점이 나 VPN 서버와 같은 네트워크 액세스 서버를 배포 하려는 경우 NPS 및 웹 서버를 배포 하는 것이 좋습니다. 네트워크 액세스 배포의 경우 보안 인증서 기반 인증 방법을 사용하는 것이 좋습니다. NPS를 사용하여 네트워크 액세스 정책을 관리하고 보안 인증 방법을 배포할 수 있습니다. 웹 서버를 사용하여 보안 인증에 대한 인증서를 제공하는 CA(인증 기관)의 CRL(인증서 해지 목록)을 게시할 수 있습니다.

> [!NOTE]
> 핵심 네트워크 부록 가이드를 사용하여 서버 인증서 및 기타 추가 기능을 배포할 수 있습니다. 자세한 내용은 [추가 기술 리소스](#BKMK_resources)를 참조하십시오.

다음 그림에서는 추가 된 NPS 및 웹 서버를 사용 하 여 Windows Server 핵심 네트워크 토폴로지를 보여줍니다.

![추가 된 NPS 및 웹 서버를 사용 하 여 Windows Server 핵심 네트워크 토폴로지](../media/Core-Network-Guide/cng16_overview_2.jpg)
...
네트워크 정책 (NPS 서버)를 사용 하면 중앙에서 구성 하 고 다음과 같은 기능을 사용 하 여 네트워크 정책을 관리할 수 있습니다. 원격 인증 전화 접속 사용자 서비스 (RADIUS) 서버 및 RADIUS 프록시입니다.

NPS는 핵심 네트워크의 선택적 구성 요소이지만 다음 중 한 가지라도 해당하는 경우에는 NPS를 설치해야 합니다.

-   Windows Server 2016, Windows Server 2012 R2, Windows Server 2012, Windows Server 2008 R2 또는 Windows Server 2008을 실행 하는 컴퓨터와 같은 RADIUS 프로토콜과 호환 되는 원격 액세스 서버를 포함 하 여 네트워크를 확장 하려는 및 라우팅 및 원격 액세스 서비스, 터미널 서비스 게이트웨이 또는 원격 데스크톱 게이트웨이.


-   무선 또는 유선에 대 한 802.1x 인증을 배포 하려고 합니다.

이 역할 서비스를 배포 하기 전에 NPS를 구성 하는 컴퓨터에서 다음 단계를 수행 해야 합니다.
...
-   [기본 도메인에서 NPS 등록](#BKMK_registerNPS)

> [!NOTE]
> 이 가이드에서는 독립 실행형 서버에 NPS를 배포 하기 위한 지침을 제공 하거나 VM 이라는 NPS1 합니다.  다른 권장 되는 배포 모델은 도메인 컨트롤러에 NPS 설치 합니다. 독립 실행형 서버 대신 도메인 컨트롤러에 NPS를 설치 합니다. 원한다 면 DC1에 NPS를 설치 합니다.
...
- 사용자 계정 데이터베이스 계획을 세웁니다. 기본적으로 Active Directory 도메인에 NPS를 실행 하는 서버를 조인 하는 경우 NPS 인증 및 수행 AD DS 사용자 계정 데이터베이스를 사용 하 여 인증 합니다. NPS를 RADIUS 프록시로 사용하여 연결 요청을 다른 RADIUS 서버로 전달하는 대규모 네트워크와 같은 경우에는 도메인 구성원이 아닌 컴퓨터에 NPS를 설치할 수 있습니다.
...
추가 역할 및 기능 마법사를 사용 하 여 네트워크 정책 (NPS 서버)를 설치 하려면이 절차를 사용할 수 있습니다. NPS는 네트워크 정책 및 액세스 서비스 서버 역할의 역할 서비스입니다.
...
5.  **서버 역할 선택**의 **역할**를 선택 **네트워크 정책 및 액세스 서비스**합니다. 경우에 네트워크 정책 및 액세스 서비스에 필요한 기능을 추가 해야 한다는 묻는 대화 상자가 열립니다. **기능 추가**를 클릭한 후 **다음**을 클릭합니다.
...
##### <a name="BKMK_registerNPS"></a>기본 도메인에서 NPS 등록
...
> [!NOTE]
> Windows PowerShell 내에서 네트워크 셸 (Netsh) 명령을 사용 하 여이 절차를 수행를 열고 PowerShell 및 다음을 입력 한 다음 ENTER 키를 누릅니다.
>
...
네트워크 정책 서버에 대 한 자세한 내용은 참조 하세요. [네트워크 정책 (NPS 서버)](../technologies/nps/nps-top.md)합니다.

#### <a name="BKMK_IIS"></a>WEB1 배포

Windows Server 2016에서 웹 서버 (IIS) 역할 웹 사이트, 서비스 및 응용 프로그램을 안정적으로 호스팅하기 위한 안전, 관리 하기 쉬운, 모듈식 하 고 확장 가능한 플랫폼을 제공 합니다. 사용 하 여 인터넷 정보 서비스 (IIS), 인터넷, 인트라넷 또는 엑스트라넷에서 사용자를 사용 하 여 정보를 공유할 수 있습니다. IIS는 IIS, ASP.NET, FTP 서비스, PHP 및 Windows Communication Foundation (WCF)를 통합 하는 통합된 웹 플랫폼.

에서 도메인 구성원 컴퓨터에서 액세스에 대 한 CRL을 게시할 수 있으므로 하는 것 외에도 웹 서버 (IIS) 서버 역할 설정 및 여러 웹 사이트, 웹 응용 프로그램 및 FTP 사이트를 관리할 수 있습니다. IIS에는 또한 다음과 같은 이점을 제공합니다.
...
-   [웹 서버 (IIS) 서버 역할 설치](#BKMK_install_IIS)

##### <a name="BKMK_install_IIS"></a>웹 서버 (IIS) 서버 역할 설치
...
3.  에 **설치 유형 선택** 페이지에서 클릭 **다음**합니다.

4.  에 **대상 서버 선택** 페이지에서 로컬 컴퓨터를 선택한 다음 클릭 **다음**합니다.

5.  에 **서버 역할 선택** 페이지를 스크롤하여 선택한 **웹 서버 (IIS)** 합니다. 합니다 **웹 서버 (IIS)에 필요한 기능 추가** 대화 상자가 열립니다. **기능 추가**를 클릭한 후 **다음**을 클릭합니다.
...
-   [Active Directory Domain Services (AD DS)에서 Windows Server 2016의에서 새로운 기능](https://technet.microsoft.com/library/mt163897.aspx)

-   [Active Directory Domain Services 개요](https://technet.microsoft.com/library/hh831484.aspx) 에서 https://technet.microsoft.com/library/hh831484.aspx합니다.

-   [도메인 이름 시스템 (DNS) 개요](https://technet.microsoft.com/library/hh831667.aspx) 에서 https://technet.microsoft.com/library/hh831667.aspx합니다.

-   [DNS 관리자 역할 구현](https://technet.microsoft.com/library/cc756152(WS.10).aspx)

-   [동적 호스트 구성 프로토콜 (DHCP) 개요](https://technet.microsoft.com/library/hh831825.aspx) 에서 https://technet.microsoft.com/library/hh831825.aspx합니다.

-   [네트워크 정책 및 액세스 서비스 개요](https://technet.microsoft.com/library/hh831683.aspx) 에서 https://technet.microsoft.com/library/hh831683.aspx합니다.

-   [웹 서버 (IIS) 개요](https://technet.microsoft.com/library/hh831725.aspx) 에서 https://technet.microsoft.com/library/hh831725.aspx합니다.

## <a name="BKMK_appendix"></a>부록 A-E
다음 섹션에서는 Windows Server 2016, Windows 10, Windows Server 2012 및 Windows 8 이외의 운영 체제를 실행 하는 컴퓨터에 대 한 추가 구성 정보를 포함 합니다. 또한 배포에 도움이 되는 네트워크 준비 워크시트 제공 됩니다.
...
다른 컴퓨터 이름을 사용 하 여 Windows Server 2008 R2, Windows 7, Windows Server 2008 및 Windows Vista를 실행 하는 컴퓨터 수 있도록이 섹션의 절차를 사용할 수 있습니다.
...
2.  **컴퓨터 이름, 도메인 및 작업 그룹 설정**에서 **설정 변경**을 클릭합니다. **시스템 속성** 대화 상자가 열립니다.
    ...
    > [!NOTE]
    > 컴퓨터에서 Windows 7을 실행 하기 전에 합니다 **시스템 속성** 대화 상자가 열리고 합니다 **사용자 계정 컨트롤** 계속 하려면 권한이 요청 대화 상자가 열립니다. 계속하려면 **계속**을 클릭합니다.
...
2.  **컴퓨터 이름, 도메인 및 작업 그룹 설정**에서 **설정 변경**을 클릭합니다. **시스템 속성** 대화 상자가 열립니다.
    ...
    > [!NOTE]
    > 컴퓨터에서 Windows Vista를 하기 전에 실행 합니다 **시스템 속성** 대화 상자가 열리고를 **사용자 계정 컨트롤** 계속 하려면 권한이 요청 대화 상자가 열립니다. 계속하려면 **계속**을 클릭합니다.
...
도메인에 Windows Server 2008 R2, Windows 7, Windows Server 2008 및 Windows Vista를 실행 하는 컴퓨터를 가입 하려면 다음이 절차를 사용할 수 있습니다.
...
##### <a name="to-join-computers-running-windows-server-2008-r2-and-windows-7-to-the-domain"></a>도메인에 Windows Server 2008 R2 및 Windows 7을 실행 하는 컴퓨터를 가입 합니다.
...
##### <a name="to-join-computers-running-windows-server-2008-and-windows-vista-to-the-domain"></a>도메인에 Windows Server 2008 및 Windows Vista를 실행 하는 컴퓨터를 가입 합니다.
...
Windows Server 2008 R2, Windows 7, Windows Server 2008 및 Windows Vista를 실행 하는 컴퓨터를 사용 하 여 도메인에 로그온 할 때 이러한 절차를 사용할 수 있습니다.
...
3.  [(선택 사항) 네트워크 정책 서버 설치](#BKMK_FndtnPrep_InstallNPS)

### <a name="BKMK_FndtnPrep_InstallAD"></a>Active Directory Domain Services 및 DNS 설치
이 섹션의 표에서 Active Directory Domain Services (AD DS)의 사전 설치 및 설치에 대 한 구성 항목 및 DNS입니다.
...
|영역 종류<br /><br />**Active Directory에 영역 저장**|선택<br />-선택 되지 않음||
|Active Directory 영역 복제 범위|이 포리스트의 모든 DNS 서버 간<br />이 도메인의 모든 DNS 서버 간<br />이 도메인의 모든 도메인 컨트롤러 간<br />이 디렉터리 파티션의 범위에 지정 된 모든 도메인 컨트롤러 간||
...
### <a name="BKMK_FndtnPrep_InstallNPS"></a>(선택 사항) 네트워크 정책 서버 설치
...
Windows Server 핵심 네트워크 NPS 배포 절차에 대 한 구성 항목 [설치 서버 NPS (네트워크 정책)](#BKMK_installNPS) 하 고 [기본 도메인에서 NPS 등록](#BKMK_registerNPS)합니다.
