...
19. 인증의 1 단계에서 사용자 id는 마스크를 지정 하려면 **Id 개인 정보 사용**, 텍스트에서 및는 익명 id 이름을 입력 하거나 텍스트 상자를 비워 둡니다.
    ...
    > - EAP id 개인 정보는 빈 또는 익명 id를 특정 EAP 방법에서 제공 \(실제 id와에서 다른\) EAP id 요청에 대 한 응답에 전송 됩니다. PEAP 인증 과정에서 두 번 id를 보냅니다. 첫 번째 단계에서는 id는 일반 텍스트로 전송 됩니다 하 고이 id는 클라이언트 인증을 위해 없습니다 라우팅 용도로 사용 됩니다. 실제 id-인증에 사용 — 첫 번째 단계에서 설정 된 보안 터널 안에서 인증, 두 번째 단계 중에 전송 됩니다. 경우 **Id 개인 정보 사용** 확인란을 선택 하면 사용자 이름 텍스트 상자에 지정 된 항목으로 대체 됩니다. 예를 들어 **Id 개인 정보 사용** 선택 및 id 개인 정보 취급 방침 별칭 **익명** 텍스트 상자에 지정 됩니다. 실제 id 별칭을 사용 하 여 사용자에 대 한 <strong>jdoe@example.com</strong>, 첫 번째 단계 인증에서 전송 하는 id로 변경 됩니다 <strong>anonymous@example.com</strong>합니다. 라우팅 목적을 위해 사용 되는 첫 번째 단계 id의 realm 부분은 수정 되지 않습니다.  
