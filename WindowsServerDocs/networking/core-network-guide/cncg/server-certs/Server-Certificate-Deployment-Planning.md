...
-   [웹 서버의 가상 디렉터리의 이름과 위치를 계획](#bkmk_virtual)  
  
-   [웹 서버에 대 한 DNS 별칭 (CNAME) 레코드를 계획 합니다.](#bkmk_cname)  
  
-   [CAPolicy.inf의 구성 계획](#bkmk_capolicy)  
  
-   [C a 1에서 CDP 및 AIA 확장의 구성을 계획합니다](#bkmk_cdp)  
  
-   [CA와 웹 서버 간의 복사 작업을 계획](#bkmk_copy)  
  
-   [CA에서 서버 인증서 템플릿 구성 계획](#bkmk_template)  
...
## <a name="bkmk_virtual"></a>웹 서버의 가상 디렉터리의 이름과 위치를 계획  
...
## <a name="bkmk_cname"></a>웹 서버에 대 한 DNS 별칭 (CNAME) 레코드를 계획 합니다.  
...
-   **URL**합니다. 예제 CAPolicy.inf 파일의 URL은 **https://pki.corp.contoso.com/pki/cps.txt**합니다. 이 가이드의 웹 서버 이름이 w e b 1 및 pki DNS CNAME 리소스 레코드를가지고 때문입니다. 웹 서버는 또한 corp.contoso.com 도메인에 가입 됩니다. 또한 인증서 해지 목록을 저장 된 "pki" 라는 웹 서버의 가상 디렉터리가입니다. 값에에서 입력 한 URL에 대 한 가상 디렉터리에 사용자 CAPolicy.inf 파일 지점을 도메인에서 웹 서버에 있는지 확인 합니다.  
...
## <a name="bkmk_cdp"></a>C a 1에서 CDP 및 AIA 확장의 구성을 계획합니다  
...
## <a name="bkmk_copy"></a>CA와 웹 서버 간의 복사 작업을 계획  
...
## <a name="bkmk_template"></a>CA에서 서버 인증서 템플릿 구성 계획  
