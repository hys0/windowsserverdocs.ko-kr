...
![ 서버 관리자를 통해 데이터 중복 제거 설치: 서버 역할 @ no__t-1에서 데이터 중복 제거를 선택 합니다.
2. **설치** 단추가 활성화될 때까지 **다음**을 클릭한 후 **설치**를 클릭합니다.  
![ 서버 관리자를 통해 데이터 중복 제거 설치: install @ no__t-1을 클릭 합니다.
...
2. Nano Server가 아닌 다른 모드에서 Windows Server 2016을 실행하는 서버 또는 RSAT([원격 서버 관리 도구](https://www.microsoft.com/download/details.aspx?id=45520))가 설치된 Windows PC에서 Nano Server 인스턴스를 명확히 참조('MyNanoServer'를 Nano Server 인스턴스의 실제 이름으로 바꿈)하여 데이터 중복 제거를 설치합니다.  
    ...
    ```  
    <br />
    <strong>--또는--</strong>@NO__T 1 @ NO__T-2<br />
...
### <a id="enable-dedup-candidate-workloads"></a>데이터 중복 제거에 적합 한 작업을 결정 합니다.
...
### <a id="enable-dedup-evaluating-sometimes-workloads"></a>데이터 중복 제거를 위한 워크 로드 평가
> [!Important]  
...
1. **워크 로드의 데이터 집합이 중복 제거를 사용 하도록 설정 하는 데 충분 한 중복이 있나요?**  
...
2. @no__t 데이터 집합에 대 한 워크 로드의 i/o 패턴은 무엇 인가요? 내 작업에 대해 어떤 성능이 필요 한가요? **  
...
3. **서버에서 내 작업의 리소스 요구 사항은 무엇 인가요?**  
...
#### <a id="enable-dedup-via-server-manager"></a>서버 관리자를 사용 하 여 데이터 중복 제거 사용
1. 서버 관리자에서 **파일 및 저장소 서비스**를 선택합니다.  
@no__t 파일 및 저장소 서비스 @ no__t-1을 클릭 합니다.
2. **파일 및 저장소 서비스**에서 **볼륨**을 선택합니다.  
![ 볼륨 @ no__t-1 클릭
3. 원하는 볼륨을 마우스 오른쪽 단추로 클릭하고 **데이터 중복 제거 구성**을 선택합니다.  
@no__t 데이터 중복 제거 구성 @ no__t-1을 클릭 합니다.
4. 드롭다운 상자에서 원하는 **사용 유형**을 선택하고 **확인**을 선택합니다.  
@no__t 0 드롭다운에서 원하는 사용 유형을 선택 합니다.
...
#### <a id="enable-dedup-via-powershell"></a>PowerShell을 사용 하 여 데이터 중복 제거 사용
...
@no__t X 워크 로드에 대 한 데이터 집합에서 데이터 중복 제거를 실행 하려고 합니다. 지원 되나요? **  
...
**데이터 중복 제거에 대 한 메모리 요구 사항은 무엇 인가요?**  
