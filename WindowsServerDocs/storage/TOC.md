# [저장소](storage.md)
## [저장소의 새로운 기능](whats-new-in-storage.md)
...
### [검사 목록: DFS 네임스페이스 조정](dfs-namespaces/checklist-tune-a-dfs-namespace.md)
...
#### [Windows Server 2008 모드로 도메인 기반 네임스페이스 마이그레이션](dfs-namespaces/migrate-a-domain-based-namespace-to-windows-server-2008-mode.md)
...
### [DFS 네임스페이스 조정](dfs-namespaces/tuning-dfs-namespaces.md)
#### [조회 및 클라이언트 장애 복구(failback) 사용 또는 사용 안 함](dfs-namespaces/enable-or-disable-referrals-and-client-failback.md)
#### [클라이언트가 조회를 캐시하는 기간 변경](dfs-namespaces/change-the-amount-of-time-that-clients-cache-referrals.md)
#### [조회 대상 순서 지정 방법 설정](dfs-namespaces/set-the-ordering-method-for-targets-in-referrals.md)
#### [대상 우선 순위를 설정하여 조회 순서 지정 무시](dfs-namespaces/set-target-priority-to-override-referral-ordering.md)
...
#### [액세스 기반 열거와 함께 상속된 사용 권한 사용](dfs-namespaces/using-inherited-permissions-with-access-based-enumeration.md)
## [DFS 복제](dfs-replication/dfsr-overview.md)
### [SYSVOL 복제를 DFS 복제로 마이그레이션](dfs-replication/migrate-sysvol-to-dfsr.md)
### [robocopy를 사용하여 DFS 복제용 파일을 미리 시드](dfs-replication/preseed-dfsr-with-robocopy.md)
### [DFS 복제: 질문과 대답(FAQ)](dfs-replication/dfsr-faq.md)
...
### [SMB: 파일 및 프린터 공유 포트를 열어야 합니다.](file-server/best-practices-analyzer/smb-open-file-sharing-ports.md)
...
#### [전자 메일 알림 구성](fsrm/configure-email-notifications.md)
#### [알림 제한 구성](fsrm/configure-notification-limits.md)
#### [저장소 보고서 구성](fsrm/configure-storage-reports.md)
...
### [저장소 보고서 관리](fsrm/storage-reports-management.md)
#### [보고서 집합 예약](fsrm/schedule-set-of-reports.md)
...
### [원격 저장소 리소스 관리](fsrm/managing-remote-storage-resources.md)
...
### [폴더에서 오프라인 파일 비활성화](folder-redirection/disable-offline-files-on-folders.md)
### [항상 오프라인 모드 활성화](folder-redirection/enable-always-offline.md)
### [최적화된 폴더 이동 활성화](folder-redirection/enable-optimized-moving.md)
...
### [질문과 대답(FAQ)](storage-migration-service/faq.md)
### [알려진 문제](storage-migration-service/known-issues.md)

## [저장소 복제본](storage-replica/storage-replica-overview.md)
### [공유 저장소를 사용하여 확장 클러스터 복제](storage-replica/stretch-cluster-replication-using-shared-storage.md)
### [서버 간 저장소 복제](storage-replica/server-to-server-storage-replication.md)
### [클러스터 간 저장소 복제](storage-replica/cluster-to-cluster-storage-replication.md)
### [Azure의 교차 지역 클러스터 간 저장소 복제](storage-replica/cluster-to-cluster-azure-cross-region.md)
### [Azure의 동일 지역 내 클러스터 간 저장소 복제](storage-replica/cluster-to-cluster-azure-one-region.md)
### [저장소 복제본: 알려진 문제](storage-replica/storage-replica-known-issues.md)
### [저장소 복제본: 질문과 대답](storage-replica/storage-replica-frequently-asked-questions.md)
## [저장소 공간](storage-spaces/overview.md)
### [독립 실행형 서버에서 저장소 공간 배포](storage-spaces/deploy-standalone-storage-spaces.md)
### [성능 상태 및 작동 상태](storage-spaces/storage-spaces-states.md)
## [저장소 공간 다이렉트](storage-spaces/storage-spaces-direct-overview.md)
### 이해
#### [캐시 이해](storage-spaces/understand-the-cache.md)
#### [내결함성 및 저장소 효율성](storage-spaces/Storage-Spaces-Fault-Tolerance.md)
#### [드라이브 대칭 고려 사항](storage-spaces/drive-symmetry-considerations.md)
#### [저장소 재 동기화 이해 및 모니터링](storage-spaces/understand-storage-resync.md)
#### [클러스터 및 풀 쿼럼](storage-spaces/understand-quorum.md)
#### [클러스터 집합](storage-spaces/cluster-sets.md)
### 계획
#### [하드웨어 요구 사항](storage-spaces/storage-spaces-direct-hardware-requirements.md)
#### [CSV 인 메모리 읽기 캐시 사용하기](storage-spaces/csv-cache.md)
...
### 배포
#### [저장소 공간 다이렉트 배포](storage-spaces/deploy-storage-spaces-direct.md)
...
#### [저장소 공간 다이렉트 클러스터를 Windows Server 2019로 업그레이드](storage-spaces/upgrade-storage-spaces-direct-to-windows-server-2019.md)
### 관리
#### [Windows Admin Center로 관리](../manage/windows-admin-center/use/manage-hyper-converged.md)
...
#### [Azure Monitor로 모니터링](storage-spaces/configure-azure-monitor.md)
### 문제 해결
#### [문제 해결 시나리오](storage-spaces/troubleshooting-storage-spaces.md)
#### [성능 상태 및 작동 상태](storage-spaces/storage-spaces-states.md)
#### [데이터 수집](storage-spaces/data-collection.md)
#### [자주 묻는 질문](storage-spaces/storage-spaces-direct-faq.md)
...
### [AD FS 및 WAP(웹 응용 프로그램 프록시)를 사용하여 클라우드 폴더 배포](work-folders/deploy-work-folders-adfs-overview.md)
...
## [저장소 QoS](storage-qos/storage-qos-overview.md)
## [저장소 클래스 메모리 상태 관리](storage-spaces/Storage-class-memory-health.md)
## [저장소 항목 변경 기록](storage-change-history.md)
