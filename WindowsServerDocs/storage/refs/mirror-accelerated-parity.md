...
- 한편 패리티는 모든 쓰기에 대해 패리티를 다시 계산해야 하므로 임의 쓰기 성능이 저하됩니다. 그러나 패리티는 사용자가 더 공간 효율적으로 데이터를 저장할 수 있게 합니다. 자세한 내용은 참조 하세요. [저장소 공간 내결함성](../storage-spaces/Storage-Spaces-Fault-Tolerance.md)합니다.
...
**쓰기:** 들어오는 reFS 서비스의 세 가지 방법으로 작성 합니다.

1.  **미러를 기록합니다.**

    - **1a.** 들어오는 쓰기가 미러에서 기존 데이터를 수정할 경우 ReFS가 데이터를 바로 수정합니다.
...
2. **미러 서버, 패리티에서 다시 할당 씁니다.**

    들어오는 쓰기가 패리티에 있는 데이터를 수정하고 ReFS가 미러에서 들어오는 쓰기를 처리하기에 충분한 여유 공간을 찾을 수 있는 경우 ReFS가 먼저 패리티의 이전 데이터를 무효화한 다음 미러에 씁니다. 이 무효화는 패리티에 대한 쓰기 성능 향상에 도움이 되는 빠르고 저렴한 메타데이터 작업입니다.
...
**다음과 같습니다.** ReFS는 관련 데이터가 포함 된 계층에서 직접 읽습니다. 패리티가 HDD로 생성된 경우 저장소 공간 다이렉트의 캐시가 이 데이터를 캐시하여 이후 읽기를 가속화합니다. 
...
**쓰기:** 위에서 설명한 쓰기의 각 형식은 자체 성능 특성과 합니다. 대체로 미러 계층에 쓰기가 재할당된 쓰기보다 훨씬 더 빠르며, 재할당된 쓰기는 패리티 계층에 직접 쓰기보다 현저히 더 빠릅니다. 이 관계는 아래의 부등호로 설명됩니다. 


- **계층 미러 > 쓰기를 다시 할당 >> 패리티 계층**


**다음과 같습니다.** 패리티에서 읽을 때 의미 있는 음수 성능 영향을 주지 않습니다.
...
>[!IMPORTANT]
> 다양 한 하위 디렉터리에 쓰기 작업이 많은 Vhd를 배치 하는 것이 좋습니다. ReFS 디렉터리 및 파일 수준에서 메타 데이터 변경 기록 때문입니다. 따라서 디렉터리 쓰기 작업이 많은 파일을 배포 하는 경우 메타 데이터 작업은 더 작은 및 앱에 대 한 대기 시간 단축, 병렬로 실행 합니다.
...
### <a name="increasing-the-size-of-the-mirrored-tier"></a>미러링된 계층의 크기 증가
