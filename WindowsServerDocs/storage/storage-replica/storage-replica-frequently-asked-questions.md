...
Azure에서 게스트 클러스터링에 추가 정보에서 찾을 수 있습니다. [Microsoft Azure에서 IaaS VM 게스트 클러스터 배포](https://blogs.msdn.microsoft.com/clustering/2017/02/14/deploying-an-iaas-vm-guest-clusters-in-microsoft-azure/)합니다.
...
2. 저장소 복제본 기반 저장소 공간 다이렉트의 클러스터링에 대 한 Azure Resource Manager 템플릿은 [Azure 지역 간에 재해 복구를 위한 저장소 복제본을 사용 하 여 저장소 공간 다이렉트 (s2d) SOFS 클러스터 만들기](https://aka.ms/azure-storage-replica-cluster)합니다.  
...
기술적으로 가능하지만 SOFS에 연결된 계산 노드에서는 사이트 인식이 지원되지 않으므로 이는 Windows Server 2016에서 권장되는 구성이 아닙니다. 캠퍼스 거리 네트워킹에 일반적으로 대기 시간이 하위 밀리초를 사용 하는 경우이 구성은 일반적으로 문제 없이 작동 합니다.   
...
Windows Server 2016에서 `Grant-SRDelegation` cmdlet을 사용하면 됩니다. 로컬 관리자 그룹의 구성원이 아니더라도 복제를 만들거나 수정하거나 제거할 수 있는 권한이 있으므로 서버 간, 클러스터 간 및 확장 클러스터 복제 시나리오에서 특정 사용자를 설정할 수 있습니다. 예를 들어 다음과 같은 가치를 제공해야 합니다.  
