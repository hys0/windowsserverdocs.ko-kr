...
또한 범위가 더 넓고 대기 시간이 더 긴 네트워크에 대해 비동기 복제도 지원합니다. 하기 때문에 검사점 기반이 아니라 지속적으로 복제, 변경 내용의 스냅숏 기반 제품 보다 훨씬 작아야 하는 경향이 있습니다. 뿐만 아니라 저장소 복제본은 파티션 계층에서 작동하므로 Windows Server 또는 백업 소프트웨어에 의해 생성된 모든 VSS 스냅숏을 복제합니다. 따라서 지정 시간 복구에 응용 프로그램 일치 데이터 스냅숏을 사용하고, 특히 구조화되지 않은 사용자 데이터를 비동기식으로 복제할 수 있습니다.  
...
Windows Server 2019 (및 Windows Server 버전 1709)에 **테스트 장애 조치** cmdlet이 추가 되었습니다. 이 구문은 이제 일시적으로 백업에 대 한 대상 볼륨의 읽기 전용 스냅숏을 탑재, 테스트 등을 지원 합니다. 참조 https://aka.ms/srfaq 자세한 정보에 대 한 합니다.
