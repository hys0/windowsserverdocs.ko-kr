...
> [!IMPORTANT]
> 이 테스트에서는 4개의 서버를 사용합니다. 각 클러스터에 Microsoft에서 지원하는 서버를 원하는 만큼 사용할 수 있습니다. 현재는 저장소 공간 다이렉트 하나당 16개, 공유 저장소 클러스터 하나당 64개가 지원됩니다.  
>   
...
9. 다음과 같이 저장소를 구성합니다.  
    ...
    -   **저장소 공간 다이렉트의 경우**  
        ...
        1.  저장소 공간 다이렉트를 배포하여 각 클러스터가 해당 사이트의 저장소 엔클로저만 볼 수 있게 합니다. (https://docs.microsoft.com/ko-kr/windows-server/storage/storage-spaces/hyper-converged-solution-using-storage-spaces-direct) 
...
4.  파일 공유 감시 또는 클라우드 감시를 구성합니다.  
    ...
    > [!WARNING]  
    > 쿼럼 구성에 대한 자세한 내용은 [Windows Server2012 장애 조치(failover) 클러스터에서 쿼럼 구성 및 관리](http://technet.microsoft.com/library/jj612870.aspx)에서 **감시 구성** 섹션을 참조하세요. `Set-ClusterQuorum` cmdlet에 대한 자세한 내용은 [Set-ClusterQuorum](http://technet.microsoft.com/library/hh847275.aspx)을 참조하세요.  

5.  **Redmond** 사이트의 디스크 하나를 클러스터 CSV에 추가합니다. 이렇게 하려면 **저장소** 섹션의 **디스크** 노드에서 원본 디스크를 마우스 오른쪽 단추로 클릭한 다음 **클러스터 공유 볼륨에 추가**를 클릭합니다.  

6.  [스케일 아웃 파일 서버 구성](http://technet.microsoft.com/library/hh831718.aspx)의 지침을 사용하여 두 클러스터 모두에서 클러스터된 스케일 아웃 파일 서버를 만듭니다.  
...
3.  각 클러스터에서 도메인 컨트롤러 또는 일부 다른 독립 서버에서 호스트되는 공유를 가리키는 파일 공유 감시 또는 클라우드(Azure) 감시를 구성합니다. 예:  
...
1.  첫 번째 클러스터의 원하는 노드에서 또는 원격으로 **Grant-ClusterAccess** cmdlet을 실행하여 첫 번째 클러스터에 다른 클러스터에 대한 모든 권한을 부여합니다.  

    ```PowerShell
    Grant-SRAccess -ComputerName SR-SRV01 -Cluster SR-SRVCLUSB  
    ```  

2.  두 번째 클러스터의 원하는 노드에서 또는 원격으로 **Grant-ClusterAccess** cmdlet을 실행하여 두 번째 클러스터에 다른 클러스터에 대한 모든 권한을 부여합니다.  
