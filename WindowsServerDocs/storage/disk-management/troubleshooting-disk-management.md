...
디스크가 누락되거나 초기화에 실패하는 이유는 여러 가지가 있으며 일반적인 이유는 디스크에 장애가 있기 때문입니다. 오류가 발생한 디스크를 고칠 수 있는 방법은 아주 많지만, 다시 작동시킬 수 있을지 알아볼 수 있는 몇 가지 단계가 있습니다. 다음 단계 중 하나를 수행한 후에 디스크가 작동하면, 다음 단계를 진행할 필요 없이, 안심하고 기뻐하면서 백업을 업데이트해도 됩니다.
...
2. 디스크 관리에 디스크가 온라인(*Online*)으로 표시되고, 정상(*Healthy*)으로 나열된 주 파티션이 있으면(아래 그림 참조) 좋은 징후입니다.
    ...
    - 파티션에 파일 시스템이 있지만 드라이브 문자(예: E:)가 없는 경우 [드라이브 문자 변경](change-a-drive-letter.md)을 참조하여 드라이브 문자를 수동으로 추가할 수 있습니다.
    - 파일 시스템(NTFS, ReFS, FAT32 또는 exFAT)이 없고 디스크가 비어 있는 것이 확실하면 파티션을 마우스 오른쪽 단추로 클릭하고 **포맷**을 선택합니다. 디스크를 포맷하면 디스크의 데이터가 모두 지워집니다. 따라서 디스크에서 파일을 복구하려면 이 방법을 시도하지 마십시오. 대신 이 단계를 건너뛰고 다음 단계를 진행하세요.
3. 외장 디스크인 경우에는 디스크를 분리했다가 다시 연결한 다음, **작업** > **디스크 다시 검사**를 선택합니다. 
...
7. 디바이스 관리자에서 문제가 있는지 확인합니다.
    시작 단추를 길게(또는 마우스 오른쪽 단추) 누른 다음, 상황에 맞는 메뉴에서 디바이스 관리자를 선택합니다. 옆에 느낌표가 있거나 다른 문제가 있는 디바이스를 찾아서 디바이스를 두 번 클릭한 다음, 상태를 읽습니다.

    [디바이스 관리자의 오류 코드](https://support.microsoft.com/help/310123/error-codes-in-device-manager-in-windows) 목록이 있지만, 문제가 되는 디바이스를 마우스 오른쪽 단추로 클릭하고 **디바이스 제거**를 선택한 다음, **작업** > **하드웨어 변경 사항 검색**을 선택하는 방법이 있습니다.

    ![알 수 없는 USB 디바이스를 보여주는 디바이스 관리자](media/device-manager.PNG)
8. 디스크를 다른 PC에 꽂아봅니다.
    
    디스크가 다른 PC에서 작동하지 않으면, PC에 문제가 있는 게 아니라 디스크에 문제가 있을 가능성이 높다는 징후입니다. 새로운 내용은 아닙니다. [외부 USB 드라이브 오류 "논리 디스크 관리자가 디스크에 액세스하기 전에 디스크를 초기화해야 합니다."](https://social.technet.microsoft.com/Forums/windows/en-US/2b069948-82e9-49ef-bbb7-e44ec7bfebdb/forum-faq-external-usb-drive-error-you-must-initialize-the-disk-before-logical-disk-manager-can?forum=w7itprohardware)에서 시도해볼 수 있는 단계가 더 있지만, [Microsoft 커뮤니티](https://answers.microsoft.com/en-us/windows) 사이트를 검색하고 도움을 요청하거나 디스크 제조업체에 문의해야 할 수도 있습니다.
...
**해결 방법:**  디스크를 다시 검사하거나 컴퓨터를 다시 시작하고 디스크 상태가 변경되는지 확인합니다. [디스크의 상태가 초기화 안 됨이거나 디스크가 없음](#a-disks-status-is-not-initialized-or-the-disk-is-missing)에 설명되어 있는 문제 해결 단계도 시도해보세요.
...
**해결 방법:**  디스크의 데이터에 액세스할 수 있도록 디스크를 컴퓨터의 시스템 구성에 추가합니다. 디스크를 컴퓨터의 시스템 구성에 추가하려면 외부 디스크를 가져옵니다(디스크를 마우스 오른쪽 단추로 클릭한 다음, **외부 디스크 가져오기** 클릭). 디스크를 가져오면 외부 디스크의 기존 볼륨이 보이고 액세스가 가능해집니다. 
...
4. [디스크의 상태가 초기화 안 됨이거나 디스크가 없음](#a-disks-status-is-not-initialized-or-the-disk-is-missing)에 설명되어 있는 문제 해결 단계를 시도해봅니다.
5. 디스크 상태가 여전히 **오프라인**이고 디스크 이름이 여전히 **없음**인 경우 디스크에 복구할 수 없는 문제가 있다고 판단되면, 디스크를 마우스 오른쪽 단추로 클릭한 다음, **디스크 제거**를 클릭하여 시스템에서 디스크를 제거할 수 있습니다. 단, 디스크를 제거하기 전에 디스크의 모든 볼륨(또는 미러)을 삭제해야 합니다. 전체 볼륨 대신 미러를 제거하여 디스크의 미러 볼륨을 저장할 수 있습니다. 볼륨을 삭제하면 볼륨의 데이터가 손상됩니다. 따라서 디스크가 영구적으로 손상되어 사용할 수 없다고 확신하는 경우에만 디스크를 제거해야 합니다.

**오프라인 상태이지만 이름이 Disk \#(없음이 아님)인 디스크를 온라인으로 설정하려면 다음 절차 중 하나 이상을 시도합니다.**

1. 디스크 관리에서 디스크를 마우스 오른쪽 단추로 클릭하고 **디스크 다시 활성화**를 클릭하여 디스크를 다시 온라인 상태로 설정합니다. 디스크 상태가 여전히 **오프라인**인 경우 케이블 및 디스크 컨트롤러를 확인한 다음, 실제 디스크가 정상인지 확인합니다. 모든 문제를 수정하고 디스크를 다시 활성화합니다. 디스크 활성화에 성공하면 디스크의 모든 볼륨이 자동으로 **정상** 상태가 됩니다.
...
- [디스크의 상태가 초기화 안 됨이거나 디스크가 없음](#a-disks-status-is-not-initialized-or-the-disk-is-missing)에 설명되어 있는 문제 해결 단계를 시도해봅니다.
