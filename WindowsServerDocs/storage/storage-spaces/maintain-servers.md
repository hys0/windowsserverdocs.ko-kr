...
불완전 하거나 작동 상태 저하 경우 일반 노드를 종료 하 고 있고 시작/중지 클러스터 노드에서 서비스 염려할 필요가 없습니다. 모든 볼륨이 온라인 상태로 유지되며 액세스 가능합니다.
...
서버를 다시 시작할 때 사용할 수 없습니다 하는 동안 발생 한 모든 새로운 쓰기 다시 동기화 해야 합니다. 이 작업은 자동으로 수행됩니다. 지능형 변경 추적을 사용하면 *모든* 데이터를 스캔하거나 동기화할 필요 없이 변경 내용만 처리하면 됩니다. 프로덕션 워크로드에 미치는 영향을 줄이기 위해 이 프로세스가 제한됩니다. 서버가 일시 중지된 시간과 새로 쓰여진 데이터의 양에 따라 작업이 완료될 때까지 다소 시간이 걸릴 수 있습니다.
...
## <a name="how-to-update-storage-spaces-direct-nodes-offline"></a>저장소 공간 다이렉트 노드를 오프 라인으로 업데이트 하는 방법
경로에 다음 단계를 저장소 공간 다이렉트 시스템을 신속 하 게 사용 합니다. 유지 관리 기간을 예약 하 고 패치를 위해 시스템을 하곤 야 합니다. 가 필요한 중요 보안 업데이트를 신속 하 게 적용 또는 패치 유지 관리 창에서 완료 해야 하는 경우이 메서드를 수 있습니다. 이 프로세스는 저장소 공간 다이렉트 클러스터 패치를 하며 다시 모두 완벽 하 게 합니다. 대신 호스트 된 리소스의 가동 중지 시간이 됩니다.

1. 프로그램 유지 관리 기간을 계획 합니다.
2. 가상 디스크를 오프 라인으로 전환 합니다.
3. 클러스터를 오프 라인으로 저장소 풀을 중지 합니다. 실행 된 **중지 클러스터** cmdlet 또는 장애 조치 클러스터 관리자를 사용 하 여 클러스터를 중지 합니다.
4. 클러스터 서비스를 설정 합니다 **사용 안 함** 각 노드에서 Services.msc에서 합니다. 이렇게 하면 시작 되 고 패치 하는 동안 클러스터 서비스 됩니다.
5. Windows Server 누적 업데이트를 적용 하 고 모든 노드에 필요한 서비스 스택 업데이트 합니다. (모든 노드에서 동시에 다운 클러스터 이므로 기다릴 필요가 없습니다 업데이트할 수 있습니다).  
6. 노드를 다시 시작한 것을 확인 합니다.
7. 클러스터 서비스를 다시 설정 **자동** 각 노드에서 합니다.
8. 클러스터를 시작 합니다. 실행 합니다 **시작 클러스터** cmdlet 또는 장애 조치 클러스터 관리자를 사용 합니다. 

   몇 분 정도 지정 합니다.  저장소 풀 정상 인지 확인 합니다.
9. 가상 디스크를 다시 온라인 상태로 전환 합니다.
10. 실행 하 여 가상 디스크의 상태를 모니터링 합니다 **Get-volume** 하 고 **Get-virtualdisk** cmdlet.


## <a name="see-also"></a>참조

- [저장소 공간 다이렉트 개요](storage-spaces-direct-overview.md)
- [클러스터 인식 업데이트 (CAU)](https://technet.microsoft.com/library/hh831694.aspx)
