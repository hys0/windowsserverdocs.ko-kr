...
3. 아래 명령 집합을 사용 하 여 클러스터 집합에 클러스터 서버를 추가 합니다.
...
8. 클러스터 집합 생성 프로세스에서 각 클러스터 구성원의 CSV 볼륨에 대 한 인프라 SOFS의 인프라 파일 서버 이름 및 Volume1으로 식별 되는 하나의 SMB 공유를 만들었는지 확인 하려면 다음을 수행 합니다.

    ```PowerShell
    Get-SmbShare -CimSession CSMASTER
    ```

9. 검토를 위해 클러스터 집합 디버그 로그를 수집할 수 있습니다. 모든 구성원 및 관리 클러스터에 대해 클러스터 집합과 클러스터 디버그 로그를 모두 수집할 수 있습니다.

    ```PowerShell
    Get-ClusterSetLog -ClusterSetCimSession CSMASTER -IncludeClusterLog -IncludeManagementClusterLog -DestinationFolderPath <path>
    ```

10. 모든 클러스터 집합 구성원 사이에서 Kerberos [제한 위임을](https://techcommunity.microsoft.com/t5/virtualization/live-migration-via-constrained-delegation-with-kerberos-in/ba-p/382334) 구성 합니다.

11. 클러스터 집합의 각 노드에서 클러스터 간 가상 머신 실시간 마이그레이션 인증 유형을 Kerberos로 구성 합니다.

    ```PowerShell
    foreach($h in $hosts){ Set-VMHost -VirtualMachineMigrationAuthenticationType Kerberos -ComputerName $h }
    ```

12. 클러스터 집합의 각 노드에 있는 로컬 관리자 그룹에 관리 클러스터를 추가 합니다.
...
메모리, CPU 용량 또는 디스크 공간이 부족 하 여 가상 컴퓨터를 추가할 수 없는 경우 다음 오류가 표시 됩니다.

```
Get-ClusterSetOptimalNodeForVM : A cluster node is not available for this operation.
```

가상 머신이 만들어지면 지정 된 특정 노드에 Hyper-v 관리자에 표시 됩니다. 클러스터 집합 가상 머신으로 추가 하 고 클러스터에 추가 하려면 아래 명령을 사용 합니다.
...
기존 가상 컴퓨터를 사용 하 여 클러스터를 추가한 경우에는 가상 컴퓨터도 클러스터 집합에 등록 해야 합니다. 모든 가상 컴퓨터를 한 번에 등록 하려면 다음 명령을 사용 합니다.

```PowerShell
Get-ClusterSetMember -Name CLUSTER3 -CimSession CSMASTER | Register-ClusterSetVM -RegisterAll -CimSession CSMASTER
```

그러나 가상 컴퓨터의 경로를 클러스터 집합 네임 스페이스에 추가 해야 하므로 프로세스가 아직 완료 되지 않았습니다.

예: 기존 클러스터를 추가 하 고 로컬 클러스터 공유 볼륨 (CSV)에 있는 미리 구성 된 가상 컴퓨터를 포함 합니다. VHDX에 대 한 경로는 "C:\ClusterStorage\Volume1\MYVM\Virtual Hard Disks\MYVM.vhdx"와 비슷합니다. CSV 경로는 단일 구성원 클러스터에 로컬로 설계 되므로 저장소 마이그레이션을 수행 해야 하므로 구성원 클러스터 간에 실시간으로 마이그레이션된 후에는 가상 컴퓨터에 액세스할 수 없습니다.
