...
ms.locfileid: "4678602"
---
# 저장소 공간 다이렉트에 서버 또는 드라이브 추가
...
![4-노드 클러스터에 서버를 추가 하는 애니메이션](media/add-nodes/Scaling-Out.gif)
...
### 서버 3개에서 4개로: 이중 패리티 잠금 해제

![3-노드 클러스터에 네 번째 서버를 추가합니다.](media/add-nodes/Scaling-3-to-4.png)
...
![추가 보여 주는 애니메이션 하는 시스템 드라이브](media/add-nodes/Scale-Up.gif)
...
## 드라이브 또는 서버를 추가한 후 드라이브 사용량을 최적화

시간이 지남에 따라 드라이브 추가 또는 제거 풀의 드라이브 간에 데이터의 수 분포가 균일 합니다. 경우에 따라이 풀의 다른 드라이브 가득 동안 차게 특정 드라이브에서 발생할 수 있습니다.

풀에서 드라이브 할당을 유지 하려면 저장소 공간 다이렉트 자동으로 사용을 최적화 드라이브 (공유 SAS 엔클로저의 사용 하는 저장소 공간 시스템에 대 한 수동 프로세스는) 풀에 서버 또는 드라이브를 추가한 후 합니다. 최적화 풀에 새 드라이브를 추가한 후 15 분을 시작 합니다. 풀 최적화 걸리는 시간 또는 일을 완료 하려면 큰 하드 드라이브를 사용 하는 경우에 특히 하므로 우선 순위가 낮은 백그라운드 작업으로 실행 됩니다.

-하나의 라는 *최적화* 및 하나의 라는 *균형을 다시 조정* -두 개의 작업을 사용 하 여 최적화 하 고 다음 명령 사용 하 여 진행 상황을 모니터링할 수 있습니다.

```powershell
Get-StorageJob
```

수동으로 [Optimize-storagepool](https://docs.microsoft.com/powershell/module/storage/optimize-storagepool?view=win10-ps) cmdlet 사용 하 여 저장소 풀을 최적화할 수 있습니다. 예를 들면 다음과 같습니다.
