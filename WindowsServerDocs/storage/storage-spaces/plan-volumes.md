...
## <a name="review-what-are-volumes"></a>검토: 볼륨은 무엇 인가요

볼륨은 Hyper-v 가상 컴퓨터용 VHDX 파일을 VHD와 같은 워크 로드에 필요한 파일을 배치 하는 위치입니다. 볼륨은 저장소 풀의 드라이브를 결합하여 저장소 공간 다이렉트의 내결함성, 확장성 및 성능 이점을 도입합니다.
...
볼륨의 수를 클러스터에 있는 서버 수의 배수로 만드는 것이 좋습니다. 예를 들어 4 명의 서버를 설정한 경우에 3 또는 5를 사용 하 여 보다 총 4 개의 볼륨을 사용 하 여 더 일관 된 성능을 발생할 수 있습니다. 이를 통해 클러스터는 서버 간에 볼륨 "소유권"(각 볼륨에 대해 하나의 서버가 메타 데이터 오케스트레이션을 처리함)을 고르게 배포할 수 있습니다.

볼륨의 총 수를 제한 하는 것이 좋습니다.

| Windows Server 2016          | Windows Server 2019          |
|------------------------------|------------------------------|
| 클러스터당 최대 32 개 볼륨 | 클러스터당 최대 64 개 볼륨 |

## <a name="choosing-the-filesystem"></a>파일 시스템 선택

저장소 공간 다이렉트에 새로운 [ReFS(복원 파일 시스템)](../refs/refs-overview.md)을 사용하는 것이 좋습니다. ReFS는 가상화를 위해 구축된 뛰어난 파일 시스템으로, 인상적인 성능 가속화 및 데이터 손상에 대한 기본 보호 기능을 제공합니다. Windows Server 버전 1709 이상에 데이터 중복 제거를 포함 하는 거의 모든 키 NTFS 기능을 지원 합니다. 참조를 참조 하세요 [기능 비교 표](../refs/refs-overview.md#feature-comparison) 세부 정보에 대 한 합니다.
...
클러스터의 두 서버를 사용 하 여 양방향 미러링을 사용할 수 있습니다. Windows Server 2019를 실행 하는 경우에 중첩 된 복원 력을 사용할 수 있습니다.

양방향 미러링을 각 서버에 있는 드라이브에서 모든 데이터의 사본을 두 개를 하나의 복사본만 유지합니다. 해당 저장소 효율성은 50%-1TB 데이터를 작성 하려면 적어도 2TB의 저장소 풀의 실제 저장소 용량입니다. 양방향 미러링을 (하나의 서버 또는 드라이브)에 한 번에 하나의 하드웨어 오류 안전 하 게 허용할 수 있습니다.

![양방향 미러](media/plan-volumes/two-way-mirror.png)

중첩 된 복원 력 (Windows Server 2019 에서만 사용 가능) 양방향 미러링을 사용 하 여 서버 간에 데이터 복원 력을 제공 다음 양방향 미러링 또는 패리티 미러 가속을 사용 하 여 서버 내에서 복원 력을 추가 합니다. 서버 다시 시작 되었거나 사용할 수 없는 경우에 데이터 복원 력을 제공 하는 중첩 합니다. 해당 저장소 효율성 중첩 양방향 미러링의 50.0 25%에서 약 35 ~ 40% 중첩된 미러 가속 패리티의 경우 안전 하 게 중첩 된 복원 력 (두 드라이브 또는 서버 및 나머지 서버에 있는 드라이브)에 한 번에 두 개의 하드웨어 오류를 허용할 수 있습니다. 이 추가 된 데이터 복원 력으로 인해 좋습니다 중첩 된 복원 력을 사용 하 여 두 서버 클러스터의 프로덕션 배포에 Windows Server 2019를 실행 하는 경우. 자세한 내용은 참조 하세요. [중첩 된 복원 력](nested-resiliency.md)합니다.

![중첩 된 미러 가속 패리티](media/nested-resiliency/nested-mirror-accelerated-parity.png)

### <a name="with-three-servers"></a>서버 세 개가 있는 경우

서버 세 개가 있는 경우 더 나은 내결함성과 성능을 위해 3방향 미러링을 사용해야 합니다. 3방향 미러링은 모든 데이터의 복사본 세 개를 각 서버에 있는 드라이브에 하나씩 유지합니다. 저장소 효율성은 33.3%입니다. 즉, 1TB의 데이터를 기록하려면 저장소 풀에 최소 3TB의 물리적 저장소 용량이 필요합니다. 3방향 미러링은 [한 번에 두 개 이상 하드웨어 문제(드라이브 또는 서버)](storage-spaces-fault-tolerance.md#examples)를 안전하게 허용할 수 있습니다. 노드 2 개를 사용할 수 없게 하는 경우 저장소 풀 디스크 2/3을 사용할 수 없는 가상 디스크에 액세스할 수 없습니다 때문 쿼럼을 손실 됩니다. 그러나 노드 다운 될 수 있습니다 다른 노드에서 하나 이상의 디스크 실패할 수 있습니다 및 가상 디스크를 온라인 상태로 유지 됩니다. 예를 들어 갑자기 또 다른 드라이브나 서버에 장애가 발생할 때 서버 하나를 재부팅하는 경우 모든 데이터가 안전하고 지속적으로 액세스할 수 있는 상태로 유지됩니다.

![3방향 미러](media/plan-volumes/three-way-mirror.png)

### <a name="with-four-or-more-servers"></a>네 개 이상의 서버가 있는 경우

4 개 이상의 서버를 사용 하 여 선택할 수 있습니다 각 볼륨에 대 한 것인지 ("지우기 코딩"), 이중 패리티 3 방향 미러링을 사용 하 여 또는 미러 가속 패리티를 사용 하 여 두 가지입니다.

이중 패리티는 3방향 미러링과 동일한 내결함성을 제공하지만 저장소 효율성은 더 높습니다. 4 대의 서버를 사용 하 여 해당 저장소 효율성은 50.0%—to 2TB 저장 데이터의 저장소 풀의 실제 저장소 용량이 4TB 해야 합니다. 7개의 서버를 통해 저장소 효율성을 66.7%로 높이고 최대 80.0%의 저장소 효율성을 유지합니다. 패리티 인코딩은 계산을 많이 사용하기 때문에 성능이 제한될 수 있습니다.

![이중 패리티](media/plan-volumes/dual-parity.png)

어떤 복원력 유형을 사용할 것인지 여부는 작업의 요구 사항에 따라 다릅니다. 어떤 워크 로드는 적합 한 각 복원 유형 뿐만 아니라 복원 력 유형별로의 성능 및 저장소 효율성을 요약 한 표는 다음과 같습니다.
...
결과로 나타나는 저장소 효율성은 선택한 비율에 따라 달라집니다. [이 데모](https://www.youtube.com/watch?v=-LK2ViRGbWs&t=36m55s)에서 몇 가지 예를 참조하세요.
   ...
   > [!TIP]
   > 데이터 수집을 통해 갑작스러운는 쓰기 성능이 저하에만 관찰 되는 경우 미러 부분 충분 한지 또는 패리티 미러 가속 사용 사례에 적합 하지는 나타낼 수 있습니다. 예를 들어 경우 400 MB/s에서 성능 감소를 40 MB/s 쓸, 미러 부분을 확장 하거나 3 방향 미러로 전환 합니다.
...
| 최대 32TB         | 최대 64TB         |
   ...
   > [!TIP]
   > 볼륨 섀도 복사본 서비스 (VSS) 및 Volsnap 소프트웨어 공급자를 사용 하는 백업 솔루션을 사용 하는 경우-파일 서버 워크 로드를 사용 하 여 흔히-성능 및 안정성 향상 됩니다 10TB 볼륨 크기를 제한 합니다. 최신 Hyper-V RCT API 및/또는 ReFS 블록 복제 및/또는 기본 SQL 백업 API를 사용하는 백업 솔루션은 최대 32TB 이상까지 제대로 작동합니다.
