...
ms.locfileid: "59849894"
---
# <a name="get-connected-in-windows-server-essentials"></a>Windows Server Essentials에서 연결

>적용 대상: Windows Server 2016 Essentials, Windows Server 2012 R2 Essentials, Windows Server 2012 Essentials
  
 Connector 소프트웨어를 사용하여 컴퓨터를 Windows Server Essentials에 연결할 수 있습니다. Connector 소프트웨어는 서버에 컴퓨터 연결 마법사를 사용하여 서버에 컴퓨터를 연결할 때 설치됩니다. 입력 하 여이 마법사를 시작할 수 있습니다 **http://<servername\>/connect**여기서 **< servername\>**  서버의 이름입니다.  
...
-   [네트워크에 Mac 컴퓨터를 연결 하기 위한 필수 구성 요소](Get-Connected-in-Windows-Server-Essentials.md#BKMK_3)  
  
-   [클라이언트 컴퓨터에 지원 되는 운영 체제](Get-Connected-in-Windows-Server-Essentials.md#BKMK_4)  
  
-   [클라이언트 컴퓨터에 변경 내용](Get-Connected-in-Windows-Server-Essentials.md#BKMK_5)  
...
###  <a name="BKMK_1"></a> Connector 소프트웨어 개요  
...
-   컴퓨터의 날짜 및 시간 설정은 서버의 설정에 맞춰집니다.  
...
###  <a name="BKMK_3"></a> 네트워크에 Mac 컴퓨터를 연결 하기 위한 필수 구성 요소  
...
###  <a name="BKMK_4"></a> 클라이언트 컴퓨터에 지원 되는 운영 체제  
...
#### <a name="windows-server-essentials"></a>Windows Server Essentials  
  이 섹션에는 Windows Server Essentials Experience 역할이 설치 된 Windows Server 2012 R2 Standard 또는 Windows Server 2012 R2 Datacenter 실행 서버 또는 Windows Server Essentials 실행 서버에 적용 됩니다. 다음 컴퓨터 운영 체제가 지원됩니다.  
  
 **Windows 7 운영 체제**  
  
-    Windows 7 Home Basic SP1 (x 86 및 x64)  
...
> [!NOTE]
>  Windows Server Essentials 대시보드에서 Mac 컴퓨터에 대 한 백업 상태를 볼 수 있습니다. 그러나 대시보드에서 컴퓨터 백업을 구성하거나 백업을 시작할 수 없습니다. 또한 원격 웹 액세스를 사용하여 Mac 컴퓨터에 연결할 수 없습니다.  
  
#### <a name="windows-server-essentials"></a>Windows Server Essentials  
  이 섹션에서는 Windows Server Essentials를 실행 하는 서버에 적용 됩니다. 다음 컴퓨터 운영 체제가 지원됩니다.  
...
###  <a name="BKMK_5"></a> 클라이언트 컴퓨터에 변경 내용  
...
-   컴퓨터 바탕 화면에 대시보드 및 실행 패드의 바로 가기를 만듭니다.  
...
-   클라이언트 컴퓨터에서 VPN을 배포 하 고 VPN 기능이 Windows Server essentials를 사용 하도록 설정할지 또는 Windows Server Essentials에서 VPN 기능을 사용 하는 경우 자동 연결 환경을 제공합니다 하는 경우 한 번 클릭 환경을 제공합니다  
...
-   네트워크에서 일상적인 작업 모니터링  
 ...
 Windows Server Essentials, Windows Server Essentials 및 Windows Server Essentials Experience 역할이 설치 된 Windows Server 2012 R2에서 네트워크 관리자를 할당할 수 있습니다 수준을 사용자 계정에 액세스 합니다. 네트워크 관리자 작업을 수행하는 데 필요한 사용 권한이 부여됩니다. 사용자에게 네트워크 관리자 액세스 수준이 할당되면 관리자 사용 권한이 필요한 작업에 대한 **사용자 액세스 제어** 프롬프트가 열립니다.  
...
-   [컴퓨터 데이터 및 설정을 이동 수동으로](Get-Connected-in-Windows-Server-Essentials.md#BKMK_12)  
  
-   [컴퓨터를 배포 하는 동안 여러 사용자 프로필 전송](Get-Connected-in-Windows-Server-Essentials.md#BKMK_Transfer)  
  
-   [Connector 소프트웨어 제거](Get-Connected-in-Windows-Server-Essentials.md#BKMK_13)  
  
-   [컴퓨터 연결 끊기 또는 서버에 컴퓨터를 다시 연결](Get-Connected-in-Windows-Server-Essentials.md#BKMK_14)  
  
-   [백업 절전 모드를 사용 하 여 작동 방법과 최대 절전 모드](Get-Connected-in-Windows-Server-Essentials.md#BKMK_Sleep)  
...
###  <a name="BKMK_9"></a> 서버에 컴퓨터 연결  
 Windows Server Essentials를 실행 하는 서버 또는 Windows Server 2012 R2에 Windows Server Essentials Experience 역할이 설치 된 컴퓨터를 연결한 경우 클라이언트 컴퓨터가 인터넷에 올바르게 연결 되어 있는지 확인 합니다.  
...
3.  주소 표시줄에 입력 **http://<servername\>/connect**, 한 다음 Enter를 누릅니다.  
    ...
    > [!NOTE]
    >  컴퓨터가 Windows Server Essentials 네트워크 외부의 원격 위치에 있는 경우 서버 마법사를 연결 하는 컴퓨터를 실행 하려면 입력 **http://<domainname\>/connect** 웹 브라우저 (사용자의 주소 표시줄에서 여기서 < 도메인\> 은 조직의 도메인 이름). 도메인 이름 정보에 대해서는 네트워크 관리자에게 문의하세요.  
...
7.  서버에 컴퓨터 연결 마법사가 나타납니다. 마법사를 완료하려면 다음을 수행합니다.  
    ...
    4.  컴퓨터는 Windows 8, Windows 8.1 또는 Windows 10를 실행 하는 경우이 단계를 건너뜁니다. 컴퓨터에서 Windows 7을 실행 중인 경우 컴퓨터를 새 네트워크에 연결한 후에 문서, 사진 또는 개인 기본 설정(예: 바탕 화면 배경, 화면 보호기, Internet Explorer 즐겨찾기)을 보존하려면 마법사의 **기존 데이터 및 설정에 대한 이동 여부를 선택하세요.** 페이지에서 **데이터 및 설정을 새 네트워크 사용자 계정으로 이동합니다.** 를 선택합니다.  
...
9. 컴퓨터는 서버에 연결 (Windows 8, Windows 8.1 또는 Windows 10, 대시보드 및 Connector 컴퓨터에서 실행 중인 경우 다음과 같이 사용할 수 있는 시작 메뉴에 Connector TrayApp 및 서버 대시보드의 바로 가기가 표시 TrayApp를 사용할 수는 컴퓨터의 시작 화면에서):  
  
    -   Windows 8, Windows 8.1 또는 Windows 10 컴퓨터에서 실행 중인, 대시보드 및 Connector TrayApp을 앱으로 검색할 됩니다.  
...
###  <a name="BKMK_10"></a> 도메인에 가입 하지 않고 Windows Server Essentials 서버에 컴퓨터 연결  
 이 항목에서는 Windows Server Essentials 네트워크에 컴퓨터 온-프레미스 클라이언트 배포에서는 Windows Server Essentials 도메인에 가입시 키 지 않고 Windows 7, Windows 8, Windows 8.1 또는 Windows 10 컴퓨터를 추가 하는 방법을 설명 합니다. 이 연결 메서드는 Windows Server Essentials 및 Windows Server Essentials에서 지원 됩니다.  
...
-   컴퓨터가 도메인에 가입 되어야 하는 모든 기능? 도메인 자격 증명, 그룹 정책 및 가상 사설망 (Vpn)을 포함 하 여?를 사용할 수 없습니다.  
...
2.  관리자 권한으로 명령 프롬프트를 엽니다.  
    ...
    -   Windows 10에서를 클릭 합니다 **시작** 단추를 선택 **모든 앱** -> **Windows 시스템 도구** -> **명령 프롬프트**명령 프롬프트를 마우스 오른쪽 단추로 클릭 한 다음 클릭 **관리자 권한으로 실행**합니다.  
  
    -   Windows 8에서에 **시작** 페이지에서 입력 **명령** 한 다음 Enter를 누릅니다. 결과에서 **명령 프롬프트**를 마우스 오른쪽 단추로 클릭하고 **관리자 권한으로 실행**을 클릭합니다.  
  
    -   Windows 7의 경우에 **시작** 메뉴에서 입력 **명령** 검색 상자에서 마우스 오른쪽 단추로 클릭 **명령 프롬프트**를 클릭 하 고 **관리자권한으로실행**.  
...
####  <a name="BKMK_SecondServer"></a> 두 번째 서버를 네트워크에 연결  
  
###### <a name="to-join-a-second-server-to-the-network"></a>네트워크에 두 번째 서버를 연결하려면  
  
1.  Windows Server Essentials 네트워크에 연결할 서버에 로그온합니다.  
  
2.  인터넷 브라우저를 열고 주소 표시줄에 입력 **http://<servername\>/connect**여기서 *< servername\>*  은 Windows Server를 실행 중인 서버의 이름 Essentials 및 Enter 키를 누릅니다.  
...
3.  Windows Server Essentials 네트워크에 연결하려는 서버에서 Internet Explorer 보안 강화 구성이 사용하도록 설정되어 있으면 다음을 완료하고, 그렇지 않으면 이 단계를 건너뜁니다.  
    ...
    2.  추가 된 **http://<servername\>/connect** 같이 신뢰할 수 있는 웹 사이트를 웹 사이트:  
    ...
    > [!NOTE]
    >  두 번째 서버를 Windows Server Essentials를 실행하는 서버에 연결하는 것은 클라이언트 컴퓨터에 연결하는 것과 다음과 같이 다릅니다.  
    >   
    >  -   사용자 프로필 마이그레이션이 없으므로 현재 프로필이 마이그레이션됩니다.  
    > -   클라이언트 컴퓨터 백업을 사용하여 두 번째 서버를 백업할 수 없고 백업을 위해 컴퓨터를 절전 모드에서 해제하는 옵션이 없습니다.  
...
-   두 번째 서버를 선택하고 **서버 속성 보기** 작업을 클릭하면 서버 속성 페이지에 **백업** 탭이 표시되지 않습니다.  
  
-   Windows Server 운영 체제에서 보안 센터가 없으므로 두 번째 서버의 보안 상태는 **적용할 수 없음**으로 표시됩니다.  
  
-   두 번째 서버의 그룹 정책 상태는 **적용할 수 없음**으로 표시됩니다.  
  
###  <a name="BKMK_11"></a> Connector 소프트웨어 설치  
 Windows Server Essentials의 Connector 소프트웨어는 서버에 컴퓨터 연결 마법사를 사용하여 서버에 컴퓨터를 연결할 때 설치됩니다. 입력 하 여이 마법사를 시작할 수 있습니다 **http://<ServerName\>/connect** 웹 브라우저의 주소 표시줄에서 (여기서 *< ServerName\>*  서버의 이름입니다).  
  
> [!NOTE]
>  컴퓨터가 원격 위치에 있는 경우 서버 마법사를 연결 하는 컴퓨터를 실행 하려면 입력 **http://<domainname\>/connect** 웹 브라우저의 주소 표시줄에서 (여기서 *< 도메인\>*  은 조직의 도메인 이름). 도메인 이름 정보에 대해서는 네트워크 관리자에게 문의하세요.  
...
1.  Windows 7, Windows 8, Windows 8.1 또는 Windows 10을 실행 중인 컴퓨터에서 엽니다 **Control Panel**, 한 다음는 **프로그램** 섹션에서 클릭 **설치 된 업데이트 보기**.  
...
5.  Windows Server Essentials에서 Windows Server Essentials Connector 페이지가 나타나면 실행 패드를 닫도록 제안, 클릭 **확인**합니다.  
  
6.  프로그램이 제거될 때까지 기다립니다. 소프트웨어가 제거되고 나면 설치된 프로그램 또는 업데이트 목록에 **Windows Server Essentials Connector**가 더 이상 나타나지 않습니다. 또한 실행 패드 및 대시보드의 바로 가기가 더 이상 컴퓨터 바탕 화면에 표시되지 않습니다.  
...
###  <a name="BKMK_14"></a> 컴퓨터 연결 끊기 또는 서버에 컴퓨터를 다시 연결  
...
3.  컴퓨터 연결 마법사를 사용하여 서버에 컴퓨터를 연결합니다. 단계별 지침에 대해서는 [Connect computers to the server](Get-Connected-in-Windows-Server-Essentials.md#BKMK_9)항목을 참조하세요.  
  
###  <a name="BKMK_Sleep"></a> 백업 절전 모드를 사용 하 여 작동 방법과 최대 절전 모드  
 서버에 컴퓨터를 연결할 때 **Wake This Computer for Backup** 옵션을 선택하면 컴퓨터를 백업할 수 있도록 매일 백업 일정에 지정된 대로 컴퓨터의 절전 또는 최대 절전 모드가 자동으로 해제됩니다. 백업이 완료되면 전원 관리 설정에 따라 컴퓨터가 절전 또는 최대 절전 모드로 돌아갑니다. 이 옵션을 선택하지 않으면 컴퓨터가 절전 또는 최대 절전 모드에 있을 경우 컴퓨터가 백업되지 않습니다. 자세한 내용은 [Manage Client Backup](../manage/Manage-Client-Computer-Backup-in-Windows-Server-Essentials.md)합니다.  
