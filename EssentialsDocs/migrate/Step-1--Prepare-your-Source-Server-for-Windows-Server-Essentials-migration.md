...
3.  [서비스 계정 설정으로 로그온 삭제](Step-1--Prepare-your-Source-Server-for-Windows-Server-Essentials-migration.md#BKMK_DeleteSvcAcctSetting)  
  
4.  [원본 서버의 상태 평가](Step-1--Prepare-your-Source-Server-for-Windows-Server-Essentials-migration.md#BKMK_EvaluateHealth)  
  
5.  [기간 업무 응용 프로그램 마이그레이션 계획 수립](Step-1--Prepare-your-Source-Server-for-Windows-Server-Essentials-migration.md#BKMK_MigrateLOB)  
...
2.  백업이 성공적으로 실행되었는지 확인합니다. 백업의 무결성을 테스트하려면 백업에서 임의 파일을 선택하여 대체 위치에 복원한 후 복원된 파일이 원본 파일과 같은지 확인합니다.  
   ...
   |Product|리소스|
   |---|---|
   |Windows Small Business Server 2003|[백업 및 복원 Windows Small Business Server 2003](https://msdn.microsoft.com/library/cc875809.aspx) 
   |Windows Small Business Server 2008|[백업 및 Windows Small Business Server 2008에서 데이터를 복원 합니다.](https://technet.microsoft.com/library/cc527505\(WS.10\).aspx)
   |Windows Server 2008 Foundation|[백업 및 복구](https://technet.microsoft.com/library/cc754097\(WS.10\).aspx)  
   |Windows Small Business Server 2011 Essentials|[서버 백업 설정에 대해 자세히 알아보기](https://technet.microsoft.com/library/server-backup-support-1.aspx)
   |Windows Small Business Server 2011 Standard|[서버 백업 관리](https://technet.microsoft.com/library/cc527488.aspx)  
   |Windows Server Essentials|[백업 관리에 Windows Server Essentials 및 복원](https://technet.microsoft.com/library/jj713536.aspx)
...
###  <a name="BKMK_DeleteSvcAcctSetting"></a> 서비스 계정 설정으로 로그온 삭제  
 Windows Small Business Server 2003 또는 Windows Server 2003에서 마이그레이션하는 경우 그룹 정책에서 **서비스로 로그온** 계정 설정을 삭제합니다.  
  
##### <a name="to-delete-the-log-on-as-a-service-account-setting"></a>서비스 계정 설정으로의 로그를 삭제 하려면  
...
1.  다음 표에는 원본 서버용 BPA(모범 사례 분석기)를 다운로드 및 설치할 수 있는 Microsoft 다운로드 센터 링크가 표시되어 있습니다.  
   ...
   |원본 서버에서 실행 중인 경우|BPA 도구에서 가져올 수 있습니다.|
   |---|---|
   ...
   |Windows SBS 2011 Essentials 또는 Windows SBS 2011 Standard|[Windows Server Solutions 모범 사례 분석기 웹 사이트](https://www.microsoft.com/download/details.aspx?id=15556) 
...
3.  탐색 창에서 **검색 시작**을 클릭합니다.  
    ...
    3.  에 <**서버** >**태스크** 창 클릭 **Best Practices Analyzer**합니다.  
...
###  <a name="BKMK_MigrateLOB"></a> 기간 업무 응용 프로그램 마이그레이션 계획 수립  
...
### <a name="create-a-plan-to-migrate-email-hosted-on-windows-sbs-2011-windows-sbs-2008-and-windows-sbs-2003"></a>Windows SBS 2011, Windows SBS 2008 및 Windows SBS 2003에서 호스트되는 메일 마이그레이션 계획 만들기  
 Windows SBS 2011, Windows SBS 2008 및 Windows SBS 2003에서 메일은 Microsoft Exchange Server를 통해 제공됩니다. 그러나 Windows Server Essentials는 받은 편지함 전자 메일 서비스를 제공 하지 않습니다. 현재 Windows SBS 2011, Windows SBS 2008 또는 Windows SBS 2003을 실행 하는 서버의 회사 전자 메일을 호스트를 사용할 경우 대체 온-프레미스를 마이그레이션해야 할 또는 호스팅 솔루션입니다.  
...
#### <a name="migrate-email-to-microsoft-office-365"></a>Microsoft Office 365로 메일 마이그레이션  
 Microsoft Office 365를 도메인의 메일 솔루션으로 사용하기로 선택한 경우 [컷오버 Exchange 마이그레이션을 사용하여 클라우드로 모든 사서함 마이그레이션](http://help.outlook.com/140/ms.exch.ecp.emailmigrationwizardexchangelearnmore.aspx) 의 지침에 따라 Office 365로의 메일 마이그레이션을 시작합니다. Windows Server Essentials를 설치 하기 전에 메일 마이그레이션을 완료 하는 것이 좋습니다.  
  
> [!NOTE]
>  원본 서버에서 온-프레미스 Exchange Server를 제거 하는 단계는 Windows Server Essentials와 Office 365 통합 하려는 경우 필수입니다. Exchange Server 공용 폴더를 Office 365로 마이그레이션하는 방법에 대한 자세한 내용은 블로그 게시물 [Office 365에 대한 Microsoft Exchange 2013 공용 폴더 마이그레이션 스크립트](http://blogs.technet.com/b/fmustafa/archive/2013/04/11/microsoft-exchange-2013-public-folders-migration-scripts-for-office-365.aspx)(영문)를 참조하세요.  
>   
>  실행 하 여 Windows Server Essentials에서 Office 365 통합 기능을 설정 해야 설치를 완료 합니다 **Microsoft Office 365와 통합** 작업 합니다.  
...
#### <a name="migrate-email-to-another-on-premises-exchange-server"></a>다른 온-프레미스 Exchange Server로 메일 마이그레이션  
 다른 전자 메일을 마이그레이션하는 방법에 대 한 내용은 온-프레미스 Exchange Server를 참조 하세요 [Windows Server Essentials를 사용 하 여 온-프레미스 Exchange Server 통합](https://technet.microsoft.com/library/jj200172.aspx)합니다. Windows Server Essentials를 설치 하 고 다음 원본 서버의 수준을 내리기 전에 메일 마이그레이션을 완료 한 후 새 온-프레미스 Exchange 서버를 설정 하는 것이 좋습니다.  
...
## <a name="next-steps"></a>다음 단계  
 Windows Server Essentials로의 마이그레이션을 위한 원본 서버를 준비 했습니다.  이제 [2 단계: Windows Server Essentials를 새 복제본 도메인 컨트롤러로 설치](Step-2--Install-Windows-Server-Essentials-as-a-new-replica-domain-controller.md)합니다.  
