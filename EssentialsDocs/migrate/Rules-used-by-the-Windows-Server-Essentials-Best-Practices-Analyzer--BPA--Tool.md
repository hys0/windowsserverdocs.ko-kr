...
이 문서에서의 Windows Server Essentials 분석기 BPA (모범 사례)를 사용 하는 규칙을 설명 합니다. BPA는 Windows Server Essentials를 실행 하 고 문제를 설명 하 고 문제 해결에 대 한 권장 사항을 제공 하는 보고서를 제공 하는 서버를 검사 합니다. 권장 사항은 Windows Server Essentials에 대 한 제품 지원 조직에서 개발 됩니다.  
  
## <a name="using-the-tool"></a>도구 사용  
 Windows Server 2011 Essentials, Windows Small Business Server 2011 Essentials, 또는 Windows Home Server 2011 마이그레이션을 마친 후 대상 서버에서 BPA를 실행 하려면 Windows Server Essentials로 마이그레이션하는 경우 표준 방법인 것에 설정 및 데이터를 제공 합니다. 언제든지 대시보드에서 도구를 실행할 수 있습니다.  
 ...
 **문제:** 서버에서 현재 IP 필터링이 사용되고 있습니다. IP 필터링을 사용하지 않도록 설정해야 합니다.  
  
 **영향:** IP 필터링을 사용하는 경우 네트워크 트래픽이 차단될 수 있습니다.  
 ...
 **영향:** 서버를 시작할 때 MSDTC 서비스가 자동으로 시작되지 않을 수 있습니다. 서비스를 중지하면 일부 SQL Server 또는 COM 함수가 실패할 수 있습니다. 따라서 Microsoft SQL Server 또는 COM 함수를 사용하는 응용 프로그램이 제대로 작동하지 않을 수도 있습니다.  
 ...
 **문제:**  Active Directory 웹 서비스가 기본 시작 모드인 자동으로 설정되어 있지 않습니다.  
  
 **영향:**  ADWS(Active Directory 웹 서비스)가 기본 시작 모드인 자동으로 설정되어 있지 않습니다. 서버에서 ADWS를 중지하거나 사용하지 않도록 설정하면 Windows PowerShell용 Active Directory 모듈 또는 Active Directory 관리 센터와 같은 클라이언트 응용 프로그램이 이 서버에서 실행 중인 디렉터리 서비스 인스턴스에 액세스하거나 관리할 수 없습니다. 자세한 내용은 참조 하세요. [AD DS의 새로운 기능: Active Directory 웹 서비스](https://technet.microsoft.com/library/dd391908\(WS.10\).aspx) (https://technet.microsoft.com/library/dd391908(WS.10).aspx) Windows Server 기술 라이브러리에서.  
...
##### <a name="to-configure-the-dhcp-client-service-to-start-automatically"></a>DHCP 클라이언트 서비스를 자동으로 시작되도록 구성하려면  
 ...
 **문제:** IIS Admin Service가 자동으로 시작되도록 구성되지 않았습니다.  
  
 **영향:** 서버를 시작할 때 IIS Admin Service가 자동으로 시작되지 않습니다. 이 서비스를 중지하면 원격 웹 액세스와 같은 서버에서 실행 중인 웹 사이트에 액세스하지 못할 수 있습니다.  
 ...
 **영향:**  서버를 시작할 때 World Wide Web Publishing 서비스가 자동으로 시작되지 않을 수 있습니다. 이 서비스를 중지하면 원격 웹 액세스와 같은 서버에서 실행 중인 웹 사이트에 액세스하지 못할 수 있습니다.  
 ...
 **영향:**  이 서비스를 중지하면 사용자가 원격 웹 액세스를 사용하여 컴퓨터에 액세스하지 못할 수 있습니다.  
 ...
 **영향:**  이 서비스를 중지하면 데이터 및 시간 동기화를 사용할 수 없습니다.  
 ...
 **문제:**  서버에서 MSDTC 서비스가 실행되고 있지 않습니다.  
  
 **영향:**  이 서비스를 중지하면 일부 SQL Server 또는 COM 함수가 실패할 수 있습니다. 따라서 Microsoft SQL Server 또는 COM 함수를 사용하는 응용 프로그램이 제대로 작동하지 않을 수도 있습니다.  
 ...
 **문제:**  서버에서 Netlogon 서비스가 실행되고 있지 않습니다.  
  
 **영향:**  서비스를 시작하지 않으면 서버가 사용자 및 서비스를 인증하지 않을 수 있습니다.  
 ...
 **문제:**  서버에서 DNS 클라이언트 서비스가 실행되고 있지 않습니다.  
  
 **영향:**  이 서비스를 시작하지 않으면 서버가 DNS 이름을 확인하지 못할 수 있습니다.  
 ...
 **문제:**  서버에서 DNS 서버 서비스가 실행되고 있지 않습니다.  
  
 **영향:**  DNS 서버 서비스를 시작하지 않으면 DNS 업데이트가 발생하지 않을 수 있습니다.  
 ...
 **문제:**  Active Directory 웹 서비스를 시작하지 않았습니다.  
  
 **영향:**  ADWS(Active Directory 웹 서비스)를 시작하지 않았습니다. 서버에서 ADWS를 중지하거나 사용하지 않도록 설정하면 Windows PowerShell용 Active Directory 모듈 또는 Active Directory 관리 센터와 같은 클라이언트 응용 프로그램이 이 서버에서 실행 중인 디렉터리 서비스 인스턴스에 액세스하거나 관리할 수 없습니다. 자세한 내용은 참조 하세요. [AD DS의 새로운 기능: Active Directory 웹 서비스](https://technet.microsoft.com/library/dd391908\(WS.10\).aspx) (https://technet.microsoft.com/library/dd391908(WS.10).aspx) Windows Server 기술 라이브러리에서.  
 ...
 **문제:**  서버에서 DHCP 클라이언트 서비스가 실행되고 있지 않습니다.  
  
 **영향:**  이 서비스를 중지하면 클라이언트 컴퓨터가 서버로부터 IP 주소를 받을 수 없습니다.  
 ...
 **문제:**  서버에서 IIS Admin Service가 실행되고 있지 않습니다.  
  
 **영향:**  이 서비스를 중지하면 원격 웹 액세스와 같은 서버에서 실행 중인 웹 사이트에 액세스하지 못할 수 있습니다.  
 ...
 **문제:**  서버에서 World Wide Web Publishing 서비스가 실행되고 있지 않습니다.  
 ...
 **문제:**  서버에서 원격 데스크톱 게이트웨이 서비스가 실행되고 있지 않습니다.  
 ...
 **문제:**  서버에서 Windows 시간 서비스가 실행되고 있지 않습니다.  
 ...
 **문제:**  MSDTC(Distributed Transaction Coordinator) 서비스에 대한 기본 로그온 계정이 변경되었습니다.  
  
 **영향:**  서비스가 예상대로 작동하는 데 필요한 권한이 없을 수 있습니다. 결과적으로, SQL Server 또는 COM 함수를 사용 하는 응용 프로그램이 제대로 작동 하지 않을 수 있습니다.  
 ...
 **문제:**  Netlogon 서비스에 대한 기본 로그온 계정이 변경되었습니다.  
  
 **영향:**  서비스가 예상대로 작동하는 데 필요한 권한이 없을 수 있습니다. 따라서 서버가 사용자 및 서비스를 인증하지 않을 수 있습니다.  
 ...
 **문제:**  DNS 클라이언트 서비스에 대한 기본 로그온 계정이 변경되었습니다.  
  
 **영향:**  서비스가 예상대로 작동하는 데 필요한 권한이 없을 수 있습니다. 따라서 서버가 DNS 이름을 확인하지 못할 수 있습니다.  
...
### <a name="the-dns-server-service-should-use-the-local-system-account-as-its-logon-account"></a>DNS 서버 서비스 로그온 계정으로 로컬 시스템 계정을 사용 해야  
 **문제:**  DNS 서버 서비스에 대한 기본 로그온 계정이 변경되었습니다.  
  
 **영향:**  서비스가 예상대로 작동하는 데 필요한 권한이 없을 수 있습니다. 따라서 DNS 업데이트가 발생하지 않을 수 있습니다.  
 ...
 **문제:**  Active Directory 웹 서비스가 기본 로그온 계정이 아닙니다. 기본적으로 로그온 계정은 **로컬 시스템 계정**으로 설정됩니다.  
 ...
 **문제:**  자동 업데이트 서비스에 대한 기본 로그온 계정이 변경되었습니다.  
  
 **영향:**  서비스가 예상대로 작동하는 데 필요한 권한이 없을 수 있습니다. 따라서 서버가 자동 업데이트를 받지 못할 수 있습니다.  
 ...
 **문제:**  DHCP 클라이언트 서비스에 대한 기본 로그온 계정이 변경되었습니다.  
  
 **영향:**  서비스가 예상대로 작동하는 데 필요한 권한이 없을 수 있습니다. 따라서 클라이언트 컴퓨터가 서버로부터 IP 주소를 받지 못합니다.  
 ...
 **문제:**  IIS Admin Service에 대한 기본 로그온 계정이 변경되었습니다.  
  
 **영향:**  서비스가 예상대로 작동하는 데 필요한 권한이 없을 수 있습니다. 따라서 원격 웹 액세스와 같은 서버에서 실행 중인 웹 사이트에 액세스하지 못할 수 있습니다.  
 ...
 **문제:**  World Wide Web Publishing 서비스에 대한 기본 로그온 계정이 변경되었습니다.  
 ...
 **문제:**  원격 데스크톱 게이트웨이 서비스에 대한 기본 로그온 계정이 변경되었습니다.  
  
 **영향:**  서비스가 예상대로 작동하는 데 필요한 권한이 없을 수 있습니다. 따라서 사용자가 원격 웹 액세스를 사용하여 컴퓨터에 액세스하지 못할 수 있습니다.  
 ...
 **문제:**  Windows 시간 서비스에 대한 기본 로그온 계정이 변경되었습니다.  
  
 **영향:**  서비스가 예상대로 작동하는 데 필요한 권한이 없을 수 있습니다. 따라서 날짜 및 시간 동기화를 사용하지 못할 수 있습니다.  
 ...
 **문제:**  기본 제공 Administrators 그룹에 일괄 작업으로 로그온할 수 있는 권한이 없습니다.  
  
 **영향:**  관리자가 경고를 만들고 관리자가 로그온하지 않은 경우 실행되도록 경고를 구성하면 경고가 실패하고 오류 코드 2147943785가 반환됩니다.  
  
 **해결 방법:**  기본 제공 관리자 그룹에 일괄 작업으로 로그온 할 수 있도록 하는 방법에 대 한 정보를 참조 하세요 [기본 제공 관리자 그룹에 일괄 작업으로 로그온 권한을 부여](https://technet.microsoft.com/library/jj635076) (https://technet.microsoft.com/library/jj635076)합니다.  
  
### <a name="the-windows-firewall-is-turned-off"></a>Windows 방화벽이 꺼져 있음  
 **문제:**  Windows 방화벽이 꺼져 있습니다. 기본값은 켜기입니다.  
  
 **영향:**  방화벽 설정에 따라 Windows 방화벽은 일부 정보가 서버를 통과하지 않도록 차단하여 서버와 네트워크를 악의적인 활동으로부터 보호하는 데 도움이 될 수 있습니다.  
 ...
 **문제:**  내부 네트워크 어댑터가 DNS에 IP 주소를 등록하도록 구성되어 있지 않습니다.  
  
 **영향:**  내부 네트워크 어댑터의 IP 주소는 DNS에 등록 되어 있지 않으면, 서버의 컴퓨터 이름을 사용 하 여 서버에 액세스를 하지 못할 수 있습니다.  
  
 **해결 방법:**  내부 네트워크 어댑터가 DNS에 등록하도록 구성되어 있는지 확인합니다.  
  
### <a name="dns-the-values-for-the-dns-forwardingtimeout-and-recursiontimeout-registry-key-are-identical"></a>DNS: DNS ForwardingTimeout 및 RecursionTimeout 레지스트리 키의 값이 같음  
 **문제:**  DNS ForwardingTimeout 레지스트리 키의 값은 RecursionTimeout 레지스트리 키의 값과 같으면 안 됩니다.  
  
 **영향:**  이름으로 인터넷 리소스에 액세스하지 못할 수 있습니다.  
  
 **해결 방법:**  레지스트리의 HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Services\DNS\Parameters에 있는 RecursionTimeout 레지스트리 키의 값을 ForwardingTimeout 키의 값보다 크게 설정합니다.  
 ...
 **영향:**  보안 동적 업데이트를 사용하도록 설정하면 권한이 있는 사용자 및 호스트만 레코드를 변경할 수 있습니다.  
 ...
 **문제:**  보안 동적 업데이트만 허용하도록 _msdcs.* 영역에 대한 정방향 조회 영역을 구성해야 합니다.  
  
 **영향:**  보안 동적 업데이트를 사용하도록 설정하면 권한이 있는 사용자 및 호스트만 _msdcs.* 영역의 레코드를 변경할 수 있습니다.  
  
 **해결 방법:**  
  
##### <a name="to-allow-secure-updates-in-the-msdcs-zone"></a>_msdcs 영역에서 보안 업데이트를 허용하려면  
 ...
 **문제:**  Internet Explorer 보안 강화 구성 (IE ESC) 현재 Administrators 그룹에 대 한 사용 되지 않습니다.  
  
 **영향:**  Administrators 그룹에 대해 Internet Explorer 보안 강화 구성을 사용하지 않도록 설정하면 웹 콘텐츠 및 응용 프로그램 스크립트를 통해 발생할 수 있는 악의적인 공격에 대한 서버 및 Internet Explorer의 노출이 증가합니다.  
...
### <a name="internet-explorer-enhanced-security-configuration-is-not-enabled"></a>Internet Explorer 보안 강화 구성이 사용되지 않음  
 **문제:**  Internet Explorer 보안 강화 구성 (IE ESC)은 현재 사용자 그룹에 대 한 사용 되지 않습니다.  
  
 **영향:**  Users 그룹에 대해 Internet Explorer 보안 강화 구성을 사용하지 않도록 설정하면 웹 콘텐츠 및 응용 프로그램 스크립트를 통해 발생할 수 있는 악의적인 공격에 대한 서버 및 Internet Explorer의 노출이 증가합니다.  
 ...
 **문제:**  Windows Small Business Server를 실행하는 원본 서버가 Active Directory 사이트 및 서비스의 Default-First-Site-Name에 여전히 있습니다.  
  
 **영향:**  원본 서버가 Active Director 사이트 및 서비스에 남아, 클라이언트 컴퓨터 연결 문제가 발생할 수 있습니다:가.  
 ...
 **문제:**  Windows Small Business Server를 실행하는 원본 서버가 Active Directory 사용자 및 컴퓨터에 여전히 있습니다.  
  
 **영향:**  원본 서버가 Active Director 사용자 및 컴퓨터에 남아, 클라이언트 컴퓨터 연결 문제가 발생할 수 있습니다:가.  
 ...
 **영향:**  도메인이 제대로 작동하려면 기본 도메인 정책이 필요합니다.  
 ...
 **영향:**  Active Directory 도메인에 대한 정방향 조회 영역에 DNS NS(이름 서버) 리소스 레코드가 없는 경우 사용자가 네트워크 또는 인터넷의 리소스에 액세스하지 못할 수 있습니다.  
 ...
 **문제:**  서버에 대한 _msdcs 영역(예: _msdcs.contoso.local)에 DNS NS(이름 서버) 리소스 레코드가 없습니다.  
  
 **영향:**  Active Directory 도메인에 대한 _msdcs 영역에 DNS NS(이름 서버) 리소스 레코드가 없는 경우 사용자가 네트워크 또는 인터넷의 리소스에 액세스하지 못할 수 있습니다.  
...
### <a name="no-dns-name-server-records"></a>DNS 이름 서버 레코드가 없음  
 **문제:**  위임된 _msdcs 정방향 조회 영역에 대한 DNS NS(이름 서버) 리소스 레코드가 없습니다.  
  
 **영향:**  위임된 _msdcs 정방향 조회 영역에 대한 DNS NS(이름 서버) 리소스 레코드가 없는 경우 DNS 서버 서비스가 도메인에 대한 DNS 리소스 레코드를 확인할 수 없으며 시작되지 않습니다.  
 ...
 **문제:**  Authenticated Users 그룹이 Pre-Windows 2000 Compatible Access 그룹의 구성원이 아닙니다.  
  
 **영향:**  기본 제공 Authenticated Users 그룹이 Pre-Windows 2000 Compatible Access 그룹의 구성원이 아닌 경우 네트워크 사용자에게 "액세스가 거부되었습니다." 오류가 발생할 수 있습니다.  
 ...
 **문제:**  DNS 클라이언트가 서버의 내부 IP 주소만 가리키도록 구성되어 있지 않습니다.  
  
 **영향:**  DNS 클라이언트가 서버의 내부 IP 주소만 가리키도록 구성되어 있지 않으면 DNS 이름 확인에 실패할 수 있습니다.  
  
 **해결 방법:**  
  
##### <a name="to-configure-dns-to-point-only-to-the-server-s-internal-ip-address"></a>S 서버 내부 IP 주소를 가리키도록 DNS를 구성 하려면  
 ...
 **문제:**  DefaultAppPool 응용 프로그램 풀에 대한 최대 작업자 프로세스 수가 기본값인 1로 설정되어 있지 않습니다.  
  
 **영향:**  사용자가 Windows Small Business Server 웹 기반 서비스에 연결하지 못할 수 있습니다.  
  
 **해결 방법:**  
  
##### <a name="to-reset-maximum-worker-processes-for-the-default-application-pool"></a>기본 응용 프로그램 풀에 대한 최대 작업자 프로세스를 다시 설정하려면  
 ...
 **문제:**  RemoteAppPool 응용 프로그램 풀이 기본 계정으로 실행되고 있지 않습니다.  
  
 **영향:**  네트워크 사용자가 원격 웹 액세스 웹 사이트에 액세스하지 못할 수 있습니다.  
  
 **해결 방법:**  
  
##### <a name="to-reset-the-remote-application-pool-to-use-the-default-account"></a>기본 계정을 사용하도록 원격 응용 프로그램 풀을 다시 설정하려면  
 ...
 **문제:**  RemoteAppPool 응용 프로그램 풀이 기본 버전의 Microsoft .NET Framework로 실행되고 있지 않습니다.  
 ...
 **문제:**  Remoteaccess.log 파일의 크기가 1GB를 초과하면 시스템 드라이브에서 디스크 공간 부족 오류가 발생할 수 있습니다.  
  
 **영향:**  Remoteaccess.log 파일이 너무 큰 경우 사용 가능한 공간 문제가 발생할 수 있습니다: c: 드라이브에 대 한 s입니다.  
 ...
 **문제:**  기본 웹 사이트의 로그 폴더의 크기가 1GB를 초과 하면 시스템 드라이브에서 부족 한 디스크 공간 오류가 발생할 수 있습니다.  
  
 **영향:**  기본 웹 사이트의 로그 폴더가 너무 크면, 사용 가능한 공간 문제가 발생할 수 있습니다: c: 드라이브에 대 한 s  
  
 **해결 방법:**  서버를 백업한 후 기본 웹 사이트가 중지된 동안 C:\inetpub\logs\LogFiles\W3SVC1 폴더에 있는 로그 파일을 삭제할 수 있습니다. 그런 다음 기본 웹 사이트를 시작합니다.  
 ...
 **영향:**  서버의 모든 IP 주소에 SSL이 바인딩되어 있지 않으면 사용자가 일부 웹 사이트를 사용할 수 없습니다.  
 ...
 **영향:**  기본 웹 사이트에 SSL이 바인딩되어 있지 않으면 사용자가 일부 웹 사이트를 사용하지 못할 수 있습니다.  
 ...
 **문제:**  서버 인증서가 30일 내에 만료됩니다.  
  
 **영향:**  서버는 만료된 인증서를 사용할 수 없습니다. 인증서가 만료되면 사용자가 원격 액세스 기능을 사용하지 못할 수 있습니다.  
  
 **해결 방법:**  인증서가 만료되지 않도록 하려면 신뢰할 수 있는 인증 기관에서 인증서를 갱신합니다.  
 ...
 **영향:**  인증서 주체가 도메인 이름 마법사에서 구성된 이름과 일치하지 않으면 일부 웹 사이트가 초기화되지 않습니다. 다른 사이트에는 오류가 표시 됩니다 "이 웹이 사이트의 보안 인증서에 문제가 있습니다."  
  
 **해결 방법:**  이 문제를 해결 하려면:에 원격 액세스 설정 마법사를 다시 실행 하 고 인증서에 대 한 올바른 도메인 이름을 제공 하거나 사용 하려는 도메인 이름과 일치 하는 새 인증서를 구입 합니다.  
  
### <a name="one-or-more-user-accounts-have-duplicate-cn-names"></a>하나 이상의 사용자 계정에 중복된 CN 이름이 있음  
 **문제:**  하나 이상의 사용자 계정에 중복 된 CN 이름이: {0}합니다.  
  
 **영향:**  사용자 계정에 중복된 CN 이름이 있으면 사용자가 네트워크에 로그온하지 못할 수 있습니다. 또한 Active Directory에서 사용자를 검색할 경우 잘못된 값이 반환될 수 있습니다.  
  
 **해결 방법:**  이 문제를 해결 하려면:, 네트워크 사용자 계정을 않습니다 중복이 없는지 확인 "CN =" 이름입니다. 이 작업을 더 쉽게 하려면 검토를 위해 Active Directory 내용을 텍스트 파일로 내보내는 것이 좋습니다. 이 작업을 수행 하는 방법에 대 한 정보를 참조 하세요 [가져오기 및 내보내기 디렉터리 개체를 Active directory (기술 자료 문서 237677)를 사용 하 여 LDIFDE](https://support.microsoft.com/kb/237677) (https://support.microsoft.com/kb/237677)합니다.  
  
### <a name="nt-backup-is-installed"></a>NT 백업이 설치되어 있음  
 **문제:**  Windows NT Backup 프로그램이 서버에 설치되어 있습니다.  
  
 **영향:**   Windows Server Essentials에서는 Windows Server Backup을 사용합니다. Windows NT Backup 프로그램도 설치되어 있으면 두 백업 프로그램 간에 충돌이 발생할 수 있습니다. 이 경우 Windows Server Backup 프로세스가 실패할 수 있습니다. 충돌로 인해 백업을 사용하여 서버를 복원하지 못할 수도 있습니다.  
 ...
 **문제:**  IIS(인터넷 정보 서비스)에서 포트 80(0.0.0.0:80) 또는 포트 443을 소유하고 있지 않습니다. 해당 포트가 현재 다른 응용 프로그램에서 바인딩되어 있습니다.  
  
 **영향:**   Windows Server Essentials 웹 응용 프로그램에는 포트 80과 포트 443 사용자에 게 서비스를 사용할 수 있도록를 사용을 해야 합니다. 다른 프로세스 또는 응용 프로그램이 이미 사용 하는 경우 포트 80 또는 포트 443, Windows Server Essentials 웹 응용 프로그램을 실행할 수 없습니다. 이 경우 사용자가 원격 웹 액세스 및 기타 응용 프로그램을 사용할 수 없습니다.  
  
 **해결 방법:**  이 문제를 해결 하려면:, 이미 포트 80 또는 포트 443 사용 하거나 다른 포트에 해당 응용 프로그램을 할당 하는 응용 프로그램을 제거 하거나 합니다.  
  
### <a name="the-default-website-is-not-running"></a>기본 웹 사이트가 실행되고 있지 않음  
 **문제:**  기본 웹 사이트는 Windows Server Essentials 환경에서 실행 되지 않습니다.  
  
 **영향:**   Windows Server Essentials 웹 응용 프로그램에 기본 웹 사이트를 사용을 해야 합니다. 기본 웹 사이트가 실행되고 있지 않으면 사용자가 원격 웹 액세스 및 기타 응용 프로그램을 사용할 수 없습니다.  
 ...
 **문제:**  /Remote 가상 디렉터리에 대한 읽기 및 스크립트 권한이 할당되어 있지 않습니다.  
  
 **영향:**  /Remote 가상 디렉터리에 대한 읽기 및 스크립트 권한이 잘못된 경우 사용자가 원격 웹 액세스를 사용할 수 없습니다. 인터넷을 찾아보려면 원격 웹 액세스를 사용 하려고 할 때 "HTTP 오류 403.1 사용할 수 없음." 오류가 발생할 수 있습니다.  
 ...
 **문제:**  /Remote 가상 디렉터리에 HTTP 리디렉션 특성이 예기치 않게 설정되어 있거나 상속되었습니다.  
  
 **영향:**  /Remote 가상 디렉터리에 HTTP 리디렉션 특성이 설정되어 있으면 원격 웹 작업 사이트가 올바르게 작동하지 않습니다.  
 ...
 **문제:**  기본 웹 사이트에 포트 80에 대한 호스트 이름이 할당되어 있습니다.  
  
 **영향:**  기본 웹 사이트에서 포트 80에 대 한 호스트 이름에 할당 된 경우 일부 Windows Server Essentials 웹 응용 프로그램에 연결할 수 수 없습니다. 호스트 이름은 필수가 아니며 이러한 상황에서 권장되지 않습니다.  
 ...
 **문제:**  Windows Server 백업에서 NTFS가 아닌 파티션이 백업에 예약되었습니다.  
  
 **영향:**  Windows Server 백업은 NTFS로 포맷된 파티션만 백업할 수 있습니다.  
  
 **해결 방법:**  NTFS가 아닌 파티션을 백업하도록 Windows Server Backup을 구성하지 마세요. 자세한 내용은 [시스템 상태 백업 (기술 자료 문서 968128)는 Windows Server 2008 기반 컴퓨터에서 실패 한 경우 이벤트 Id 12290 및 16387이 기록](https://support.microsoft.com/kb/968128) (https://support.microsoft.com/kb/968128)합니다.  
  
### <a name="the-most-recent-backup-did-not-succeed"></a>가장 최근의 백업이 실패함  
 **문제:**  가장 최근의 백업 시도가 성공적으로 완료되지 않았습니다.  
  
 **영향:**  시스템의 백업 상태가 잘못되었습니다.  
  
 **해결 방법:**  이벤트 로그 및 백업 로그에서 가장 최근의 백업 중에 발생한 오류를 검토합니다.  
  
### <a name="the-startup-type-for-the-file-replication-service-is-not-set-to-automatic"></a>파일 복제 서비스의 시작 유형이 자동으로 설정되어 있지 않음  
 **문제:**  시작 유형이 기본값인 자동으로 설정되어 있지 않으면 FRS(파일 복제 서비스)가 시작되지 않을 수 있습니다.  
  
 **영향:**  파일 복제 서비스가 실행되고 있지 않으면 도메인 컨트롤러가 서비스 알림을 중지할 수 있습니다. 이 경우 로그온 오류 및 그룹 정책 오류와 같은 다른 문제가 발생할 수 있습니다.  
 ...
 **영향:**  파일 복제 서비스가 실행되고 있지 않으면 도메인 컨트롤러가 서비스 알림을 중지할 수 있습니다. 이 동작으로 인해 로그온 오류 및 그룹 정책 오류와 같은 다른 문제가 발생할 수 있습니다.  
 ...
 **문제:**  파일 복제 서비스가 로컬 시스템 계정을 기본 로그온 계정으로 사용하도록 구성되어 있지 않습니다.  
  
 **영향:**  파일 복제 서비스가 로컬 시스템을 기본 로그온 계정으로 사용하지 않는 경우 사용 권한 관련 오류가 발생할 수 있습니다. 이러한 오류는 다른 오류를 트리거할 수 있으며, 결국 도메인 컨트롤러가 해당 서비스의 알림을 중지하게 만들 수 있습니다.  
 ...
 **문제:**  시작 유형이 기본값인 자동으로 설정되어 있지 않으면 DFS 복제 서비스가 시작되지 않을 수 있습니다.  
  
 **영향:**  DFS 복제 서비스가 실행되고 있지 않으면 도메인 컨트롤러가 서비스 알림을 중지할 수 있습니다. 이 경우 로그온 오류 및 그룹 정책 오류와 같은 다른 문제가 발생할 수 있습니다.  
 ...
 **문제:**  현재 DFS 복제 서비스가 실행되고 있지 않습니다.  
  
 **영향:**  DFS 복제 서비스가 실행되고 있지 않으면 도메인 컨트롤러가 서비스 알림을 중지할 수 있습니다. 이 동작으로 인해 로그온 오류 및 그룹 정책 오류와 같은 다른 문제가 발생할 수 있습니다.  
 ...
 **문제:**  DFS 복제 서비스가 로컬 시스템 계정을 기본 로그온 계정으로 사용하도록 설정되어 있지 않습니다.  
  
 **영향:**  DFS 복제 서비스가 로컬 시스템을 기본 로그온 계정으로 사용하지 않는 경우 사용 권한 관련 오류가 발생할 수 있습니다. 이러한 오류는 다른 오류를 트리거할 수 있으며, 결국 도메인 컨트롤러가 해당 서비스의 알림을 중지하게 만들 수 있습니다.  
 ...
 **문제:**  Windows Server Office 365 통합 서비스가 로컬 시스템 계정을 기본 로그온 계정으로 사용하도록 설정되어 있지 않습니다.  
  
 **영향:**  Windows Server Office 365 통합 서비스가 로컬 시스템을 기본 로그온 계정으로 사용하지 않으면 Office 365의 일부 기능이 제대로 작동하지 않을 수 있습니다. 사용 권한 관련 오류가 발생할 수도 있습니다.  
 ...
 **문제:**  현재 Windows Server Office 365 통합 서비스가 실행되고 있지 않습니다.  
  
 **영향:**  Windows Server Office 365 통합 서비스가 실행되고 있지 않으면 Office 365의 클라우드 기반 기능을 사용할 수 없습니다.  
 ...
 **문제:**  시작 유형이 기본값인 자동으로 설정되어 있지 않으면 Windows Server Office 365 통합 서비스가 시작되지 않을 수 있습니다.  
 ...
 **문제:**  HKEY_LOCAL_MACHINE\Software\Microsoft\Rpc\RpcProxy 아래의 레지스트리 키가 잘못된 값을 포함하거나 없습니다.  
  
 **영향:**  RPCProxy 레지스트리 키가 올바르게 설정 하는 경우 다음과 유사한 오류 메시지가 나타날 수 있습니다. "원격 데스크톱 게이트웨이 서버를 일시적으로 사용할 수 없어 컴퓨터가 원격 컴퓨터에 연결할 수 없습니다. 나중에 다시 연결하거나 네트워크 관리자에게 문의하세요."  
...
3.  "웹 사이트" 라는 문자열을 기본 웹 사이트의 데이터 값에 있는지 확인 합니다.  
  
    -   데이터 값이 잘못된 경우 올바른 값을 사용하도록 문자열을 수정합니다.  
  
    -   문자열 존재 하지 않는 경우 "웹 사이트" 라는 새 문자열을 만들고 기본 웹 사이트에 데이터 값을 설정 합니다. "  
 ...
 **문제:**  블록 수준 백업 엔진 서비스에서 기본 시작 유형인 수동을 사용하고 있지 않습니다.  
  
 **영향:**  시작 유형이 수동으로 설정되어 있지 않으면 블록 수준 백업 엔진 서비스가 시작되지 않을 수 있습니다. 이번 호의이 내용: Windows Server Backup 작업이 발생할 수 있습니다.  
 ...
 **영향:**  블록 수준 백업 엔진 서비스가 로컬 시스템을 기본 로그온 계정으로 사용하지 않는 경우 사용 권한 관련 오류가 발생할 수 있습니다. 이러한 오류로 인해 Windows Server Backup 작업이 성공적으로 완료되지 않을 수 있습니다.  
 ...
 **문제:**  IIS의 WSS 인증서 웹 서비스 웹 사이트에 잘못된 인증서가 바인딩되어 있습니다. 이 인증서의 일반 이름이 서버 이름과 일치하지 않습니다.  
  
 **영향:**  WSS 인증서 웹 서비스 웹 사이트에 잘못된 인증서를 바인딩하면 연결 마법사가 제대로 작동하지 않을 수 있습니다.  
 ...
 **영향:**  원격 데스크톱 게이트웨이 서비스에 대한 인증서가 올바르게 구성되어 있지 않으면 사용자가 원격 웹 액세스에 연결할 수 없습니다.  
...
-   관리자 권한으로 명령 프롬프트를 열고 다음 명령을 입력합니다.  
    ...
    ```  
     ...
     자세한 내용은 [(기술 자료 문서 2472211) Windows Server Essentials에서 원격 데스크톱 게이트웨이 서비스를 관리 하는 방법](https://support.microsoft.com/kb/2472211) (https://support.microsoft.com/kb/2472211)합니다.
