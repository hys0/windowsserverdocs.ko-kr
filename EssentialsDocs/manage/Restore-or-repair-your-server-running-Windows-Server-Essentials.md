...
>적용 대상: Windows Server 2016 Essentials, Windows Server 2012 R2 Essentials, Windows Server 2012 Essentials
 ...
 이 항목에서는 개요와 지원 절차를 복원 또는 Windows Server Essentials를 실행 하는 서버 복구를 제공 하 고 다음 섹션이 포함 되어 있습니다.  
  
-   [서버 시스템 복원 개요](Restore-or-repair-your-server-running-Windows-Server-Essentials.md#BKMK_Overview)  
  
-   [시스템 드라이브를 복구 또는 복원](Restore-or-repair-your-server-running-Windows-Server-Essentials.md#BKMK_Restore)  
...
###  <a name="BKMK_RestoreFromBackup"></a> 서버 백업에서 복원  
 ...
 백업을 사용할 수 있는 경우에 적합 한 서버를 복원 하 외부 백업에서 복원 하려면 제조업체의 설치 미디어를 사용 하는 것입니다. 복원하면 선택한 백업에서 서버 설정 및 폴더가 복구됩니다. 백업 후 만들어진 설정을 구성하고 데이터를 복원하기만 하면 됩니다.  
...
-   모든 데이터를 유지하도록 선택하는 경우 시스템 볼륨의 모든 데이터는 삭제되지만 다른 볼륨의 데이터는 보존됩니다.  
    ...
    > [!CAUTION]
    >  디스크 설정이 기본 설정과 일치하지 않으면 디스크에 있는 모든 데이터가 삭제됩니다. 시스템 디스크를 교체 하는 경우 새 디스크가 원본 디스크의 시스템 볼륨 보다 커야 합니다.  
    >   
...
-   **서버 다시 구성.** 서버에서 서버 구성 마법사를 사용하여 구성 설정을 다시 입력합니다. 클라이언트 컴퓨터에서 원격으로 관리 되는 Windows Server Essentials 서버를 구성 하려면 웹 브라우저를 열고 입력 **http://***<YourServerName\>*  주소 표시줄에 있습니다.  
  
-   **서버에 클라이언트 컴퓨터 다시 연결.** 이전 컴퓨터 서버에 연결 된, 경우 다시 컴퓨터 서버에 연결 하기 전에 Windows Server Essentials Connector 소프트웨어를 컴퓨터에서 제거 해야 합니다. 자세한 내용은 [Uninstall the Connector software](../use/Get-Connected-in-Windows-Server-Essentials.md#BKMK_13) 및 [Connect computers to the server](../use/Get-Connected-in-Windows-Server-Essentials.md#BKMK_9)을 참조하세요.  
  
##  <a name="BKMK_Restore"></a> 시스템 드라이브를 복구 또는 복원  
...
-   [복구 DVD를 사용하여 클라이언트 컴퓨터에서 서버 복원 또는 다시 설정](Restore-or-repair-your-server-running-Windows-Server-Essentials.md#BKMK_Restore_2). Windows Server Essentials를 실행 하는 원격 관리 서버를 복원 해야 할 경우 서버 제조업체의 복원 DVD를 사용 하 여 클라이언트 컴퓨터에서 복원이 수행 해야 합니다.  
  
###  <a name="BKMK_Restore_1"></a> 복원 하거나 설치 미디어를 사용 하 여 서버를 복구 합니다.  
 다음 절차에는 Windows Server Essentials 설치 미디어를 사용 하 여 백업에서 서버 시스템 드라이브를 복원 하는 방법을 설명 합니다. (설치 미디어를 사용하여 공장 기본 설정으로 복원하는 방법을 알아보려면 서버 제조업체의 설명서를 참조하세요.)  
  
> [!NOTE]
>  서버에 저장소 공간을 사용 하 고 데이터를 새 서버로 복원 하는 경우 먼저 시스템 드라이브를 복구 및 Windows Server Essentials 대시보드에 로그온, 이전 서버에서와 비슷한 방식으로 저장소 공간 구성를 dat 복구 볼륨입니다.  
  
##### <a name="to-restore-the-server-system-drive-from-a-backup-using-installation-media"></a>설치 미디어를 사용하여 백업에서 서버 시스템 드라이브를 복원하려면  
  
1.  서버 DVD 드라이브에 Windows Server Essentials 설치 DVD를 넣습니다, 그리고 서버를 다시 시작 및 DVD에서 시작 하려면 아무 키나 누릅니다.  
...
5.  에 **시스템 이미지 복구** 페이지에서 현재 시스템? 어느 **Windows Server Essentials** 또는 **Windows Server Essentials**합니다.  
...
###  <a name="BKMK_Restore_2"></a> 복원 또는 복구 DVD를 사용 하 여 클라이언트 컴퓨터에서 서버를 다시 설정  
 Windows Server Essentials에서 시작할 수 있습니다 서버에서 부팅 가능 USB 플래시 드라이브를 사용자가 만든 하 고 복구 서버 제조업체에서 받은 DVD를 사용 하 여 클라이언트 컴퓨터에서 서버를 복구 하는 다음입니다. 클라이언트 컴퓨터는 서버와 동일한 네트워크에 있어야 합니다. 이 방법은 Windows Server Essentials에서 사용할 수 없습니다.  
...
1.  클라이언트 컴퓨터에서 서버 제조업체에서 받은 Windows Server Essentials 서버 복구 미디어를 삽입 합니다.  
...
###  <a name="BKMK_RestoreFilesFromBackup"></a> 서버 백업에서 파일 및 폴더 복원  
 파일 및 폴더 복원 마법사를 사용하면 하드 디스크의 작동이 중지되거나 파일이 실수로 지워진 경우 데이터를 보호하는 데 도움이 됩니다. Windows Server Essentials Backup을 사용 하 여 하드 드라이브에 모든 데이터의 복사본을 만들고 하 외부 저장 장치에 데이터를 저장 합니다. 하드 드라이브에 있는 원래 데이터가 실수로 지워지거나, 덮어써지거나, 오작동으로 인해 액세스 불가능하게 되는 경우 백업에서 데이터를 복원할 수 있습니다. 파일 또는 폴더 복원 마법사를 사용하면 기존 백업에서 단일 파일 또는 폴더, 여러 파일 또는 폴더나 전체 하드 드라이브를 복원할 수 있습니다.  
...
> [!WARNING]
>  백업 및 복원 파일 및 폴더에 대 한 자세한 내용은 참조 하세요. [Manage Backup and Restore](Manage-Backup-and-Restore-in-Windows-Server-Essentials.md)합니다.  
  
###  <a name="BKMK_ConfigreSharedFolders"></a> 서버의 공유 폴더 복원  
 공유 폴더는 여전히 데이터 파티션에 또는 데이터 파티션으로 복원 된 경우 s 서버 시스템 드라이브를 복원한 후에 폴더를 인식 하도록 서버에 대 한 공유 폴더를 다시 구성 해야 합니다. 다음 절차에서는 이전에 공유된 공유 폴더를 추가하는 방법을 설명합니다.  
...
3.  Windows Server Essentials 대시보드에 로그온을 클릭 합니다 **저장소** 탭을 클릭 **폴더를 추가** 에 **서버 폴더 작업** 창입니다.  
  
     폴더 추가 마법사가 열립니다.  
  
4.  **이름** 상자에 공유의 이름을 입력합니다.  
  
5.  클릭 **찾아보기**, 이동할 *< 드라이브\>\\< 서버 이름\>* \ServerFolders (예를 들어 *d:\Contoso\ServerFolders*)을 공유 하 고 클릭 하려는 폴더를 선택 **확인**합니다.  
...
7.  2단계에서 기록해 두었던 사용 권한을 지정한 후 **폴더 추가**를 클릭합니다.  
    ...
    > [!IMPORTANT]
    >  이러한 사용 권한은 Windows Server Essentials 대시보드를 사용 하 여 폴더에 추가 되지 않았습니다 하는 모든 기존 사용 권한이 바뀝니다.  
