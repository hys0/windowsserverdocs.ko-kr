...
> [!NOTE]
>  Windows Server Essentials 및 Windows Server Essentials Experience 역할이 설치 된 Windows Server 2012 R2에서 서버 및 네트워크의 클라이언트 컴퓨터에 대 한 상태 경고는 경고 뷰어에 더 이상 표시 됩니다 있지만 대신 볼 수 없습니다는  **상태 보고서** 탭의 **홈** 페이지입니다.  
  
 Windows Server Essentials 서버에 연결 되 고 경고 문제를 관리자 상태와 관련 된 시스템 s, 중요 업데이트를 포함 하 여 맬웨어 보호를 클라이언트에서 오래 된 바이러스 정의 누락 된 모든 컴퓨터를 적극적으로 모니터링 컴퓨터 및 작업을 필요로 하는 다른 중요 한 문제입니다. 이러한 문제는 대시보드의 서버 또는 실행 패드 또는 Windows Server Essentials에서 클라이언트 컴퓨터에서 시작할 수 있는 경고 뷰어에 경고로 표시 되는 **상태 보고서** Windows Server Essentials에서 탭 합니다. 기본적으로 경고는 30분마다 새로 고쳐지지만 **상태 보고서** 탭 또는 경고 뷰어에서 **새로 고침**을 클릭하여 언제든지 네트워크에 경고가 있는지 평가할 수 있습니다.  
  
 다음 항목은 경고 뷰어에서 경고를 이해하고 확인하며 이에 응답하도록 도와주고 전자 메일로 경고 알림을 받도록 서버를 구성하는 지침을 제공합니다.  
  
-   [상태 보고서 추가 기능](Manage-System-Health-in-Windows-Server-Essentials.md#BKMK_AddIn)  
...
##  <a name="BKMK_AddIn"></a> 상태 보고서 추가 기능  
...
-   [요청 시 또는 일정에 보고서를 생성 합니다.](Manage-System-Health-in-Windows-Server-Essentials.md#BKMK_Generate)  
  
-   [보고서의 콘텐츠를 사용자 지정](Manage-System-Health-in-Windows-Server-Essentials.md#BKMK_Customize)  
  
-   [전자 메일 보고서](Manage-System-Health-in-Windows-Server-Essentials.md#BKMK_emailreport)  
  
> [!NOTE]
>  **Windows Server Essentials의 경우:** 상태 보고서 추가 기능을 Windows Server essentials에서 다운로드할 수 있습니다 합니다 [Microsoft 다운로드 센터](https://go.microsoft.com/fwlink/p/?LinkId=266342)합니다.  
>   
>  **Windows Server Essentials의 경우:** 기본적으로 상태 보고서 추가 기능에 Windows Server Essentials 또는 Windows Server 2012 R2를 사용 하 여 Windows Server Essentials Experience 역할이 설치 된와 통합 상태 보고서에 표시 되는 **상태 보고서** 탭 s 대시보드의 **홈** 페이지입니다.  
  
###  <a name="BKMK_Generate"></a> 요청 시 또는 일정에 보고서를 생성 합니다.  
 ...
 보고서는 수동으로 생성할 뿐만 아니라 매일 또는 매시간 일정에 따라 자동으로 생성할 수도 있습니다. 이 작업을 하려면 작업 창에서 **상태 보고서 설정 사용자 지정**, **일정 및 전자 메일** 탭을 차례로 클릭합니다. **일정** 기능은 기본적으로 꺼져 있고 **예약된 시간에 상태 보고서 생성** 확인란을 선택하여 기능을 켤 수 있습니다.  
...
-   **저장소** 드라이브 및 해당 용량 목록이 **세부 정보** 섹션에 제공됩니다.  
 ...
 기본적으로 보고서에 포함되지 않은 일부 데이터 요소가 필요하지 않으면 작업 창에서 **상태 보고서 설정 사용자 지정**을 클릭하고 **콘텐츠** 탭을 클릭하여 보고서 콘텐츠를 사용자 지정할 수 있습니다. Don t 보고서에 표시 되는 콘텐츠에 대 한 확인란의 선택을 취소 합니다. 예를 들어 고유한 서버 백업 계획이 있고 안 서버 백업에 대 한 경고를 보려는 경우 백업을 제외할 수 있습니다 서버 보고서에서 선택을 취소 하 여 합니다 **서버 백업** 확인란 합니다.  
  
###  <a name="BKMK_emailreport"></a> 전자 메일 보고서  
 ...
 **상태 보고서 설정 사용자 지정** 대화 상자에서 전자 메일을 사용하도록 설정하고 SMTP 설정을 변경하고 전자 메일 받는 사람 목록을 지정하고 나면 작업 창에 새 작업이 표시됩니다. **상태 보고서를 전자 메일로 보냅니다**. SMTP 설정에 대한 자세한 내용은 [Set up email notifications for alerts](Manage-System-Health-in-Windows-Server-Essentials.md#BKMK_Email)을 참조하세요.  
...
###  <a name="BKMK_list"></a> 생성 되는 경고 전자 메일 알림  
...
-   라이선스 오류: Active Directory Domain Services.  
...
-   드라이브 스냅숏을 가져올 수 없어 서버를 백업하지 못했습니다.  
...
> [!NOTE]
>  상태 보고서 추가 기능을 Windows Server essentials에서 다운로드할 수 있습니다 합니다 [Microsoft 다운로드 센터](https://go.microsoft.com/fwlink/p/?LinkId=266342)합니다.  
...
4.  **SMTP 설정** 창에서 다음을 수행합니다.  
    ...
    1.  **보낸 사람 전자 메일 주소**에 대해 전자 메일 경고를 보내는 데 사용할 전자 메일 주소를 입력합니다. 이 전자 메일 주소는 경고 알림에 보낸 s 주소로 표시 됩니다.  
    ...
    |smtp.comcast.net|예|아니오|587|인증을 위해 도메인 이름과 암호가 포함된 전체 전자 메일 주소를 제공합니다.|  
...
> [!NOTE]
>  기본적으로 상태 보고서 추가 기능에 Windows Server Essentials 또는 Windows Server 2012 R2를 사용 하 여 Windows Server Essentials Experience 역할이 설치 된와 통합 상태 보고서에 표시 되는 **상태 보고서** 탭 s 대시보드의 **홈** 페이지입니다.  
...
4.  **일정 및 전자 메일** 탭의 **전자 메일** 섹션에서 다음을 수행합니다.  
    ...
    1.  **사용**을 클릭하고 상태 보고서를 보내는 데 사용할 전자 메일 주소를 입력합니다. 이 전자 메일 주소는 전자 메일로 전송 되는 상태 보고서에 보낸 s 주소로 표시 됩니다.  
...
|하드 드라이브에 사용 가능한 공간이 부족합니다.|사용 가능한 공간이 없으면 추가 정보를 저장할 수 없습니다. 컴퓨터에서 사용 가능한 공간을 늘리려면 다음 옵션을 고려하세요.<br /><br /> 새 하드 드라이브를 추가 합니다.<br /><br /> -Run **디스크 정리** 오래 된 파일과 임시 파일을 제거 합니다.<br /><br /> -다른 컴퓨터에서 공유 폴더에 파일을 이동 합니다.<br /><br /> -CD, DVD 또는 외부 하드 드라이브와 같은 이동식 미디어에 파일을 보관 합니다.|  
